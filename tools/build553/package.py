"""Build553 patch: continuous courses and new artwork, delta over Build552."""
...
for f in sorted(root.rglob('*')):
 ...
 if rel.name=='BUILD553_MANIFEST.json' or str(rel).startswith('docs/build553/') and f.suffix in {'.png','.jpeg','.webm'}:continue
 if not (base/rel).exists() or f.read_bytes()!=(base/rel).read_bytes():paths.append(str(rel))
manifest={'build':553,'version':'3.1.232','baseBuild':552,'type':'patch','games':['pinball','junkgp'],'ricochetVersion550':4,'requiredServerRestart':True,'files':[{'path':n,'size':(root/n).stat().st_size,'sha256':hashlib.sha256((root/n).read_bytes()).hexdigest()} for n in paths]}
(root/'BUILD553_MANIFEST.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n');out.parent.mkdir(parents=True,exist_ok=True)
with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED,compresslevel=9) as z:
 for n in [*paths,'BUILD553_MANIFEST.json']:z.write(root/n,n)
