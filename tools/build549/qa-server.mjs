...
const push=id=>send(id,{type:'raceState451',selfId:id,serverNow:now,cartVersion543:5,cart:cartSnapshot543(c,current(),id)});
function reset({round=2}={}){
 ...
 c={data:{cartRooms543:{LOCAL:g},parties462:{party:{id:'party',hostId:'p0',members:members.map(p=>({...p,cartVersion543:5,ready:true}))}}},sessions,subscribers:new Set(sessions.keys()),now:()=>now,transaction:fn=>fn(),send,broadcast(){for(const id of sockets.keys())push(id)},push:s=>{if(s)push(s.playerId)}};
...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(p.startsWith('/qa/')&&req.method==='POST'){let body='';for await(const chunk of req)body+=chunk;const data=JSON.parse(body||'{}');let out={ok:true};
  ...
  if(p==='/qa/push-start'){const g=reset({round:1});Object.assign(g.players[0],{x:-.6,y:4});Object.assign(g.players[1],{x:-2.3,y:4});Object.assign(g.players[2],{x:2.9,y:14});Object.assign(g.players[3],{x:-2.9,y:18});c.broadcast();out={id:g.id}}
  if(p==='/qa/intermission'){const g=reset({round:4});g.phase='intermission';g.phaseAt=now;g.players.forEach((q,i)=>Object.assign(q,{launched:true,launchAt:0,y:10+[60,99,88,78][i]/100*(R.CART543.edge-R.CART543.radius-10),vx:0,vy:0,score:[137,286,88,254][i],roundScore:[60,99,88,78][i]}));paused=true;c.broadcast();out={id:g.id}}
...
server.listen(8549,'127.0.0.1',()=>console.log('cart549 QA ready'));process.on('SIGTERM',()=>{clearInterval(timer);wss.close();server.close();process.exit()});
