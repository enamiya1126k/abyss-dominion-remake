...
const out=fileURLToPath(new URL('../../docs/build549/',import.meta.url));await mkdir(out,{recursive:true});
...
const base='http://127.0.0.1:8549',errors=[],missing=[],report={engine:await browser.version(),scope:'Production cart View / Controls / Renderer with real CartCoordinator over two WebSocket clients; no public deployment or physical iPhone',viewports:[],checks:[]};
...
try{
 ...
 await shot(p,'cart549-pulling');check(await p.evaluate(()=>!Array.from(window.drawnLabels548).some(s=>['P','海まで飛ぶ！','停車の目安','金枠ねらい！','この先、海！'].includes(s))),'prediction or parking text is still drawn');check(await p.locator('[data-ct-power]').innerText()!=='POWER 0%','old precision readout');await p.waitForTimeout(1600);const held=await p.evaluate(()=>c.cartUI543.pull547.power);check(held===pull.power,'holding changes power');await t.up();
 ...
 check((await api('state')).players[1].power===second,'second player did not launch independently');await p.waitForTimeout(400);await shot(p,'cart549-launched');report.checks.push('No stopping forecast, landing marker, hazard text, parking P letters or floating joystick; actual canvas labels inspected');report.checks.push('Two connected clients agree on exact released power; holding 1.6 seconds does not increase it');
 // A real second client pushes the unreleased local cart during its pull.
 const pushed=await api('push-start');for(const page of [p,peer])await page.waitForFunction(id=>c.state.cart.id===id,pushed.id);
 a=await anchor(p);await t.down([point(a.x,a.y)]);await t.move([point(a.x,a.y+54)]);
 const beforeX=await p.evaluate(()=>c.state.cart.players[0].x);
 await peer.evaluate(()=>{const g=c.state.cart;c.raw('cartInput543',{gameId:g.id,round:g.round,seq:100,action:'shoot',power:.24,angle:Math.PI/2})});
 await p.waitForFunction(x=>c.state.cart.players[0].x>x+.12,beforeX);
 check(await p.evaluate(()=>!c.state.cart.players[0].launched&&c.cartUI543.pull547?.active),'impact consumed or cancelled the pending shot');
 await t.up();await p.waitForFunction(()=>c.state.cart.players[0].launched);await peer.waitForFunction(()=>c.state.cart.players[0].launched);
 for(const page of [p,peer])check(await page.evaluate(()=>{const q=c.state.cart.players[0];return q.vx>1&&q.vy>4}),'release discarded lateral carry');
 report.checks.push('A peer collides with an unreleased cart while its finger stays down; local and peer agree that the subsequent shot retains lateral carry');
 ...
 for(const [width,height]of [[320,690],[390,844],[430,932],[844,390]]){
  ...
  const q=await anchor(page);await finger.down([point(q.x,q.y)]);await finger.move([point(q.x-12,q.y+55)]);await page.waitForTimeout(90);await shot(page,`cart549-${width}x${height}`);await finger.up();await page.waitForFunction(()=>c.state.cart.players[0].launched);await page.evaluate(()=>dispose547());await page.close();
 }
 ...
 for(const [width,height]of [[320,690],[390,664],[430,932],[844,390]]){
  await api('reset');const page=await open(width,height);const {id}=await api('intermission');await page.waitForFunction(id=>c.state.cart.id===id&&c.state.cart.phase==='intermission',id);await page.waitForTimeout(100);
  check(await page.evaluate(()=>getComputedStyle(document.querySelector('.ct-pawns543')).visibility==='hidden'),'world avatars cover the results');
  const layout=await page.evaluate(()=>{const grid=document.querySelector('[data-ct-roundresults]').getBoundingClientRect(),stage=document.querySelector('[data-ct-stage]').getBoundingClientRect();return{top:grid.top,bottom:grid.bottom,stageTop:stage.top,stageBottom:stage.bottom,scores:[...document.querySelectorAll('[data-ct-score]')].map(e=>({width:e.clientWidth,content:e.scrollWidth,unit:e.querySelector('small')?.textContent}))}});
  check(await page.locator('[data-ct-help]').innerText()==='今回 ＋60点','round footer still shows provisional points');check(layout.top>=layout.stageTop&&layout.bottom<=layout.stageBottom,'round results clipped');check(layout.scores.every(e=>e.content<=e.width&&e.unit==='点'),'three-digit scores wrap or overflow');
  await shot(page,`cart549-result-${width}x${height}`);await page.evaluate(()=>dispose547());await page.close();
 }
 await api('reset',{round:4});const beltPage=await open();await api('pause',{value:true});await beltPage.waitForTimeout(120);await shot(beltPage,'cart549-conveyor');await beltPage.evaluate(()=>dispose547());await beltPage.close();
 report.checks.push('Result overlays keep field avatars behind the panel; three-digit score units and result cards fit in four layouts including Safari-sized 390x664');
