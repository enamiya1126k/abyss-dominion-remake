...
const browser=await chromium.launch({headless:true,executablePath:process.env.CHROMIUM_PATH,args:['--no-sandbox']});const base='http://127.0.0.1:8558',api=(n,d={})=>fetch(base+'/qa/'+n,{method:'POST',body:JSON.stringify(d)}).then(r=>r.json()),errors=[],events=[],screens=[],shots=[];
try{
 ...
 let seen=0,zoomCapture=null;
 for(let i=0;i<740;i++){
  let g=await api('state');if(g.phase==='result')break;
  const q=g.players[0];
  if(g.phase==='play'&&!q.launched&&q.fallenAt==null&&g.elapsed>=650){
   const plan=BOT_COURSES556[g.courseId556][q.lane][1],metrics=await p.evaluate(async()=>{const {point543}=await import('/src/cart/Renderer543.js'),r=c.cartUI543.renderer,q=c.state.cart.players[0],a=point543(r,q.x,q.y),b=point543(r,q.x+1,q.y),d=point543(r,q.x,q.y+1);return{width:r.width,height:r.height,xScale:b.x-a.x,yScale:a.y-d.y,skewX:d.x-a.x}}),distance=12+plan.power*(Math.max(72,Math.min(120,metrics.width*.28,metrics.height*.34))-12),vx=Math.sin(plan.angle),vy=Math.cos(plan.angle),dx=-(vx*metrics.xScale+vy*metrics.skewX),dy=vy*metrics.yScale,k=distance/Math.hypot(dx,dy),box=await p.locator('[data-ct-stage]').boundingBox(),x=box.x+box.width*.5,y=box.y+box.height*.5;
  ...
  if(g.courseId556==='icefork'&&!zoomCapture&&g.phase==='play'&&g.elapsed>5000){const zoom=await p.evaluate(()=>c.cartUI543.renderer.zoom);if(zoom>1.3){zoomCapture={zoom,elapsed:g.elapsed};await p.screenshot({path:out+'cart-icefork-zoom-real.png'})}}
 }
 const g=await api('state');await p.waitForFunction(()=>c.state.cart.phase==='result');await p.screenshot({path:out+'cart-result-real.png',fullPage:true});const report={source:'Production game modules, one automated human using actual mouse drags, three AI; server clock accelerated in 25 ms simulation steps. Normal seeded random draw. No course, score, position, velocity or outcome injection.',phase:g.phase,rounds:g.history,results:g.results,shots,screens,zoomCapture,eventCounts:Object.fromEntries([...new Set(events.map(e=>e.type))].map(t=>[t,events.filter(e=>e.type===t).length])),errors,stderr};await writeFile(out+'capture-report.json',JSON.stringify(report,null,2));console.log(JSON.stringify({phase:g.phase,shots,zoomCapture,errors,stderr}));if(errors.length||stderr||shots.length!==5||g.history.length!==5||!zoomCapture)throw Error('capture failed');
