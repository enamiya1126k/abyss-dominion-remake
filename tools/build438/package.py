"""Create the cumulative overwrite ZIP from the delivered Build437 archive."""
...
manifest_name='docs/build438/patch-manifest.json';changed.discard(manifest_name)
for name in changed:
 p=Path(name)
...
assert contents['src/core/config.js'].find(b'APP_VERSION="3.1.117"')>=0
assert len(json.loads(contents['docs/build438/browser-result.json'])['checks'])>=69
assert json.loads(contents['docs/build438/browser-result.json'])['errors']==[]
assert json.loads(contents['docs/build438/browser-result.json'])['missing']==[]
assert b'# pass 97' in contents['docs/build438/tests.tap'] and b'# fail 0' in contents['docs/build438/tests.tap']
manifest={'build':438,'version':'3.1.117','baseline_build':424,'kind':'cumulative-overwrite','save_schema':84,'data_files_included':False,'removed':[],
 ...
 'updated_in_438':sorted(changed),'validation':{'targeted_tests':97,'passed':97,'browser_checks':len(json.loads(contents['docs/build438/browser-result.json'])['checks']),'production_deployed':False,'real_safari_tested':False},
...
print(json.dumps({'file':str(out),'files':len(contents),'updated_in_438':len(changed),'bytes':out.stat().st_size,'sha256':hashlib.sha256(out.read_bytes()).hexdigest()},ensure_ascii=False))
