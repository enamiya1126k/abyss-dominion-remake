...
previous = root.parent / 'downloads458/ABYSS_Build457_integrated.zip'
out = root.parent / 'deliverables/ABYSS_Build458_integrated.zip'
out.parent.mkdir(exist_ok=True)
previous_hash = hashlib.sha256(previous.read_bytes()).hexdigest()
assert previous_hash == 'd1fbe26e0b01b0baf605c4b7ddf47180b2a977f116d3ededcddd2b0c44ac2aff'
with zipfile.ZipFile(previous) as z:
    assert z.testzip() is None
    old = {i.filename: z.read(i) for i in z.infolist() if not i.is_dir()}
changed_runtime = json.loads((root / 'docs/build458/changed-runtime.json').read_text())
new = {
    'README_Build458.md', 'world-raid-offline458-sw.js',
    'world-raid-offline458-assets.json', 'src/Styles/build458-race-grand-prix.css',
    'online-server/server.js', 'tests/build458-race-ux.test.mjs',
    'tests/build453-race-finish.test.mjs', *changed_runtime,
}
for folder in ['docs/build458', 'tools/build458']:
    new.update(p.relative_to(root).as_posix() for p in (root / folder).rglob('*')
               if p.is_file() and p.suffix not in ['.png', '.pyc']
               and '__pycache__' not in p.parts
               and p.name not in ['edit-client.py', 'smoke.mjs', 'test-results-initial.txt', 'package-result.txt'])
new.discard('docs/build458/patch-manifest.json')
entries = set(old) | new
...
manifest = {
    'build': 458, 'version': '3.1.137', 'baseline_build': 424,
    ...
    'updated_in_458': updated,
    'files': [{'path': p, 'bytes': len(b), 'sha256': hashlib.sha256(b).hexdigest()}
              for p, b in files.items()],
}
p = root / 'docs/build458/patch-manifest.json'
...
print(json.dumps({
    ...
    'preserved_457_files': len(old), 'frozen_runtime_preserved': True,
}))
