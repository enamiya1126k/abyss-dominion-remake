...
try{
 ...
 await p.waitForSelector('.ct-lobby543');await p.waitForFunction(()=>c.ready());await p.waitForTimeout(600);await start(p);await sync(p,3050);
 ...
 await api('scene');await p.waitForTimeout(350);await shot(p,'cart543-production');await shot(p,'cart543-before-crash');await sync(p,550);await shot(p,'cart543-crash');await api('scene',{kind:'fall'});await sync(p,450);await shot(p,'cart543-fall');await sync(p,160000);await p.waitForSelector('.ct-result543');await p.locator('[data-party-result490="again"]').click();await p.waitForSelector('.ct-lobby543');await start(p);await sync(p,3050);const t2=await touch(p);await t2.down();await sync(p,250);await p.evaluate(()=>window.dispatchEvent(new Event('blur')));await sync(p,100);check((await api('state')).players[0].chargeAt==null,'production blur cancels without launch');check(!(await api('state')).players[0].launched,'blur not launch');
 check(errors.length===0,'errors: '+errors.join(';'));check(missing.length===0,'missing: '+missing.join(';'));await writeFile(out+'/production-report543.json',JSON.stringify({productionRaceClient:true,partyStyles:stylePaths.length,fit,refreshWhileHolding:true,release:true,replay:true,blurCancel:true,errors,missing},null,2));console.log('production client OK',JSON.stringify(fit));await p.evaluate(()=>c.dispose());await p.close();
}catch(e){console.error({errors,missing});for(const p of browser.contexts().flatMap(c=>c.pages())){await shot(p,'debug-production543');console.error(await p.evaluate(()=>({error:c.error,party:c.state?.party,phase:c.state?.cart?.phase,choice:c.state?.cart?.members,ready:c.ready(),html:document.querySelector('[data-party-action462=ready]')?.outerHTML})));}throw e}finally{await browser.close();server.kill('SIGTERM')}
