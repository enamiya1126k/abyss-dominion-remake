"""Build552 patch: continuous courses and new artwork, delta over Build551."""
...
for f in sorted(root.rglob('*')):
 ...
 if rel.name=='BUILD552_MANIFEST.json' or str(rel).startswith('docs/build552/') and f.suffix in {'.png','.jpeg','.webm'}:continue
 if not (base/rel).exists() or f.read_bytes()!=(base/rel).read_bytes():paths.append(str(rel))
manifest={'build':552,'version':'3.1.231','baseBuild':551,'type':'patch','games':['pinball','junkgp'],'ricochetVersion550':3,'requiredServerRestart':True,'files':[{'path':n,'size':(root/n).stat().st_size,'sha256':hashlib.sha256((root/n).read_bytes()).hexdigest()} for n in paths]}
(root/'BUILD552_MANIFEST.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n');out.parent.mkdir(parents=True,exist_ok=True)
with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED,compresslevel=9) as z:
 for n in [*paths,'BUILD552_MANIFEST.json']:z.write(root/n,n)
