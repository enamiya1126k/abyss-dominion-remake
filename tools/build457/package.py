...
previous = root.parent / 'deliverables/ABYSS_Build456_integrated.zip'
out = root.parent / 'deliverables/ABYSS_Build457_integrated.zip'
out.parent.mkdir(exist_ok=True)
previous_hash = hashlib.sha256(previous.read_bytes()).hexdigest()
assert previous_hash == 'bac96a67096cc955309c27e0ebde4c1fb0d8fa15117887459315a972cdb6ebb4'
with zipfile.ZipFile(previous) as z:
    old = {i.filename: z.read(i) for i in z.infolist() if not i.is_dir()}
changed_runtime = json.loads((root / 'docs/build457/changed-runtime.json').read_text())
new = {
    'README_Build457.md', 'world-raid-offline457-sw.js',
    'world-raid-offline457-assets.json', 'src/Styles/build457-race-cockpit.css',
    'online-server/server.js', 'tests/build457-race-cockpit.test.mjs', 'tests/build453-race-finish.test.mjs', 'tests/build451-race.test.mjs',
    *changed_runtime,
}
for folder in ['docs/build457', 'tools/build457']:
...
new.discard('docs/build457/patch-manifest.json')
entries = set(old) | new
...
manifest = {
    'build': 457, 'version': '3.1.136', 'baseline_build': 424,
    ...
    'updated_in_457': updated,
    'files': [{'path': p, 'bytes': len(b), 'sha256': hashlib.sha256(b).hexdigest()}
              for p, b in files.items()],
}
p = root / 'docs/build457/patch-manifest.json'
...
print(json.dumps({
    ...
    'preserved_456_files': len(old), 'frozen_runtime_preserved': True,
}))
