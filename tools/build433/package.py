"""Create the cumulative overwrite ZIP from the delivered Build432 archive."""
...
manifest_name='docs/build433/patch-manifest.json';changed.discard(manifest_name)
for name in changed:
 p=Path(name)
...
assert contents['src/core/config.js'].find(b'APP_VERSION="3.1.112"')>=0
assert json.loads(contents['docs/build433/browser-result.json'])['errors']==[]
assert b'# pass 105' in contents['docs/build433/tests.tap'] and b'# fail 0' in contents['docs/build433/tests.tap']
manifest={'build':433,'version':'3.1.112','baseline_build':424,'kind':'cumulative-overwrite','save_schema':84,'data_files_included':False,'removed':[],
 ...
 'updated_in_433':sorted(changed),'validation':{'targeted_tests':105,'passed':105,'browser_checks':13,'production_deployed':False,'real_safari_tested':False},
...
with zipfile.ZipFile(out) as z:
 ...
 for n in json.loads(z.read('world-raid-offline433-assets.json')):
  assert n.removeprefix('./') in contents or (root/n.removeprefix('./')).is_file(),n
print(json.dumps({'file':str(out),'files':len(contents),'updated_in_433':len(changed),'bytes':out.stat().st_size,'sha256':hashlib.sha256(out.read_bytes()).hexdigest()},ensure_ascii=False))
