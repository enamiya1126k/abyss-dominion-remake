"""Build a reviewable delta from a complete Build544 baseline; no saved game data."""
...
for p in sorted(root.rglob('*')):
    if not p.is_file() or ('docs/build545/' in str(p) and p.suffix=='.png'):
        continue
    rel = p.relative_to(root)
    if any(part in {'node_modules', '__pycache__', '.git'} for part in rel.parts):
        continue
    if str(rel).startswith('online-server/data/') or p.name.startswith('debug545') or p.name in {'luck-qa.mjs','first-report.json','regression.txt'} or p.name == 'BUILD545_MANIFEST.json':
        continue
    original = base / rel
    if not original.exists() or p.read_bytes() != original.read_bytes():
        files.append(p)
manifest = {'build':545, 'version':'3.1.224', 'baseBuild':544, 'type':'patch', 'requiredServerRestart':True,
            'files':[{'path':str(p.relative_to(root)), 'size':p.stat().st_size, 'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in files]}
manifest_path = root / 'BUILD545_MANIFEST.json'
