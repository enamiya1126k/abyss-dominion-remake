"""Create the cumulative overwrite ZIP from the delivered Build435 archive."""
...
manifest_name='docs/build435/patch-manifest.json';changed.discard(manifest_name)
for name in changed:
 p=Path(name)
...
assert contents['src/core/config.js'].find(b'APP_VERSION="3.1.114"')>=0
assert json.loads(contents['docs/build435/browser-result.json'])['errors']==[]
assert json.loads(contents['docs/build435/browser-result.json'])['missing']==[]
assert json.loads(contents['docs/build435/final-ui-result.json'])['errors']==[]
assert b'# pass 140' in contents['docs/build435/tests.tap'] and b'# fail 0' in contents['docs/build435/tests.tap']
manifest={'build':435,'version':'3.1.114','baseline_build':424,'kind':'cumulative-overwrite','save_schema':84,'data_files_included':False,'removed':[],
 ...
 'updated_in_435':sorted(changed),'validation':{'targeted_tests':140,'passed':140,'browser_checks':23,'production_deployed':False,'real_safari_tested':False},
...
print(json.dumps({'file':str(out),'files':len(contents),'updated_in_435':len(changed),'bytes':out.stat().st_size,'sha256':hashlib.sha256(out.read_bytes()).hexdigest()},ensure_ascii=False))
