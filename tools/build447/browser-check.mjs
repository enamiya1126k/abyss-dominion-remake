...
try{
 ...
 const expected=await page.evaluate(async()=>{
  ...
  s.records.combatPower={scaleVersion:6,highest:510224,previous:420812,history:[{power:510224,previous:420812,delta:89412,floor:100,at:'2026-09-15T00:00:00Z'}],legacyRecord446:{scaleVersion:5,highest:500000,history:[]},legacyRecord445:{scaleVersion:4,highest:23652,history:[]}};
 ...
 await page.evaluate(()=>qa438.power());assert.ok((await page.locator('#app').textContent()).includes(expected.total));assert.equal(await page.locator('.power-legacy-record445').count(),3);
 assert.deepEqual(errors,[]);const result={passed:true,heroLabels:expected.labels,total:expected.total,homeViewports:[320,393],formationTotalMatches:true,recordTotalMatches:true,legacyArchives:3,errors};fs.writeFileSync('docs/build447/browser-results.json',JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result));
