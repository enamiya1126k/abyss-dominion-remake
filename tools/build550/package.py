"""Build550 patch: both ricochet games, delta over Build549."""
...
for f in sorted(root.rglob('*')):
 ...
 if rel.name=='BUILD550_MANIFEST.json' or str(rel).startswith('docs/build550/') and f.suffix=='.png':continue
 if not (base/rel).exists() or f.read_bytes()!=(base/rel).read_bytes():paths.append(str(rel))
manifest={'build':550,'version':'3.1.229','baseBuild':549,'type':'patch','games':['pinball','junkgp'],'ricochetVersion550':1,'requiredServerRestart':True,'files':[{'path':n,'size':(root/n).stat().st_size,'sha256':hashlib.sha256((root/n).read_bytes()).hexdigest()} for n in paths]}
(root/'BUILD550_MANIFEST.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n');out.parent.mkdir(parents=True,exist_ok=True)
with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED,compresslevel=9) as z:
 for n in [*paths,'BUILD550_MANIFEST.json']:z.write(root/n,n)
