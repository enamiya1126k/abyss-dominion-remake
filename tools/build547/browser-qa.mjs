...
const out=fileURLToPath(new URL('../../docs/build547/',import.meta.url));await mkdir(out,{recursive:true});
...
const base='http://127.0.0.1:8547',errors=[],missing=[],report={engine:await browser.version(),scope:'Production cart View / Controls / Renderer with real CartCoordinator over two WebSocket clients; no public deployment or physical iPhone',viewports:[],checks:[]};
...
async function open(width=390,height=844,self='p0'){
 ...
 await p.goto(base+'/preview.html?self='+self);await p.waitForFunction(()=>window.qaReady);await p.evaluate(()=>document.fonts.ready);await p.waitForFunction(()=>{const r=c.cartUI543.renderer;return ['track','cart','wax','sale','crates'].every(k=>r[k].complete&&r[k].naturalWidth)});return p;
}
...
try{
 ...
 const pull=await p.evaluate(()=>({...c.cartUI543.pull547}));check(pull.active&&pull.power>0&&pull.power<1,'pull not sampled');check(await p.locator('[data-ct-tether]').isVisible(),'tether invisible');await p.waitForFunction(power=>c.state.cart.players[0].pullPower547===power,pull.power);
 await shot(p,'cart547-pulling');await p.waitForTimeout(1600);const held=await p.evaluate(()=>c.cartUI543.pull547.power);check(held===pull.power,'holding changes power');await t.up();
 ...
 check((await api('state')).players[1].power===second,'second player did not launch independently');await p.waitForTimeout(400);await shot(p,'cart547-launched');report.checks.push('Two connected clients agree on exact released power; holding 1.6 seconds does not increase it');
 ...
 for(const [width,height]of [[320,690],[390,844],[430,932],[844,390]]){
  ...
  check(layout.scrollWidth===width,'horizontal overflow');check(layout.footer.bottom<=height+.5,'footer clipped');check(layout.oldControls===0,'old controls still present');check(layout.stage.height>=200,'stage collapsed');report.viewports.push(layout);
  const q=await anchor(page);await finger.down([point(q.x,q.y)]);await finger.move([point(q.x-12,q.y+55)]);await page.waitForTimeout(90);await shot(page,`cart547-${width}x${height}`);await finger.up();await page.waitForFunction(()=>c.state.cart.players[0].launched);await page.evaluate(()=>dispose547());await page.close();
 }
...
}catch(e){console.error({errors,missing});for(const page of browser.contexts().flatMap(c=>c.pages())){await shot(page,'debug547');console.error(await page.evaluate(()=>({text:document.body.innerText,ui:window.c?.cartUI543?{pull:c.cartUI543.pull547,pending:c.cartUI543.pending547}:null})))}throw e}
