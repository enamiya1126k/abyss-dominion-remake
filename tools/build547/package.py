"""Package only the Build547 delta over the supplied Build546 baseline."""
...
for p in sorted(root.rglob('*')):
 ...
 if rel.name=='BUILD547_MANIFEST.json' or (str(rel).startswith('docs/build547/') and p.suffix=='.png'):continue
 if not(base/rel).exists() or p.read_bytes()!=(base/rel).read_bytes():paths.append(str(rel))
manifest={'build':547,'version':'3.1.226','baseBuild':546,'type':'patch','requiredServerRestart':True,'cartVersion543':4,
 'files':[{'path':p,'size':(root/p).stat().st_size,'sha256':hashlib.sha256((root/p).read_bytes()).hexdigest()} for p in paths]}
manifest_path=root/'BUILD547_MANIFEST.json'
manifest_path.write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n')
out.parent.mkdir(parents=True,exist_ok=True)
with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED,compresslevel=9) as z:
 for p in [*paths,'BUILD547_MANIFEST.json']:z.write(root/p,p)
