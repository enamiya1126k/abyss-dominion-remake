...
const dest=new URL('../../docs/build411/',import.meta.url);fs.mkdirSync(dest,{recursive:true});fs.writeFileSync(new URL('combat-samples.json',dest),JSON.stringify(rows));fs.writeFileSync(new URL('combat-summary.json',dest),JSON.stringify({count:rows.length,teams,foes,summary},null,2));console.log(JSON.stringify({count:rows.length,summary}));
