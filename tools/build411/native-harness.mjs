...
const expected=JSON.parse(fs.readFileSync(new URL('./source-identity.json',import.meta.url),'utf8'));if(createHash('sha256').update(source).digest('hex')!==expected.main_sha256)throw new Error('This harness requires the exact Build411 main.js recorded in source-identity.json');
...
export async function run(partyIds,enemyIds,{seed=1,level=1500,maxRounds=30,gear=false,pairEnabled=true,trace=false,enemyHp=8,enemyPower=1.3,circles=false,boss=false,speed=1,resumeAt=0,inspect=false}={}){
 const random=rng(seed),old=Math.random;Math.random=random;
 try{
 const party=partyIds.map(id=>monster(id,level,gear)),enemies=enemyIds.map((id,i)=>{const m=monster(id,level,gear),stats=rawCalculatedStats410(m),e=createEnemyBattleState(SPECIES[m.speciesId],{id:'e'+i,level,combatRarity:SPECIES[m.speciesId].rarity},100);Object.assign(e,stats,{id:'e'+i,speciesId:m.speciesId,hp:Math.floor(stats.hp*enemyHp),maxHp:Math.floor(stats.hp*enemyHp),atk:Math.floor(stats.atk*enemyPower),matk:Math.floor(stats.matk*enemyPower),currentMp:m.maxMp,maxMp:m.maxMp,level,boss,divineBarrier:0});return e;});
