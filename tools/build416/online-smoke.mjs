...
const teams=[LATE_TEAMS415[0],LATE_TEAMS415[3],LATE_TEAMS415[11]],out='docs/build416/online-smoke.jsonl',chunks=[];const append=(_,s)=>chunks.push(s);
...
for(const boss of COOP_BOSS_CATALOG)for(const[team,ids]of teams.entries()){
 ...
 append(out,JSON.stringify({mode:'coop',target:boss.id,team,ids,level:1000,participants:4,seeds:5,wins,rounds:rounds/5})+'\n');
}
fs.writeFileSync(out+'.tmp',chunks.join(''));fs.renameSync(out+'.tmp',out);console.log(out);
