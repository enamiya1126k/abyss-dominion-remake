...
previous = root.parent / 'downloads459/ABYSS_Build458_integrated.zip'
out = root.parent / 'deliverables/ABYSS_Build459_integrated.zip'
out.parent.mkdir(exist_ok=True)
previous_hash = hashlib.sha256(previous.read_bytes()).hexdigest()
assert previous_hash == 'a1b99e0ade216ce7df378af188472fe9686501e300e4f0e89d6b01768d7667c5'
with zipfile.ZipFile(previous) as z:
    assert z.testzip() is None
    old = {i.filename: z.read(i) for i in z.infolist() if not i.is_dir()}
changed_runtime = json.loads((root / 'docs/build459/changed-runtime.json').read_text())
new = {
    'README_Build459.md', 'world-raid-offline459-sw.js',
    'world-raid-offline459-assets.json', 'src/Styles/build459-race-courses.css',
    'online-server/server.js', 'tests/build459-race-courses.test.mjs',
    'tests/build453-race-finish.test.mjs', *changed_runtime,
}
for folder in ['docs/build459', 'tools/build459']:
    new.update(p.relative_to(root).as_posix() for p in (root / folder).rglob('*')
               if p.is_file() and p.suffix not in ['.png', '.pyc']
               and '__pycache__' not in p.parts
               and p.name not in ['implement-core.py', 'implement-ui.py', 'tests-first.txt', 'tests-new.txt', 'tests-baseline.txt', 'package-result.txt'])
new.discard('docs/build459/patch-manifest.json')
entries = set(old) | new
...
manifest = {
    'build': 459, 'version': '3.1.138', 'baseline_build': 424,
    ...
    'updated_in_459': updated,
    'files': [{'path': p, 'bytes': len(b), 'sha256': hashlib.sha256(b).hexdigest()}
              for p, b in files.items()],
}
p = root / 'docs/build459/patch-manifest.json'
...
print(json.dumps({
    ...
    'preserved_458_files': len(old), 'frozen_runtime_preserved': True,
}))
