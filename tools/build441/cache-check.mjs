...
const html=fs.readFileSync('index.html','utf8'),map=JSON.parse(html.match(/<script type="importmap">([\s\S]*?)<\/script>/)[1]).imports,changed=JSON.parse(fs.readFileSync('docs/build441/changed-runtime.json')),assets=JSON.parse(fs.readFileSync('world-raid-offline441-assets.json'));
for(const name of changed){assert.equal(map['./'+name],'./'+name+'?v=3.1.120-build441');assert.ok(assets.includes('./'+name));for(const [from,to]of Object.entries(map))if(from.split('?')[0]==='./'+name)assert.equal(to,'./'+name+'?v=3.1.120-build441');}
assert.ok(!Object.keys(map).some(k=>k.includes('/runtime430/')));assert.ok(html.includes('world-raid-offline441-sw.js'));assert.ok(html.includes('build441.css?v=3.1.120-build441'));assert.ok(fs.readFileSync('src/worldRaid/WorldRaidOfflineCache430.js','utf8').includes('world-raid-offline441-assets.json'));
for(const p of fs.readdirSync('assets/magic-circles/build441'))assert.ok(assets.includes('./assets/magic-circles/build441/'+p));
