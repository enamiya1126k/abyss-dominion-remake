...
const push=id=>{const g=current();send(id,{type:'raceState451',selfId:id,serverNow:now,ricochetVersion550:2,partyResultActions490:1,available:true,ricochet:g?ricochetSnapshot550(c,g,id):null,party:P.partyView462(c,c.sessions.get(id))})};
...
const server=http.createServer(async(req,res)=>{try{const u=new URL(req.url,'http://localhost'),p=decodeURIComponent(u.pathname);if(p.startsWith('/qa/')&&req.method==='POST'){let body='';for await(const chunk of req)body+=chunk;const data=JSON.parse(body||'{}');let out={ok:true};
 ...
 if(p==='/qa/push'){const g=reset({mode:data.mode,phase:'play'});Object.assign(g.players[0],{x:.1,y:4});Object.assign(g.players[1],{x:-1.4,y:4});Object.assign(g.players[2],{x:3,y:12});Object.assign(g.players[3],{x:-3,y:14});c.broadcast();out={id:g.id}}
