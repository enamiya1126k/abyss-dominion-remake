...
const push=id=>{const g=current();send(id,{type:'raceState451',selfId:id,serverNow:now,ricochetVersion550:6,cartVersion543:6,partyResultActions490:1,available:true,cart:g?cartSnapshot543(c,g,id):null,party:P.partyView462(c,c.sessions.get(id))})};
...
server.listen(8556,'127.0.0.1',()=>console.log('cart556 QA ready'));process.on('SIGTERM',()=>{clearInterval(timer);wss.close();server.close();process.exit()});
