...
const raidProfile=sanitizeProfile(buildOnlinePartyProfile(f.context.save.state)),raidSession={playerId:'AD-QA435',connected:true,profile:raidProfile},raidCoordinator=new WorldRaidCoordinator437({sessions:new Map([['AD-QA435',raidSession]]),send:()=>{},random:()=>.5});raidCoordinator.start(raidSession,{requestId:'local-qa435-start-0001',campaignId:raidCoordinator.ledger.state.current.id});const raidState=raidCoordinator.snapshot(raidSession.playerId);
...
const script="import {raidResult432,raidHeader432,raidLobby432} from '/src/worldRaid/WorldRaidView432.js';import {worldRaidBattleView428} from '/src/worldRaid/WorldRaidView428.js';\n"+imports+'\n'+classes+'\n'+functions+'\n'+constants+`\n
...
window.qa436={...window.qa435,
 ...
 chapterGacha(crystals=19012){stopGame();battle=null;app.classList.remove('world-raid-active432');app.innerHTML='';save.state.player.crystals=crystals;save.state.campaign100.finalCompleted=true;save.state.chapterTwo376={unlocked:true,started:true,areaClears378:{4:1}};openChapterTwoGacha397();},
...
window.qa437.menu();window.ready437=true;
window.qa434.field();window.ready435=true;window.ready436=true;
...
}).listen(8437,'127.0.0.1',()=>console.log('Preview http://127.0.0.1:8437'));
