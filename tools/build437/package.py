...
manifest_name='docs/build437/patch-manifest.json';changed.discard(manifest_name)
for name in changed:
 p=Path(name)
...
assert contents['src/core/config.js'].find(b'APP_VERSION="3.1.116"')>=0
assert json.loads(contents['docs/build437/browser-result.json'])['errors']==[]
assert json.loads(contents['docs/build437/browser-result.json'])['missing']==[]
assert b'# pass 219' in contents['docs/build437/tests.tap'] and b'# fail 0' in contents['docs/build437/tests.tap']
manifest={'build':437,'version':'3.1.116','baseline_build':424,'kind':'cumulative-overwrite','save_schema':84,'data_files_included':False,'removed':[],
 ...
 'updated_in_437':sorted(changed),'validation':{'targeted_tests':219,'passed':219,'browser_checks':27,'production_deployed':False,'real_safari_tested':False},
...
with zipfile.ZipFile(out) as z:
 ...
 for n in json.loads(z.read('world-raid-offline437-assets.json')):
  assert n.removeprefix('./') in contents or (root/n.removeprefix('./')).is_file(),n
print(json.dumps({'file':str(out),'files':len(contents),'updated_in_437':len(changed),'bytes':out.stat().st_size,'sha256':hashlib.sha256(out.read_bytes()).hexdigest()},ensure_ascii=False))
