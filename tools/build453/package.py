...
previous = root.parent / 'deliverables/ABYSS_Build452_integrated.zip'
out = root.parent / 'deliverables/ABYSS_Build453_integrated.zip'
out.parent.mkdir(exist_ok=True)
previous_hash = hashlib.sha256(previous.read_bytes()).hexdigest()
assert previous_hash == '8d766d41577c8194b59d831e1d837393c23329f66c4a730eb032e56b66c2705d'
with zipfile.ZipFile(previous) as z:
    old = {i.filename: z.read(i) for i in z.infolist() if not i.is_dir()}
changed_runtime = json.loads((root / 'docs/build453/changed-runtime.json').read_text())
new = {
    'README_Build453.md', 'world-raid-offline453-sw.js',
    'world-raid-offline453-assets.json', 'src/Styles/build453-race-finish.css',
    'assets/race453/sand.png', 'assets/race453/gate.png', 'tests/build453-race-finish.test.mjs', 'online-server/server.js',
    *changed_runtime,
}
for folder in ['docs/build453', 'tools/build453']:
...
new.discard('docs/build453/patch-manifest.json')
entries = set(old) | new
...
manifest = {
    'build': 453, 'version': '3.1.132', 'baseline_build': 424,
    ...
    'updated_in_453': updated,
    'files': [{'path': p, 'bytes': len(b), 'sha256': hashlib.sha256(b).hexdigest()}
              for p, b in files.items()],
}
p = root / 'docs/build453/patch-manifest.json'
...
print(json.dumps({
    ...
    'preserved_452_files': len(old), 'frozen_runtime_preserved': True,
}))
