...
base=${1:?release178 baseline directory is required}
work=${2:-$(cd "$(dirname "$0")" && pwd)}
output=${3:-$(cd "$work/.." && pwd)/abyss-dominion-remake-main-179-patch-from-178.zip}
expected_tree="b666b4d0555807fc56136ae086c1c9fbdfeb8e17f4b9d4c15cd01d9f3e812ad0"

if [[ ! -d "$base" ]]; then
  echo "release178 baseline directory not found: $base" >&2
  exit 1
fi
if [[ ! -d "$work" ]]; then
  echo "release179 working directory not found: $work" >&2
  exit 1
fi
...
if [[ "$actual_tree" != "$expected_tree" ]]; then
  echo "release178 baseline mismatch: $actual_tree" >&2
  exit 1
fi

payload_files=(
  "BUILD238_GUILD_REMINDERS_AND_SETTLEMENT_DURABILITY_UPDATE.md"
  "README.md"
  "README_179.txt"
  "README_PATCH_179.txt"
  "build_release179_patch.sh"
  "index.html"
  ...
  "online-server/src/SettlementJournal.js"
  "online-server/src/TradeCoordinator.js"
  "online-server/tests/build238-guild-plan-reminders.test.js"
  "online-server/tests/settlement-journal.test.js"
  "src/Styles/build238.css"
  "src/main.js"
  ...
  "tests/build238-guild-plan-day-of-ux-regression.mjs"
)

declare -A listed=()
for relative in "${payload_files[@]}"; do
  if [[ -z "$relative" || "$relative" = /* || "$relative" = .* || "$relative" == */../* || "$relative" == ../* || "$relative" == */.. ]]; then
    echo "unsafe release179 payload path: $relative" >&2
    exit 1
  fi
  if [[ -n "${listed[$relative]:-}" ]]; then
    echo "duplicate release179 payload path: $relative" >&2
    exit 1
  fi
  if is_excluded "$relative"; then
    echo "excluded path cannot be included in release179: $relative" >&2
    exit 1
  fi
  listed[$relative]=1
done
...
if ((${#unexpected[@]})); then
  echo "release179 has changed files missing from the explicit payload list:" >&2
  printf '  %s\n' "${unexpected[@]}" >&2
  exit 1
fi
if ((${#deleted[@]})); then
  echo "release179 contains deletions, but this patch supports replacement files only:" >&2
  printf '  %s\n' "${deleted[@]}" >&2
  exit 1
fi
...
while IFS= read -r relative; do
  ...
  if [[ ! -f "$source_file" ]]; then
    echo "release179 payload file is missing: $relative" >&2
    exit 1
  fi
  if [[ ! -s "$source_file" ]]; then
    echo "release179 payload file is empty: $relative" >&2
    exit 1
  fi
  if [[ -f "$base/$relative" ]] && cmp -s "$source_file" "$base/$relative"; then
    echo "release179 payload file is unchanged from release178: $relative" >&2
    exit 1
  fi
...
if find "$root" -type f -empty -print -quit | grep -q .; then
  echo "release179 patch contains an empty file" >&2
  exit 1
fi

echo "release179 patch payload:"
...
if [[ ! -f "$verify_root/PATCH_FILES.txt" || ! -f "$verify_root/PATCH_MANIFEST.sha256" ]]; then
  echo "release179 patch metadata is missing after extraction" >&2
  exit 1
fi
