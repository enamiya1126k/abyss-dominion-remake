...
test('actual server dispatcher allows authenticated background raids, rejects room entry, and persists a reconnect',()=>{
 ...
 try{
  ...
  const context={RoomStore,WorldRaidCoordinator430,http:{createServer:fn=>{health=fn;return fakeHttp;}},process:{env,on:()=>{}},WebSocketServer:FakeWss,WebSocket:{OPEN:1},URL,Date,JSON,Set,Map,console:{log:()=>{},warn:()=>{},error:()=>{}},setInterval:()=>({unref(){}}),clearInterval:()=>{},setTimeout:()=>({unref(){}}),clearTimeout:()=>{}};
  ...
  worldRaid.ledger.transact(s=>{s.current.hp=10;return {ok:true};});send(resumed,{...start,requestId:'dispatch-second-429'});const second=worldRaid.snapshot(hello.friendId).attempt;send(resumed,{type:'worldRaidAuto',attemptId:second.id,round:1,enabled:false});send(resumed,{type:'worldRaidAction',attemptId:second.id,round:1,actorId:hello.friendId,kind:'attack',enemyTargetId:second.raid.boss.id});
