...
test("build157 isolates active online play and keeps the composer persistent",async()=>{
 ...
 assert.match(client,/_syncImmersiveMode/);assert.match(client,/_setChatExpanded/);assert.match(client,/online-chat-keyboard-open/);assert.match(client,/if\(!this\.chatExpanded\)this\.chatUnread/);assert.match(index,/ASSET_BUILD = "build161"/);
});
...
test("exclusive weapon resonance is owner-only, canonical and server-authoritative",async()=>{
 ...
 assert.equal(signatureWeaponState(state,owner).status,"専用共鳴 1/6");assert.equal(signatureWeaponState(state,guest).status,"専用効果 未発動");
