...
test("build157 isolates active online play and keeps the composer persistent",async()=>{
 ...
 assert.match(client,/_syncImmersiveMode/);assert.match(client,/_setChatExpanded/);assert.match(client,/online-chat-keyboard-open/);assert.match(client,/if\(!this\.chatExpanded\)this\.chatUnread/);assert.match(index,/ASSET_BUILD = "build160"/);
});
