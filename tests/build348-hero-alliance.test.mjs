...
test('living count changes reduction immediately; shields cap at 40% and can fully absorb',()=>{
 const b=online(),u=b.players.myth_rion;assert.equal(H.mitigateHeroDamage(b,'ally',u,100),50);b.players.myth_yori.hp=0;assert.equal(H.mitigateHeroDamage(b,'ally',u,100),70);b.players.myth_hide.hp=0;assert.equal(H.mitigateHeroDamage(b,'ally',u,100),85);u.hp=u.maxHp;assert.equal(H.heroOverheal(b,'ally',u,999999,u.maxHp),40000);assert.equal(H.heroOverheal(b,'ally',u,100,u.maxHp),0);assert.equal(H.mitigateHeroDamage(b,'ally',u,100),0);assert.equal(u.heroShield348,39915);
});
...
test('browser resume keeps spent emergency response, shield, chains and action budget',()=>{
 ...
 data.enemies[0].heroLoadoutVersion348=2;data.enemies[0].heroShield348=Math.floor(data.enemies[0].maxHp*.18);data.enemies[0].hp=data.enemies[0].maxHp*.5;const beforeMp=data.enemies[0].currentMp/data.enemies[0].maxMp;assert.equal(c.resumeSavedBattle(),true);assert.ok(Math.abs(c.battle.enemies[0].hp/c.battle.enemies[0].maxHp-.5)<.001);assert.equal(c.battle.enemies[0].heroLoadoutVersion348,3);assert.ok(Math.abs(c.battle.enemies[0].heroShield348/c.battle.enemies[0].maxHp-.18)<.001);assert.ok(Math.abs(c.battle.enemies[0].currentMp/c.battle.enemies[0].maxMp-beforeMp)<.01);
});
