...
test('browser resume keeps spent emergency response, shield, chains and action budget',()=>{
 ...
 delete data.enemies[0].heroLoadoutVersion348;data.enemies[0].hp=data.enemies[0].maxHp*.5;const beforeMp=data.enemies[0].currentMp/data.enemies[0].maxMp;assert.equal(c.resumeSavedBattle(),true);assert.ok(Math.abs(c.battle.enemies[0].hp/c.battle.enemies[0].maxHp-.5)<.001);assert.equal(c.battle.enemies[0].heroLoadoutVersion348,1);assert.ok(Math.abs(c.battle.enemies[0].currentMp/c.battle.enemies[0].maxMp-beforeMp)<.01);
});
test('browser hero damage adapter uses actual enemy mitigation once and respects guards on allies',()=>{
 ...
 const c={battle:b,calculatedStats,maxMp,allLearnedSkills:H.heroAuthoredSkills,effectiveSkillMpCost,signatureResonance:u=>u.heroSignature348,heroId:H.heroId,mitigateHeroDamage:H.mitigateHeroDamage,tryHeroLastStand:H.tryHeroLastStand,applyEnemyDamage,enemyDamageMultiplier:()=>1,endgameIncomingDamageMultiplier:()=>1,magicCircleDamageMultiplier:()=>1,recordBattleDamage:()=>{},registerWeaponFinisher:()=>{},tryUnyielding:()=>{},recordBattleTaken:()=>{},handleMagicCircleDeath:()=>{},Object,Math,Number};vm.createContext(c);vm.runInContext(source.slice(from,to),c);const environment=c.heroBattleEnvironment348(),before=enemy.hp;environment.damage(enemy,'enemy',100,{source:party[0],element:'neutral',damageClass:'physical'});assert.equal(before-enemy.hp,100);const allyBefore=party[0].currentHp;environment.damage(party[0],'ally',50,{source:enemy,element:'neutral',damageClass:'physical'});assert.equal(allyBefore-party[0].currentHp,50);
});
