...
test('10 pre-clear home renderings are identical to Build375 aside from release label',{skip:!oldHome},()=>{
 ...
 for(const floor of [1,10,50,99,100])for(const lost of [false,true]){
  ...
  assert.equal(HomeScreen(structuredClone(s)).replaceAll('3.1.56','VERSION'),oldHome(structuredClone(s)).replaceAll('3.1.55','VERSION'),`floor ${floor}, defeat ${lost}`);
 }
});
test('campaign generation, hero stats, rewards, skills and existing online logic are unchanged',{skip:!oldHome},()=>{
 const strip=s=>s.replace(/\?v=[\w.\-]+/g,'?v=VERSION');
 for(const file of ['src/core/Campaign100System.js','src/core/CampaignDungeonLayoutSystem.js','src/core/CampaignHeroLoadoutSystem.js','src/core/CampaignEndgameBalance.js','src/core/CampaignRewardSystem.js','src/core/CampaignHeroEncounterSystem.js','src/battle/EnemyAI.js','src/battle/SkillSystem.js','src/online/OnlinePartyClient.js','src/ui/screens/ExploreScreen.js'])assert.equal(strip(readFileSync(new URL('../'+file,import.meta.url),'utf8')),strip(readFileSync(new URL(file,base),'utf8')),file);
});
