...
function testEndgameRules(){
  ...
  Math.random=()=>0;
  // 132 hotfix: exploration no longer pauses for random altar/contact screens.
  // Deep Abyss / Ten-God units enter ordinary encounters directly after floor 1000.
  assert.equal(shouldTriggerEmergency(state),false);
  assert.equal(state.endgame.emergency.pendingEncounter,null);
  assert.ok(createEmergencyEncounter(state).boss.id);
...
function testSaveMigration(){
  ...
  assert.equal(fresh.state.schemaVersion,52);
  ...
  assert.deepEqual(fresh.state.endgame.trials,{battle:1,loop:1,cleared:[],run:null,dailyKey:null,dailyAttempts:0});
  ...
  assert.equal(migrated.state.schemaVersion,52);
  ...
  assert.deepEqual(migrated.state.lastMigration.to,52);
  migrated.reset();
...
function testStaticReferences(){
  ...
  for(const modulePath of [
    ...
    assert.ok(main.includes(`${modulePath}?v=2.6.0`),`Stale release cache tag: ${modulePath}`);
  }
  const audio=fs.readFileSync(path.join(root,"src/core/AudioSystem.js"),"utf8");
  assert.match(audio,/main-bgm\.mp3/);
  assert.match(audio,/dungeon-bgm\.mp3/);
  assert.match(audio,/ten-gods-bgm\.mp3/);
