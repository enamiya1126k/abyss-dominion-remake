...
function testSaveMigration(){
  ...
  assert.equal(fresh.state.schemaVersion,45);
  ...
  assert.equal(migrated.state.schemaVersion,45);
  ...
  assert.deepEqual(migrated.state.lastMigration.to,45);
  migrated.reset();
...
function testStaticReferences(){
  ...
  for(const modulePath of [
    ...
    assert.ok(main.includes(`${modulePath}?v=2.1.0-release`),`Stale release cache tag: ${modulePath}`);
  }
  ...
  assert.match(index,/2\.1\.0-release/);
}
