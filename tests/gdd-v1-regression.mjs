...
function testSaveMigration(){
  ...
  assert.equal(fresh.state.schemaVersion,47);
  ...
  assert.deepEqual(fresh.state.endgame.trials,{battle:1,loop:1,cleared:[],run:null});
  ...
  assert.equal(migrated.state.schemaVersion,47);
  ...
  assert.deepEqual(migrated.state.lastMigration.to,47);
  migrated.reset();
...
function testStaticReferences(){
  ...
  for(const modulePath of [
    ...
    assert.ok(main.includes(`${modulePath}?v=2.3.0`),`Stale release cache tag: ${modulePath}`);
  }
