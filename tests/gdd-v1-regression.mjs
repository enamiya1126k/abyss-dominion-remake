...
function testSaveMigration(){
  ...
  assert.equal(fresh.state.schemaVersion,53);
  ...
  assert.equal(migrated.state.schemaVersion,53);
  ...
  assert.deepEqual(migrated.state.lastMigration.to,53);
  migrated.reset();
...
function testStaticReferences(){
  ...
  for(const modulePath of [
    ...
    assert.ok(main.includes(`${modulePath}?v=2.7.0`),`Stale release cache tag: ${modulePath}`);
  }
