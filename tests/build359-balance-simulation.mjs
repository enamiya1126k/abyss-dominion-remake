...
export function finaleFixture(seed=1,ids=HERO_ORDER,{legacyLR=false}={}){
 ...
 const enemies=ids.map(id=>{const e=applyCampaignHeroLoadout({id,speciesId:id,campaignHeroId:id,level:1000});e.name=id;e.effects=[];e.cooldowns={};e.skills=heroAuthoredSkills(e).map(s=>({...s,mp:e.heroSkillCosts348[s.id]}));return e;});
...
export function simulateFinale(seed=1,{ids=HERO_ORDER,basicOnly=false,legacyLR=false,maxRounds=35,trace=false}={}){
 const {store,battle:b,room}=finaleFixture(seed,ids,{legacyLR}),rows=[];let peakHeroHit=0,heroActions=0,ultimateCasts=0;
...
if(process.argv[1]?.endsWith('build359-balance-simulation.mjs')){
 const n=Number(process.env.BALANCE_SAMPLES)||40;
 for(const ids of [...HERO_ORDER.map(id=>[id]),HERO_ORDER]){const rows=Array.from({length:n},(_,i)=>simulateFinale(i+1,{ids}));console.log(JSON.stringify({ids,n,wins:rows.filter(r=>r.won).length,timeouts:rows.filter(r=>r.outcome==='timeout').length,meanRounds:rows.reduce((s,r)=>s+r.rounds,0)/n,ultimateCasts:rows.reduce((s,r)=>s+r.ultimateCasts,0),heroActions:rows.reduce((s,r)=>s+r.heroActions,0)/n}));}
}
