...
const RELEASE_QUERY = "?v=3.0.5-build305";
...
test("Build305 publishes one client release identity and loads its final CSS last", async () => {
  ...
  assert.ok(build305Index > build304Index, "Build305 CSS must load after Build304 CSS");
});

test("Build305 removes the superseded Build304 cache identity from active src imports", async () => {
  const currentEdges = [];
  for (const importer of await javascriptFiles()) {
    ...
    assert.doesNotMatch(
      source,
      /\?v=3\.0\.4-build304/,
      `${relative(SRC_ROOT, importer)} still contains the Build304 browser cache identity`,
    );
  ...
  assert.ok(currentEdges.length >= 70, `expected the Build305 dependency graph, checked only ${currentEdges.length} current edges`);
});

test("Build305 refreshes every incoming edge of each changed browser module", async () => {
...
test("Build305 online package metadata and live protocol agree on 1.16.0", async () => {
  ...
  assert.equal(packageLock.version, "1.16.0");
  assert.equal(packageLock.packages?.[""]?.version, "1.16.0");
  assert.match(client, /const ONLINE_PROTOCOL = "1\.16\.0";/);
  assert.match(server, /message\.protocol!=="1\.16\.0"/);
  assert.match(server, /type:"helloAck",protocol:"1\.16\.0"/);
