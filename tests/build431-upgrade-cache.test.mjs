...
function fixture({current=false,failRegistration=false}={}){
 const stored=new Map([['abyss-dominion-asset-version',current?'3.1.111-build432':'3.1.110-build431'],['save-record','{"pendingResult":true,"gutsUsed":true}']]),events=[],listeners=new Map(),scope='https://game.test/abyss/';
 const workers={controller:{scriptURL:scope+(current?'world-raid-offline432-sw.js':'world-raid-offline431-sw.js')},getRegistrations:async()=>[{scope,unregister:async()=>events.push('unregister-own')},{scope:'https://game.test/other/',unregister:async()=>events.push('unregister-other')}],register:async()=>{if(failRegistration)throw new Error('offline');events.push('register');},addEventListener:(type,fn)=>listeners.set(type,fn),removeEventListener:type=>listeners.delete(type)};
 ...
 vm.runInNewContext(source,context);return {stored,events,context,activate(){workers.controller={scriptURL:scope+'world-raid-offline432-sw.js'};events.push('activated');listeners.get('controllerchange')?.();}};
}
test('upgrade waits for the new worker, preserves saved results and warms code without reserving another ticket',async()=>{
 ...
 assert.ok(f.events.indexOf('activated')<f.events.indexOf('load-main'));assert.ok(f.events.includes('warm-boot'));assert.equal(f.stored.get('save-record'),'{"pendingResult":true,"gutsUsed":true}');assert.equal(f.stored.get('abyss-dominion-asset-version'),'3.1.111-build432');
