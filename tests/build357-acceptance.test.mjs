...
test('shield starts outside the HP bar; skill effect markup includes every description line',()=>{
 const s=fs.readFileSync(new URL('../src/ui/screens/BattleScreen.js',import.meta.url),'utf8'),start=s.indexOf('function shieldLabel'),end=s.indexOf('function hpBar',start),c={battleInteger:n=>n.toLocaleString('ja-JP')};vm.createContext(c);vm.runInContext(s.slice(start,end),c);const shield=c.shieldLabel({heroShield348:53893},100000);assert.match(shield,/battle-shield-gauge has-shield/);assert.match(shield,/width:53.893%/);assert.match(shield,/盾 53,893/);assert.match(c.shieldLabel({heroShield348:0},100000),/is-empty/);assert.doesNotMatch(s,/HP .*?盾/);
