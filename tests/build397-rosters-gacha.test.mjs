...
test('real summon button handler prevents double-click grants and permits retry after a failed save',()=>{
 ...
 const c={save,battle:null,app:{insertAdjacentHTML(){}},Modal:()=>'',topModal:()=>modal,bindGachaTabs397(){},chapterTwoGachaUnlocked397:G.chapterTwoGachaUnlocked397,chapterTwoGachaBody397,chapterTwoGachaRates397,drawChapterTwoGacha397:G.drawChapterTwoGacha397,showToast(){},showSummonResults:()=>shown++,chapterTwoCommit:f=>fail?{ok:false,saveFailed:true}:f()};
...
test('provided-rate UI lists all names, individual percentages and no-guarantee conditions; results return to this gacha',()=>{
 const html=chapterTwoGachaRates397();assert.equal((html.match(/<li>/g)??[]).length,70);assert.equal((html.match(/<details>/g)??[]).length,7);for(const ids of Object.values(G.CHAPTER_TWO_GACHA_POOLS397))for(const id of ids)assert.ok(html.includes(SPECIES[id].name));assert.ok(html.includes('0.1%'));assert.ok(html.includes('独立抽選'));assert.ok(html.includes('天井はありません'));assert.ok(html.includes('通常召喚の保証カウントには影響しません'));
