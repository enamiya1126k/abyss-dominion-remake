...
test('cache maps route every edited module to the new build while frozen raid runtime stays isolated',()=>{
 ...
 for(const path of ['src/main.js','src/primordial/Cycle426.js','src/ui/RoyalChamberField.js','src/ui/MotherDial425.js','src/ui/screens/BattleScreen.js','src/core/config.js']){const entries=Object.entries(map).filter(([k])=>k.split('?')[0]==='./'+path);assert.ok(entries.length);assert.ok(entries.every(([,v])=>v.endsWith(['src/main.js','src/core/config.js'].includes(path)?'3.1.116-build437':['src/ui/RoyalChamberField.js','src/ui/screens/BattleScreen.js'].includes(path)?'3.1.114-build435':'3.1.112-build433')));}
 assert.match(html,/world-raid-offline437-sw/);const assets=JSON.parse(fs.readFileSync('world-raid-offline433-assets.json','utf8'));assert.ok(assets.includes('./src/primordial/FieldHit433.js'));assert.ok(assets.includes('./src/Styles/build433-mother.css'));assert.ok(!Object.keys(map).some(k=>k.includes('/runtime430/')));
});
