...
test('finishing an action releases busy state before the next action; last action reaches endRound once',async()=>{
 for(const finished of [false,true]){let advanced=0,continued=0,ended=0;const b={busy:true,actionCommitted:true,party:[],enemies:[]};const ctx=vm.createContext({battle:b,endMotherAttack426(){},settleSingleDeaths410(){},flushSingleCues410:async()=>{},observePairImpact409(){},flushPairSynergyCues409:async()=>{},flushPairCounters390:async()=>false,flushUltimateEvents358:async()=>{},currentTurnEntry:()=>({type:'ally',id:'x'}),advanceQueue:()=>advanced++,queueFinished:()=>finished,endRound:()=>ended++,renderBattle(){},wait:async()=>{},continueBattleFlow:()=>{assert.equal(b.busy,false);continued++}});
