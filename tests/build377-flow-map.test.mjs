...
test('first-chapter ExploreScreen output and mutation match the old version at all 100 floors',()=>{
...
test('map shows visited rooms and adjacent unknown rooms without leaking distant facilities',()=>{
 ...
 assert.match(chapterTwoObjective(r).title,/0\/2/);r.defeated=['west'];assert.match(chapterTwoObjective(r).title,/1\/2/);r.defeated.push('east');assert.equal(chapterTwoObjective(r).title,'最深部へ向かう');r.room=5;assert.equal(chapterTwoObjective(r).title,'世界樹の残響を倒す');r.completed=true;assert.equal(chapterTwoObjective(r).title,'境界の森を解放した');
