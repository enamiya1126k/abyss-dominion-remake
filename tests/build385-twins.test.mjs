...
test('formation and battle show the partner requirement, active state and used quotas without losing overhead names',()=>{
 const b=state();assert.match(twinStatus385([b.party[0]]),/ロゼを編成で発動/);assert.match(twinStatus385(b.party),/共鳴中/);reserveTwin385(b,b.party[0]);assert.match(twinStatus385(b.party,{battle:b,compact:true}),/残り1\/2/);for(const p of TWIN_PAIRS385)assert.ok(twinCodex385(p.members[0]).includes(p.name));b.enemies=encounter('roam3_4');b.turnQueue=[];const html=BattleScreen(b,{},{});for(const e of b.enemies){const start=html.indexOf(`data-enemy-target="${e.id}"`),end=html.indexOf('side-unit-sprite',start);assert.ok(html.slice(start,end).includes(e.name));assert.ok(html.slice(start,end).includes('combat-rank-badge'));}assert.ok(html.includes('天穹・双翼聖歌'));
});
