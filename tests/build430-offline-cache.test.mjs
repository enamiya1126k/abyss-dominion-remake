...
test('service worker caches boot files, then serves modules with version queries and navigation offline',async()=>{
 ...
 let response;f.handlers.fetch({request:{method:'GET',url:'https://game.test/abyss/src/main.js?v=3.1.110-build431',mode:'cors'},respondWith:p=>response=p});assert.match(await(await response).text(),/src\/main.js/);
...
test('offline boot manifest includes every runtime dependency and only files that exist',()=>{
 const urls=JSON.parse(fs.readFileSync('world-raid-offline431-assets.json','utf8'));assert.ok(urls.includes('./index.html'));assert.ok(urls.includes('./src/worldRaid/WorldRaidClient430.js'));for(const u of urls)assert.ok(fs.existsSync(u),u);
