...
test('438 boot and offline assets include every new material and runtime module',()=>{
 const html=fs.readFileSync('index.html','utf8'),map=JSON.parse(html.match(/<script type="importmap">([\s\S]*?)<\/script>/)[1]).imports,assets=JSON.parse(fs.readFileSync('world-raid-offline438-assets.json'));
 for(const n of ['src/main.js','src/ui/screens/EquipmentScreen.js','src/online/OnlinePartyClient.js','src/worldRaid/WorldRaidSpeed438.js','src/worldRaid/WorldRaidClient432.js','src/worldRaid/WorldRaidReplay430.js']){assert.match(map['./'+n],/3\.1\.117-build438$/);assert.ok(assets.includes('./'+n));}
 for(const n of ['frame','button','archive','raid'])assert.ok(assets.includes('./assets/ui/build438/'+n+'.webp'));for(const n of assets)assert.ok(fs.existsSync(n),n);assert.ok(!Object.keys(map).some(k=>k.includes('/runtime430/')));assert.match(html,/world-raid-offline438-sw.js/);
});
