...
test("critical/down exploration and readable status details are wired to both modes",async()=>{
 ...
 for(const token of["battle-status-effect-entries","最終効果","発動元：","build159CriticalBlink","equipment-archetype-chip"])assert.ok(`${main}\n${styles}`.includes(token),token);
 assert.match(index,/ASSET_BUILD = "build159"/);
});
