...
test("探索の重い区画走査・ミニマップ・オンライン描画へ接続済み",async()=>{const main=await readFile(new URL("../src/main.js",import.meta.url),"utf8"),index=await readFile(new URL("../index.html",import.meta.url),"utf8");assert.match(main,/shouldPaintExploreFrame\(game,now/);assert.match(main,/performanceProfile\.pixelRatio/);assert.match(main,/lastMiniMapPaintAt/);assert.match(main,/for\(let y=bounds\.minY;y<=bounds\.maxY;y\+\+\)/);assert.match(main,/game\.performanceProfile\?\.constrained/);assert.match(index,/ASSET_BUILD = "build316"/)});
