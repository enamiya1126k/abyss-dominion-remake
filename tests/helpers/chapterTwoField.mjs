...
export function fieldFixture(room=0,{area=0,onContact=null}={}){
 const s=fresh();chapterTwoState(s).introComplete=true;beginChapterTwoRun(s);const run=s.chapterTwo376.run;run.room=room;run.area=area;run.visited=[0,room];run.position={...chapterTwoLayout(run.serial,area).sections[room].center};
 ...
 const g={};context.game=g;mountChapterTwoField(g,{canvas,Entity:context.Entity,Camera:context.Camera,findPath:context.path,drawScene:()=>draws++,bindInput:context.bindInput,updateTrail:()=>{},TILE:88,run,onSave(){},onContact:o=>{contacts.push(o);if(onContact)onContact(o,{s,g,run,context});else g.paused=true}});
 let now=performance.now();return {s,g,run,context,contacts,canvas,listeners,get draws(){return draws},tick(n){for(let i=0;i<n;i++){now+=40;callback(now)}},tap(x,y){const q=g.camera.world((x+.5)*88,(y+.5)*88),e={pointerId:1,clientX:q.x,clientY:q.y};canvas.onpointerdown(e);canvas.onpointerup(e)},cleanup(){g.disposeChapterTwo();for(const [k,v]of Object.entries(old))if(v===undefined)delete globalThis[k];else globalThis[k]=v;}};
}
