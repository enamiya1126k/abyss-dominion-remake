...
assert.match(main,/claimOnlinePartyReward/);assert.match(main,/claimedRewards/);assert.match(save,/onlineParty:\{claimedRewards:\[\]/);assert.match(css,/\.online-coop-lobby/);assert.match(css,/\.online-dungeon-board/);assert.match(css,/\.online-expedition-event/);assert.match(index,/ASSET_BUILD = "build14[67]"/);

const dungeon=createSharedDungeon({roomId:"ABC234",floor:245,runId:"TEST"});assert.equal(dungeon.cols,17);assert.equal(dungeon.rows,17);assert.equal(dungeon.tiles.length,17);assert.equal(dungeon.objects.filter(object=>object.type==="chest").length,3);assert.equal(dungeon.objects.filter(object=>object.type==="bone").length,2);assert.equal(dungeon.objects.filter(object=>object.type==="shrine").length,1);assert.equal(dungeon.objects.filter(object=>object.type==="exit").length,1);for(const object of dungeon.objects)assert.equal(dungeon.tiles[object.y][object.x],".");
