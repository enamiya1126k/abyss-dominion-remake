...
assert.match(main,/claimOnlinePartyReward/);assert.match(main,/claimedRewards/);assert.match(save,/onlineParty:\{claimedRewards:\[\]/);assert.match(css,/\.online-coop-lobby/);assert.match(css,/\.online-dungeon-board/);assert.match(css,/\.online-expedition-event/);assert.match(index,/ASSET_BUILD = "build(?:14[6-9]|15[0-7])"/);
