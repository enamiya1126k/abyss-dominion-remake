...
import {mainFixture550} from '../tools/build556/main-fixture.mjs';
const {RaceCoordinator451}=await mainFixture550();
test('main coordinator advertises cart v6, opens a random-course match and forwards authorized shots',()=>{
 let now=0;const session={playerId:'p0',clientKey:'fixture-key',connected:true,profile:{displayName:'あなた'}},sent=[],c=new RaceCoordinator451({sessions:new Map([['p0',session]]),now:()=>now,send:(id,m)=>sent.push(m)}),raw=(op,p={})=>c.handle(session,{op,rulesVersion:8,cartVersion543:6,ricochetVersion550:6,roster:[{id:'m',speciesId:'slime'}],slotOne476:'m',...p});
 raw('partyCreate462',{game:'cart'});let v=c.view(session);assert.equal(v.cartVersion543,6);assert.equal(v.cart.game,'cart');assert.equal(v.room,null);
