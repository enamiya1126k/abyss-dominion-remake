...
assert.equal(campaignEndingForResult({partyWon:true}),"complete");assert.equal(campaignEndingForResult({sairanWon:true}),"defeat");assert.equal(campaignEndingForResult({won:true,partySurvivors:2}),"comeback");assert.equal(campaignEndingForResult({}),"defeat");
