...
base=${1:?release176 baseline directory is required}
work=${2:-$(cd "$(dirname "$0")" && pwd)}
output=${3:-$(cd "$work/.." && pwd)/abyss-dominion-remake-main-177-patch-from-176.zip}
expected_tree="d237f0d6800bf872245f129f424ed8727278171d906afc2fbf850f3aaa6d0145"

if [[ ! -d "$base" ]]; then
  echo "release176 baseline directory not found: $base" >&2
  exit 1
fi
if [[ ! -d "$work" ]]; then
  echo "release177 working directory not found: $work" >&2
  exit 1
fi
...
if [[ "$actual_tree" != "$expected_tree" ]]; then
  echo "release176 baseline mismatch: $actual_tree" >&2
  exit 1
fi

payload_files=(
  "BUILD236_GUILD_PLANS_UPDATE.md"
  "README.md"
  "README_177.txt"
  "README_PATCH_177.txt"
  "build_release177_patch.sh"
  "index.html"
  "online-server/server.js"
  "online-server/src/GuildCoordinator.js"
  "online-server/src/RoomStore.js"
  "online-server/tests/build233-guilds.test.js"
  "online-server/tests/build235-guild-activity-history.test.js"
  "online-server/tests/build236-guild-plans.test.js"
  "src/Styles/build236.css"
  "src/main.js"
...
for relative in "${payload_files[@]}"; do
  if [[ -n "${listed[$relative]:-}" ]]; then
    echo "duplicate release177 payload path: $relative" >&2
    exit 1
  fi
  if is_excluded "$relative"; then
    echo "excluded path cannot be included in release177: $relative" >&2
    exit 1
  fi
  listed[$relative]=1
done
...
if ((${#unexpected[@]})); then
  echo "release177 has changed files missing from the explicit payload list:" >&2
  printf '  %s\n' "${unexpected[@]}" >&2
  exit 1
fi
if ((${#deleted[@]})); then
  echo "release177 contains deletions, but this patch supports replacement files only:" >&2
  printf '  %s\n' "${deleted[@]}" >&2
  exit 1
fi
...
while IFS= read -r relative; do
  ...
  if [[ ! -f "$source_file" ]]; then
    echo "release177 payload file is missing: $relative" >&2
    exit 1
  fi
  if [[ ! -s "$source_file" ]]; then
    echo "release177 payload file is empty: $relative" >&2
    exit 1
  fi
  if [[ -f "$base/$relative" ]] && cmp -s "$source_file" "$base/$relative"; then
    echo "release177 payload file is unchanged from release176: $relative" >&2
    exit 1
  fi
...
if find "$root" -type f -empty -print -quit | grep -q .; then
  echo "release177 patch contains an empty file" >&2
  exit 1
fi

echo "release177 patch payload:"
...
echo "release177 patch verified:"
sha256sum "$output"
