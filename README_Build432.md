...
1. ゲーム公開側をこの変更を含むコミットへ更新します。
2. サーバー側も `online-server/` と、その隣の `src/` を同じコミットにそろえます。追加の本番依存パッケージはありません。
3. 既存と同じ作業ディレクトリ・設定・保存先でサーバーを再起動します。`data/` や環境設定は上書き・削除しません。
4. ゲームを再読み込みし、表示が `v3.1.111` になったことを確認します。ルートの `world-raid-offline432-sw.js` と `world-raid-offline432-assets.json` も更新対象です。
