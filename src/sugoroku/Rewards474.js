...
export function checkpoint474(g,p,node){
 ...
 p.crystalStops474??=[];if(p.crystalStops474.includes(node))return 0;p.crystalStops474.push(node);initCrystals477(g);const amount=Math.min(checkpointAmount474(g.economy474?.fee??500),Math.max(0,crystalCap477(g)-p.crystals474));p.crystals474+=amount;p.crystalShrines477+=amount;return amount;
}
