...
function snapshot466(g){return{status465:(g.players??[]).map(p=>({playerId:p.playerId,pos:p.pos,handCount:p.hand.length,crystals474:p.crystals474??0,special:p.special,skip:p.skip,finished:p.finished})),turnPlayerId466:current463(g)?.playerId,turnNumber466:g.turnNumber,step466:g.step,private466:Object.fromEntries((g.players??[]).map(p=>[p.playerId,{hand:p.hand.map(uid=>({uid,cardId:g.cards[uid],ward:uid===p.ward,playable:false})),score:score463(g,p)}]))}}
...
function executeBody466(g,e){
 ...
 if(!e.presented464&&!['draw','drawOne','roll','move','direct','special','shrine469','piece','crystal477'].includes(e.type)){present464(g,'effect',{actorId:e.actor,targetIds:[p.playerId],cardId:e.cardId??null,effectType:e.type,n:e.type==='draw'?(e.n??1)*multiplier(p,e):e.n??null,source:e.source,specialId466:e.source==='special'?player463(g,e.actor)?.special:null,tile:e.tile??null,harmful:!!e.harmful});e.presented464=true}
