import{greedMultiplier528,greedLeft528,ensureGreed528,markGreedTurn528,setSpecial528,finishGreedTurn528}from'./Greed528.js';
import{gameColors501,freezeColors501}from'../party/GameColors501.js';
...
function snapshot466(g){return{status465:(g.players??[]).map(p=>({playerId:p.playerId,pos:p.pos,handCount:p.hand.length,crystals474:p.crystals474??0,special:p.special,greedTurns528:greedLeft528(p),visited469:[...(p.visited469??[])],skip:p.skip,finished:p.finished})),turnPlayerId466:current463(g)?.playerId,turnNumber466:g.turnNumber,step466:g.step,private466:Object.fromEntries((g.players??[]).map(p=>[p.playerId,{hand:p.hand.map(uid=>({uid,cardId:g.cards[uid],ward:uid===p.ward,playable:false})),score:score463(g,p)}]))}}
...
const multiplier=greedMultiplier528;
export function makeLobby463({id,code,partyId,hostId,members,now=0,seed=1}){return{id,code,game:'sugoroku',partyId462:partyId,hostId,rulesVersion:19,rules469:RULES469.id,phase:'lobby',createdAt:now,updatedAt:now,seed:seed||1,members:members.map(m=>({...m,choice:null,ai:false})),players:[],revision:0,seen:{},log:[],eventId:0,effectId:0,choiceId:0,deadline:0}}
export function start463(g,now=0){
 if(g.phase!=='lobby')fail('すでに開始しています');
 g.rules469=RULES469.id;g.rulesVersion=19;g.presentation464=[];g.presentationSequence464=0;g.cards={};g.deck=[];g.discard=[];g.specialDeck=shuffle463(g,SPECIALS463.map(s=>s.id));g.specialDiscard=[];g.queue=[];g.pending=null;g.turnNumber=1;g.turn=0;g.extraChain=0;g.finishOrder=[];g.blocked={};g.step='draw';g.usedSet={};g.startedAt=now;
...
function safeSpecial(g,p,id,remaining=2){if(!id)return;if(p.special)ask(g,p.playerId,'special','残す特殊カードを選ぶ',[{value:p.special,label:special(p).name,special:p.special},{value:id,label:SPECIAL_BY_ID463[id].name,special:id}],{old:p.special,next:id,nextGreed528:remaining});else{setSpecial528(g,p,id,remaining);awaken466(g,p);log463(g,`${p.name}が「${SPECIAL_BY_ID463[id].name}」を獲得！`)}}
function takeSpecial(g,p,e={}){if(!g.specialDeck.length)g.specialDeck=shuffle463(g,g.specialDiscard.splice(0));const id=g.specialDeck.pop();if(id){present464(g,'specialDraw470',{actorId:p.playerId,specialId466:id,replacing470:!!p.special,tile:e.tile??null,source:e.source??null,sourceCardId466:e.cardId??null});safeSpecial(g,p,id)}else log463(g,'特殊カードの山札は空だった')}
function loseSpecial(g,p){if(p.special){log463(g,`${p.name}は「${special(p).name}」を失った`);g.specialDiscard.push(p.special);p.special=null;p.ward=null;delete p.greedTurns528;delete p.greedActiveTurn528}}
...
function goal(g,p){
 ...
 finishRewards474(g);g.phase='result';g.queue=[];g.pending=null;g.endedAt=g.updatedAt;log463(g,`${p.name}が先着ゴール！ 優勝！`);
}
function landing(g,p,e){if(p.pos===GOAL463){goal(g,p);return}if(!e.event||p.finished)return;const node=NODES463[p.pos];const gems474=checkpoint474(g,p,node.id);if(gems474){present464(g,'crystal474',{actorId:p.playerId,n:gems474,tile:node.id,total474:p.crystals474});log463(g,`${p.name}が💎${gems474}獲得（終了時に精算）`)}if(p.special==='power'&&node.tone==='black'){log463(g,`${p.name}は権力で黒マスを無効化`);return}log463(g,`${p.name}が「${node.name}」に止まった`);if(node.crystalMode477){p.crystalVisits477??=[];if(p.crystalVisits477.includes(node.id)){log463(g,'この💎マスは獲得・処理済み');return}p.crystalVisits477.push(node.id);}if(node.landmark469||node.kind==='special')present464(g,'arrival472',{actorId:p.playerId,tile:node.id,source:'tile',forcedStop472:!!e.forcedStop472,hasSpecial472:!!p.special});const m=meta(g,p.playerId,null,'tile');add(g,node.effects.map(fx=>({...fx,...m,target:fx.target??p.playerId,tile:node.id,...(fx.type==='move'?{event:false}:{})})),true)}
function move(g,p,e){
 if(p.finished)return;let n=Math.trunc((e.n??0)*multiplier(p,e));if(!n)return;
 ...
 while(n>0&&!p.finished){const node=NODES463[p.pos];if(!node.next.length)break;
  if(node.next.length>1&&!e.path){movementPresentation464(g,p,origin,route465);ask(g,p.playerId,'fork','近道か、宝庫への寄り道か',[{value:node.next[0],label:'近道を進む',note:'ゴールへ最短の道'},{value:node.next[1],label:'宝庫へ寄り道',note:'4マス長い。補充・特殊カードのチャンス'}],{effect:{...e,n,noGreed:true}});return}
  ...
  if(p.pos===GOAL463){reachedGoal=true;break}if(NODES463[p.pos].kind==='gate'&&!p.visited469?.includes(p.pos)){p.visited469??=[];p.visited469.push(p.pos);e.event=true;e.forcedStop472=true;break}
 }
...
function defend(g,p,e){
 ...
 if(p.special==='tsundere'&&e.actor!==p.playerId&&!e.area&&!e.reflected&&p.reflectTurn!==g.turnNumber)options.push({value:'passive-reflect',label:'ツンデレで跳ね返す',special:'tsundere'});
...
function expandTarget(g,e){
 ...
 if(e.target==='choose'||e.target==='chooseAny'){
  let choices=active(g).filter(p=>e.target==='chooseAny'||p.playerId!==e.actor);if(['steal','swapHand','discard'].includes(e.type))choices=choices.filter(p=>p.hand.length);if(e.type==='crystal477'&&e.mode==='steal')choices=choices.filter(p=>(p.crystals474??0)>0);if(e.type==='crystal477'&&e.mode==='steal'&&!choices.length)log463(g,'奪える💎を持つ相手はいなかった');if(['stealSpecial','loseSpecial'].includes(e.type))choices=choices.filter(p=>p.special);
  if(!choices.length)return true;
  ask(g,e.actor,'target','対象のプレイヤーを選ぶ',choices.map(p=>({value:p.playerId,label:p.name,note:e.type==='crystal477'?`対戦用 💎${p.crystals474??0}`:`残り${distance463(p.pos)}マス・手札${p.hand.length}枚`})),{effect:e});return true;
 }return false;
}
...
function executeBody466(g,e){
 ...
 if(!e.presented464&&!['draw','drawOne','roll','move','direct','special','shrine469','piece','crystal477'].includes(e.type)){present464(g,'effect',{actorId:e.actor,targetIds:[p.playerId],cardId:e.cardId??null,effectType:e.type,n:e.n!=null?e.n*multiplier(e.type==='steal'?player463(g,e.actor):p,e):null,source:e.source,specialId466:e.source==='special'?player463(g,e.actor)?.special:null,tile:e.tile??null,harmful:!!e.harmful});e.presented464=true}
 ...
 case'bonus':{const n=e.n*multiplier(p,e);p.mods.bonus+=n;log463(g,`${p.name}の合計出目${n<0?'−':'＋'}${Math.abs(n)}`);break}
 case'dice':p.mods.dice=Math.min(5,p.mods.dice+e.n*multiplier(p,e));log463(g,`${p.name}のサイコロが${p.mods.dice}個に`);break;
 case'resonance':{const n=(p.hand.some(uid=>hasAttr463(definition463(g,uid),e.attr))?4:2)*multiplier(p,e);p.mods.bonus+=n;log463(g,`${p.name}の共鳴で出目＋${n}`);break}
 ...
 case'shrine469':if(!p.special)takeSpecial(g,p,e);else add(g,[{...e,type:'draw',n:1,mode:'safe',noGreed:true}],true);break;
 ...
 case'roll':{let count=Math.min(5,p.mods.dice);p.buddhaReady=false;const dice=Array.from({length:count},()=>rng463(g,6)+1),transformed=p.special==='worker'?dice.map(n=>n>=4?1:n):dice;const rawTotal528=transformed.reduce((a,b)=>a+b,0),rollMultiplier528=multiplier(p,e);let n=rawTotal528*rollMultiplier528+p.mods.bonus;
 if(p.special==='genius')n=Math.max(7,transformed.reduce((a,b)=>a+b,0))+p.mods.bonus;if(p.hand.some(uid=>definition463(g,uid)?.curse==='heavy'))n-=2;n+=Math.max(0,Math.floor((g.turnNumber-48)/12));n=Math.max(1,n);g.lastRoll={eventId:g.eventId+1,playerId:p.playerId,dice,transformed,total:n,rawTotal528,greed528:rollMultiplier528===2,bonus528:p.mods.bonus,genius:p.special==='genius'};present464(g,'dice',{actorId:p.playerId,dice,transformed,total:n,rawTotal528,greed528:rollMultiplier528===2,bonus528:p.mods.bonus,genius:p.special==='genius'});log463(g,`${p.name}：${n}マス進む`);move(g,p,{...e,type:'move',n,event:true,noGreed:true});break}
 ...
 case'discardAttr':drop(g,p,shuffle463(g,p.hand.filter(uid=>hasAttr463(definition463(g,uid),e.attr))).slice(0,(e.n??2)*multiplier(p,e)));break;
 case'cleanse':drop(g,p,p.hand.filter(uid=>definition463(g,uid)?.kind==='curse'));log463(g,`${p.name}の呪いを浄化`);break;
 case'catastrophe':drop(g,p,shuffle463(g,[...p.hand]).slice(0,(e.n??2)*multiplier(p,e)));break;
 ...
 case'swapHand':{if(actor.finished)return;const n=Math.min(e.n*Math.max(multiplier(actor,e),multiplier(p,e)),actor.hand.length,p.hand.length);const fromA=shuffle463(g,[...actor.hand]).slice(0,n),fromB=shuffle463(g,[...p.hand]).slice(0,n);actor.hand=actor.hand.filter(uid=>!fromA.includes(uid));p.hand=p.hand.filter(uid=>!fromB.includes(uid));for(const uid of fromA){if(actor.ward===uid)actor.ward=null;receiveCard(g,p,uid,'normal',{...e,fromPlayerId466:actor.playerId,ordinal466:fromA.indexOf(uid)+1,total466:n})}for(const uid of fromB){if(p.ward===uid)p.ward=null;receiveCard(g,actor,uid,'normal',{...e,fromPlayerId466:p.playerId,ordinal466:fromB.indexOf(uid)+1,total466:n})}log463(g,`${actor.name}と${p.name}が手札${n}枚を交換`);break}
 case'special':takeSpecial(g,p,e);break;
 case'loseSpecial':loseSpecial(g,p);break;
 case'swapSpecial':if(!actor.finished){const a=actor.special,b=p.special,ac=greedLeft528(actor),bc=greedLeft528(p);setSpecial528(g,actor,b,bc);setSpecial528(g,p,a,ac);awaken466(g,actor);awaken466(g,p);log463(g,`${actor.name}と${p.name}が特殊カードを交換`)}break;
 case'stealSpecial':if(p.special&&!actor.finished){const s=p.special,remaining=greedLeft528(p);setSpecial528(g,p,null);log463(g,`${actor.name}が${p.name}の特殊カードを奪った`);safeSpecial(g,actor,s,remaining)}break;
 case'skip':if(p.special!=='worker'){p.skip=Math.min(2,p.skip+e.n*multiplier(p,e));log463(g,`${p.name}は${p.skip}回休み`)}else log463(g,`${p.name}は休みを無効化`);break;
 ...
 case'recover':{const n=Math.min(e.n*multiplier(p,e),g.discard.length);if(n)ask(g,p.playerId,'recover',`捨て札から${n}枚回収できる`,g.discard.map(uid=>({value:uid,label:definition463(g,uid).name,cardId:g.cards[uid]})),{remaining:n});break}
...
function nextTurn(g){if(g.phase!=='playing')return;const last=current463(g);if(finishGreedTurn528(g,last)){log463(g,`${last.name}の強欲は2手番を終えた`);loseSpecial(g,last)}if(last?.extraTurn&&!last.finished&&!g.extraChain){last.extraTurn=false;g.extraChain=1}else{g.extraChain=0;do{g.turn=(g.turn+1)%4}while(current463(g).finished)}g.turnNumber++;g.step='draw';g.blocked={};const p=current463(g);p.mods={bonus:0,dice:2};p.boostAttr=null;p.grudgeUsed=false;p.specialUsed=false;p.cardsUsed469=0;p.playedTurn={};log463(g,`${p.name}の手番${g.extraChain?'（追加）':''}`)}
...
function resolveChoice(g,p,value){const q=g.pending;if(!q||q.playerId!==p.playerId)fail('今はあなたの選択ではありません');if(Array.isArray(value)){if(q.kind!=='discard'||value.length!==Math.min(q.data.remaining,p.hand.length)||new Set(value).size!==value.length||!value.every(v=>typeof v==='string'&&q.options.some(o=>o.value===v)&&p.hand.includes(v)))fail('必要な枚数の手札を選んでください')}else if(!q.options.some(o=>o.value===value))fail('選択肢を選んでください');g.pending=null;const d=q.data,e=d.effect;
 ...
 case'special':{const unused=value===d.old?d.next:d.old;if(unused)g.specialDiscard.push(unused);if(value!==d.old){setSpecial528(g,p,value,d.nextGreed528??2);awaken466(g,p)}log463(g,`${p.name}は「${special(p).name}」を選んだ`);break}
 ...
 case'defense':{
  ...
  const reflected=!!(reflect&&e.actor!==p.playerId&&(e.source!=='tile'||e.type==='crystal477'&&e.mode==='steal')&&e.source!=='instant'&&!e.reflected);
  ...
  if(reflect&&e.actor!==p.playerId&&(e.source!=='tile'||e.type==='crystal477'&&e.mode==='steal')&&e.source!=='instant'&&!e.reflected){const group=meta(g,p.playerId,null).group;add(g,[e,...remaining].map(x=>({...x,actor:p.playerId,target:e.actor,group,reflected:true,checked:false,presented464:false})),true);log463(g,`${p.name}が効果を跳ね返した！`)}else log463(g,`${p.name}が効果を無効化した！`);break}
...
export function action463(g,playerId,m,now=0){
 if(g.phase!=='playing')fail('ゲームは進行中ではありません');const p=player463(g,playerId);if(!p||p.finished)fail('観戦中は操作できません');g.updatedAt=now;initCrystals477(g);for(const owner of g.players)ensureGreed528(owner);
 ...
 if(m.kind==='draw'){
  ...
  g.step='pre';markGreedTurn528(g,p);const base=meta(g,p.playerId,null,'turn');const fx=[];
...
export function botAction463(g,playerId){
 ...
 if(q){if(q.playerId!==playerId)return null;let value=q.options[0]?.value;
  ...
  if(q.kind==='target')value=[...q.options].sort((a,b)=>q.data?.effect?.type==='crystal477'?(player463(g,b.value)?.crystals474??0)-(player463(g,a.value)?.crystals474??0):distance463(player463(g,a.value)?.pos)-distance463(player463(g,b.value)?.pos))[0].value;
