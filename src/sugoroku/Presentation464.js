...
function write464(c,f,art,token){
 const layer=c.root?.querySelector('.sg-presentation464');if(!layer)return;const viewport=c.root.querySelector('.sg-viewport'),game=c.root.querySelector('.sg-game'),full=!!f.active&&f.active.event.kind!=='move'&&viewport?.clientHeight<280;if(full){game.append(layer);layer.style.setProperty('--sg-stage-top471',(viewport.getBoundingClientRect().top-game.getBoundingClientRect().top)+'px')}else if(viewport&&layer.parentElement!==viewport)viewport.append(layer);layer.classList.toggle('sg-stage-overlay471',full);
 ...
 const height=Math.max(160,layer.clientHeight||layer.parentElement?.clientHeight||300),cardHeight=Math.max(180,Math.min(310,height-70));
 layer.style.setProperty('--sg-stage-h',height+'px');layer.style.setProperty('--card-h465',cardHeight+'px');layer.style.setProperty('--card-w465',Math.round(cardHeight*.68)+'px');layer.style.setProperty('--fx-age',`-${age}ms`);layer.dataset.kind=e.kind;layer.classList.toggle('is-compact470',height<340);layer.style.setProperty('--event-duration469',`${f.active.duration}ms`);layer.classList.toggle('is-dark-instant467',e.kind==='card'&&card?.kind==='bad'&&e.activation466==='instant');layer.classList.toggle('sg-motion-reduced',f.reduced);
