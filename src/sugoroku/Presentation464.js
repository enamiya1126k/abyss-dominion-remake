import{signed477}from'./Interface477.js';
...
import{duration465,PRESENTATION_SCALE481}from'./Pacing465.js';
const escape=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
export function tileLabel464(n){if(n.label477)return n.label477;if(n.landmark469)return n.name;return({start:'START',goal:'GOAL',draw:'2枚引く',safe:'安全に2枚',move:'3マス進む',back:'2マス戻る',skip:'1回休み',trade:'1枚交換',discard:'2枚捨てる',special:'特殊獲得',lose:'特殊を失う',steal:'1枚奪う',rest:'1枚引く',cleanse:'呪いを解除',wager:'1枚捨て3枚',attribute:'属性札−2枚',gate:'神殿で絵柄'})[n.kind]??n.name}
...
function queueCompleted468(f,hand){const completed=completedSeries468(f.seriesHand468??hand,hand);f.seriesHand468=hand.map(h=>({...h}));for(const set468 of completed.reverse())f.queue.unshift({kind:'series468',set468});}
...
function write464(c,f,art,token){
 ...
 const height=Math.max(80,layer.clientHeight||layer.parentElement?.clientHeight||300),width=Math.max(180,layer.clientWidth||layer.parentElement?.clientWidth||360),cardHeight=Math.max(60,Math.min(310,height-44,(width-112)*88/63));
 // Fixed-duration subanimations follow the authoritative event clock, including replayed events.
 layer.style.setProperty('--fx-scale481',String(f.active.duration*PRESENTATION_SCALE481/duration465(e)));
 layer.style.setProperty('--sg-stage-h',height+'px');layer.style.setProperty('--card-h465',cardHeight+'px');layer.style.setProperty('--card-w465',Math.round(cardHeight*63/88)+'px');layer.style.setProperty('--fx-age',`-${age}ms`);layer.dataset.kind=e.kind;layer.classList.toggle('is-other-draw472',e.kind==='draw'&&e.actorId!==c.transport.selfId);layer.classList.toggle('is-short472',height<280);layer.classList.toggle('is-compact470',height<340);layer.style.setProperty('--event-duration469',`${f.active.duration}ms`);layer.classList.toggle('is-dark-instant467',e.kind==='card'&&card?.kind==='bad'&&e.activation466==='instant');layer.classList.toggle('sg-motion-reduced',f.reduced);
 if(e.kind==='card'&&card){layer.innerHTML=`${card.kind==='bad'&&e.activation466==='instant'?'<div class="sg-dark-title467"><small>引いてしまった…！</small><b>闇の速攻</b><span>強制発動</span></div>':''}<div class="sg-cast465"><div class="sg-caster465">${pawn465(c,actor,token,'is-source')}<small>${e.activation466==='instant'?'速攻カードを引いた！<br>強制発動':e.activation466==='blocked'?'悪い速攻を引いた<br>無効化！':'このカードを使用'}</small><button data-sg-action="card" data-card="${card.id}" class="sg-fx-detail469">効果を読む</button></div><div class="sg-flip464"><div class="sg-flip-inner464">${back466()}<div class="sg-front464 ${cardClass476(card)}" style="${cardStyle476(card)}">${cardBody476(card,art)}</div></div></div>${e.activation466==='blocked'?'<div class="sg-safe-block469"><b>無効化！</b><span>闇の速攻を防いだ</span></div>':''}<div class="sg-event-progress465"></div></div>`}
  ...
  else layer.innerHTML=`<div class="sg-draw470 ${own?'is-own':'is-hidden'}"><div class="sg-draw-title470"><small>${escape(from?name465(c,from)+'の手札から':source)}</small><b>${escape(name465(c,actor))}が1枚獲得 <em>${e.ordinal466??1} / ${e.total466??1}</em></b></div><div class="sg-draw-layout470"><div class="sg-draw-table470">${deck469()}<div class="sg-draw-card466"><div class="sg-draw-flipper466">${back466()}${drawn?`<div class="sg-front464 ${cardClass476(drawn)}" style="${cardStyle476(drawn)}">${cardBody476(drawn,art)}</div>`:back466()}</div></div></div>${drawn?`<div class="sg-draw-cue470"><span class="sg-cue-symbol470 cue-${cue.key}">${effectIcon470(cue.key)}</span><small>${escape(cue.category)}</small><strong>${escape(drawn.summary466)}</strong><span>${escape(cue.timing)}</span><button data-sg-action="card" data-card="${drawn.id}">詳しく読む</button></div>`:'<span class="sg-hidden-cue470">中身は本人だけに表示</span>'}</div><div class="sg-reading470"><span></span></div><small class="sg-draw-foot470">${drawn?'獲得後も「直近の獲得」で読み返せます':'相手の手札へ'}</small></div>`;
 }
 else if(e.kind==='crystal477'){layer.innerHTML=`<div class="sg-crystal-event477 ${['half','lose'].includes(e.mode477)?'is-loss':''}"><small>${escape(NODES463[e.tile]?.name??'魔晶石の運命')}</small><b>${({gain:'魔晶石を獲得',steal:'魔晶石を略奪！',double:'獲得分が2倍！',half:'獲得分が半減…',lose:'獲得分を失った…'})[e.mode477]}</b>${e.changes477.map(x=>`<div><span>${escape(name465(c,g.players.find(p=>p.playerId===x.playerId)))}</span><strong class="${x.delta<0?'is-loss':'is-profit'}">💎 ${signed477(x.delta)}</strong><small>獲得分 ${x.total.toLocaleString()} 💎</small></div>`).join('')}<small>${g.economy474?.mode==='crystal'?'終了時に順位賞金と合計して精算':'無料練習 · 仮想の💎'}</small></div>`;}
 else if(e.kind==='crystal474'){layer.innerHTML=`<div class="sg-crystal-arrival474"><small>${escape(name465(c,actor))} · 神殿の宝石</small><b>💎 +${e.n.toLocaleString()}</b><span>道中の獲得分 ${e.total474.toLocaleString()} 💎</span><small>${g.economy474?.mode==='crystal'?'終了時に本編の所持💎へ加算':'無料練習 · 仮想の💎'}</small></div>`;}
  ...
  const defender=CARD_BY_ID463[e.cardId];layer.innerHTML=`<div class="sg-defense469"><div class="sg-defense-face469">${defender?`<div class="sg-flip464"><div class="sg-flip-inner464">${back466()}<div class="sg-front464 ${cardClass476(defender)}" style="${cardStyle476(defender)}">${cardBody476(defender,art)}</div></div></div>`:pawn465(c,actor,token)}</div><div class="sg-defense-outcome469"><span class="sg-shield469">${e.reflected469?'↩':'✦'}</span><small>${escape(name465(c,actor))}</small><b>${e.reflected469?'反射成功！':'防御成功！'}</b><strong>${escape(defenseResult469(e))}</strong><span>この攻撃は通らない</span></div></div>`;
 }
 else if(e.kind==='dice'){const sum=e.dice.reduce((s,n)=>s+n,0),delta=e.total-sum;layer.innerHTML=`<div class="sg-dice-table469"><small>${escape(name465(c,actor))}のサイコロ · 上の面が出目</small><div class="sg-dice-line469" style="--dice-count469:${e.dice.length}">${e.dice.map((n,i)=>die469(n,true,i)).join('')}</div><div class="sg-dice-answer469"><b>${e.total}<small>マス進む</small></b><p>${e.greed528?`強欲 ×2：出目 ${e.rawTotal528} ×2${e.bonus528?` ＋ 補正 ${e.bonus528}`:''} → ${e.total}マス`:e.genius?'天才：素の出目は最低7＋カード補正':delta?`${sum} ${delta>0?'+':'−'} ${Math.abs(delta)} = ${e.total}（補正込み）`:e.dice.join(' + ')+' = '+e.total}</p></div></div>`}
