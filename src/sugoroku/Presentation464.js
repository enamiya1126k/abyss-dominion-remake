...
function write464(c,f,art,token){
 ...
 layer.style.setProperty('--sg-stage-h',height+'px');layer.style.setProperty('--card-h465',cardHeight+'px');layer.style.setProperty('--card-w465',Math.round(cardHeight*.68)+'px');layer.style.setProperty('--fx-age',`-${age}ms`);layer.dataset.kind=e.kind;layer.style.setProperty('--event-duration469',`${f.active.duration}ms`);layer.classList.toggle('is-dark-instant467',e.kind==='card'&&card?.kind==='bad'&&e.activation466==='instant');layer.classList.toggle('sg-motion-reduced',f.reduced);
