...
function queueCompleted468(f,hand){const completed=completedSeries468(f.seriesHand468??hand,hand);f.seriesHand468=hand.map(h=>({...h}));for(const set468 of completed.reverse())f.queue.unshift({kind:'series468',set468,duration465:2600});}
...
function write464(c,f,art,token){
 ...
 if(e.kind==='card'&&card){layer.innerHTML=`${card.kind==='bad'&&e.activation466==='instant'?'<div class="sg-dark-title467"><small>引いてしまった…！</small><b>闇の速攻</b><span>強制発動</span></div>':''}<div class="sg-cast465"><div class="sg-caster465">${pawn465(c,actor,token,'is-source')}<small>${e.activation466==='instant'?'速攻カードを引いた！<br>強制発動':e.activation466==='blocked'?'悪い速攻を引いた<br>無効化！':'このカードを使用'}</small></div><div class="sg-flip464"><div class="sg-flip-inner464">${back466()}<div class="sg-front464" style="${rankStyle466(card)}">${photoCardArt468(card,art)}<small class="sg-rank466">${escape(card.rank466)}</small><b>${escape(card.name)}</b><p>${escape(card.text)}</p></div></div></div><div class="sg-event-progress465"></div></div>`}
...
export function presentationAfter464(c,art,token,focus){
 ...
 if(!f||f.gameId!==g.id){if(f?.raf)cancelAnimationFrame(f.raf);f=c.sgFX464={gameId:g.id,seen:g.presentationSequence464??0,queue:[],active:null,positions:{},visual466:capture466(g),seriesHand468:g.hand.map(h=>({...h})),reduced:matchMedia('(prefers-reduced-motion: reduce)').matches};}
