...
function write464(c,f,art,token){
 ...
 else if(e.kind==='dice'){layer.innerHTML=`<div class="sg-dice-stage464">${pawn465(c,actor,token)}<div><small>サイコロを振る</small><div class="sg-dice-row464">${e.dice.map(dice464).join('')}</div><b class="sg-dice-total464">${e.total}<small>マス進む</small></b><p>${e.genius?'特殊能力：手札の枚数で移動':e.total!==e.dice.reduce((s,n)=>s+n,0)?'カード・特殊能力の補正込み':''}</p></div></div>`}
...
export function presentationAfter464(c,art,token,focus){
 ...
 const frame=()=>{
  ...
  if(!f.active&&f.queue.length){const e=f.queue.shift(),duration=e.duration465??duration465(e);f.active={event:e,start:performance.now(),duration,lastStep:-1};if(e.kind==='move')focus(e.from);if(e.kind==='awaken')c.sound452?.play('win');write464(c,f,art,token);}
