...
export function cardBody476(c,art,{special=false,state='',usable=false}={}){
 ...
 return `${ornaments476}<span class="sg-tcg-top476"><b class="sg-gem476">${esc(rank)}</b><small>${esc(code)}</small></span><span class="sg-frame-art476">${special?art(c.art):photoCardArt468(c,art)}${dark?skull476:special?seal476:''}<span class="sg-art-label476">${esc(category)}</span></span><strong class="sg-title476">${esc(c.name)}</strong><span class="sg-paper476"><span class="sg-card-caption">${esc(summary)}</span><small>${esc(timing)}</small></span><span class="sg-tcg-footer476"><span class="sg-affinities476">${special?'✦ 覚醒能力':c.attrs?.length?c.attrs.map(id=>{const a=ATTRS463.find(x=>x.id===id);return `<i style="--attr476:${a?.color??'#d5c094'}">${esc(a?.name??id)}</i>`}).join(''):'<i>無属性</i>'}</span><span class="sg-foil476" aria-hidden="true">${special?'✦':dark?'☾':(c.rankIndex466??0)>=6?'✧':'◆'}</span></span>${state?`<span class="sg-card-state464">${usable?'✦ ':''}${esc(state)}</span>`:''}`;
}
