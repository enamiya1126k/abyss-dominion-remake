...
export const dashDistance527=p=>{const q=clamp523((p.charge??0)/SUMO523.charge,0,1);return(7+5*q+p.power*.025)*(.28+q*.12)};
...
export function makeSumo523({id,code,partyId,hostId,members,now=0}){return{id,code,game:'sumo',rules523:2,partyId462:partyId,hostId,members:members.map(m=>({...m})),phase:'lobby',revision:0,createdAt:now,updatedAt:now,players:[],crystals:[],events:[],eventSeq:0}}
...
export function motion523(g,p,input={},dt=1/60){if(!p.alive)return;let x=Number(input.x)||0,y=Number(input.y)||0,d=len(x,y);if(d>1){x/=d;y/=d}
 ...
 p.vx=attacking?p.dx*(7+5*p.attackPower+p.power*.025):stunned?0:x*speed;
 p.vy=attacking?p.dy*(7+5*p.attackPower+p.power*.025):stunned?0:y*speed;
...
function hit(g,a,b,nx,ny,impulses){if(g.elapsed>=a.attackUntil||a.hitIds.includes(b.playerId))return;a.hitIds.push(b.playerId);a.hits++;
 ...
 const opening=g.elapsed<30000?.78:1;const force=(5+12*a.attackPower+a.power*.10)*side*guard*opening/mass523(b);
...
export function brace527(g,p,collapsed=false){
 ...
 p.kx=-n.x*1.8;p.ky=-n.y*1.8;p.vx=0;p.vy=0;p.attackUntil=0;p.charging=false;p.charge=0;p.stunUntil=g.elapsed+240;
 emit(g,'brace',{playerId:p.playerId,x:p.x,y:p.y});return true;
}
function magnet527(g){if((g.pickups??=[]).length)return;const a=random(g)*Math.PI*2,d=(.25+random(g)*.48)*g.radius;g.pickups.push({id:++g.pickupSeq,x:Math.cos(a)*d,y:Math.sin(a)*d,born:g.elapsed,expires:g.elapsed+16000,type:'magnet'});emit(g,'magnetSpawn',{x:Math.cos(a)*d,y:Math.sin(a)*d})}
...
export function migrateSumo527(g){
 ...
 Object.assign(g,{rules523:2,elapsed,radius:rad,startAt:(g.startAt??0)-shift,pickups:[],pickupSeq:0,nextMagnet:elapsed+10000,nextCrystal:elapsed+700,events:[]});return true;
}
...
export function bot523(g,p){let group=brains.get(g);if(!group){group=new Map();brains.set(g,group)}let b=group.get(p.playerId);if(!b||g.elapsed>=b.until){
 const enemies=g.players.filter(v=>v.alive&&v!==p),near=[...enemies].sort((a,b)=>len(a.x-p.x,a.y-p.y)-len(b.x-p.x,b.y-p.y))[0],danger=enemies.find(v=>len(v.x-p.x,v.y-p.y)<3.5&&(v.charging||v.attackUntil>g.elapsed)),distance=len(p.x,p.y),limit=(nextShrink523(g.elapsed)-g.elapsed<SUMO523.warning?radius523(nextShrink523(g.elapsed)):g.radius)-.8;
 ...
 else if(near&&len(near.x-p.x,near.y-p.y)<3.4&&g.elapsed>=p.coolUntil&&(g.elapsed>18000||len(near.x-p.x,near.y-p.y)<1.5)){tx=near.x+near.vx*.15-p.x;ty=near.y+near.vy*.15-p.y;attack=true}
 else{const gems=[...g.crystals,...(g.pickups??[]).map(v=>({...v,value:p.magnetUntil>g.elapsed?1:5}))].filter(v=>len(v.x,v.y)<limit).sort((a,b)=>len(a.x-p.x,a.y-p.y)/a.value-len(b.x-p.x,b.y-p.y)/b.value);const target=gems[0]??near;if(target){tx=target.x-p.x;ty=target.y-p.y}}
 const v=unit(tx,ty);b={...v,attack,until:g.elapsed+150};group.set(p.playerId,b);
 }
 const dangerEdge=len(p.x,p.y)>g.radius-.8;
 const dot=p.x*b.x+p.y*b.y,edge=Math.max(.5,g.radius-.35),room=-dot+Math.sqrt(Math.max(0,dot*dot+edge*edge-p.x*p.x-p.y*p.y)),safe=room>1.6;
 return{x:b.x,y:b.y,press:b.attack&&safe&&!p.charging,release:p.charging&&(p.charge>=800+(p.attackSerial%3)*250||dashDistance527(p)>=Math.max(1.8,room-.15)||!b.attack),cancel:p.charging&&dangerEdge&&dot>0};
}
export function advanceSumo523(g,now,inputs=new Map(),autoIds=new Set()){
 ...
 let steps=0;while(g.lastAt+SUMO523.step<=now+.001&&g.phase==='play'&&steps++<30){g.lastAt+=SUMO523.step;g.elapsed=g.lastAt-g.startAt;const old=g.radius;g.radius=radius523(g.elapsed);const collapsed=old-g.radius>.3;if(collapsed)emit(g,'collapse',{from:old,to:g.radius});
  ...
  for(const p of g.players){if(!p.alive)continue;if(len(p.x,p.y)>g.radius+body523(p)*.22){if(!brace527(g,p,collapsed)){out(g,p);continue}}
   for(let i=(g.pickups??[]).length-1;i>=0;i--){const v=g.pickups[i];if(len(p.x-v.x,p.y-v.y)<body523(p)+.4){g.pickups.splice(i,1);p.magnetUntil=g.elapsed+SUMO523.magnetDuration;emit(g,'magnet',{playerId:p.playerId,x:p.x,y:p.y})}}
  ...
  if(g.elapsed>=(g.nextMagnet??14000)){magnet527(g);g.nextMagnet=g.elapsed+22000+random(g)*6000}
...
export function publicSumo523(g){return{id:g.id,code:g.code,game:'sumo',rules523:2,partyId462:g.partyId462,hostId:g.hostId,phase:g.phase,revision:g.revision,members:g.members.map(({playerId,name,choice,ai,color499,departed})=>({playerId,name,choice,ai,color499,departed})),players:g.players.map(({hitIds,...p})=>({...p})),crystals:g.crystals.map(v=>({...v})),pickups:(g.pickups??[]).map(v=>({...v})),events:g.events.map(v=>({...v})),serverAt:g.lastAt,startAt:g.startAt,elapsed:g.elapsed,radius:g.radius,winnerIds:g.winnerIds,reason:g.reason,results:g.results,finaleUntil:g.finaleUntil}}
