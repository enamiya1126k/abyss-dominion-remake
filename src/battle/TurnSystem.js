import{ultimateIsolated}from"../core/EndgameUltimateSystem.js?v=3.1.58-build378";
...
export function buildTurnQueue(battle){
 const entries=battleParty(battle).filter(monster=>monster.currentHp>0&&!stunned(battle,"ally",monster.id)).map(monster=>{const base=calculatedStats(monster).spd??0,rate=Math.max(.2,1+effectAmount(battle,"ally",monster.id,"spdUp")-effectAmount(battle,"ally",monster.id,"spdDown")),priority=battle.turn<=1&&Number(monster._seriesEffects?.firstStrike)>0?1:0;return{type:"ally",id:monster.id,name:displayName(monster),spd:Math.max(0,base*rate),priority,tie:Math.random()}});
 aliveEnemies(battle).filter(enemy=>!(enemy.motherSummon426&&enemy.readyRound426>battle.turn)&&!stunned(battle,"enemy",enemy.id)).forEach(enemy=>{const rate=Math.max(.2,1+effectAmount(battle,"enemy",enemy.id,"spdUp")-effectAmount(battle,"enemy",enemy.id,"spdDown"));entries.push({type:"enemy",id:enemy.id,name:enemy.name,spd:Math.max(0,(enemy.spd??0)*rate),priority:0,tie:Math.random()})});
...
export function currentAlly(battle){const entry=currentTurnEntry(battle);if(entry?.type!=="ally")return null;return battleParty(battle).find(monster=>monster.id===entry.id&&monster.currentHp>0)??null}
...
export function skipInvalidEntries(battle){while(battle.queueIndex<(battle.turnQueue?.length??0)){const entry=currentTurnEntry(battle);if(entry?.type==="enemy"&&aliveEnemies(battle).some(e=>e.id===entry.id&&!(e.motherSummon426&&e.readyRound426>battle.turn))&&!stunned(battle,"enemy",entry.id))return entry;if(entry?.type==="ally"&&battleParty(battle).some(m=>m.id===entry.id&&m.currentHp>0)&&!stunned(battle,"ally",entry.id))return entry;battle.queueIndex++}return null}
