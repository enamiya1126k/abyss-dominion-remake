...
export function skipInvalidEntries(battle){while(battle.queueIndex<(battle.turnQueue?.length??0)){const entry=currentTurnEntry(battle);if(entry?.type==="enemy"&&aliveEnemies(battle).some(e=>e.id===entry.id)&&!stunned(battle,"enemy",entry.id))return entry;if(entry?.type==="ally"&&battleParty(battle).some(m=>m.id===entry.id&&m.currentHp>0)&&!stunned(battle,"ally",entry.id))return entry;battle.queueIndex++}return null}
