...
export function applyPersistentAilment(battle,targetId,ailment){
 ...
 const resistance=Math.max(0,Math.min(.9,(Number(target?._equipmentAffixes?.statusResistance)||0)/100)),chance=ailment.chance==null?1:Math.max(0,Math.min(1,Number(ailment.chance)||0));
...
export function clearPersistentAilments(battle,targetId){battle.allyAilments??={};battle.allyAilments[targetId]=[];syncPersistentAilments(battle,targetId)}
