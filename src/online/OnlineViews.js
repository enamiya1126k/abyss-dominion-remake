import{displayPower447}from"../core/PowerScale447.js";
import{formatCombatPower}from"../core/CombatPower.js";
...
export function renderOnlineHome(room, selfId, state = {}) {
  ...
  return `<section class="online-gathering-hall" data-online-hall-stage aria-label="オンライン集会所">
    ...
    <div class="online-hall-world ${state.exploreChatOpen ? "chat-open" : ""}">
      ...
      <div class="online-hall-social-tools"><button type="button" data-online-chat-toggle class="online-hall-chat-tool ${state.exploreChatOpen ? "active" : ""}" aria-controls="onlineHallQuickChat" aria-expanded="${state.exploreChatOpen ? "true" : "false"}">${pixelIcon("notice")}<small>チャット</small></button></div>
...
export function renderSharedBattle({ mode, room, battle, selfId, selectedTarget = null, selectedAlly = null, title = "共闘バトル", enemies = [], allowCapture = false, readOnly = false, skillMenu = false, itemMenu = false, itemTargetMenu = false, hpTrails = {}, presentationKoIds = [], autoSupported = false, biomePanelCollapsed = false }) {
  ...
  const enemyMagicCircleArt = Object.fromEntries(foes.filter(enemy => enemy.magicCircleAsset).map(enemy => [enemy.id, `<span class="magic-circle magic-circle-black enemy-battle-magic-circle" data-circle-id="${escapeOnlineHtml(enemy.circleId || enemy.id || "raid-boss")}" aria-hidden="true"><img class="magic-circle-frame magic-circle-frame-1" src="${escapeOnlineHtml(enemy.magicCircleAsset)}" alt="" draggable="false"><i class="magic-circle-ring-a"></i><i class="magic-circle-ring-b"></i></span>`]));
...
export function renderOnlineExplore(room, selfId, state = {}) {
  ...
  if (state.expeditionReport && (state.expeditionReport.multiplayer ?? (room?.members?.length ?? 0) >= 2)) {
    ...
    const reportLead = floorBossClear ? `${number(reportFloor)}階の階層支配者を撃破しました。探索は続行できます。` : report.completed ? `${number(reportFloor)}階を共に踏破しました。` : report.reason === "defeat" ? isWorldOwner ? "通常探索と同じ敗北結果を反映しました。" : "お手伝い結果と自分のHP・MPを保存しました。通常探索の進行や敗北ペナルティは変わりません。" : "今回の共闘記録を確認できます。";
...
export function renderOnlineRaid(room, selfId, state = {}) {
  ...
  const weekly = room?.weeklyRaid ?? {}, boss = weekly.boss ?? { id: "abyss-amalga", name: "終焉融骸・アビス＝マルガ", level: 50, maxHp: 50_000, heroAsset: "./assets/online/raid-abyss-amalgam.png", materialName: "融骸核片", contractName: "融骸幼体アマルガ", equipmentName: "終焉喰らいの大刃", circleId: "reincarnation", circleName: "輪廻の魔法陣", intro: "与ダメージ・回復・蘇生・防御・補助を貢献度として集計します。" }, modifier = weekly.modifier ?? { name: "通常環境", description: "特殊ルールなし" };
  ...
  const exchanges = [
    ...
    [`circle:${boss.id}`, 120, "ボス専用魔法陣", boss.circleName || "専用魔法陣", Number(exchangeCount[`circle:${boss.id}`] ?? (boss.id === "abyss-amalga" ? exchangeCount.circle : 0)) > 0],
  ];
...
export function renderOnlineTeam(room, selfId, state = {}) {
  ...
  const teamPower = members => members.reduce((sum, member) => sum + displayPower447(member.profile?.power,member.profile), 0);
  ...
  return `${screenHeader("team", "FREE TEAM BATTLE 2.0", "ルールと勝敗形式を選び、最大4人で報酬を賭けない安全な模擬戦を楽しめます。")}
    ...
    <section class="online-team-balance ${powerGap >= 1.8 && settings.ruleset !== "balanced" ? "warning" : ""}"><div><small>紅組 TOTAL POWER</small><b>${formatCombatPower(sunPower)}</b></div><span>${settings.ruleset === "balanced" ? "戦力・人数補正 ON" : powerGap >= 1.8 ? "戦力差が大きいため均衡ルール推奨" : "戦力差を確認してください"}</span><div><small>蒼組 TOTAL POWER</small><b>${formatCombatPower(moonPower)}</b></div></section>
