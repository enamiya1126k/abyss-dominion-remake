...
import{worldPresentationForFloor}from"../../core/WorldSystem.js?v=3.0.1-build301";
import{partyCombatPower,formatCombatPower}from"../../core/CombatPower.js?v=3.0.1-build301";
import{manualReturnPreview}from"../../core/ReturnRewardSystem.js?v=3.0.1-build301";
...
import{CAMPAIGN_KEYS_PER_FLOOR,campaignDayDefinition,campaignFloorState}from"../../core/Campaign100System.js?v=3.0.1-build301";
...
export function ExploreScreen(state,options={}){
 ...
 const day=campaignDayDefinition(floor),floorState=campaignFloorState(state,floor),keys=Math.max(0,Math.min(CAMPAIGN_KEYS_PER_FLOOR,Number(options.campaignKeys??floorState?.keysCollected)||0));
 ...
 return`<section class="screen explore-screen explore-screen-dungeon world-${world.id} scenery-${scenery.id} ${hudCollapsed?"party-hud-collapsed":""} ${online?"online-shared-explore":""} ${options.className??""}" ${online?'data-online-explore-view="shared"':""} data-biome="${biome.theme}" data-scenery="${scenery.id}" data-scenery-variant="${scenery.variant}" data-world="${world.id}" data-music-profile="${world.musicProfile}" style="--biome-accent:${world.phase===1?world.accent:biome.accent};--scenery-accent:${scenery.accent};--scenery-dark:${scenery.minimapWall}">
  ...
  <div class="explore-stage">${stageContent}<span class="explore-floor-plate"><small>予言 ${day.day}日目・${day.title}</small>${floor}階</span><span class="campaign-key-counter" data-campaign-key-counter><small>戦利品の鍵</small><span class="campaign-key-locks">${Array.from({length:CAMPAIGN_KEYS_PER_FLOOR},(_,index)=>`<i data-key-lock class="${index<keys?"acquired":""}">⚿</i>`).join("")}</span><b>${keys}/${CAMPAIGN_KEYS_PER_FLOOR}</b></span><span class="explore-scenery-badge" aria-label="現在の景観 ${scenery.name} ${scenery.variantName}"><i></i><span><small>ENVIRONMENT</small><b>${scenery.name}</b><em>${scenery.variantName}</em></span></span>
