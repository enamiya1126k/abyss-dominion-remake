import{APP_VERSION}from"../../core/config.js?v=2.10.0-build163";
import{calculatedStats,displayName}from"../../models/Monster.js?v=2.10.0-build163";
import{maxMp}from"../../battle/SkillSystem.js?v=2.10.0-build163";
import{biomeForFloor,biomeProgress}from"../../data/biomes.js?v=2.10.0-build163";
import{dungeonThemeForFloor}from"../../data/dungeonThemes.js?v=2.10.0-build163";
import{worldPresentationForFloor}from"../../core/WorldSystem.js?v=2.10.0-build163";
import{partyCombatPower,formatCombatPower}from"../../core/CombatPower.js?v=2.10.0-build163";
import{manualReturnPreview}from"../../core/ReturnRewardSystem.js?v=2.10.0-build163";
import{monsterVisual}from"../MonsterVisual.js?v=2.10.0-build163";
import{SPECIES}from"../../data/species.js?v=2.10.0-build163";
import{resourceHud,pixelIcon}from"../components/GameChrome.js?v=2.10.0-build163";
...
export function ExploreScreen(state){
 const combatPower=partyCombatPower(state),run=manualReturnPreview(state),biome=biomeForFloor(state.player.currentFloor),progress=biomeProgress(state,biome),world=worldPresentationForFloor(state.player.currentFloor),scenery=dungeonThemeForFloor(state.player.currentFloor);
 ...
 return`<section class="screen explore-screen explore-screen-dungeon world-${world.id} scenery-${scenery.id} ${hudCollapsed?"party-hud-collapsed":""}" data-biome="${biome.theme}" data-scenery="${scenery.id}" data-scenery-variant="${scenery.variant}" data-world="${world.id}" data-music-profile="${world.musicProfile}" style="--biome-accent:${world.phase===1?world.accent:biome.accent};--scenery-accent:${scenery.accent};--scenery-dark:${scenery.minimapWall}">
  ...
  <div class="explore-stage"><canvas id="gameCanvas"></canvas><span class="explore-floor-plate">${state.player.currentFloor}階</span><span class="explore-scenery-badge" aria-label="現在の景観 ${scenery.name} ${scenery.variantName}"><i></i><span><small>ENVIRONMENT</small><b>${scenery.name}</b><em>${scenery.variantName}</em></span></span>
