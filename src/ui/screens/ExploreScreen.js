...
import{biomeForFloor,biomeProgress}from"../../data/biomes.js?v=2.3.0";
import{worldPresentationForFloor}from"../../core/WorldSystem.js?v=2.3.0";
import{partyCombatPower,formatCombatPower}from"../../core/CombatPower.js?v=2.3.0";
import{manualReturnPreview}from"../../core/ReturnRewardSystem.js?v=2.3.0";
import{monsterVisual}from"../MonsterVisual.js?v=2.3.0";
import{SPECIES}from"../../data/species.js?v=2.3.0";
import{resourceHud,pixelIcon}from"../components/GameChrome.js?v=2.3.0";
...
export function ExploreScreen(state){
 ...
 const party=(state.party??[]).map(id=>state.monsters.find(monster=>monster.id===id)).filter(Boolean),hudCollapsed=Boolean(state.settings?.explorePartyHudCollapsed),autoMode=state.settings?.exploreAutoMode??"off";
