...
import{resourceHud,pixelIcon}from"../components/GameChrome.js?v=1.14.4-delta117";
...
export function ExploreScreen(state){
 ...
 return`<section class="screen explore-screen explore-screen-dungeon world-${world.id}" data-biome="${biome.theme}" data-world="${world.id}" data-music-profile="${world.musicProfile}" style="--biome-accent:${world.phase===1?world.accent:biome.accent}">
  ...
  <div class="explore-party-strip">${party.map((monster,index)=>{
   ...
   return`<button type="button" data-explore-monster="${monster.id}" class="${longName?"long-name":""}">
    <em>${index+1}</em>${monsterVisual(monster,species.emoji??"MONSTER",{className:"explore-party-monster-visual"})}
    <div><b>${name}</b><small>Lv.${monster.level}・★${monster.stars??1}・+${monster.plus??0}・絆${monster.affection??0}</small>
