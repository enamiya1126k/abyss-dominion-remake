import{APP_VERSION}from"../../core/config.js?v=3.1.59-build379";
import{calculatedStats,displayName}from"../../models/Monster.js?v=3.1.59-build379";
...
import{worldPresentationForFloor}from"../../core/WorldSystem.js?v=3.1.55-build375";
import{partyCombatPower,formatCombatPower}from"../../core/CombatPower.js?v=3.1.59-build379";
import{manualReturnPreview}from"../../core/ReturnRewardSystem.js?v=3.1.55-build375";
...
export function ExploreScreen(state,options={}){
 const chapter=options.chapterTwo, floor=Math.max(1,Number(options.floor??state.player.currentFloor)||1),combatPower=options.combatPower??partyCombatPower(state),run=options.run??manualReturnPreview(state),biome=biomeForFloor(floor),world=worldPresentationForFloor(floor),savedAttribute=options.currentAttribute??state.expeditionSnapshot?.world?.currentAttribute??null,roomProfile=campaignRoomProfile(savedAttribute),scenery=options.scenery??(savedAttribute?dungeonThemeForAttribute(savedAttribute,floor):dungeonThemeForFloor(floor));
 const day=campaignDayDefinition(floor),regionProgress=chapter?{percent:0,from:1,to:6,cleared:0,total:6}:campaignRegionProgress(state,floor),progress=regionProgress.percent,floorState=chapter?{}:campaignFloorState(state,floor),keys=Math.max(0,Math.min(CAMPAIGN_KEYS_PER_FLOOR,Number(options.campaignKeys??campaignKeysHeld(floorState))||0));
