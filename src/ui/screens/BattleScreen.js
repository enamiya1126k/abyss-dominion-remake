...
import{learnedSkills,maxMp,skillElementLabel,effectiveSkillMpCost}from"../../battle/SkillSystem.js?v=1.9.0-monster-catalog";
...
function renderEnemies(battle,enemies,target){
 return enemies.map(enemy=>{
  ...
  return `<button id="enemy-${enemy.id}" data-enemy-target="${enemy.id}" class="combatant enemy-combatant ${enemy.elite?"elite-enemy":""} ${target?.id===enemy.id?"targeted":""}">
   ...
   <div class="enemy-orb" style="background:${enemy.color}">${monsterVisual(enemy,enemy.emoji??"👾",{frame:enemy.hp<=0?"down":"idle",className:"battle-enemy-visual"})}</div>
...
function renderParty(battle,actor){
 ...
 const stats=calculatedStats(m),mp=maxMp(m),need=expNeedFor(m);
  ...
  return `<button id="ally-${m.id}" data-battle-detail="${m.id}" class="battle-unit combatant ${actor?.id===m.id?"active":""} ${m.currentHp<=0?"dead":""}">
   <div class="unit-head"><span class="unit-orb" style="background:${colorValue(m)}">${monsterVisual(m,battle.species?.[m.speciesId]?.emoji??"●",{frame:m.currentHp<=0?"down":"idle",className:"battle-ally-visual"})}</span><b>${displayName(m)} Lv.${m.level}</b></div>
