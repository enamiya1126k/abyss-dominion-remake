 ...
 <section class="battle-screen" data-speed="${settings.battleSpeed}">
  ...
  <div class="battle-arena">
   ...
   <div class="battle-party">
    ${battle.party.map(m=>{
     const stats=calculatedStats(m);
     return`<div id="ally-${m.id}" class="battle-unit combatant ${actor?.id===m.id?"active":""} ${m.currentHp<=0?"dead":""}">
      <div class="unit-head"><span class="unit-orb" style="background:${colorValue(m)}"></span><b>${displayName(m)} Lv.${m.level}</b></div>
      <small>HP ${m.currentHp}/${stats.hp}</small>
      <div class="battle-bar ally"><i style="width:${Math.max(0,m.currentHp/stats.hp*100)}%"></i></div>
     </div>`;
    }).join("")}
   </div>
  ...
  <div class="battle-command">
   ...
   <div class="command-grid">
    <button data-command="attack">たたかう</button>
    <button data-command="guard">ガード</button>
    <button data-command="skill">スキル</button>
    <button data-command="item">アイテム</button>
    <button data-command="capture">捕獲</button>
   </div>
  </div>
 </section>`;
}
