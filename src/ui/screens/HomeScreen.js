...
import{dailyTeamAttempts,TEAM_BATTLE_UNLOCK_FLOOR,EMERGENCY_UNLOCK_FLOOR,ENDGAME_BOSSES,emergencyFragmentStatus}from"../../core/EndgameSystem.js?v=0.9.14-alpha.2";

export function HomeScreen(state){
  const party=state.party.map(id=>state.monsters.find(m=>m.id===id)).filter(Boolean);
  const team=dailyTeamAttempts(state),teamUnlocked=state.player.maxFloor>=TEAM_BATTLE_UNLOCK_FLOOR,emergencyUnlocked=state.player.maxFloor>=EMERGENCY_UNLOCK_FLOOR,revealed=state.player.maxFloor>=1000||state.flags?.deepAbyssUnlocked;
