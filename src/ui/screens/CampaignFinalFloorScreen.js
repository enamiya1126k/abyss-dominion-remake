import{ExploreScreen}from"./ExploreScreen.js?v=3.1.82-build402";
...
export function CampaignFinalFloorScreen({heroes=[],party=[],state,room}={}){
 ...
 const hint=room.phase==='cleared'?'勇者に触れて会話・手合わせ。玉座では結末の回想':room.phase==='victory'?'玉座へ進み、守り抜いた世界の結末を見届けよう':room.phase==='ready'?'準備ができたら、勇者に触れて最終決戦へ':'絨毯の先へ進み、王室で待つ者と対面しよう';
 return ExploreScreen(state,{title:'魔王城-王室',party,className:'royal-explore-360',stageContentHtml:'<canvas id="royalCanvas" tabindex="0" aria-label="王室。勇者をタップで会話・挑戦。床をタップで移動、ドラッグでカメラ移動"></canvas><div class="royal-location-seal"><small>THE LAST THRESHOLD</small><b>魔王城-王室</b></div>',stageToolsHtml:'',autoToggleHtml:'',miniMapHtml:'',navHtml:`<button data-royal-formation>${pixelIcon('formation')}編成</button><button data-royal-equipment>${pixelIcon('equipment')}装備</button><button data-royal-center>${pixelIcon('event')}現在地</button><button data-royal-exit>${pixelIcon('rest')}拠点へ</button>`})+`<aside class="royal-objective-360" role="status">${hint}<span>${room.phase==='cleared'?'予言を越えた世界':`勇者 ${remaining}/4人`}</span></aside>`;
}
