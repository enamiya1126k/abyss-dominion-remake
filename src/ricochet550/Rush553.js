...
export function canBurst553(g,p,at=g.simAt){return !!(g.game==='pinball'&&g.phase==='play'&&p?.picked&&p.launched&&!overdrive553(p,at)&&p.burstCharge553>=100&&at<g.deadline)}
...
export function chargeRush553(g,p,strength,event){
 if(overdrive553(p,g.simAt))return;const before=p.burstCharge553??0;p.burstCharge553=Math.min(100,before+5+Math.min(7,strength*.13));
...
export function burst553(g,p,power,angle,{limit,event}){
 ...
 const carryX=p.vx,carryY=p.vy,speed=(7+13*power+21*power**3)*RUSH553.impulse*(1+.15*(p.parts?.spring??0));
 p.vx+=Math.sin(angle)*speed;p.vy+=Math.cos(angle)*speed;limit(p);
 p.nextShotAt555=g.simAt+ARENA555.reload;p.burstCharge553=0;p.burstUsed553=true;p.bursts553++;p.pulling=false;p.pullKind553=null;p.overdriveUntil553=g.simAt+RUSH553.duration;
...
export function queueJackpot553(g,p,base,x,y,mult,{random,event}){
 const roll=random(g),tier=roll<.6?1:roll<.92?3:7,final=final555(g);
...
export function resolveJackpot553(g,event,force=false){
 ...
 const from=rank(),value=j.base*j._tier553;p.roundScore+=value;if(p.breakdown555)p.breakdown555.jackpot+=value;
