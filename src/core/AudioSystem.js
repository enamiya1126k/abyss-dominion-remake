...
export class AudioSystem{
 constructor(settings=()=>({})){
  this.settings=settings;this.scene="home";this.context=null;this.sfxGain=null;this.unlocked=false;this.current=null;this.fadeToken=0;this.cache=new Map();
 }
 track(scene){
 ...
 async unlock(){
  ...
  this.unlocked=true;this.applySettings();await this.switchTrack(this.scene,true);return true;
 }
 applySettings(){
  ...
  if(!enabled)this.cache.forEach(track=>track.pause());else if(this.unlocked)this.switchTrack(this.scene,true);
 }
 setScene(scene){
  if(!TRACKS[scene])scene="home";
  const changed=this.scene!==scene;this.scene=scene;
  if(this.unlocked&&(changed||!this.current||this.current.paused))this.switchTrack(scene,!changed);else this.applySettings();
 }
 async switchTrack(scene,immediate=false){
  if(!this.unlocked||this.settings()?.audioEnabled===false)return;
  ...
  const steps=immediate?1:12,delay=immediate?0:35;
 ...
 sfx(kind="select"){
  if(!this.context||!this.sfxGain||this.settings()?.audioEnabled===false)return;
