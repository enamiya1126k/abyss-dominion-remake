...
function partyScene(ledger,payload,definition){
 const heroId=definition.heroId,name=HERO_NAMES[heroId],outcome=payload.outcome,hurt=payload.hurtPercent,others=HERO_PARTY_IDS.filter(id=>id!==heroId&&!ledger.heroes?.[id]?.defeated),dialogue=[];
 if(outcome==="repelled"){
  dialogue.push(line(null,`${name}は野営地へ戻らなかった。残された仲間は、途切れた足音の意味を理解した。`,"narration"));
  if(others.includes("myth_hide"))dialogue.push(line("myth_hide",`${name}の反応が消えた。捜索より先に、残った三人の生存率を計算する。`,"serious"));
  ...
  if(heroId!=="myth_enami"&&others.includes("myth_enami"))dialogue.push(line("myth_enami",outcome==="hero-victory"?"勝った顔してるけど、傷まで無かったことにはせえへんで。":"戻ってきたならええ。次は四人で話を終わらせる。","gentle"));
 ...
 return storyScene(definition,{id:payload.id,part:"party",title:"同じ頃、勇者一行は",summary:`第${payload.floor}階の遭遇結果は、勇者側の会話と次の判断にも残った。`,dialogue,location:"勇者一行・夜営地",eyebrow:"HEROES / SAME TIMELINE",variant:outcome})
}
