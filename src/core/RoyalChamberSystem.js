import {memoryProgress,memoryLevel,MEMORY_MAX_STAGE} from './RoyalMemory379.js?v=3.1.59-build379';
...
export function beginRoyalAttempt(state,ledger,{memory=false,stage=null}={}){
 const room=royalState(state);if(room.attempt||room.phase==='rewind'||room.phase==='victory')return false;
 const progress=memory?memoryProgress(state):null,chosen=stage==null?progress?.unlocked:Number(stage);if(memory&&(!state.campaign100.finalCompleted||!Number.isInteger(chosen)||chosen<1||chosen>progress.unlocked))return false;
 room.attempt={memory,stage:memory?chosen:Math.max(1,(Number(room.memoryWins)||0)+1),...(memory?{memoryLevel379:memoryLevel(chosen)}:{}),partyIds:[...(state.party??[])],items:Object.fromEntries(ROYAL_ITEM_KEYS.map(k=>[k,Math.max(0,Number(state.inventory?.[k])||0)])),gold:Math.max(0,Number(state.player?.gold)||0),heroes:copy(ledger.heroes),startedAt:Date.now()};
