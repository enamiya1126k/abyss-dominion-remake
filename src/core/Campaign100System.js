...
export function createCampaignFloorState(floor){return{floor:capFloor(floor),keysCollected:0,keyIds:[],bossDiscovered:false,bossDefeated:false,trophyLocksOpened:0,trophyClaimed:false,hotSpringUsed:false,exitUnlocked:false,visitedRoomIds:[]}}
...
export function beginCampaignFloorRun(state,floor,runId){const entry=campaignFloorState(state,floor),id=String(runId??"");if(entry.runId===id)return entry;entry.runId=id;entry.keyIds=[];entry.keysCollected=0;entry.bossDiscovered=false;entry.bossDefeated=false;entry.trophyLocksOpened=0;entry.hotSpringUsed=false;entry.exitUnlocked=false;entry.visitedRoomIds=[];return entry}
