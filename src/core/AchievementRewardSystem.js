import{COMPLETE_MONSTER_CODEX,codexCollectionSummary,rewardDescription}from"./CollectionRewardSystem.js?v=3.1.28-build348";
...
export const ACHIEVEMENT_DEFINITIONS=Object.freeze([
 ...
 definition("abyss-1","契約","summon","深淵との契約","深淵を1体獲得", "abyssOwned",1,reward(1000000,500,{abyssKeys:10})),
 definition("abyss-7","契約","summon","七深淵の集結","深淵7体を獲得", "abyssOwned",7,reward(10000000,3000,{abyssKeys:70,mythicEquipment:1,equipmentPlus:70})),
 definition("tengod-1","契約","event","神域への接触","十神を1体獲得", "tenGodOwned",1,reward(3000000,1000,{abyssKeys:20})),
 definition("tengod-10","契約","event","十神集結","十神10体を獲得", "tenGodOwned",10,reward(30000000,7500,{abyssKeys:100,mythicEquipment:2,equipmentPlus:99})),
 definition("serial-4","契約","notice","四つの限定契約","シリアル限定魔物を4体獲得", "serialOwned",4,reward(2000000,1000,{captureCrystals:200})),

 definition("coop-1","オンライン","formation","初めての共同探索","共同探索で1階踏破", "coopExpeditions",1,reward(250000,150,{captureCrystals:20})),
 definition("coop-25","オンライン","formation","共闘の熟練者","共同探索で25階踏破", "coopExpeditions",25,reward(2500000,750,{captureCrystals:100,abyssKeys:10})),
 definition("coop-battles-100","オンライン","crossed-swords","百戦共闘","オンライン戦闘で100勝", "coopBattles",100,reward(5000000,1250,{experienceItemsUltra:10})),
 definition("raid-1","オンライン","event","レイド初討伐","ワールドレイドで1勝", "raidWins",1,reward(1000000,500,{abyssKeys:10})),
...
export function achievementRewardId(achievementId,version=1){
 const id=String(achievementId??"").trim(),revision=Math.max(1,Math.floor(Number(version)||1));
 return id?`achievement-${id}-v${revision}`:""
}

export function achievementIconKeyForId(value,{fallback="event"}={}){
 const raw=String(value&&typeof value==="object"?value.id??"":value??"").trim();
 const direct=ACHIEVEMENT_DEFINITION_BY_ID.get(raw);if(direct)return direct.iconKey;
 for(const entry of ACHIEVEMENT_DEFINITIONS){
  const prefix=`achievement-${entry.id}-v`;
  if(raw.startsWith(prefix)&&/^\d+$/.test(raw.slice(prefix.length)))return entry.iconKey
 }
 return ACHIEVEMENT_ICON_KEY_SET.has(fallback)?fallback:"event"
}
...
function floorBossVictoryCount(state){
 ...
 return FLOOR_BOSS_CATALOG.filter(boss=>{
  const campaignFloor=floorBossCampaignDisplayFloor(boss),keys=[boss.floor,campaignFloor].filter(value=>value!=null).map(String);
  return keys.some(key=>number(player.bossKills?.[key])>0||Object.prototype.hasOwnProperty.call(player.bossRewards??{},key)||Object.prototype.hasOwnProperty.call(player.pendingBossRewards??{},key))||Boolean(victories[boss.id])
 }).length
}
