import {createMonster,calculatedStats} from '../models/Monster.js?v=3.1.34-build354';
import {maxMp,effectiveSkillMpCost,allLearnedSkills} from '../battle/SkillSystem.js?v=3.1.28-build348';
import {createSignatureEquipment,signatureStatBonuses,signatureSetState} from './SignatureWeaponSystem.js?v=3.1.28-build348';
...
export function createCampaignHeroLoadout(speciesId,level=1000){
 ...
 const equipment=[0,1].map(index=>{const item=createSignatureEquipment(speciesId,index);item.id=`hero348:${speciesId}:weapon:${index}`;item.level=CAMPAIGN_HERO_WEAPON_LEVEL;item.plus=CAMPAIGN_HERO_WEAPON_PLUS;item.affixes=[];item.equippedBy=monster.id;return item});
 monster.equipment={weaponRight:equipment[0].id,weaponLeft:equipment[1].id};
...
export function applyCampaignHeroLoadout(enemy){
 ...
 Object.assign(enemy,{...stats,hp:stats.hp,maxHp:stats.hp,maxMp,currentMp:maxMp,rank:monster.rank,enemyGear:equipment,equipped:true,enemyEquipmentSlots:2,enemyEquipmentLevel:CAMPAIGN_HERO_WEAPON_LEVEL,enemyEquipmentRarity:'神話',enemyMagicCircle:null,heroSignature348:signature,heroSkillCosts348:monster.heroSkillCosts348,heroLoadoutVersion348:1,bossStatusResist:0,bossPowerMultiplier:1,hiddenDamageTaken:1,hiddenStatusResist:0,hiddenProfile:{active:false},enraged:false});
 return enemy;
}
