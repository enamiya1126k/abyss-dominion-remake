...
export function start500(g,now){
 ...
 Object.assign(g,{rules492:1,rules494:1,rules496:1,rules500:1,phase:'countdown',simAt:g.startAt-100,nextSpawnAt:g.startAt+500,updatedAt:now,enemies:[],events:[],eventSerial:0,enemySerial:0,hp:100,captured:0,breaches:0,points:0,netEnergy:0,netMax:CANAL500.netMax,netUntil:0,megaCount:0,teamCatches:0,bySpecies:[0,0,0,0],bossMask:0,missiles:[],missileSerial:0,intercepted:0});return true;
}
function spawn(g,at,lane,kind){
 ...
 g.enemies.push({path500:1,nextShotAt:kind===3?at+2000:null,id:++g.enemySerial,kind,lane,hp:s.hp,maxHp:s.hp,spawnAt:at,travelMs:s.travelMs*(.90+rand(g)*.20)*(elapsed>=50000?.62:.90),routeX,routeEnd,curve,wiggle:rand(g),motionAt:at,motionProgress:0,slowUntil:0,snareSeat:null,contributors:[]});return true;
}
function hit(g,p,e,damage,at,mega){
 ...
 e.hp-=damage;p.hits++;const base={path500:1,enemyId:e.id,kindIndex:e.kind,lane:e.lane,progress:progress500(e,at),wiggle:e.wiggle,routeX:e.routeX,routeEnd:e.routeEnd,curve:e.curve,seat:p.seat,mega};
 if(e.hp>0){e.motionProgress=progress500(e,at);e.motionAt=at;e.slowUntil=at+850;e.snareSeat=p.seat;event(g,'hit',at,{...base,hp:e.hp,slow:true});return}
 g.enemies=g.enemies.filter(x=>x.id!==e.id);const s=SPECIES500[e.kind];g.captured++;g.points+=s.points;p.captured++;p.points+=s.points;g.bySpecies[e.kind]++;
 ...
 if(!mega)g.netEnergy=Math.min(CANAL500.netMax,g.netEnergy+(helper?2:1));
...
function missiles500(g,at){
 ...
 for(const e of g.enemies)if(e.kind===3&&at>=e.nextShotAt){
  const from=route500(e,progress500(e,at));g.missiles.push({id:++g.missileSerial,enemyId:e.id,spawnAt:at,arriveAt:at+CANAL500.missileMs,fromX:from.x,fromY:Math.min(.67,from.y+.05),damage:CANAL500.missileDamage});
  e.nextShotAt+=CANAL500.missileEveryMs;
 }
 for(const m of [...g.missiles])if(at>=m.arriveAt){g.missiles=g.missiles.filter(x=>x.id!==m.id);g.hp=Math.max(0,g.hp-m.damage);event(g,'missileImpact',at,{missileId500:m.id,damage:m.damage})}
}
...
export function advance500(g,now,connected=()=>true){
 ...
 while(g.simAt+100<=until&&g.phase==='playing'){
  ...
  missiles500(g,at);
  for(const e of [...g.enemies])if(progress500(e,at)>=1){const damage=SPECIES500[e.kind].damage;g.hp=Math.max(0,g.hp-damage);g.breaches++;g.enemies=g.enemies.filter(x=>x.id!==e.id);event(g,'breach',at,{lane:e.lane,kindIndex:e.kind,damage})}
...
export function public500(g,selfId,now,connected=()=>true){return{
 ...
 missiles:(g.missiles??[]).map(m=>({...m})),intercepted:g.intercepted??0,netEnergy:g.netEnergy,netMax:CANAL500.netMax,netUntil:g.netUntil,megaCount:g.megaCount,teamCatches:g.teamCatches,bySpecies:[...g.bySpecies],
 ...
 players:g.players.map(p=>({playerId:p.playerId,name:p.name,color499:color499(p.color499,p.seat).id,choice:{id:p.choice.id,speciesId:p.choice.speciesId},seat:p.seat,ai:p.ai,auto:p.auto,connected:p.ai||!!connected(p.playerId),lane:p.lane,held:p.held,captured:p.captured,assists:p.assists,teamwork:p.teamwork,rescues:p.rescues,points:p.points,bursts:p.bursts,hits:p.hits,intercepted:p.intercepted??0,lastAction:p.lastAction?{...p.lastAction}:null,...(p.playerId===selfId?{lastSeq:p.lastSeq,nextFireAt:p.nextFireAt}:{})})),
