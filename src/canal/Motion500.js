...
export function route500(e,progress){const t=clamp(progress),sway=(e.kindIndex??e.kind)===2?Math.sin(t*Math.PI*8+(e.wiggle??0)*Math.PI*2)*Math.sin(Math.min(1,t*8)*Math.PI/2)*.062*(1-t*.4):0;return{x:(e.routeX??.5)*(1-t)+(e.routeEnd??.5)*t+Math.sin(t*Math.PI)*(e.curve??0)+sway,y:e.path541===1?.17+t*.51:.055+t*.625}}
