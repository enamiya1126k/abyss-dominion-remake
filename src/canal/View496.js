...
const gRules500=c=>game(c)?.rules500===1;
...
export function target496(c,id){if(id<0)return interceptTarget500(c,-id);const u=ui(c),e=game(c)?.enemies.find(e=>e.id===id);if(!playable(c)||!e||cooldown496(mine(c),u,clock(c),Date.now())>0)return false;if(!submit(c,{pulse:true,targetId:id,lane:e.lane}))return false;u.localFire=Date.now();u.preview={kind:'stroke',localAt:Date.now(),lane:e.lane,seat:u.seat,enemyId:e.id,point:u.frameLayout.find(x=>x.id===id)};sound(c,'net');tick496(c);return true}
export function interceptTarget500(c,id){const u=ui(c),g=game(c),at=clock(c),m=g?.missiles?.find(m=>m.id===id);if(g?.rules500!==1||!playable(c)||!m||at>=m.arriveAt||u.pending.some(x=>x.missileId500===id))return false;if(!submit(c,{pulse:true,missileId500:id}))return false;sound(c,'net');tick496(c);return true}
...
const unbind=bindFocus496(stage,{tap:id=>target496(c,id),instantMissiles:gRules500(c)}),unlock=lockPlayZoom486(c.root.querySelector('.cn496'));u.cleanup=()=>{unbind();unlock();u.targets.clear()};tick496(c)}
...
export function receive496(c){const u=ui(c),g=game(c);if(u.gameId!==g?.id)return;acknowledgeCanal489(u,mine(c));
 ...
 for(const e of g.events??[]){if(e.id<=u.seen)continue;u.seen=e.id;if(clock(c)-e.at>1800)continue;u.effects.push({...e,point:e.kind==='intercept'?null:u.targets.get(e.enemyId)?.point,localAt:Date.now()});
  ...
  if(e.kind==='intercept'&&!e.mega){if(e.seat===u.seat)sound(c,'catch');u.feedback={until:Date.now()+800,text:who(e.seat)+'がミサイルを迎撃！'}};
  ...
  if(e.kind==='boss')u.callout={until:Date.now()+1500,text:g.rules500===1?'巨大将軍！ 20回で捕獲・ミサイルは1タップ迎撃！':'ザリ将軍が来た！ みんなの網を重ねろ！'};
  if(e.kind==='missileImpact'){sound(c,'alarm');u.callout={until:Date.now()+600,text:'ミサイル直撃！ 家に −5！'}};
