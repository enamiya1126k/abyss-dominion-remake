...
export function bindFocus494(surface, { tap, swipe }, win=globalThis, pointer=!!globalThis.PointerEvent) {
  ...
  const begin=(id,x,y,target,e)=>{
    ...
    const b=target.closest?.('[data-cn-target494]');
    active={id,x,y,max:0,target:b&&!b.disabled?Number(b.dataset.cnTarget494):null};
  };
  ...
  const end=(id,x,y,cancel=false)=>{
    ...
    if(!cancel&&!blocked&&a?.id===id){const dx=x-a.x,dy=y-a.y,d=Math.max(a.max,Math.hypot(dx,dy));
      if(Math.abs(dx)>=40&&Math.abs(dx)>Math.abs(dy)*1.25)swipe(dx<0?1:-1);
      else if(d<=14&&a.target!==null)tap(a.target);
    }
