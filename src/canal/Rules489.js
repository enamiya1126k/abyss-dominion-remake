import{start500,sweep500,input500,advance500,public500}from'./Rules500.js';
...
export function startCanal489(g,now){if(g.rules500===1)return start500(g,now);if(g.rules496===1)return start496(g,now);if(g.rules494===1)return start494(g,now);if(g.rules492===1)return start492(g,now);
...
export function sweep489(g,p,lane,at,burst=false){if(g.rules500===1)return sweep500(g,p,lane,at,burst);if(g.rules496===1)return sweep496(g,p,lane,at,burst);if(g.rules494===1)return sweep494(g,p,lane,at,burst);if(g.rules492===1)return sweep492(g,p,lane,at,burst);
...
export function inputCanal489(g,p,input,receivedAt){if(g.rules500===1)return input500(g,p,input,receivedAt);if(g.rules496===1)return input496(g,p,input,receivedAt);if(g.rules494===1)return input494(g,p,input,receivedAt);if(g.rules492===1)return input492(g,p,input,receivedAt);
...
export function advanceCanal489(g,now,connected=()=>true){if(g.rules500===1)return advance500(g,now,connected);if(g.rules496===1)return advance496(g,now,connected);if(g.rules494===1)return advance494(g,now,connected);if(g.rules492===1)return advance492(g,now,connected);
