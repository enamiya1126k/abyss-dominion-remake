...
export const CABBAGE484=Object.freeze({version:4,duration:45000,countdown:3500,warning:700,grace:180,minTap:20,maxAge:1000,points:10,penalty:80,repair:950,comboGap:900,maxSequence:4500,maxBatch:32,maxQueue:256});
const rng=g=>{let x=g.seed|0;x^=x<<13;x^=x>>>17;x^=x<<5;g.seed=x>>>0;return g.seed/4294967296};
export function makeCabbage484({id,code,partyId,hostId,members,now=0,seed=1}){return{id,code,game:'cabbage',scoringVersion491:1,partyId462:partyId,hostId,phase:'lobby',createdAt:now,updatedAt:now,revision:0,seed:seed||1,members:members.map(m=>({...m,choice:m.choice??null})),players:[],windows:[]}}
export function startCabbage484(g,now){
 ...
 g.scoringVersion491=1;g.phase='countdown';g.startAt=now+CABBAGE484.countdown;g.endAt=g.startAt+CABBAGE484.duration;g.updatedAt=now;
...
export function tap484(g,p,{seq,side,at},received){
 ...
 if(w.kind==='stop'){
  const perTap=g.scoringVersion491===1;
  if(!perTap&&at<w.at+CABBAGE484.grace)return'grace';
  p.boardWindow=w.index;
  p.boardDamage=(p.boardDamage??0)+1;p.lockedUntil=w.until;p.combo=0;p.lastSide=null;
  // Build491: every accepted forbidden tap costs 80, with no score floor.
  // A running pre-update match finishes under its original scoring contract.
  if(!perTap&&p.brokenWindow===w.index)return event(p,'damage',at,{damage:p.boardDamage,seq});
  p.score=perTap?p.score-CABBAGE484.penalty:Math.max(0,p.score-CABBAGE484.penalty);p.breaks++;p.brokenWindow=w.index;return event(p,'break',at,{delta:-CABBAGE484.penalty,damage:p.boardDamage,seq});
 }
...
export function advanceCabbage484(g,now){
 ...
 for(const p of g.players.filter(p=>p.ai)){
  let guard=0;while(p.nextBotAt<=end&&guard++<400){const at=p.nextBotAt,w=window484(g,at);p.nextBotAt+=p.botInterval+((p.lastSeq*47+p.botSeed)%71);
   if(w.kind==='stop'&&((w.index+p.botSeed)%4!==0||(g.scoringVersion491===1&&p.brokenWindow===w.index)))continue;
