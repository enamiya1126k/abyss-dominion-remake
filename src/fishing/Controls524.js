import {inWater526} from './Water526.js';
export function controls524(root,on,input,send,enabled,unlock,aim){const reel=root.querySelector('[data-fi-reel]'),ease=root.querySelector('[data-fi-ease]'),stage=root.querySelector('[data-fi-stage]'),holds=new Set(),eases=new Set();let castPointer=null;
 ...
 on(stage,'pointerup',e=>{if(e.pointerId!==castPointer)return;castPointer=null;if(!enabled())return;const b=stage.getBoundingClientRect(),x=(e.clientX-b.left)/b.width,y=(e.clientY-b.top)/b.height;if(inWater526(x,y)){unlock();aim(x,y)}});
