...
export function publicFishing524(g,{compact=false}={}){return{...(compact?{compact540:true}:{}),id:g.id,code:g.code,theme532:g.theme532??'river',game:'fishing',rules524:8,partyId462:g.partyId462,hostId:g.hostId,phase:g.phase,revision:g.revision,members:g.members.map(({playerId,name,choice,ai,color499,departed})=>({playerId,name,choice,ai,color499,departed})),players:g.players.map(({nextAI,bot526,recent532,biteAt,fish,pendingBait530,baitItems530,...p})=>({...p,records:compact?undefined:p.records.map(f=>({...f})),chainTrail525:compact?undefined:[...(p.chainTrail525??[])],fish:['fight','landing','choice','rest'].includes(p.mode)&&fish?{...fish}:null,biteAt:undefined,pendingBait530:pendingBait530?{...pendingBait530}:null,baitItems530:compact?undefined:(baitItems530??[]).map(f=>({...f}))})),shoals:g.shoals.map(s=>({...s})),events:(compact?g.events.filter(e=>g.elapsed-e.at<8000):g.events).map(e=>({...e})),serverAt:g.lastAt,startAt:g.startAt,elapsed:g.elapsed,boss:g.boss,lord526:g.lord526?{phase:g.lord526.phase,ownerId:g.lord526.ownerId,speciesId531:g.lord526.phase==='hidden'?null:g.lord526.speciesId531}:null,winnerIds:g.winnerIds,results:g.results,finaleUntil:g.finaleUntil}}
// Frequent frames carry only live state. Full results and catch records remain authoritative.
export const fishingFrame540=g=>publicFishing524(g,{compact:g.phase!=='result'});
