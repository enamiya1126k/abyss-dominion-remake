...
export function raceDomSignature452(client){return JSON.stringify({room:client.state?.room,available:client.state?.available,rulesVersion:client.state?.rulesVersion,connected:client.connected(),gold:client.save.state.player.gold,pending:client.bank()?.pending,error:client.error,rewardError:client.rewardError})}
