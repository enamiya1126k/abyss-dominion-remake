...
export function paradeIndex452(room,at,pinned=null){return Number.isInteger(pinned)&&pinned>=0&&pinned<8?pinned:Math.floor(Math.max(0,at-room.phaseAt)/2000)%8}
...
export function raceDomSignature452(client){return JSON.stringify({room:client.state?.room,available:client.state?.available,fatigue455:client.state?.fatigue455,rulesVersion:client.state?.rulesVersion,connected:client.connected(),gold:client.save.state.player.gold,pending:client.bank()?.pending,error:client.error,rewardError:client.rewardError})}
