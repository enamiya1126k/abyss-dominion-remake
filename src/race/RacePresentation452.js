...
export function raceDomSignature452(client){const source=client.state?.room,room=source?.rulesVersion>=4?{...source,live456:null,finishMs:source.phase==='result'?source.finishMs:null}:source;return JSON.stringify({room,canal:canalSignature489(client),cabbage:cabbageSignature484(client),sugoroku:client.state?.sugoroku,party:client.state?.party,available:client.state?.available,fatigue455:client.state?.fatigue455,rulesVersion:client.state?.rulesVersion,connected:client.connected(),gold:client.save.state.player.gold,crystals474:client.save.state.player.crystals,crystalPending474:client.key?.()?client.save.state.sugoroku474?.accounts?.[client.key()]?.pending:null,pending:client.bank()?.pending,error:client.error,rewardError:client.rewardError})}
