import{signature550}from'../ricochet550/Rules550.js';
import{signature543}from'../cart/Rules543.js';
import{signature542}from'../bomb/Rules542.js';
import{signature539}from'../tetra/Rules539.js';
import{signature536}from'../hide/Rules536.js';
import{signature524}from'../fishing/Rules524.js';
import{signature523}from'../sumo/Rules523.js';
...
export function raceDomSignature452(client){const source=client.state?.room,room=source?.rulesVersion>=4?{...source,live456:null,finishMs:source.phase==='result'?source.finishMs:null}:source;return JSON.stringify({room,ricochet:signature550(client.state?.ricochet),cart:signature543(client.state?.cart),bomb:signature542(client.state?.bomb),tetra:signature539(client.state?.tetra),hide:signature536(client.state?.hide),fishing:signature524(client.state?.fishing),sumo:signature523(client.state?.sumo),tower:towerSignature517(client.state?.tower),quiz:quizSignature513(client.state?.quiz),luck:luckSignature507(client.state?.luck),gorilla:gorillaSignature506(client.state?.gorilla),canal:canalSignature489(client),cabbage:cabbageSignature484(client),sugoroku:client.state?.sugoroku,party:(client.state?.ricochet&&!['lobby','result'].includes(client.state.ricochet.phase)||client.state?.cart&&!['lobby','result'].includes(client.state.cart.phase)||client.state?.bomb&&!['lobby','result'].includes(client.state.bomb.phase)||client.state?.tetra&&!['lobby','result'].includes(client.state.tetra.phase)||client.state?.hide&&!['lobby','result'].includes(client.state.hide.phase)||client.state?.fishing&&!['lobby','result'].includes(client.state.fishing.phase)||client.state?.sumo&&!['lobby','result'].includes(client.state.sumo.phase)||client.state?.tower&&!['lobby','result'].includes(client.state.tower.phase)||client.state?.quiz&&!['lobby','result'].includes(client.state.quiz.phase)||client.state?.luck&&!['lobby','result'].includes(client.state.luck.phase))?{...client.state?.party,phase:'play'}:client.state?.party,available:client.state?.available,fatigue455:client.state?.fatigue455,rulesVersion:client.state?.rulesVersion,connected:client.connected(),gold:client.save.state.player.gold,crystals474:client.save.state.player.crystals,crystalPending474:client.key?.()?client.save.state.sugoroku474?.accounts?.[client.key()]?.pending:null,pending:client.bank()?.pending,resultPending490:client.resultPending490,dismissedResult490:client.dismissedResult490,error:client.error,rewardError:client.rewardError})}
