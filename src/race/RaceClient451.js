import{towerView517,towerBefore517,towerAfter517,towerDispose517,towerClick517,towerInput517,towerKey517,towerReceive517,towerFrame517}from'../tower/View517.js';
import{quizView513,quizBefore513,quizAfter513,quizDispose513,quizClick513,quizInput513,quizKey513,quizTick513,quizReceive513,quizError513}from'../quiz/View513.js';
import{luckView507,luckBefore507,luckAfter507,luckDispose507,luckClick507,luckInput507,luckKey507,luckTick507,luckReceive507,luckError507}from'../luck/Bridge511.js';
...
export class RaceClient451{
 constructor({transport,save,displayName=()=>'',onBack=()=>{},toast=()=>{},audio=null}){
  ...
  transport._handleMessage=(m,socket)=>{if(socket&&socket!==transport.ws)return;if(m.type==='towerFrame517'){towerFrame517(this,m);return}if(m.type==='cabbageClock484'){cabbageClockReply484(this,m);return}if(m.type==='raceState451'){this.receive(m);return}if(m.type==='raceError451'){quizError513(this,m);luckError507(this,m);gorillaError505(this,m);memberError485(this,m);resultError490(this,m);if(m.op==='boost'&&(!this.boostPending456||m.raceId==null||m.raceId===this.boostPending456.raceId&&(m.seq??20)>=this.boostPending456.seq))this.boostPending456=null;if(m.op==='sg463'&&this.sgUI463)this.sgUI463.inFlight=null;this.error=m.message;this.actionPending458=null;this.render();return}const result=original(m,socket);if(m.type==='helloAck'){this.presenceKey462=null;if(this.state?.selfId!==this.transport.selfId)this.state=null;this.refresh()}return result};
 ...
 receive(message){
  ...
  const oldTower=this.state?.tower,old=this.state?.room,oldQuiz=this.state?.quiz,oldLuck=this.state?.luck,oldGorilla=this.state?.gorilla,oldCanal=this.state?.canal,oldCabbage=this.state?.cabbage,oldBoard=this.state?.sugoroku;if(this.sgUI463&&oldBoard?.revision!==message.sugoroku?.revision)this.sgUI463.inFlight=null;this.offset=message.serverNow-Date.now();this.state=message;towerReceive517(this);quizReceive513(this);luckReceive507(this);gorillaReceive505(this);memberReceive485(this);if(this.sgUI463?.leaveAfter480&&(message.sugoroku?.paused480||message.sugoroku?.players?.find(p=>p.playerId===this.transport.selfId)?.auto480)){this.sgUI463.leaveAfter480=false;this.onBack()}if(oldTower?.id!==message.tower?.id||oldQuiz?.id!==message.quiz?.id||oldLuck?.id!==message.luck?.id||oldGorilla?.id!==message.gorilla?.id||oldCanal?.id!==message.canal?.id||old?.id!==message.room?.id||oldBoard?.id!==message.sugoroku?.id||oldCabbage?.id!==message.cabbage?.id)this.partyBrowse462=false;resultReceive490(this);const presenceKey=message.party?.id+':'+!!this.root;if(message.party&&this.presenceKey462!==presenceKey){this.presenceKey462=presenceKey;this.raw('partyPresence462',{atHome:!this.root});if(this.root)this.raw('partyRoster462',{roster:this.roster()})}
 ...
 onInput(event){const e=event.target;if(towerInput517(this,e))return;if(quizInput513(this,e))return;if(luckInput507(this,e))return;if(gorillaInput505(this,e))return;if(canalInput489(this,e))return;if(cabbageInput484(this,e))return;if(sugorokuInput463(this,e))return;if(e.name==='raceSort'){this.sort458=e.value;this.rosterLimit453=24;this.render();return}if(e.name==='roomCode')this.draft.code=e.value.toUpperCase();if(e.name==='betAmount'){this.draft.amount=e.value;updateRaceClock451(this)}if(e.name==='monsterSearch'){this.rosterLimit453=24;this.draft.search=e.value;this.render()}}
 onClick(event){if(memberClick485(this,event))return;if(Date.now()<(this.suppressClick458??0))return;const b=event.target.closest('button');if(!b||b.disabled||!this.root?.contains(b))return;this.sound452?.unlock();if(resultClick490(this,b))return;if(towerClick517(this,b))return;if(quizClick513(this,b))return;if(luckClick507(this,b))return;if(gorillaClick505(this,b,event))return;if(canalClick489(this,b,event))return;if(cabbageClick484(this,b,event))return;if(sugorokuClick463(this,b))return;if(partyClick462(this,b))return;if(spectator462(this)&&(b.dataset.betPick!==undefined||b.dataset.betKind||['bet','pass','boost456','candidate458'].includes(b.dataset.raceAction)))return;
