import{gorillaView503,gorillaBefore503,gorillaAfter503,gorillaDispose503,gorillaClick503,gorillaInput503,gorillaKey503,gorillaTick503,gorillaReceive503,gorillaError503}from'../gorilla/View503.js';
...
export class RaceClient451{
 ...
 receive(message){
  ...
  const old=this.state?.room,oldGorilla=this.state?.gorilla,oldCanal=this.state?.canal,oldCabbage=this.state?.cabbage,oldBoard=this.state?.sugoroku;if(this.sgUI463&&oldBoard?.revision!==message.sugoroku?.revision)this.sgUI463.inFlight=null;this.offset=message.serverNow-Date.now();this.state=message;gorillaReceive503(this);memberReceive485(this);if(this.sgUI463?.leaveAfter480&&(message.sugoroku?.paused480||message.sugoroku?.players?.find(p=>p.playerId===this.transport.selfId)?.auto480)){this.sgUI463.leaveAfter480=false;this.onBack()}if(oldGorilla?.id!==message.gorilla?.id||oldCanal?.id!==message.canal?.id||old?.id!==message.room?.id||oldBoard?.id!==message.sugoroku?.id||oldCabbage?.id!==message.cabbage?.id)this.partyBrowse462=false;resultReceive490(this);const presenceKey=message.party?.id+':'+!!this.root;if(message.party&&this.presenceKey462!==presenceKey){this.presenceKey462=presenceKey;this.raw('partyPresence462',{atHome:!this.root});if(this.root)this.raw('partyRoster462',{roster:this.roster()})}
