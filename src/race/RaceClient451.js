...
export class RaceClient451{
 constructor({transport,save,displayName=()=>'',onBack=()=>{},toast=()=>{},audio=null}){
  ...
  transport._handleMessage=(m,socket)=>{if(socket&&socket!==transport.ws)return;if(m.type==='raceState451'){this.receive(m);return}if(m.type==='raceError451'){if(m.op==='boost'&&(!this.boostPending456||m.raceId==null||m.raceId===this.boostPending456.raceId&&(m.seq??5)>=this.boostPending456.seq))this.boostPending456=null;this.error=m.message;this.render();return}const result=original(m,socket);if(m.type==='helloAck'){this.state=null;this.refresh()}return result};
 ...
 ready(){return this.connected()&&this.transport.capabilities?.has('monsterRaceV1')&&this.state?.available===true&&this.state?.rulesVersion>=5}
 connectionMessage(){if(!this.connected())return '接続待ちです。復帰後に現在のレースと保留中の受取を確認します。';if(!this.state)return 'サーバーへレース状況を確認しています。';if(!(this.state.rulesVersion>=5))return '操作できるレースの新ルールにはゲームとサーバー両方の更新が必要です。サーバーをBuild457へ更新・再起動してください。';return 'サーバーのレース保存状態を確認しています。受取・購入の記録を保持して再接続します。'}
 ...
 raw(op,payload={}){if(!this.connected()||!this.transport.capabilities?.has('monsterRaceV1'))return false;return this.transport._send('raceRequest451',{op,rulesVersion:5,...payload})}
 ...
 receive(message){
  ...
  if(old?.id!==message.room?.id){this.draft.ticket={kind:'win',picks:[]};this.paradePinned=null;this.watchPlayer452=null;this.predictionTab457='watch';this.newsOpen457=false;this.detailsOpen457=false;this.error='';this.sound452?.stop()}
 ...
 onClick(event){const b=event.target.closest('button');if(!b||b.disabled||!this.root?.contains(b))return;this.sound452?.unlock();
  ...
  if(b.dataset.predictionTab){this.predictionTab457=b.dataset.predictionTab;this.render();this.root.scrollTop=0;return}
  ...
  if(b.dataset.paradeIndex!==undefined){this.paradePinned=Number(b.dataset.paradeIndex);this.predictionTab457='watch';this.render();this.root.scrollTop=0;return}
