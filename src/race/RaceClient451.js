...
export class RaceClient451{
 ...
 ready(){return this.connected()&&this.transport.capabilities?.has('monsterRaceV1')&&this.state?.available===true&&this.state?.rulesVersion>=7}
 connectionMessage(){if(!this.connected())return '接続待ちです。復帰後に現在のレースと保留中の受取を確認します。';if(!this.state)return 'サーバーへレース状況を確認しています。';if(!(this.state.rulesVersion>=7))return '操作できるレースの新ルールにはゲームとサーバー両方の更新が必要です。サーバーをBuild459へ更新・再起動してください。';return 'サーバーのレース保存状態を確認しています。受取・購入の記録を保持して再接続します。'}
 ...
 raw(op,payload={}){if(!this.connected()||!this.transport.capabilities?.has('monsterRaceV1'))return false;return this.transport._send('raceRequest451',{op,rulesVersion:7,...payload})}
