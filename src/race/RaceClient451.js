...
import{raceDomSignature452,presentation452}from'./RacePresentation452.js';
...
export class RaceClient451{
 constructor({transport,save,displayName=()=>'',onBack=()=>{},toast=()=>{},audio=null}){
  ...
  transport._handleMessage=(m,socket)=>{if(socket&&socket!==transport.ws)return;if(m.type==='raceState451'){this.receive(m);return}if(m.type==='raceError451'){this.error=m.message;this.render();return}const result=original(m,socket);if(m.type==='helloAck'){this.state=null;this.refresh()}return result};
  this.retryClock=setInterval(()=>{if(this.connected()&&transport.capabilities?.has('monsterRaceV1')){this.refresh();this.retryPurchase()}if(this.root)this.renderConnection()},5000);
 ...
 mount(root){if(this.root!==root){this.unmount();this.root=root;root.addEventListener('click',this.click);root.addEventListener('input',this.input);this.animationClock=setInterval(()=>updateRaceClock451(this),33)}this.render();this.transport.startBackground?.();this.refresh();this.retryPurchase()}
 unmount(){if(this.root){this.root.removeEventListener('click',this.click);this.root.removeEventListener('input',this.input)}clearInterval(this.animationClock);this.sound452?.stop();this.root=null}
 ...
 renderConnection(){if(this.lastConnected!==this.connected()){this.lastConnected=this.connected();this.render()}}
 ...
 onClick(event){const b=event.target.closest('button');if(!b||b.disabled||!this.root?.contains(b))return;this.sound452?.unlock();
  ...
  if(b.dataset.betPick!==undefined){const t=this.draft.ticket,i=Number(b.dataset.betPick),max=t.kind==='win'?1:2;t.picks=t.picks.includes(i)?t.picks.filter(n=>n!==i):max===1?[i]:[...t.picks,i].slice(-max);this.render();return}
