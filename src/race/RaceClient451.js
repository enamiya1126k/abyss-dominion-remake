...
export class RaceClient451{
 ...
 onInput(event){const e=event.target;if(sugorokuInput463(this,e))return;if(e.name==='raceSort'){this.sort458=e.value;this.rosterLimit453=24;this.render();return}if(e.name==='roomCode')this.draft.code=e.value.toUpperCase();if(e.name==='betAmount'){this.draft.amount=e.value;updateRaceClock451(this)}if(e.name==='monsterSearch'){this.rosterLimit453=24;this.draft.search=e.value;this.render()}}
 onClick(event){if(Date.now()<(this.suppressClick458??0))return;const b=event.target.closest('button');if(!b||b.disabled||!this.root?.contains(b))return;this.sound452?.unlock();if(sugorokuClick463(this,b))return;if(partyClick462(this,b))return;if(spectator462(this)&&(b.dataset.betPick!==undefined||b.dataset.betKind||['bet','pass','boost456','candidate458'].includes(b.dataset.raceAction)))return;
