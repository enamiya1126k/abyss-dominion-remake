...
export class RaceClient451{
 constructor({transport,save,displayName=()=>'',onBack=()=>{},toast=()=>{},audio=null}){
  ...
  transport._handleMessage=(m,socket)=>{if(socket&&socket!==transport.ws)return;if(m.type==='raceState451'){this.receive(m);return}if(m.type==='raceError451'){if(m.op==='boost'&&(!this.boostPending456||m.raceId==null||m.raceId===this.boostPending456.raceId&&(m.seq??20)>=this.boostPending456.seq))this.boostPending456=null;this.error=m.message;this.actionPending458=null;this.render();return}const result=original(m,socket);if(m.type==='helloAck'){this.presenceKey462=null;if(this.state?.selfId!==this.transport.selfId)this.state=null;this.refresh()}return result};
 ...
 onClick(event){if(Date.now()<(this.suppressClick458??0))return;const b=event.target.closest('button');if(!b||b.disabled||!this.root?.contains(b))return;this.sound452?.unlock();if(partyClick462(this,b))return;if(spectator462(this)&&(b.dataset.betPick!==undefined||b.dataset.betKind||['bet','pass','boost456','candidate458'].includes(b.dataset.raceAction)))return;
