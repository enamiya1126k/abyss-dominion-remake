...
export function raceForecast455(racers,course){const weights=raceWeights455(racers,course),sum=weights.reduce((a,b)=>a+b,0);return weights.map((w,i)=>({index:i,rank:1+weights.filter(v=>v>w+1e-10).length,probability:w/sum,odds:odds451(racers,course,{kind:'win',picks:[i]})}))}

// Keep even extreme matchups uncertain and minimum 1.1x tickets below 100% EV.
export function raceWeights455(racers,course){const w=racers.map(r=>raceWeight451(r,course));if(racers.every(r=>r.rulesVersion>=3)){const sum=w.reduce((a,b)=>a+b,0),max=Math.max(...w),i=w.indexOf(max);if(max>4*(sum-max))w[i]=4*(sum-max)}return w}
