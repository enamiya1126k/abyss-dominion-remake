...
export function advanceSimulation456(s,racers,target,autoIndices=[],record=true){const auto=new Set(autoIndices),until=Math.min(s.maxMs??SIM456.maxMs,Math.floor(Math.max(0,target)/SIM456.step)*SIM456.step);while(s.elapsed<until&&s.order.length<8){
 ...
 for(let i=0;i<8;i++){const x=s.runners[i],r=racers[i];if(x.finishMs!==null)continue;
  ...
  if(auto.has(i)&&x.boosts<5&&s.elapsed>=autoAt(r,x.boosts)*scale&&(s.version>=5||s.elapsed-x.lastBoost>=SIM456.cooldown)&&x.stamina>=SIM456.boostCost)boost456(s,i,x.boostSeq+1);
 ...
 for(let i=0;i<8;i++){const x=s.runners[i],r=racers[i];if(x.finishMs!==null)continue;const t=s.elapsed/1000/scale,style=r.profile.style,pace=style==='逃げ'?(t<7?1.055:t>18?.97:1):style==='追込'?(t<9?.955:t>17?1.07:1):style==='差し'?(t<10?.98:t>13?1.035:1):1.005;
  ...
  x.velocity=(s.version>=7?length/s.track459.seconds:42)*corner*pacePressure*Math.max(.7,1+(x.score-75)*.005)*x.luck*pace*boost*wind*draft*slow*energy*(.99+random(s)*.02);
  const old=x.distance;x.distance=Math.min(length,old+x.velocity*dt);const drain=s.version>=7?(1.9-(r.profile.stamina-70)*.02)/scale*(length===500?.8:length===3000?1.3:length===1600?1.1:1)*(s.front459>=3&&style==='逃げ'&&t<7?1.2:1):1.9-(r.profile.stamina-70)*.008;x.stamina=Math.max(0,x.stamina-dt*drain);
