...
export function createOfflineTicketBattle430(session,campaign,id,seed,issuedAt,ruleVersion=1){
 const engine=new (ruleVersion===2?WorldRaidBattle432:WorldRaidBattle428)({session,now:()=>issuedAt,random:seededRandom430(seed)}),r=engine.create(session,campaign,id);if(!r.ok)throw new Error(r.message);
 return {...r.room,members:[...r.room.members]};
}
export class WorldRaidReplay430{
 constructor(ticket){
  if(![1,2].includes(ticket.ruleVersion))throw new Error('この挑戦権の戦闘形式は未対応です。');
  ...
  this.engine=new (ticket.ruleVersion===2?WorldRaidBattle432:WorldRaidBattle428)({session:this.session,now:()=>this.time,random:this.random,broadcast:(_,message)=>this.events.push(...message.events??[])});
 }
