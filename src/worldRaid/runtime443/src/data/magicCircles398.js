...
export function magicCircleEffects398(id,level=1){
 ...
 if(id==='ch2_prism398'){const perExtraElement=round(.15+.07*p);return{perExtraElement,maxExtraElements:3,summary:`生存味方の属性が1種類増すごとに与ダメ +${pct(perExtraElement)}（2種類以上・最大+${pct(perExtraElement*3)}）`};}
 if(id==='ch2_hex398'){const perAilment=round(.20+.10*p);return{perAilment,maxAilments:3,summary:`敵の状態異常1種類ごとに与ダメ +${pct(perAilment)}（最大3種類・+${pct(perAilment*3)}）`};}
 if(id==='ch2_reserve398'){const damageRate=round(.45+.25*p);return{damageRate,mpThreshold:.8,summary:`命中直前のMP80%以上で与ダメ +${pct(damageRate)}（消費後のMPで判定）`};}
 return null;
}
