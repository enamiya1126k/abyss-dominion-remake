import {worldRaidPercentHp428} from '../worldRaid/WorldRaidDamage428.js';
...
function forceDeath(b,u,source,label){if(ultimateHp(u)<=0)return;if(u.worldRaidBoss428===true){damage(b,source,u,worldRaidPercentHp428(u,ultimateHp(u))*.25,{label:label+'・巨体耐性',ignoreShield:true});return}const before=ultimateHp(u);setHp(u,0);if(tryHeroFortitude(b,u,before)){emit(b,'damage',u,'勇者のふんばり！',before-1,source);return}emit(b,'ultimateDeath',u,label,before,source)}
...
export function replayUltimateOrdinary(b,u,s,{team=ultimateTeam(b,u),scale=1,charge=true,targetId=null,random=Math.random}={}){
 ...
 if((s.power??0)>0)for(const target of targets)for(let hit=0;hit<(s.hits??1)&&ultimateHp(target)>0;hit++){
  ...
  if(s.currentHpDamage&&ultimateHp(target)>0)total+=damage(b,u,target,worldRaidPercentHp428(target,ultimateHp(target))*Math.min(.25,s.currentHpDamage)*scale,{label:s.name});
