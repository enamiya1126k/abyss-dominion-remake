...
export const MAGIC_CIRCLES=freeze([
 ...
 circle("slot_fate","運命の三桁環","777","violet",180_000_000,"行動開始時に000〜999。000は休止、999は即死判定。それ以外は抽選で威力が変わる。","slot"),
 circle("last_life","不屈の残光","1","gold",120_000_000,"戦闘中1回、致死ダメージをHP1で耐える。","lastLife"),
 circle("reincarnation","輪廻の魔法陣","∞","rose",360_000_000,"装着者の与ダメージが常時大幅上昇。戦闘不能時に一度だけ高いHP・MPで蘇生する。","revive"),
 ...
 circle("opening_rite","開戦共鳴陣","20","red",280_000_000,"戦闘開始時、味方全体の与ダメージ・会心率を少し上げる。","openingBuff"),
 ...
 circle("gold_power","黄金換力陣","G","gold",1_100_000_000,"所持GOLDに応じて攻撃上昇（強い逓減・Lv.1は最大+20%、Lv.99は最大+30%）。行動ごとの消費は最大10万G。","goldPower"),
 ...
 circle("raid_zero_sovereign","零界凍結陣","氷","blue",980_000_000,"強大な氷晶の障壁で味方全体を守り、装着者の与ダメージも常時大幅上昇。","shield","raid-zero-sovereign",{staticArt:true}),
 circle("raid_vajra_beast","天雷轟界陣","雷","gold",1_080_000_000,"装着直後から高火力。被弾でさらに与ダメージが上昇し、少ない被弾で追加連撃を得る。","rage","raid-vajra-beast",{staticArt:true}),
 ...CHAPTER_TWO_CIRCLES394,
 ...MAGIC_CIRCLES398
]);

// Tree node 22/31 are the final stretch; chapter-II trophies share that tier.
const LATE_TREE443=new Set(['sacrifice_lottery','sole_survivor','judgment20','weak_critical','crimson_threshold','death_mirror']);
export function magicCircleTier443(id){
 if(id==='none')return {rank:0,label:'装着なし'};
 if(isRaidExclusiveMagicCircle(id))return {rank:3,label:'最上位・レイド限定'};
 if(isChapterTwoCircle394(id)||isMagicCircle398(id))return {rank:2,label:'上位・第二章'};
 if(LATE_TREE443.has(id))return {rank:2,label:'上位・深淵ツリー終盤'};
 return {rank:1,label:'基礎〜中上位・深淵ツリー'};
}
 ...
 * online profile from inventing different values.  Build443 orders player circles by acquisition tier.
...
export function magicCircleLevelEffect(entryOrId,level=1){
 ...
 const base={id:entry.id,effect:entry.effect,level:safeLevel,progress:roundedRate(progress),balanceVersion443:443};
 ...
 if(entry.effect==="slot"){
  const damageMin=roundedRate(.65+.15*progress),damageMax=1.45+.25*progress;
  return freeze({...base,damageMin,damageMax:roundedRate(damageMax),instantKillRoll:999,summary:`通常抽選 ${damageMin.toFixed(2)}〜${damageMax.toFixed(2)}倍`});
 }
 if(entry.effect==="lastLife"){
  const surviveHpRate=.20*progress;
 ...
 if(entry.effect==="revive"){
  const raidExclusive=isRaidExclusiveMagicCircle(entry.id),reviveHpRate=(raidExclusive?0.85:0.40)+(raidExclusive?0.15:0.30)*progress,reviveMpRate=(raidExclusive?0.75:0.25)+(raidExclusive?0.25:0.25)*progress;
  return freeze({...base,reviveHpRate:roundedRate(reviveHpRate),reviveMpRate:roundedRate(reviveMpRate),baseDamageRate:raidExclusive?roundedRate(.60+.25*progress):0,summary:`蘇生 HP${percentText(reviveHpRate)}・MP${percentText(reviveMpRate)}（1戦1回）${raidExclusive?` ／ 常時与ダメ +${percentText(.60+.25*progress)}`:""}`});
 }
 if(entry.effect==="manaReversal"){
  const damageMultiplier=1.15+.10*progress;
 ...
 if(entry.effect==="endgameNoCrit"){
  const damageReductionRate=.10+.10*progress;
 ...
 if(entry.effect==="shield"){
  const raidExclusive=isRaidExclusiveMagicCircle(entry.id),shieldRate=(raidExclusive?1.50:0.50)+(raidExclusive?0.50:0.20)*progress;
  return freeze({...base,shieldRate:roundedRate(shieldRate),baseDamageRate:raidExclusive?roundedRate(.60+.25*progress):0,summary:`味方全体に開戦障壁 最大HP${percentText(shieldRate)}${raidExclusive?` ／ 装着者の常時与ダメ +${percentText(.60+.25*progress)}`:""}`});
 }
 if(entry.effect==="openingBuff"){
  const damageRate=.08+.04*progress,criticalRate=.08+.04*progress;
 ...
 if(entry.effect==="rage"){
  const raidExclusive=isRaidExclusiveMagicCircle(entry.id),damagePerHit=(raidExclusive?0.15:0.04)+(raidExclusive?0.05:0.02)*progress,maxDamageBonus=(raidExclusive?1.50:0.30)+(raidExclusive?0.50:0.20)*progress;
  return freeze({...base,damagePerHit:roundedRate(damagePerHit),maxDamageBonus:roundedRate(maxDamageBonus),baseDamageRate:raidExclusive?roundedRate(.75+.25*progress):0,firstChainHits:raidExclusive?1:4,secondChainHits:raidExclusive?3:9,summary:`${raidExclusive?`常時与ダメ +${percentText(.75+.25*progress)} ／ `:""}被弾ごと +${percentText(damagePerHit)}（被弾分の上限+${percentText(maxDamageBonus)}） ／ ${raidExclusive?"1・3":"4・9"}回被弾で追加連撃1・2回`});
 }
 if(entry.effect==="weakCrit"){
  const criticalCeiling=.48+.12*progress;
  return freeze({...base,criticalCeiling:roundedRate(criticalCeiling),minimumCriticalBonus:.05,damageRate:roundedRate(.30+.15*progress),summary:`与ダメ +${percentText(.30+.15*progress)}・弱攻撃の会心補正 最大+${percentText(criticalCeiling-.10)}`});
 }
 if(entry.effect==="sacrifice"){
  const survivorShieldRate=.45+.20*progress;
 ...
 if(entry.effect==="inheritance"){
  const attackRate=.15+.10*progress,defenseRate=.15+.10*progress,speedRate=.10+.05*progress,turns=Math.round(3+2*progress);
 ...
 if(entry.effect==="deathDrain"){
  const enemyMpDrainRate=.45+.20*progress;
  return freeze({...base,enemyMpDrainRate:roundedRate(enemyMpDrainRate),allyMpRecoveryRate:roundedRate(.15+.10*progress),summary:`戦闘不能時、敵MP −${percentText(enemyMpDrainRate)}・生存味方MP +${percentText(.15+.10*progress)}（1戦1回）`});
 }
 ...
 if(entry.effect==="deathMirror"){
  const reflectedHealRate=.15+.15*progress;
  return freeze({...base,reflectedHealRate:roundedRate(reflectedHealRate),openingShieldRate:roundedRate(.35+.25*progress),summary:`開戦時に自身へHP${percentText(.35+.25*progress)}障壁・最初の即死を反射${reflectedHealRate?`・反射後HP${percentText(reflectedHealRate)}回復`:""}`});
 }
...
function circleMarkup(entry,{className=""}={}){
 ...
 const sources=entry.frames?.length?entry.frames:[entry.asset];
 const frames=sources.map((source,index)=>`<img class="magic-circle-frame magic-circle-frame-${index+1}" src="${source}" alt="" draggable="false">`).join("");
 return`<span class="magic-circle magic-circle-${entry.tone} ${high} ${slot} ${sources.length===1?"magic-circle-static413":""} ${className}" data-circle-id="${entry.id}" data-circle-level="${entry.level}" aria-hidden="true">${frames}<i class="magic-circle-ring-a"></i><i class="magic-circle-ring-b"></i><b>${entry.glyph}</b></span>`;
}
...
export function slotDamageMultiplier(value,level=1){
 ...
 const e=magicCircleLevelEffect("slot_fate",level);
 return Number((e.damageMin+(roll/999)*(e.damageMax-e.damageMin)).toFixed(3));
}
...
export function autoEquipMagicCircle(state,monster){
 ...
 choices.sort((a,b)=>b.level-a.level||magicCircleTier443(b.circleId).rank-magicCircleTier443(a.circleId).rank||Number(b.instanceId===monster.magicCircleInstanceId)-Number(a.instanceId===monster.magicCircleInstanceId));
