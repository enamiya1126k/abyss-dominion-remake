...
export async function prepareOfflineCache430(){
 ...
 if(!registration.active?.scriptURL?.includes('world-raid-offline463-sw.js'))throw new Error('画面を再読み込みしてから挑戦権を取得してください。');
 const response=await fetch('./world-raid-offline463-assets.json',{cache:'no-store'});if(!response.ok)throw new Error('オフライン用データを取得できません。');const urls=await response.json();
