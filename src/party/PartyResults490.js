...
export const resultGame490=c=>c.state?.bomb??c.state?.tetra??c.state?.hide??c.state?.fishing??c.state?.sumo??c.state?.tower??c.state?.quiz??c.state?.luck??c.state?.gorilla??c.state?.canal??c.state?.cabbage??c.state?.sugoroku??c.state?.room;
