...
export function resize517(r,width,height,dpr=1){
 const key=[width,height,Math.min(1.5,dpr),!!r.overview520,r.art?.naturalWidth??0].join(':');if(r.sizeKey522===key)return;r.sizeKey522=key;
...
// Copy only the visible pixels of the tall cached canvases.
function viewport522(c,layer,r){const y=(r.cameraY??0)*r.dpr,h=Math.min(r.canvas.height,layer.height-y);c.drawImage(layer,0,y,r.canvas.width,h,0,0,r.canvas.width,h)}
export function paint517(r,g,players,at,reduced=false){const{ctx:c,width:w,height:h,cell:s,left:l,bottom:b}=r;if(!w)return;c.setTransform(1,0,0,1,0,0);viewport522(c,r.background,r);if(r.boardRevision!==g.boardRevision){r.sc.setTransform(1,0,0,1,0,0);r.sc.clearRect(0,0,r.settled.width,r.settled.height);r.sc.setTransform(r.dpr,0,0,r.dpr,0,0);for(const t of g.board)r.sc.drawImage(tile(r,t.type),l+t.x*s,r.worldBottom-(t.y+1)*s,s,s);r.boardRevision=g.boardRevision}viewport522(c,r.settled,r);c.setTransform(r.dpr,0,0,r.dpr,0,0);
 ...
 if(e.type==='squeeze529'){c.save();c.globalAlpha=(1-t)*.7;c.strokeStyle='#d1f6dd';c.lineWidth=2;for(let j=0;j<3;j++){c.beginPath();c.moveTo(l+e.from*s,y+(j-1)*s*.12);c.lineTo(x+(e.x-e.from)*s*t,y+(j-1)*s*.12);c.stroke()}c.restore()}
