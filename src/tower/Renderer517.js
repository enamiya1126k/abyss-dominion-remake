...
export function resize517(r,width,height,dpr=1){
 ...
 r.cameraY=null;r.bottom=r.worldBottom;r.top=r.bottom-16*r.cell;const{bg:b}=r,wh=r.worldHeight;
 ...
 if(r.art?.complete&&r.art.naturalWidth){b.drawImage(r.art,0,0,width,wh);b.fillStyle='#00120d70';b.fillRect(r.left,r.top,r.cell*10,r.cell*16)}
 b.strokeStyle='#bfddab14';b.lineWidth=.6;
 for(let y=0;y<=16;y++){const py=r.bottom-y*r.cell;b.beginPath();b.moveTo(r.left,py);b.lineTo(width-r.left,py);b.stroke()}
 for(let x=0;x<=10;x++){b.beginPath();b.moveTo(r.left+x*r.cell,r.top);b.lineTo(r.left+x*r.cell,r.bottom);b.stroke()}
 for(const x of[r.left-8,width-r.left+1]){const g=b.createLinearGradient(x,0,x+7,0);g.addColorStop(0,'#574025');g.addColorStop(.45,'#edcf87');g.addColorStop(.65,'#8a6936');g.addColorStop(1,'#33261b');b.fillStyle=g;b.fillRect(x,r.top,7,16*r.cell+12);
  for(let y=0;y<=16;y+=2){const py=r.bottom-y*r.cell;b.fillStyle='#e9ce8a';b.fillRect(x-2,py,11,3);if(y>0&&y<16){b.font='7px sans-serif';b.textAlign=x<width/2?'left':'right';b.fillStyle='#d7cca6aa';b.fillText(String(y),x<width/2?x+12:x-4,py-4)}}}
...
export function paint517(r,g,players,at,reduced=false){const{ctx:c,width:w,height:h,cell:s,left:l,bottom:b}=r;if(!w)return;c.setTransform(1,0,0,1,0,0);c.drawImage(r.background,0,-(r.cameraY??0)*r.dpr);if(r.boardRevision!==g.boardRevision){r.sc.setTransform(1,0,0,1,0,0);r.sc.clearRect(0,0,r.settled.width,r.settled.height);r.sc.setTransform(r.dpr,0,0,r.dpr,0,0);for(const t of g.board)r.sc.drawImage(tile(r,t.type),l+t.x*s,r.worldBottom-(t.y+1)*s,s,s);r.boardRevision=g.boardRevision}c.drawImage(r.settled,0,-(r.cameraY??0)*r.dpr);c.setTransform(r.dpr,0,0,r.dpr,0,0);
 ...
 if(!f&&g.phase==='play'){c.globalAlpha=.65;const a=g.aim,shape=shape517(g.hand[a.slot],a.rotation);for(const [dx,dy] of shape)c.drawImage(tile(r,g.hand[a.slot]),l+(a.x+dx)*s,b-(16.15+dy+1)*s,s,s);c.globalAlpha=1}
 ...
 if(r.worldHeight>h+1&&!r.overview520){const mx=w-45,my=h-80,ms=3.2;c.fillStyle='#031711d9';c.fillRect(mx-4,my-13,40,70);c.strokeStyle='#c5b17677';c.strokeRect(mx-4,my-13,40,70);c.font='7px sans-serif';c.textAlign='center';c.fillStyle='#e4d2a2';c.fillText('全体',mx+16,my-4);
  c.fillStyle='#698d70';for(const t of g.board)c.fillRect(mx+t.x*ms,my+(15-t.y)*ms,ms-.4,ms-.4);
  for(const p of players)if(p.role==='run'&&p.alive){c.fillStyle=p.playerId===r.self520?'#fff3ba':'#a3f6dd';c.fillRect(mx+p.x*ms-1,my+(16-p.y)*ms-2,2.5,2.5)}
  if(f){c.fillStyle='#ffc27a';for(const[dx,dy]of f.cells)if(f.y+dy<16)c.fillRect(mx+(f.x+dx)*ms,my+(15-f.y-dy)*ms,ms-.4,ms-.4)}
  const top=Math.max(0,(r.cameraY-(r.worldBottom-16*s))/s),vh=Math.min(16-top,h/s);c.strokeStyle='#f5e3a5';c.lineWidth=1;c.strokeRect(mx,my+top*ms,32,vh*ms);
 }
