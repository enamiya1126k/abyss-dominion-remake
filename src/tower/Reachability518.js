...
function topology(g) {
 ...
 const occupied=new Uint8Array(10*G.height), outside=new Uint8Array(10*G.height), queue=[];
 for(const b of g.board) if(b.x>=0&&b.x<10&&b.y>=0&&b.y<G.height) occupied[b.y*10+b.x]=1;
 for(let x=0;x<10;x++) if(!occupied[(G.height-1)*10+x]) { outside[(G.height-1)*10+x]=1;queue.push((G.height-1)*10+x); }
 for(let i=0;i<queue.length;i++) { const n=queue[i],x=n%10,y=Math.floor(n/10); for(const v of [x>0?n-1:-1,x<9?n+1:-1,y>0?n-10:-1,y<G.height-1?n+10:-1]) if(v>=0&&!occupied[v]&&!outside[v]){outside[v]=1;queue.push(v);} }
...
export function hasPlacement518(g) {
 for(const type of new Set(g.hand)) {let cells=shapes[names.indexOf(type)]?.map(c=>[...c]);if(!cells)continue;
  for(let rot=0;rot<4;rot++) { const w=1+Math.max(...cells.map(c=>c[0]));for(let x=0;x<=10-w;x++){let y=0;for(const [dx,dy] of cells)for(const b of g.board)if(b.x===x+dx)y=Math.max(y,b.y+1-dy);if(cells.every(([,dy])=>y+dy+1<=G.height))return true;}
...
export function impossible518(g) {
 ...
 const connected=p=>p.y+.41>=G.height||outside[Math.max(0,Math.min(10*G.height-1,Math.floor(p.y+.41)*10+Math.floor(p.x)))];
 ...
 const heights=[0,...g.board.filter(b=>b.y===G.height-1||!occupied[(b.y+1)*10+b.x]).map(b=>b.y+1)].sort((a,b)=>a-b);
 // A cooldown will expire: never declare defeat merely because the boost is not ready yet.
 const reach=G.superRise+.01;
 for(const h of heights)if(h<=high+reach)high=Math.max(high,h);
 return high+reach<G.height?'unreachable':null;
}
export function sealedRunners520(g){
 if(g.phase!=='play'||g.falling)return [];
 const {outside}=topology(g);
 return g.players.filter(p=>p.role==='run'&&p.alive&&!p.escaped&&p.y+.41<G.height&&!outside[Math.max(0,Math.min(10*G.height-1,Math.floor(p.y+.41)*10+Math.floor(p.x)))]);
}
