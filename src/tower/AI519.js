...
function projectedRisk(g,p,A){const f=g.falling;if(!f)return 0;let risk=0;for(const[dx,dy]of f.cells){const bottom=f.y+dy,eta=(bottom-p.y-A.C.ph)/A.fallSpeed(g.elapsed);if(f.land+dy<p.y+A.C.ph&&Math.abs(f.x+dx+.5-p.x)<.82&&eta>=0&&eta<3)risk=Math.max(risk,(3-eta)*10)}return risk}
...
// Stay planted when there is no higher foothold, reachable exit or visible threat.
// Search still handles useful climbing, cooperative head jumps and evasion.
export function botRunner519(g,p,A){if(!p.alive||p.escaped)return{axis:0};let group=brains.get(g);if(!group){group=new Map();brains.set(g,group)}let memory=group.get(p.playerId);const ledgesNow=boardInfo(g,A).ledges;const useful=ledgesNow.some(t=>t.y>p.y+.15&&t.y<=p.y+4.5&&Math.abs(t.x-p.x)<4.4)||p.y>A.C.height-2.2||(g.elapsed>=(p.superReady??0)&&p.y>=A.C.height-A.C.superJump**2/(2*A.C.gravity)+.02);if(p.grounded&&!useful&&!projectedRisk(g,p,A)){group.delete(p.playerId);return{axis:0}}const mates=alive(g).filter(o=>o!==p&&Math.abs(o.x-p.x)<1.15&&Math.abs(o.y-p.y)<1.3);const needsLift=boardInfo(g,A).ledges.some(t=>t.y>p.y+2.17&&t.y<p.y+4.5&&Math.abs(t.x-p.x)<3.3);if(p.grounded&&g.elapsed<(p.superReady??0)&&needsLift&&mates.some(o=>o.seat>p.seat)&&g.elapsed%1700<150&&!projectedRisk(g,p,A))return{axis:0,jump:true};const falling=g.falling?.id??0;
