...
export function layout520(width,height,overview=false){
 const cell=Math.max(4,overview?Math.min((width-24)/10,(height-68)/G.height):(width-24)/10);
 const worldHeight=Math.max(height,cell*(G.height+2.6)+16),worldBottom=worldHeight-14;
...
export function camera520(r,g,focus,dt=0,reduced=false){
 ...
 if(y==null){const runners=g.players.filter(p=>p.role==='run'&&p.alive&&!p.escaped);y=Math.max(0,...runners.map(p=>p.y));
  if(focus?.role==='drop'){const landing=g.falling?.land;y=Math.max(y,Math.min(G.height-1,(landing??y)+1));}}
 ...
 r.cameraY=clamp(r.cameraY,0,max);r.bottom=r.worldBottom-r.cameraY;r.top=r.bottom-G.height*r.cell;
 return{offset:r.cameraY,target,visibleBottom:Math.max(0,(r.worldBottom-r.cameraY-r.height)/r.cell),visibleTop:Math.min(G.height,(r.worldBottom-r.cameraY)/r.cell)};
}
