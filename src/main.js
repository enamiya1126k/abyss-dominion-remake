...
import{equipmentExpNeed,equipmentMaterialExp,enhancementMaterialCandidates,consumeEquipmentMaterial,EQUIPMENT_LEVEL_CAP}from"./services/EquipmentEnhancement.js?v=0.9.15-alpha.4-part1";
...
function equipmentEnhancementBody(item){
 const materials=enhancementMaterialCandidates(save.state,item.id),need=equipmentExpNeed(item.level??1),level=item.level??1,progress=level>=EQUIPMENT_LEVEL_CAP?100:Math.floor(((item.exp??0)/need)*100);
 const rows=materials.slice(0,80).map(material=>{const exp=equipmentMaterialExp(material,item),same=material.name===item.name;return`<button class="equipment-material-row ${same?"same-name":""}" data-equipment-material="${material.id}"><span><b>[${material.rarity}] ${material.name}</b><small>Lv.${material.level??1}${material.plus?` / +${material.plus}`:""}${same?" / 同名ボーナス×5":""}</small></span><strong>+${exp.toLocaleString()} EXP</strong></button>`}).join("")||'<div class="empty">素材にできる装備がありません。<br><small>装備中・お気に入り・ロック中は表示されません。</small></div>';
 return`<div class="equipment-enhancement"><div class="enhancement-target"><small>ENHANCEMENT</small><h3>[${item.rarity}] ${item.name}</h3><b>Lv.${level}${level>=EQUIPMENT_LEVEL_CAP?" MAX":""}</b><div class="equipment-exp large"><i style="width:${progress}%"></i></div><p>${level>=EQUIPMENT_LEVEL_CAP?"現在のレベル上限です":`EXP ${(item.exp??0).toLocaleString()} / ${need.toLocaleString()}`}</p></div><p class="muted">不要装備を1個ずつ合成します。同名装備は獲得EXPが5倍です。</p><div class="equipment-material-list">${rows}</div></div>`
}
function openEquipmentEnhancement(id){
 const item=save.state.equipment.find(i=>i.id===id);if(!item)return;if((item.level??1)>=EQUIPMENT_LEVEL_CAP)return alert("現在のレベル上限に到達しています。");
 app.insertAdjacentHTML("beforeend",Modal("🔨 装備育成",equipmentEnhancementBody(item),"閉じる"));const modal=topModal();
 modal.querySelectorAll("[data-equipment-material]").forEach(button=>button.onclick=()=>{const material=save.state.equipment.find(i=>i.id===button.dataset.equipmentMaterial);if(!material)return;const beforeOwner=item.equippedBy?save.state.monsters.find(m=>m.id===item.equippedBy):null,beforeStats=beforeOwner?calculatedStats(beforeOwner):null,beforeMp=beforeOwner?maxMp(beforeOwner):null;if(!confirm(`${material.name}を素材にしますか？\n獲得EXP ${equipmentMaterialExp(material,item).toLocaleString()}${material.name===item.name?"（同名×5）":""}`))return;const result=consumeEquipmentMaterial(save.state,item.id,material.id);if(!result.ok)return alert(result.message);if(beforeOwner)preserveVitals(beforeOwner,beforeStats,beforeMp);save.save();modal.remove();showToast(result.gained?`${item.name} Lv.${item.level}へ！`:`${result.amount} EXP獲得`);render();openEquipmentEnhancement(item.id)});
 modal.querySelector("#closeGameModal").onclick=()=>modal.remove();
}
...
function win(caught,m){
 ...
 const victorySubtitle=boss?`👑 ${String(boss.name??SPECIES[boss.speciesId]?.name??"BOSS").replace(/^⚔️\s*/,"")}撃破！`:caught?`${m.nickname}を捕獲！`:"";const result=`<div class="battle-result-cinematic ${boss?"boss-clear":""} ${caught?"capture-clear":""}"><div class="victory-rays"></div><div class="victory-title">${boss?"BOSS DEFEATED":caught?"CAPTURE SUCCESS":"VICTORY"}</div>${victorySubtitle?`<div class="victory-subtitle">${victorySubtitle}</div>`:""}</div><div class="reward-summary cinematic-rewards"><b>🪙 +${gold}G</b>${crystalRoll+eliteBonusCrystals?`<b>💎 +${crystalRoll+eliteBonusCrystals}</b>`:""}${eliteBonusGold?`<b class="elite-reward">🜲 エリート討伐 +${eliteBonusGold.toLocaleString()}G</b>`:""}<small>総EXP ${totalExp} / 生存 ${survivors.length}体で分配${firstBoss?"・初回ボス撃破ボーナス":""}</small>${drop?`<b>[${drop.rarity}] ${drop.name}（${slotLabel(drop.slot)}）</b><small>${dropReceipt.message}</small>`:""}${keyDrop?`<b>🗝️ 深淵の鍵を獲得</b>`:""}${caught?`<b>${m.nickname}を捕獲！</b>`:""}</div><div class="exp-results compact">${progress.map(p=>{const hpMax=p.afterStats.hp,mpMax=maxMp(p.x),remaining=Math.max(0,p.need-p.x.exp),diff=k=>p.afterStats[k]-(p.before.stats[k]??0);return`<div class="${p.alive?"":"exp-defeated"} ${p.levels?"level-up-card level-up-reveal":""}"><span>${SPECIES[p.x.speciesId].emoji}</span><section><b>${displayName(p.x)} ${p.levels?`Lv.${p.before.level} → Lv.${p.x.level}`:`Lv.${p.x.level}`}</b><div class="result-vitals"><small>HP ${p.x.currentHp}/${hpMax}</small><small>MP ${p.x.currentMp}/${mpMax}</small><small>${p.alive?`次まであと${remaining}EXP`:"戦闘不能：EXP 0"}</small></div><i class="result-exp"><u style="width:${Math.min(100,p.x.exp/p.need*100)}%"></u></i>${p.levels?`<div class="level-gains"><span>HP ${p.before.stats.hp} → ${p.afterStats.hp} <strong>+${diff("hp")}</strong></span><span>ATK ${p.before.stats.atk} → ${p.afterStats.atk} <strong>+${diff("atk")}</strong></span><span>DEF ${p.before.stats.def} → ${p.afterStats.def} <strong>+${diff("def")}</strong></span><span>SPD ${p.before.stats.spd} → ${p.afterStats.spd} <strong>+${diff("spd")}</strong></span></div>`:""}</section></div>`}).join("")}</div>`;
