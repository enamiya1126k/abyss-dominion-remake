...
import{SkillScreen}from"./ui/screens/SkillScreen.js?v=0.9.15-alpha.9-skill-phase45";
...
import{ENEMY_ACTIONS,createEnemyBattleState,chooseEnemyAction,enemyDamageMultiplier,enemyHealAmount,enemyAttackMultiplier,specialActionMultiplier,specialActionInfo}from"./battle/EnemyAI.js?v=0.9.15-alpha.13-endgame-roster";
...
import{WORLD_MAX_FLOOR,TEAM_BATTLE_UNLOCK_FLOOR,ENDGAME_BOSSES,normalizeEndgameState,dailyTeamAttempts,createTeamBattleEncounter,shouldTriggerEmergency,createEmergencyEncounter,recordEmergencyResult,awardEmergencyFragments,emergencyFragmentStatus,craftEndgameEquipment,hasCleared1000,mark1000FloorCleared,worldRegionForFloor,endgamePreludeOptions,resolveEndgamePrelude,applyPreludeToEncounter}from"./core/EndgameSystem.js?v=0.9.15-alpha.14-endgame-events";
import{baseEnemyLevelForFloor,enemyLevelForFloor as scaledEnemyLevelForFloor,rollEnemyRank,visibleEnemyRank,enemyRankStatMultiplier,equipmentHolderRateForFloor,equipmentSlotsForFloor,rollEnemyEquipmentRarity,bossProfileForFloor,bossLevelForFloor}from"./core/EnemyScalingSystem.js?v=0.9.15-alpha.12-enemy-scaling";
...
function showSecondWorldRandomEvent(){
 ...
 const modal=topModal(),finish=(choice="leave")=>{const outcome=resolveSecondWorldRandomEvent(event,choice);if(!outcome.ok){showToast(outcome.message);return}modal.remove();game.paused=false;if(outcome.elite){pauseModal("⚠️ 裂け目の番人",`<p>${outcome.message}</p><p class="muted">通常より強い敵だ。勝利すれば高い報酬が期待できる。</p>`);const warning=topModal();warning.querySelector("#closeGameModal").textContent="戦う";warning.querySelector("#closeGameModal").onclick=()=>{warning.remove();game.paused=false;beginEncounter(createEliteEncounter({...randomEnemy(),level:enemyLevelForFloor(event.floor)+18,boss:false,equipped:true},event.floor,{forced:true}))};return}pauseModal(event.title,`<p>${outcome.message}</p>`)};
...
function openTeamBattle(){const team=dailyTeamAttempts(save.state);if(save.state.player.maxFloor<TEAM_BATTLE_UNLOCK_FLOOR)return alert("100階突破で解放されます");if(team.dailyAttempts>=50)return alert("本日の挑戦回数50回を使い切りました");app.insertAdjacentHTML("beforeend",Modal("⚔️ チームバトル",`<div class="team-battle-intro"><h2>第${team.stage}試練</h2><p>4体対4体。戦闘前に全回復します。</p><p><b>敗北ペナルティなし</b> / 本日 ${team.dailyAttempts}/50戦</p></div>`,`挑戦する`));const modal=topModal();modal.querySelector("#closeGameModal").onclick=()=>{modal.remove();const prior=capturePartyVitals();fullyRecoverParty();team.dailyAttempts++;save.save();startSpecialBattle(createTeamBattleEncounter(save.state),{type:"team",title:`TEAM BATTLE・第${team.stage}試練`,subtitle:"4 VS 4 / 敗北ペナルティなし",priorVitals:prior})}}
function triggerEmergencyEncounter(){let event=createEmergencyEncounter(save.state);const prior=capturePartyVitals(),options=endgamePreludeOptions(event.boss);fullyRecoverParty();save.save();app.insertAdjacentHTML("beforeend",Modal(event.boss.faction==="tenGod"?"――神が降臨しました。":"――深淵反応を検知。",`<div class="emergency-warning ${event.boss.faction}"><div class="warning-icon">${event.boss.icon}</div><small>${event.manifestation.label} / ${event.manifestation.percent}%</small><h2>${event.boss.name}</h2><p>${event.boss.title}</p><blockquote>${event.boss.encounterText??""}</blockquote><p><b>固有能力：</b>${event.boss.passive??"不明"}</p><p><b>戦闘傾向：</b>${event.boss.ai??"不明"}</p>${event.boss.trial?`<p><b>試練：</b>${event.boss.trial}</p>`:""}<div class="endgame-prelude-grid">${options.map(o=>`<button data-endgame-prelude="${o.id}"><span>${o.icon}</span><b>${o.title}</b><small>${o.desc}</small></button>`).join("")}</div><p class="muted">どの選択でも戦闘は必ず発生。味方は全回復、敗北ペナルティなし。</p></div>`,`選択してください`));const modal=topModal();modal.querySelector("#closeGameModal").disabled=true;modal.querySelectorAll("[data-endgame-prelude]").forEach(button=>button.onclick=()=>{const prelude=resolveEndgamePrelude(save.state,event.boss.id,button.dataset.endgamePrelude);event=applyPreludeToEncounter(event,prelude);save.save();modal.querySelector(".emergency-warning").innerHTML=`<div class="warning-icon">${event.boss.icon}</div><h2>${prelude.title}</h2><p>${prelude.resultText}</p>`;setTimeout(()=>{modal.remove();snapshot=currentSnapshot();stopGame();startSpecialBattle(event.enemies,{type:"emergency",title:event.boss.name,subtitle:`${event.manifestation.label}・${prelude.title}`,priorVitals:prior,bossId:event.boss.id,powerPercent:event.manifestation.percent,bonusFragments:prelude.bonusFragments,preludeId:prelude.id})},700)})}
function startSpecialBattle(enemies,options={}){startBattle(enemies,{specialBattle:true,specialBattleType:options.type,specialTitle:options.title,specialSubtitle:options.subtitle,priorVitals:options.priorVitals,specialBossId:options.bossId,powerPercent:options.powerPercent,bonusFragments:options.bonusFragments??0,preludeId:options.preludeId??null})}
function finishSpecialBattle(won){const type=battle.specialBattleType,prior=battle.priorVitals,bossId=battle.specialBossId;if(type==="team"){const team=dailyTeamAttempts(save.state);won?(team.totalWins++,team.stage++):team.totalLosses++}let fragments=0;if(type==="emergency"){recordEmergencyResult(save.state,battle,won);fragments=awardEmergencyFragments(save.state,bossId,won,battle.bonusFragments??0)}restorePartyVitals(prior);clearPartySynergy();clearBattleCheckpoint();document.querySelector(".battle-screen")?.remove();const boss=bossId?ENDGAME_BOSSES[bossId]:null,status=bossId?emergencyFragmentStatus(save.state,bossId):null;const title=won?"SPECIAL VICTORY":"DEFEAT";const body=won?`<div class="special-result win"><h2>${boss?boss.name:"チームバトル"}を撃退！</h2><p>${type==="team"?"次の試練が解放されました。":(boss?.victoryText??"世界異変を退けました。")}</p>${type==="emergency"?`<div class="fragment-reward"><b>${boss.icon} ${boss.name}の欠片 ×${fragments}</b><small>所持 ${status.count}/${status.required}${status.canCraft?"　製作可能！":""}</small></div>`:""}</div>`:`<div class="special-result lose"><h2>${boss?boss.name:"第試練"}には届かなかった…</h2><p>所持品・階層・仲間へのペナルティはありません。</p>${type==="emergency"?`<div class="fragment-reward"><b>${boss.icon} ${boss.name}の欠片 ×${fragments}</b><small>敗北しても欠片は必ず残る。所持 ${status.count}/${status.required}</small></div>`:""}</div>`;save.save();battle=null;activeEnemy=null;app.insertAdjacentHTML("beforeend",Modal(title,body,type==="team"?"拠点へ戻る":"探索へ戻る"));const modal=topModal();modal.querySelector("#closeGameModal").onclick=()=>{modal.remove();if(type==="team"){snapshot=null;go("home")}else{screen="explore";render()}}}
...
function openEndgameForge(){
 ...
 const modal=topModal();modal.querySelector("#closeGameModal").onclick=closeTopModal;modal.querySelectorAll("[data-craft-endgame]").forEach(b=>b.onclick=()=>craftEndgameGear(b.dataset.craftEndgame));
}
...
function bindSkills(){
 ...
 document.querySelectorAll("[data-skill-filter]").forEach(button=>button.addEventListener("click",()=>{
  ...
  const tag=button.dataset.skillFilter;document.querySelectorAll("[data-skill-filter]").forEach(b=>b.onclick=()=>{document.querySelectorAll("[data-skill-filter]").forEach(x=>x.classList.toggle("active",x===b));document.querySelectorAll("[data-skill-card]").forEach(card=>card.hidden=b.dataset.skillFilter!=="すべて"&&card.dataset.skillTag!==b.dataset.skillFilter)});document.querySelectorAll("[data-skill-card]").forEach(card=>card.hidden=tag!=="すべて"&&card.dataset.skillTag!==tag);
 }));
 document.querySelectorAll("[data-skill-filter]").forEach(b=>b.onclick=()=>{document.querySelectorAll("[data-skill-filter]").forEach(x=>x.classList.toggle("active",x===b));document.querySelectorAll("[data-skill-card]").forEach(card=>card.hidden=b.dataset.skillFilter!=="すべて"&&card.dataset.skillTag!==b.dataset.skillFilter)});document.querySelectorAll("[data-skill-card]").forEach(card=>card.addEventListener("click",()=>{
...
function openHomePartyEditor(){app.insertAdjacentHTML("beforeend",Modal("パーティー編成",partyEditorBody("home"),"確定"));const modal=topModal();modal.dataset.partyEditorMode="home";bindPartyEditor(modal);modal.querySelector("#closeGameModal").onclick=()=>{modal.remove();render()}}
...
function showSummonResults(results,deep=false){save.save();document.querySelector(".game-modal")?.remove();app.insertAdjacentHTML("beforeend",Modal(deep?"深淵召喚結果":"召喚結果",`<div class="gacha-results">${results.map(r=>`<div class="rarity-${r.rarity} ${r.displayRarity==="深淵"?'rarity-deep':''}"><span>${r.icon}</span><b>[${r.displayRarity??r.rarity}] ${r.name}</b><small>${r.type==="monster"?"モンスター":"装備"} NEW</small></div>`).join("")}</div><p class="muted">残り 💎${save.state.player.crystals}</p>`,"ホームへ"));topModalButton().onclick=()=>{document.querySelector(".game-modal")?.remove();render()}}
...
function openMonsterCodexDetail(speciesId,seen,index){const sp=SPECIES[speciesId];if(!sp)return;if(!seen){app.insertAdjacentHTML("beforeend",Modal(`No.${String(index+1).padStart(3,"0")} ？？？？？`,`<div class="codex-detail unknown"><p>まだ遭遇していない魔物です。</p><p>深淵を探索して情報を集めよう。</p></div>`,"図鑑へ戻る"));topModalButton().onclick=closeTopModal;return}const owned=save.state.monsters.filter(m=>m.speciesId===speciesId),captured=save.state.codex.captures[speciesId]??owned.length,base=sp.baseStats??{},sample=[...owned].sort((a,b)=>b.level-a.level)[0],all=allSpeciesSkills(speciesId);const skillRows=all.map(skill=>{const unlocked=sample&&sample.level>=(skill.unlock?.value??1),progress=sample?skillProgressFor(sample,skill.id):null;return`<div class="codex-skill-row ${unlocked?"":"locked"}"><b>${unlocked?skill.name:"？？？？"}</b><small>${unlocked?`${skill.tag} / ${skill.target} / MP${skill.mp}${progress?.level>=10?" / AWAKENED":` / Skill Lv.${progress?.level??1}`}`:`Lv.${skill.unlock?.value??1}で判明`}</small></div>`}).join("");app.insertAdjacentHTML("beforeend",Modal(`${sp.emoji} No.${String(index+1).padStart(3,"0")} ${sp.name}`,`<div class="codex-detail"><div class="codex-detail-head"><span>${sp.emoji}</span><div><b>${codexVisibleRarity(sp.rarity)} / ${elementLabel(sp.element)}</b><small>${sp.race??"不明"} / ${sp.role??"不明"}</small></div></div><div class="detail-stat-grid"><span>HP ${base.hp??"-"}</span><span>ATK ${base.atk??"-"}</span><span>DEF ${base.def??"-"}</span><span>SPD ${base.spd??"-"}</span></div><div class="codex-info-list"><p><b>遭遇</b>${save.state.codex.encounters[speciesId]??0}回</p><p><b>捕獲</b>${captured}回</p><p><b>出現階層</b>${sp.minFloor??"?"}階〜</p><p><b>捕獲率</b>${Math.round((sp.captureRate??0)*100)}%</p></div><h3 class="codex-skill-title">専用スキル 12種</h3><div class="codex-skill-list">${skillRows}</div></div>`,"図鑑へ戻る"));topModalButton().onclick=closeTopModal}
...
function openEquipmentEnhancement(id){
 ...
 modal.querySelector("#closeGameModal").onclick=()=>modal.remove();refreshPreview();
}
...
function openShopMenu(type){
 ...
 const items=type==="herb"?[
  ["potions","🌿","薬草","単体HP100回復",160],
  ["highPotions","🧪","ハイポーション","単体HP300回復",480],
  ["partyPotions","💚","全体回復薬","生存者全員HP80回復",620],
  ["manaPotions","💧","マナポーション","単体MP30回復",240],
  ["highManaPotions","🔷","ハイマナポーション","単体MP100回復",680],
  ["partyManaPotions","🌊","全体マナポーション","生存者全員MP30回復",900],
...
function openFieldItems(){
 game.paused=true;const items=[["potions","❤️","単体回復","HP100回復"],["partyPotions","💚","全体回復","全員HP50回復"],["manaPotions","💧","単体MP回復","MP30回復"],["highManaPotions","🔷","単体MP大回復","MP100回復"],["partyManaPotions","🌊","全体MP回復","全員MP30回復"],["fullManaPotions","💠","単体MP全回復","MPを全回復"],["partyFullManaPotions","🌀","全体MP全回復","全員のMPを全回復"],["statusCures","🩹","状態異常回復・単体","単体の状態異常を解除"],["partyStatusCures","💨","状態異常回復・全体","全員の状態異常を解除"],["fullHeals","✨","全回復＋異常回復・単体","単体を完全回復"],["partyFullHeals","🌟","全回復＋異常回復・全体","全員を完全回復"]],targets=save.state.party.map(id=>save.state.monsters.find(m=>m.id===id)).filter(Boolean);let targetId=targets[0]?.id;
...
function createEnemyGearLoadout(floor,rank,forced=false){
 const equipped=forced||Math.random()<equipmentHolderRateForFloor(floor);if(!equipped)return[];
 const slots=["weapon","armor","accessory","weapon","armor","accessory"],count=equipmentSlotsForFloor(floor);
 return slots.slice(0,count).map(slot=>{const rolled=rollEnemyEquipmentRarity(floor,rank),abyss=rolled==="ABYSS",rarity=abyss?"LR":rolled,item=createEquipment(slot,{rarity});if(abyss)item.name=`深淵・${item.name}`;return item})
}
function randomEnemy(){const floor=save.state.player.currentFloor;if(floor===1)return{speciesId:"slime",level:1,boss:false,equipped:false,gear:null,gearLoadout:[],enemyRank:"N",visibleRank:"N"};if(floor>=2&&Math.random()<.006)return{speciesId:"baby_slime",level:Math.max(1,enemyLevelForFloor(floor)),boss:false,equipped:false,gear:null,gearLoadout:[],enemyRank:"N",visibleRank:"N",rareExp:true};const pool=speciesPoolForFloor(floor).filter(s=>s.id!=="baby_slime"),picked=pool[Math.floor(Math.random()*pool.length)],speciesId=picked.id,enemyRank=rollEnemyRank(floor),gearLoadout=createEnemyGearLoadout(floor,enemyRank);return{speciesId,level:enemyLevelForFloor(floor),boss:false,equipped:gearLoadout.length>0,gear:gearLoadout[0]??null,gearLoadout,enemyRank,visibleRank:visibleEnemyRank(enemyRank,floor),rankStatMultiplier:enemyRankStatMultiplier(enemyRank)}}
...
function floorBossEnemy(){const floor=save.state.player.currentFloor,pool=speciesPoolForFloor(Math.max(floor,10)).filter(s=>s.minFloor<=floor),speciesId=(pool[Math.floor(seeded(floorSeed(floor)+991)()*pool.length)]??SPECIES.slime).id,enemyRank=rollEnemyRank(floor),bossProfile=bossProfileForFloor(floor),gearLoadout=createEnemyGearLoadout(floor,enemyRank,true);return{speciesId,level:Math.max(14,bossLevelForFloor(floor)),boss:true,bossTier:bossProfile.tier,bossProfile,equipped:true,gear:gearLoadout[0]??null,gearLoadout,enemyRank,visibleRank:visibleEnemyRank(enemyRank,floor),rankStatMultiplier:enemyRankStatMultiplier(enemyRank)}}
function loop(now){
 if(!game?.running)return;
...
async function animateSkillEffect(skill,target="enemy"){const layer=document.getElementById("battleFxLayer"),el=battleTarget(target);if(!layer||!el)return;const lr=layer.getBoundingClientRect(),r=el.getBoundingClientRect(),fx=document.createElement("div"),element=skill?.element??"neutral";fx.className=`skill-cast-fx element-${element} ${skill?.awakened?"awakened":""}`;fx.innerHTML=`<span>${({fire:"🔥",water:"❄️",earth:"🪨",wind:"🌪️",dark:"🌑",light:"✨",poison:"☠️",lightning:"⚡"})[element]??"✦"}</span>`;fx.style.left=`${r.left-lr.left+r.width/2}px`;fx.style.top=`${r.top-lr.top+r.height*.45}px`;layer.appendChild(fx);battleFlash(skill?.awakened?"awakened":element);burstParticles(target,element,skill?.awakened?24:14);await wait(skill?.awakened?620:380);fx.remove()}
...
function makeBattleEnemy(e,index=0){const sp=SPECIES[e.speciesId],danger={stats:1},scaled={...e,level:Math.max(1,e.level??1)},enemy=createEnemyBattleState(sp,scaled,save.state.player.currentFloor);enemy.dangerLevel=e.boss?5:e.speciesId==="mimic"?3:e.equipped?3:((e.level??1)>baseEnemyLevelForFloor(save.state.player.currentFloor)*1.05?2:1);if(e.nameOverride)enemy.name=e.nameOverride;const rankMult=Number(e.rankStatMultiplier)||1;if(rankMult!==1){enemy.maxHp=Math.max(1,Math.round(enemy.maxHp*rankMult));enemy.hp=enemy.maxHp;enemy.atk=Math.max(1,Math.round(enemy.atk*rankMult));enemy.def=Math.max(0,Math.round(enemy.def*rankMult));enemy.spd=Math.max(1,Math.round(enemy.spd*Math.sqrt(rankMult)))}if(e.statMultiplier){const mult=Number(e.statMultiplier)||1;enemy.maxHp=Math.max(1,Math.round(enemy.maxHp*mult));enemy.hp=enemy.maxHp;enemy.atk=Math.max(1,Math.round(enemy.atk*mult));enemy.def=Math.max(0,Math.round(enemy.def*mult));enemy.spd=Math.max(1,Math.round(enemy.spd*Math.sqrt(mult)))}if(e.bossProfile){enemy.maxHp=Math.max(1,Math.round(enemy.maxHp*e.bossProfile.hp));enemy.hp=enemy.maxHp;enemy.atk=Math.max(1,Math.round(enemy.atk*e.bossProfile.atk));enemy.def=Math.max(0,Math.round(enemy.def*e.bossProfile.def));enemy.spd=Math.max(1,Math.round(enemy.spd*e.bossProfile.spd));enemy.statusResistance=e.bossProfile.statusResist;enemy.bossTier=e.bossProfile.tier}enemy.enemyRank=e.enemyRank??sp.rarity??"N";enemy.visibleRank=e.visibleRank??enemy.enemyRank;enemy.endgameBossId=e.endgameBossId??null;enemy.faction=e.faction??null;enemy.powerRate=e.powerRate??null;enemy.manifestationLabel=e.manifestationLabel??null;enemy.endgameSupport=Boolean(e.endgameSupport);enemy.uncapturable=Boolean(e.uncapturable);enemy.id=`enemy-${Date.now()}-${index}-${Math.random().toString(36).slice(2,7)}`;enemy.maxHp=Math.max(1,Math.round(enemy.maxHp*danger.stats));enemy.hp=enemy.maxHp;enemy.atk=Math.max(1,Math.round(enemy.atk*danger.stats));enemy.def=Math.max(0,Math.round(enemy.def*danger.stats));enemy.spd=Math.max(1,Math.round(enemy.spd*(1+(danger.stats-1)*.35)));applyEliteModifiers(enemy,e);const loadout=e.gearLoadout?.length?e.gearLoadout:(e.gear?[e.gear]:[]);if(loadout.length){enemy.gear=loadout[0];enemy.gearLoadout=loadout;enemy.name=`⚔️ ${enemy.name}`;for(const gear of loadout){enemy.atk+=gear.stats.atk??0;enemy.def+=gear.stats.def??0;enemy.spd+=gear.stats.spd??0;enemy.maxHp+=gear.stats.hp??0}enemy.hp=enemy.maxHp}return enemy}
function saveBattleCheckpoint(){if(!battle)return;save.state.activeBattle={floor:save.state.player.currentFloor,enemies:battle.enemies,turn:battle.turn,turnQueue:battle.turnQueue,queueIndex:battle.queueIndex,targetEnemyId:battle.targetEnemyId,auto:battle.auto,escapePending:false,guards:battle.guards,cooldowns:battle.cooldowns,enemyStatuses:battle.enemyStatuses,log:battle.log,specialBattle:battle.specialBattle,specialBattleType:battle.specialBattleType,specialTitle:battle.specialTitle,specialSubtitle:battle.specialSubtitle,priorVitals:battle.priorVitals,specialBossId:battle.specialBossId,powerPercent:battle.powerPercent,bonusFragments:battle.bonusFragments??0,preludeId:battle.preludeId??null};save.save()}
...
async function useBattleItem(type,targetId){
 ...
 if(type==="highPotions")target.currentHp=Math.min(calculatedStats(target).hp,target.currentHp+300);
 if(type==="partyPotions")list.filter(m=>m.currentHp>0).forEach(m=>m.currentHp=Math.min(calculatedStats(m).hp,m.currentHp+80));
 if(type==="manaPotions")target.currentMp=Math.min(maxMp(target),target.currentMp+30);
 if(type==="highManaPotions")target.currentMp=Math.min(maxMp(target),target.currentMp+100);
 if(type==="partyManaPotions")list.filter(m=>m.currentHp>0).forEach(m=>m.currentMp=Math.min(maxMp(m),m.currentMp+30));
...
async function command(type,skillId=null){
 ...
 if(type==="skill"&&skillId){
  ...
  if(skill.type==="selfHeal"||skill.type==="stance"&&skill.heal){
   ...
   if(skill.type==="drain"||hasEffect(battle,a.id,"lifeSteal")){const rate=(skill.drain??0)+effectValue(battle,a.id,"lifeSteal"),h=Math.max(1,Math.floor(total*rate));a.currentHp=Math.min(s.hp,a.currentHp+h);await floatText(`+${h}`,a.id,"heal")}
   if(skill.status&&e.hp>0&&Math.random()<skill.status.chance){applyEnemyStatus(battle,{...skill.status,sourceMonsterId:a.id},e.id);addBattleLog(battle,`${e.name}は${skill.status.name}状態になった`);await floatText(skill.status.name,e.id,skill.status.id)}applySkillEffects(skill,a,e)
  }
 }
 ...
 if(type==="capture"){
  ...
  const chance=e.boss?Math.max(.01,Math.min(.10,(.01+(1-e.hp/e.maxHp)*.04)*currentDanger().bossCapture)):Math.max(.08,Math.min(.88,.2+(1-e.hp/e.maxHp)*.55+(Math.max(...battle.party.map(m=>m.level+m.stars*2+m.plus))-e.level)*.012));
...
async function enemyTurn(){
 ...
 if(action===ENEMY_ACTIONS.guard){await floatText("GUARD",e.id,"guard")}
 ...
 else if(specialActionInfo(action)){
  const info=specialActionInfo(action);await battleBanner(info.label,e.name,e.faction==="tenGod"?"boss":"skill",720);battleFlash(e.faction==="tenGod"?"boss":"danger");
  const alive=battle.party.filter(m=>m.currentHp>0),base=specialActionMultiplier(action)*(e.enraged?1.25:1);let targets=[];
  if(info.pattern==="singleWeak")targets=[chooseEnemyTarget(e,"weak")].filter(Boolean);
  ...
  let dealt=0;for(const target of targets){await animateAttack(e.id,true);dealt+=await dealEnemyHit(e,target,base,`${info.label} `,info.pattern==="singleWeak"?.22:.14)}
  if(info.drain){const h=Math.max(1,Math.floor(dealt*info.drain));e.hp=Math.min(e.maxHp,e.hp+h);await floatText(`+${h}`,e.id,"heal")}
  if(info.selfHeal){const h=Math.max(1,Math.floor(e.maxHp*info.selfHeal));e.hp=Math.min(e.maxHp,e.hp+h);await floatText(`+${h}`,e.id,"heal")}
  if(info.selfAtk){e.atk=Math.floor(e.atk*(1+info.selfAtk));await floatText("ATK UP",e.id,"enrage")}
  if(info.selfDef){e.def=Math.floor(e.def*(1+info.selfDef));await floatText("DEF UP",e.id,"guard")}
  if(info.selfSpd){e.spd=Math.floor(e.spd*(1+info.selfSpd));await floatText("SPD UP",e.id,"charge")}
  if(info.copyAtk){const strongest=Math.max(...alive.map(m=>calculatedStats(m).atk),0);e.atk=Math.max(e.atk,Math.floor(strongest*1.15));await floatText("COPY",e.id,"enrage")}
  if(info.barrier){e.divineBarrier=Math.max(e.divineBarrier??0,info.barrier);await floatText("BARRIER",e.id,"guard")}
  if(info.slow){for(const target of targets){target.currentMp=Math.max(0,(target.currentMp??0)-Math.max(1,Math.floor(maxMp(target)*info.slow)));await floatText("MP DOWN",target.id,"miss")}}
 }else{
...
function win(caught,m){
 ...
 const rewardMult=eliteDefeated.length?1.65:1;const gold=Math.round(defeated.reduce((sum,e)=>sum+(e.boss?(firstBoss?80+e.level*14:28+e.level*7):16+e.level*5),0)*rewardMult);
...
function lose(){
 ...
 const modal=topModal(),returnHome=()=>{modal?.remove();battle=null;go("home")};modal._onDismiss=returnHome;modal.querySelector("#closeGameModal").onclick=returnHome
}
