import{SaveService,normalizeRaidJuvenileContract}from"./services/SaveService.js?v=3.1.1-build311";
import{CONTENT_TEST_MODE,BATTLE_SPEED_OPTIONS,CAMERA_DRAG_THRESHOLD_PX,WATER_RULES,MONSTER_STAR_MAX,MONSTER_STORAGE_CAP,ENDGAME_MAX_LEVEL,premiumCrystalCost,normalizeBattleSpeed,contentUnlockFloor,isContentUnlocked}from"./core/config.js?v=3.1.1-build311";
...
import{HomeScreen,homePartySlots}from"./ui/screens/HomeScreen.js?v=3.1.1-build311";
...
import{OnlinePartyController,resetCurrentWeeklyRaidForFullReset}from"./online/OnlinePartyClient.js?v=3.1.1-build311";
...
import{CampaignFinalFloorScreen}from"./ui/screens/CampaignFinalFloorScreen.js?v=3.1.1-build311";
...
import{AbyssSkillTreeScreen}from"./ui/screens/AbyssSkillTreeScreen.js?v=3.1.1-build311";
import{InventoryScreen,ArmoryScreen}from"./ui/screens/InventoryScreen.js?v=3.1.1-build311";
import{abyssEquipmentRarityBonus,abyssExplorationChance,abyssSkillEffectTotal,abyssSkillEffects,abyssSkillMultiplier,abyssSkillNodeById,abyssSkillTreeSummary,learnAbyssSkill}from"./core/AbyssSkillTreeSystem.js?v=3.1.1-build311";
...
import{ENEMY_ACTIONS,createEnemyBattleState,chooseEnemyAction,enemyActionMpCost,enemyDamageMultiplier,enemyDamageAfterDefense,enemyHealAmount,enemyAttackMultiplier,specialActionMultiplier,specialActionInfo}from"./battle/EnemyAI.js?v=3.1.1-build311";
...
import{generateSectionDungeon,sectionIdAt,sectionRoute,portalTowardSection,sectionBounds,safeSectionExitCandidates,chooseSafeSectionExitCell}from"./core/DungeonSectionSystem.js?v=3.1.1-build311";
...
import{applyGameMasterReward,applySerialReward,commitSerialRedemption,validateGameMasterCode,validateSerialCode}from"./core/SerialCodeSystem.js?v=3.1.1-build311";
import{runConfirmedFullReset}from"./core/FullResetSystem.js?v=3.1.1-build311";
import{NOTICE_DEFINITIONS,DAILY_NOTICE_GIFT,markNoticeRead,normalizeNoticeState,dailyNoticeGiftStatus,claimDailyNoticeGift,noticeAttentionCount,pendingNoticeRewards,claimNoticeReward,enqueueNoticeReward}from"./core/NoticeSystem.js?v=3.1.1-build311";
import{COMPLETE_MONSTER_CODEX,codexCollectionSummary,syncCollectionRewardInbox,rewardDescription}from"./core/CollectionRewardSystem.js?v=3.1.1-build311";
import{achievementSummary,syncAchievementRewardInbox,achievementIconKeyForId}from"./core/AchievementRewardSystem.js?v=3.1.1-build311";
...
const SCREEN_SESSION_KEY="abyss-dominion:current-screen",INVITE_SESSION_KEY="abyss-dominion:last-party-invite",REFRESHABLE_SCREENS=new Set(["home","formation","onlineParty","monsters","settings","explore","campaignFinalFloor","gauntlet","equipment","shop","skills","abyssSkills","inventory","armory"]);
...
function render(){
 ...
 queueCampaignStoryScenes({delay:220});
}
...
function settleAbandonedCampaignHeroPursuit(reason="field-abandoned"){
 ...
 normalizeCampaignState(save.state).heroEncounters310=settled.state;
...
function showCampaignEnding(ending){
 const outcome=recordCampaignEnding(save.state,ending),copy={complete:{title:"完全勝利",lead:"四体すべてが立ったまま、残った勇者を正面から退けた。",detail:"道中で刻んだ傷も、十日間で築いた魔王軍も、すべてが予言を打ち破る力になった。"},narrow:{title:"辛勝",lead:"最後に立っていた仲間が戦線をつなぎ、勇者一行を退けた。",detail:"倒れた仲間が残した一撃まで、すべてが紙一重の勝利へつながった。"},"all-preempted":{title:"予言外の結末",lead:"勇者四人は魔王城へ着く前に、全員が道中で退けられた。",detail:"決戦場に戦う者はいない。リオネルの予言を最も鮮やかに覆した結末だ。"},defeat:{title:"敗北",lead:"現在の部隊は勇者一行に届かなかった。",detail:"リオネルが予言を九日目へ巻き戻す。育成・装備・与えた傷は失われない。"}}[outcome.ending];
 const ledger=campaignHeroLedger(),finalHeroIds=campaignRemainingHeroes(ledger).map(entry=>entry.heroId),finalVoices=outcome.victorious?campaignHeroFinalVoiceHtml("finalPlayerWin",finalHeroIds):"";
 ledger.finalArena={...ledger.finalArena,completed:outcome.victorious,lastEnding:outcome.ending,battleStarted:false};normalizeCampaignState(save.state).heroEncounters310=ledger;if(outcome.victorious){mark10000FloorCleared(save.state);save.state.flags.ending10000Played=true}restoreCampaignFinalParty();clearPartySynergy();clearBattleCheckpoint({saveNow:false});document.querySelector(".battle-screen")?.remove();activeEnemy=null;battle=null;save.state.player.inRun=false;clearExpeditionSnapshot();try{sessionStorage.setItem(SCREEN_SESSION_KEY,"home")}catch{}
 let persisted=Boolean(save.save());app.insertAdjacentHTML("beforeend",Modal(`予言の十日間・${copy.title}`,`<div class="campaign-ending ending-${outcome.ending}"><small>予言10日目・勇者軍最終決戦</small><h2>${copy.title}</h2><p>${copy.lead}</p><p>${copy.detail}</p>${finalVoices}<b>${persisted?`${outcome.victorious?"勝利":"敗北"}の記録を保存しました。 探索・育成データは失われません。`:"記録をまだ保存できていません。空き容量を確認して再試行してください。"}</b></div>`,persisted?"魔王城へ戻る":"保存して魔王城へ戻る"));const modal=configureCampaignOutcomeModal(topModal(),"campaignEndingTitle"),finish=()=>{if(!persisted){persisted=Boolean(save.save());if(!persisted){showToast("結末を保存できませんでした。容量を確認してください");return}}modal?.remove();go("home")};modal._onDismiss=finish;modal.querySelector("[data-modal-primary]").onclick=finish
}
function showCampaignDefeatRewind(resultId){
 recordCampaignEnding(save.state,"defeat");let ledger=campaignHeroLedger();const finalHeroIds=campaignRemainingHeroes(ledger).map(entry=>entry.heroId),finalVoices=campaignHeroFinalVoiceHtml("finalHeroesWin",finalHeroIds),rewind=beginCampaignDay9Rewind(ledger,{resultId,reason:"final-defeat"});ledger=rewind.state;normalizeCampaignState(save.state).heroEncounters310=ledger;restoreCampaignFinalParty();clearPartySynergy();clearBattleCheckpoint({saveNow:false});document.querySelector(".battle-screen")?.remove();activeEnemy=null;battle=null;save.state.player.inRun=false;save.state.player.currentFloor=81;clearExpeditionSnapshot();
 try{sessionStorage.setItem(SCREEN_SESSION_KEY,"home")}catch{}
 let persisted=Boolean(save.save());app.insertAdjacentHTML("beforeend",Modal("予言の十日間・敗北",`<div class="lionel-rewind"><header><small>予言10日目・勇者軍最終決戦</small><strong class="lionel-rewind-defeat">敗北</strong><p>現在の部隊は勇者一行に届かなかった。</p><h2>まだ、結末は定まっていない</h2><p>リオネルが時間を予言9日目へ巻き戻す。</p></header>${finalVoices}<section class="lionel-rewind-stage"><span class="lionel-rewind-art">${monsterVisual({speciesId:"space_archon",visualSpeciesId:"196_star_oracle_noctiel"},"リオネル",{className:"lionel-rewind-visual"})}</span><div class="lionel-rewind-clock"><span class="lionel-rewind-day is-day-10"><small>PROPHECY</small><b>10日目</b></span><span class="lionel-rewind-day is-day-9"><small>REWIND</small><b>9日目</b></span></div></section><p class="lionel-rewind-note"><b>81階から再開。</b> 仲間・装備・所持品と、道中で勇者へ与えた傷はすべて保持されます。最終決戦中だけの損傷は巻き戻ります。${persisted?"":"<br>巻き戻しをまだ保存できていません。容量を確認して再試行してください。"}</p></div>`,persisted?"予言9日目から再開":"保存して予言9日目へ"));const modal=configureCampaignOutcomeModal(topModal(),"campaignRewindTitle"),finish=()=>{if(!persisted){persisted=Boolean(save.save());if(!persisted){showToast("巻き戻しを保存できませんでした。容量を確認してください");return}}modal?.remove();go("home")};modal.classList.add("lionel-rewind-modal");modal._onDismiss=finish;modal.querySelector("[data-modal-primary]").onclick=finish
}
function finishCampaignFinalBattle(won){
 const current=battle,survivingAllies=(current?.party??[]).filter(monster=>Number(monster?.currentHp)>0).length,resultId=`${current?.battleId??Date.now()}:final`,ending=campaignHeroEndingForResult(campaignHeroLedger(),{partyWon:Boolean(won),partySurvivors:survivingAllies,partySize:4}),victorious=ending!=="defeat";audio.setScene(victorious?"victory":"defeat");audio.sfx(victorious?"victory":"defeat");
 if(!victorious)return showCampaignDefeatRewind(resultId);
 return showCampaignEnding(ending)
}
...
function renderCampaignFinalFloor(){
 const ledger=campaignHeroLedger();if(ledger.rewind?.active||!ledger.finalArena?.unlocked||ledger.finalArena?.completed){screen="home";render();return}const party=(save.state.party??[]).map(id=>save.state.monsters.find(monster=>monster.id===id)).filter(Boolean).slice(0,4),heroes=CAMPAIGN_HERO_IDS.map(id=>({id,name:campaignHeroName(id),...(ledger.heroes?.[id]??{})}));app.innerHTML=CampaignFinalFloorScreen({heroes,party,rewindCount:ledger.rewind?.count??0});
 document.querySelector("[data-final-floor-return]")?.addEventListener("click",()=>go("home"));document.querySelector("[data-final-floor-formation]")?.addEventListener("click",()=>{formationOrigin="campaignFinalFloor";go("formation")});document.querySelector("[data-final-floor-approach]")?.addEventListener("click",()=>openCampaignFinalPreparation())
}
...
function finishCampaignHeroEncounterBattle(won,{retreated=false}={}){
 const current=battle,enemy=(current?.enemies??[]).find(entry=>entry.campaignHeroId)??current?.enemies?.[0],heroId=current?.campaignHeroId??enemy?.campaignHeroId,encounterId=current?.campaignHeroEncounterId??enemy?.campaignHeroEncounterId,resultId=`${current?.battleId??Date.now()}:hero-field:${retreated?"retreat":won?"repelled":"overwhelmed"}`,hpRate=won?0:Math.max(0,Math.min(1,Number(enemy?.hp)/Math.max(1,Number(enemy?.maxHp)||1))),settled=settleCampaignHeroEncounter(campaignHeroLedger(),{encounterId,resultId,heroId,outcome:won?"repelled":retreated?"escaped":"hero-victory",floor:save.state.player.currentFloor,hpRate,repelled:won});normalizeCampaignState(save.state).heroEncounters310=settled.state;
...
function openExploreFloorSelector(){
 const heroLedger=campaignHeroLedger(),rewindMax=heroLedger.rewind?.active?Math.max(81,Math.min(CAMPAIGN_MAX_FLOOR,Number(heroLedger.rewind.currentFloor)||81)):null,reachedMax=rewindMax??Math.min(WORLD_MAX_FLOOR,Math.max(1,Number(save.state.player.maxFloor)||1));
...
function hydrateExpeditionSnapshot(source){
 ...
 // Build303 snapshots use the same disconnected-section contract and remain
 // playable. Build307 adds per-section scale/shape metadata, but does not make
 // that metadata a requirement for restoring a run already in progress.
 const bossIds=campaignBossIdsForFloor(source.floor);if(!game?.online&&![303,307,308,310].includes(Number(world.layoutVersion)))return null;
...
function orderedCampaignStoryCharacters(scene){
 const preferred=scene.kind==="opening"?["lionel","sairan"]:HERO_PARTY_IDS,order=new Map(preferred.map((id,index)=>[id,index]));
...
function campaignStoryCharacterArt(character){
 ...
 return monsterVisual(subject,"人物",{frame:"idle1",className:"campaign-story-character-art"})
}
function campaignStoryPresentationBody(scene){
 ...
 const heading=scene.kind==="opening"?(scene.title??"予言の十日間"):`予言 ${scene.day}日目`,eyebrow=scene.kind==="opening"?"PROLOGUE / 魔王城":"HEROES ON THE ROAD",route=scene.kind==="opening"?`<div class="campaign-story-countdown"><span>予言の日まで</span><b>残り10日</b></div>`:`<div class="campaign-story-route"><span>西の大陸</span><i role="progressbar" aria-label="魔王城への進軍度" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${progress}"><em style="width:${progress}%"></em></i><b>魔王城</b></div>`;
 return`<section class="campaign-story-presentation variant-${escapeAttribute(scene.variant??"default")}" data-story-kind="${escapeAttribute(scene.kind)}" style="--story-backdrop:url('${background}');--story-progress:${progress}%;--story-castle-scale:${castleScale};--story-castle-opacity:${castleOpacity}"><header><small>${eyebrow}</small><h3>${escapeAttribute(heading)}</h3><b>${escapeAttribute(scene.location??"")}</b><p>${escapeAttribute(scene.summary??"")}</p></header>${route}<div class="campaign-story-stage"><div class="campaign-story-scenery" aria-hidden="true"><i></i><u></u></div><div class="campaign-story-cast" data-story-cast>${cast}</div><article class="campaign-story-dialogue" data-story-dialogue aria-live="polite"><small data-story-speaker-title></small><b data-story-speaker></b><p data-story-text></p></article></div><nav class="campaign-story-line-progress" data-story-line-progress aria-label="会話の進行"></nav></section>`
}
function showCampaignStoryScene(scene){
 if(!scene||campaignStoryPresenting)return false;
 const campaign=save.state.campaign100??(save.state.campaign100={}),hadStory=Object.prototype.hasOwnProperty.call(campaign,"story309"),storyBefore=hadStory?(typeof structuredClone==="function"?structuredClone(campaign.story309):JSON.parse(JSON.stringify(campaign.story309))):null,receipt=acknowledgeCampaignStoryScene(save.state,scene.id,{seenAt:new Date().toISOString()});
 if(!receipt.recorded||!save.save()){if(hadStory)campaign.story309=storyBefore;else delete campaign.story309;if(receipt.recorded)showToast("物語の記録を保存できませんでした");return false}
 campaignStoryPresenting=true;const resumeRunningGame=Boolean(game?.running&&!game.paused);if(game?.running)game.paused=true;
 app.insertAdjacentHTML("beforeend",Modal(scene.kind==="opening"?"序章":"勇者一行の道中",campaignStoryPresentationBody(scene),"次へ"));
...
function refreshCampaignHeroChaseHud(){
 const stage=document.querySelector(".explore-stage"),pursuit=game?.world?.campaignHeroPursuit;if(!stage)return;if(!pursuit||pursuit.state==="resolved"){stage.classList.remove("is-hero-hunt");stage.querySelectorAll(".campaign-hero-chase-hud,.campaign-hero-edge-chip").forEach(node=>node.remove());return}
 const heroId=pursuit.heroId,name=campaignHeroName(heroId),record=campaignHeroLedger().heroes?.[heroId],percent=Math.round(Math.max(0,Math.min(1,record?.remainingHpRate??1))*100);stage.classList.add("is-hero-hunt");let hud=stage.querySelector(".campaign-hero-chase-hud");if(!hud){hud=document.createElement("div");hud.className="campaign-hero-chase-hud is-discovered";stage.appendChild(hud)}hud.classList.toggle("is-tracking",pursuit.state==="pursuing");hud.classList.toggle("is-contact",pursuit.state==="contact");hud.innerHTML=`<span class="campaign-hero-chase-portrait">${monsterVisual({speciesId:heroId,visualSpeciesId:heroId},name,{className:"campaign-hero-chase-visual"})}</span><span class="campaign-hero-chase-copy"><small>${pursuit.state==="observing"?"視線を感じる":"勇者に発見された"}</small><b>${name}が${pursuit.state==="observing"?"こちらを観察している":"追跡中"}</b></span><span class="campaign-hero-chase-state">残存HP ${percent}%<i class="campaign-hero-wound-meter" style="--hero-hp:${percent}%"><i></i></i></span>`;
...
function drawCampaignSectionPortal(portal){
 ...
 // The portal is a continuation of the floor which feathers into darkness,
 // not a black panel laid on top of it. All four exits share one organic mouth
 // and rotate toward their authored cardinal direction.
 c.save();c.translate(p.x+size*.5,p.y+size*.5);c.rotate(angle);
 const mouthPath=()=>{c.beginPath();c.moveTo(-size*.86,size*.74);c.bezierCurveTo(-size*.78,size*.32,-size*.72,-size*.2,-size*.55,-size*1.28);c.quadraticCurveTo(0,-size*1.43,size*.55,-size*1.28);c.bezierCurveTo(size*.72,-size*.2,size*.78,size*.32,size*.86,size*.74);c.quadraticCurveTo(0,size*.58,-size*.86,size*.74);c.closePath()};
 c.save();c.filter=`blur(${Math.max(2,size*.11)}px)`;c.globalAlpha=.5;const feather=c.createLinearGradient(0,size*.8,0,-size*1.38);feather.addColorStop(0,"rgba(0,0,0,0)");feather.addColorStop(.3,"rgba(0,0,0,.52)");feather.addColorStop(.78,"rgba(0,0,0,.96)");feather.addColorStop(1,"rgba(0,0,0,.68)");c.fillStyle=feather;mouthPath();c.fill();c.restore();
 const tunnel=c.createLinearGradient(0,size*.74,0,-size*1.36);tunnel.addColorStop(0,"rgba(2,3,6,0)");tunnel.addColorStop(.18,"rgba(1,2,5,.24)");tunnel.addColorStop(.46,"rgba(0,0,0,.9)");tunnel.addColorStop(.8,"rgba(0,0,0,.99)");tunnel.addColorStop(1,`${targetTheme.accent}34`);c.fillStyle=tunnel;mouthPath();c.fill();
 const rim=c.createLinearGradient(-size*.9,0,size*.9,0);rim.addColorStop(0,`${targetTheme.accent}0a`);rim.addColorStop(.18,"rgba(0,0,0,.82)");rim.addColorStop(.5,"rgba(0,0,0,0)");rim.addColorStop(.82,"rgba(0,0,0,.82)");rim.addColorStop(1,`${targetTheme.accent}0a`);c.fillStyle=rim;mouthPath();c.fill();
 c.globalCompositeOperation="screen";c.globalAlpha=.24+.18*pulse;c.fillStyle=`${targetTheme.accent}70`;c.beginPath();c.moveTo(-size*.5,-size*1.25);c.quadraticCurveTo(0,-size*1.38,size*.5,-size*1.25);c.quadraticCurveTo(0,-size*1.18,-size*.5,-size*1.25);c.closePath();c.fill();c.globalCompositeOperation="source-over";c.globalAlpha=.38+.24*pulse;c.strokeStyle=targetTheme.accent;c.lineWidth=Math.max(1,size*.026);c.beginPath();c.moveTo(-size*.73,size*.51);c.bezierCurveTo(-size*.67,-size*.05,-size*.63,-size*.72,-size*.51,-size*1.24);c.moveTo(size*.73,size*.51);c.bezierCurveTo(size*.67,-size*.05,size*.63,-size*.72,size*.51,-size*1.24);c.stroke();c.restore();
 drawExploreParticles(portal,targetTheme.accent,3,portal.x+portal.y,.4);const interiorOffset={north:{x:0,y:.92},east:{x:-1.04,y:0},south:{x:0,y:-.92},west:{x:1.04,y:0}}[portal.direction]??{x:0,y:.92},distance=Math.abs(Number(game.player?.x)-Number(portal.x))+Math.abs(Number(game.player?.y)-Number(portal.y));if(distance<=5){const label=`${profile.name}の区画へ`,labelX=Math.max(size*1.2,Math.min(game.canvas.width-size*1.2,p.x+size*(.5+interiorOffset.x))),labelY=Math.max(size*.32,Math.min(game.canvas.height-size*.32,p.y+size*(.5+interiorOffset.y)));c.save();c.textAlign="center";c.textBaseline="middle";c.font=`900 ${Math.max(8,size*.12)}px "Yu Mincho",serif`;c.strokeStyle="#020305";c.lineWidth=Math.max(2,size*.05);c.strokeText(label,labelX,labelY,size*2.5);c.fillStyle="#f0e7d3";c.fillText(label,labelX,labelY,size*2.5);c.restore()}
}
...
async function enemyTurn(){
 ...
 if(action===ENEMY_ACTIONS.guard){await floatText("防御",e.id,"guard")}
  ...
  const target=chooseEnemyTarget(e,e.campaignHeroTargetMode??(e.endgameBossId?"threat":"normal"));if(!target){battle.busy=false;return lose()};await animateAttack(e.id,action===ENEMY_ACTIONS.power);
