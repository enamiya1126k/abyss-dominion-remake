import{SaveService}from"./services/SaveService.js?v=1.8.0";
import{SPECIES}from"./data/species.js?v=1.8.0";
import{HomeScreen}from"./ui/screens/HomeScreen.js?v=1.8.0";
import{FormationScreen}from"./ui/screens/FormationScreen.js?v=1.8.0";
import{MonsterListScreen}from"./ui/screens/MonsterListScreen.js?v=1.8.0";
import{MonsterDetailScreen}from"./ui/screens/MonsterDetailScreen.js?v=1.8.0";
import{SettingsScreen}from"./ui/screens/SettingsScreen.js?v=1.8.0";
import{ExploreScreen}from"./ui/screens/ExploreScreen.js?v=1.8.0";
import{BattleScreen}from"./ui/screens/BattleScreen.js?v=1.8.0";
import{Modal}from"./ui/components/Modal.js?v=0.9.15-alpha.28-phase10-6-consistency";
import{createMonster,displayName,calculatedStats,TRAITS,expNeedFor,limitBreakGrowth,affectionBonuses,totalExperience,applyTotalExperience}from"./models/Monster.js?v=1.8.0";
...
import{recordWeaponKill,weaponMasteryDamageMultiplier,weaponMasterySummary}from"./services/WeaponMastery.js?v=1.8.0";
...
import{EquipmentScreen}from"./ui/screens/EquipmentScreen.js?v=1.8.0";
...
import{SkillScreen}from"./ui/screens/SkillScreen.js?v=1.8.0";
...
import{maxMp,learnedSkills,allLearnedSkills,equipSkill,skillById,canUseSkill,effectiveSkillMpCost,skillDamage,affixOutgoingDamageMultiplier,chooseAutoSkill,skillProgressFor,recordSkillUse}from"./battle/SkillSystem.js?v=1.8.0";
...
import{buildTurnQueue,currentTurnEntry,currentAlly,currentEnemy,aliveEnemies,selectedEnemy,advanceQueue,queueFinished,skipInvalidEntries}from"./battle/TurnSystem.js?v=1.8.0";
...
import{monsterCombatPower,partyCombatPower,formatCombatPower,recordPartyCombatPower}from"./core/CombatPower.js?v=1.8.0";
...
import{applySerialReward,commitSerialRedemption,validateSerialCode}from"./core/SerialCodeSystem.js?v=1.8.0";
import{monsterSpriteUrl,monsterVisual,setMonsterVisualFrame}from"./ui/MonsterVisual.js?v=1.8.0";
...
function idleReturnPreviewBody(preview){
 return`<div class="return-reward-report idle-return-preview"><div class="idle-return-emblem">🎁</div><p><span>放置探索時間</span><b>${compactElapsedText(preview.elapsedMs)}${preview.capped?"・上限到達":""}</b></p><p><span>探索地点</span><b>最高到達の${Math.round(preview.expeditionRate*100)}%・${preview.expeditionFloor}階層帯</b></p><p><span>換算探索量</span><b>${preview.floorUnits}階層分</b></p><p class="return-reward-gold"><span>受取GOLD</span><b>${preview.gold.toLocaleString()}G</b></p><p><span>装備ドロップ</span><b>${preview.equipmentCount}個</b></p>${returnRarityTable()}<small>5分ごとに報酬が増加。現在の上限は${preview.maxHours}時間です。</small></div>`;
}
...
function bindHome(){
 ...
 document.getElementById("openAbyssSkillTree")?.addEventListener("click",()=>{abyssSkillCategory="economy";go("abyssSkills")});
 document.getElementById("openTeamBattle")?.addEventListener("click",openTeamBattle);
 document.getElementById("openEndgameForge")?.addEventListener("click",openEndgameForge);
 ...
 document.getElementById("openDeepGacha")?.addEventListener("click",openDeepGacha);
 document.getElementById("openWorldRecord")?.addEventListener("click",openWorldRecord);
 document.getElementById("openCodexHub")?.addEventListener("click",openCodexHub);
 ...
 document.getElementById("openExplore").onclick=()=>{
  ...
  app.insertAdjacentHTML("beforeend",Modal("探索開始",`<p>再開する階層を選択</p><input id="floorSelect" type="number" min="1" max="${max}" value="${max}"><p class="muted">1〜${max}階。到達済みの階層から再開できます。</p>`,`出発`));
  const modal=topModal(),button=modal.querySelector("[data-modal-primary]");
...
async function redeemSettingsSerialCode(event){
 ...
 app.insertAdjacentHTML("beforeend",Modal(`${result.icon} ${result.title}`,`<div class="serial-reward-result"><div>${result.icon}</div><p><b>${result.message}</b></p><small>このコードは使用済みとして記録されました。</small></div>`,"確認"));
...
function formationPickerBody(replacingId=null){
 ...
 const rows=reserve.map(monster=>{const species=SPECIES[monster.speciesId]??{},stats=calculatedStats(monster),power=monsterCombatPower(monster),searchText=escapeAttribute(`${displayName(monster)} ${species.name??""} ${species.race??""}`.toLowerCase());return`<button class="formation-picker-row" data-formation-pick="${monster.id}" data-search="${searchText}"><span>${monsterVisual(monster.speciesId,species.emoji??"👹",{className:"formation-picker-monster-visual"})}</span><div><b>${displayName(monster)}</b><small>${monsterVisibleRarity(monster)}・${elementLabel(species.element)}・Lv.${monster.level}・★${monster.stars??1}・+${monster.plus??0}</small><em>累計EXP ${totalExperience(monster).toLocaleString()} / HP ${stats.hp} / ATK ${stats.atk} / DEF ${stats.def} / SPD ${stats.spd}</em></div><strong><b>戦力 ${formatCombatPower(power)}</b><small>${replacingId?"交代":"編成"}</small></strong></button>`}).join("");
...
function confirmFormationReplacement(outgoingId,incomingId){
 ...
 app.insertAdjacentHTML("beforeend",Modal("部隊メンバーを交代",`<div class="formation-transfer">
  <div class="formation-transfer-route"><span>${monsterVisual(outgoing.speciesId,outgoingSpecies.emoji??"👹",{className:"formation-transfer-monster-visual"})}<b>${displayName(outgoing)}</b><small>Lv.${outgoing.level}</small></span><i>→</i><span>${monsterVisual(incoming.speciesId,incomingSpecies.emoji??"👹",{className:"formation-transfer-monster-visual"})}<b>${displayName(incoming)}</b><small>現在 Lv.${incoming.level}</small></span></div>
...
function partyEditorBody(mode="home"){
 const current=save.state.party.map((id,i)=>{const m=save.state.monsters.find(x=>x.id===id);return m?`<button data-party-slot="${m.id}"><span>${i+1}</span><b>${monsterVisual(m.speciesId,SPECIES[m.speciesId].emoji)} ${displayName(m)}</b><small>Lv.${m.level}</small></button>`:`<button disabled><span>${i+1}</span><b>空き</b></button>`}).join("");
 const elements=["all","neutral","fire","water","wind","earth","lightning","light","dark","poison","nature"];
 const rows=partyEditorMonsters().map(m=>{const sp=SPECIES[m.speciesId],st=calculatedStats(m),active=save.state.party.includes(m.id),rarity=monsterVisibleRarity(m);return`<article class="party-compare-card ${active?"selected":""}"><button class="party-card-main" data-home-party-toggle="${m.id}"><span class="party-monster-icon">${monsterVisual(m.speciesId,sp.emoji)}</span><div><div class="party-card-title"><b>${displayName(m)}</b></div><small>${rarity} / ${elementLabel(sp.element)} / Lv.${m.level} / +${m.plus??0}</small><small>才能 ${"★".repeat(m.stars??1)}　なつき ${m.affection??0}</small><div class="party-stat-line"><span>HP ${st.hp}</span><span>ATK ${st.atk}</span><span>DEF ${st.def}</span><span>SPD ${st.spd}</span></div></div></button><button class="party-detail-button" data-party-detail="${m.id}">詳細</button></article>`}).join("")||'<div class="empty">条件に合うモンスターがいません</div>';
...
function explorationSpriteImage(speciesId,frame){
 const url=monsterSpriteUrl(speciesId,frame);if(!url)return null;
...
function drawExplorationMonster(position,monster,glow=false,scale=1,index=0){
 ...
 const image=explorationSpriteImage(monster.speciesId,frame);
