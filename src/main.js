import{SaveService}from"./services/SaveService.js?v=0.9.15-alpha.3-part4";
...
import{BattleScreen}from"./ui/screens/BattleScreen.js?v=0.9.15-alpha.3-part4";
...
import{equipmentExpNeed,equipmentMaterialExp,enhancementMaterialCandidates,consumeEquipmentMaterial,consumeEquipmentMaterials,projectEquipmentGrowth,equipmentLevelCap,equipmentLimitBreak,canLimitBreakEquipment,limitBreakMaterialCandidates,limitBreakEquipment,EQUIPMENT_LIMIT_BREAK_MAX}from"./services/EquipmentEnhancement.js?v=0.9.15-alpha.5-part6";
...
import{EquipmentScreen}from"./ui/screens/EquipmentScreen.js?v=0.9.15-alpha.5-part7-layout";
...
function equipmentEnhancementBody(item){
 const materials=enhancementMaterialCandidates(save.state,item.id),level=item.level??1,cap=equipmentLevelCap(item),breaks=equipmentLimitBreak(item),atCap=level>=cap,maxBreak=breaks>=EQUIPMENT_LIMIT_BREAK_MAX,need=equipmentExpNeed(level),progress=atCap?100:Math.floor(((item.exp??0)/need)*100),breakMaterials=limitBreakMaterialCandidates(save.state,item.id);
 const rows=materials.slice(0,120).map(material=>{const exp=equipmentMaterialExp(material,item),same=material.name===item.name;return`<label class="equipment-material-row ${same?"same-name":""}"><input type="checkbox" data-equipment-material="${material.id}" ${atCap?"disabled":""}><span><b>[${material.rarity}] ${material.name}</b><small>Lv.${material.level??1}${material.plus?` / +${material.plus}`:""}${same?" / 同名ボーナス×5":""}</small></span><strong>+${exp.toLocaleString()} EXP</strong></label>`}).join("")||'<div class="empty">素材にできる装備がありません。<br><small>装備中・お気に入り・ロック中は表示されません。</small></div>';
 const breakRows=breakMaterials.map(material=>`<label class="equipment-break-row"><input type="radio" name="limitBreakMaterial" value="${material.id}"><span><b>[${material.rarity}] ${material.name}</b><small>Lv.${material.level??1}${material.plus?` / +${material.plus}`:""}</small></span><strong>上限 +10</strong></label>`).join("")||'<div class="empty">使用できる同名装備がありません。</div>';
 ...
 return`<div class="equipment-enhancement"><div class="enhancement-target compact"><div class="enhancement-title-row"><div><small>ENHANCEMENT</small><h3>[${item.rarity}] ${item.name}</h3></div><strong>Lv.${level}<small>/${cap}</small></strong></div><div class="enhancement-status-row"><span>上限 ${cap}</span><span>突破 ${breaks}/${EQUIPMENT_LIMIT_BREAK_MAX}</span><span>${atCap?maxBreak?"最終上限":"突破可能":`次Lvまで ${(need-(item.exp??0)).toLocaleString()}`}</span></div><div class="equipment-exp large"><i style="width:${progress}%"></i></div><div class="limit-break-pips compact">${Array.from({length:EQUIPMENT_LIMIT_BREAK_MAX},(_,i)=>`<i class="${i<breaks?"active":""}"></i>`).join("")}</div><p>${atCap?maxBreak?"最大まで育成済み":"同名装備1個でレベル上限を10解放":`EXP ${(item.exp??0).toLocaleString()} / ${need.toLocaleString()}`}</p></div>${growthRecords}${atCap&&!maxBreak?`<section class="limit-break-panel"><div class="spread"><b>◆ 限界突破</b><span>Lv.${cap} → ${cap+10}</span></div><p class="muted">同名装備を1個消費します。装備中・保護中の装備は使用できません。</p><div class="equipment-break-list">${breakRows}</div><button id="executeEquipmentLimitBreak" class="primary" ${breakMaterials.length?"":"disabled"}>同名装備で限界突破</button></section>`:""}${!atCap?`<section class="equipment-feed-panel"><div class="section-label">素材を選択</div><div class="enhancement-tools"><button data-material-preset="same">同名</button><button data-material-preset="low">N・R</button><button data-material-preset="all">全選択</button><button data-material-preset="none">解除</button></div><div class="enhancement-preview" id="enhancementPreview"><b>素材 0個</b><span>獲得EXP 0</span><small>強化後 Lv.${level} / ${cap}</small></div><p class="muted compact-note">同名装備は獲得EXP5倍。装備中・お気に入り・ロック中は候補から除外。</p><div class="equipment-material-list">${rows}</div><button id="executeEquipmentEnhancement" class="primary sticky-enhance-button" disabled>選択した装備で強化</button></section>`:""}</div>`
}
function openEquipmentEnhancement(id){
 const item=save.state.equipment.find(i=>i.id===id);if(!item)return;
 app.insertAdjacentHTML("beforeend",Modal("🔨 装備育成",equipmentEnhancementBody(item),"閉じる"));const modal=topModal(),selectedMaterials=new Set(),cap=equipmentLevelCap(item);
 const refreshPreview=()=>{const ids=[...selectedMaterials],materials=ids.map(mid=>save.state.equipment.find(i=>i.id===mid)).filter(Boolean),total=materials.reduce((sum,m)=>sum+equipmentMaterialExp(m,item),0),projected=projectEquipmentGrowth(item,total),preview=modal.querySelector("#enhancementPreview"),execute=modal.querySelector("#executeEquipmentEnhancement");if(!preview||!execute)return;preview.innerHTML=`<b>素材 ${materials.length}個</b><span>獲得EXP ${total.toLocaleString()}</span><small>強化後 Lv.${projected.level} / ${equipmentLevelCap(item)}${projected.level>item.level?`（+${projected.level-item.level}）`:""}</small>`;execute.disabled=!materials.length};
...
function openShopMenu(type){
 ...
 const items=type==="herb"?[
  ...
  ["manaPotions","💧","マナポーション","単体MP10回復",380],
  ["partyManaPotions","🌊","全体マナポーション","生存者全員MP5回復",1200],
...
async function useBattleItem(type,targetId){
 ...
 const hasAilment=m=>(m.statuses?.length??0)||(m.ailments?.length??0)||m.status;
 const usable=type==="potions"?target.currentHp<calculatedStats(target).hp:type==="highPotions"?target.currentHp<calculatedStats(target).hp:type==="partyPotions"?list.some(m=>m.currentHp>0&&m.currentHp<calculatedStats(m).hp):type==="manaPotions"?target.currentMp<maxMp(target):type==="partyManaPotions"?list.some(m=>m.currentHp>0&&m.currentMp<maxMp(m)):type==="reviveLeaves"?target.currentHp<=0:type==="statusCures"?hasAilment(target):type==="partyStatusCures"?list.some(hasAilment):type==="fullHeals"?(target.currentHp<calculatedStats(target).hp||target.currentMp<maxMp(target)||hasAilment(target)):list.some(m=>m.currentHp>0&&(m.currentHp<calculatedStats(m).hp||m.currentMp<maxMp(m)||hasAilment(m)));
 ...
 if(type==="manaPotions")target.currentMp=Math.min(maxMp(target),target.currentMp+10);
 if(type==="partyManaPotions")list.filter(m=>m.currentHp>0).forEach(m=>m.currentMp=Math.min(maxMp(m),m.currentMp+5));
