...
import{OnlinePartyScreen}from"./ui/screens/OnlinePartyScreen.js?v=2.10.0-build147";
import{OnlinePartyController}from"./online/OnlinePartyClient.js?v=2.10.0-build147";
...
function claimOnlinePartyReward({rewardId,reward={},source={}}={}){
 ...
 save.state.player.gold=Math.min(Number.MAX_SAFE_INTEGER,(Number(save.state.player.gold)||0)+gold);save.state.inventory.captureCrystals=Math.min(Number.MAX_SAFE_INTEGER,(Number(save.state.inventory.captureCrystals)||0)+captureCrystals);online.claimedRewards.push(id);online.claimedRewards=online.claimedRewards.slice(-200);online.totalGold=(Number(online.totalGold)||0)+gold;online.totalCaptureCrystals=(Number(online.totalCaptureCrystals)||0)+captureCrystals;if(source.kind==="completion")online.expeditionsCompleted=(Number(online.expeditionsCompleted)||0)+1;if(source.kind==="battle")online.battlesWon=(Number(online.battlesWon)||0)+1;save.save();const compact=value=>{const number=Math.max(0,Number(value)||0);if(number>=1e9)return`${Number((number/1e9).toFixed(1))}B`;if(number>=1e6)return`${Number((number/1e6).toFixed(1))}M`;if(number>=1e4)return`${Number((number/1e3).toFixed(1))}K`;return Math.floor(number).toLocaleString()};const goldHud=document.getElementById("goldHud"),captureHud=document.getElementById("captureHud");if(goldHud)goldHud.textContent=compact(save.state.player.gold);if(captureHud)captureHud.textContent=compact(save.state.inventory.captureCrystals);if(gold)showResourceToast("gold",gold);if(captureCrystals)setTimeout(()=>showResourceToast("capture",captureCrystals),260);return{ok:true,gold,captureCrystals,captureSuccess,captureStorageFull,captureNoCrystal,captureName}
}
