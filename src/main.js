import{SaveService}from"./services/SaveService.js?v=0.9.14-alpha.1";
import{SPECIES}from"./data/species.js?v=0.9.14-alpha.1";
import{HomeScreen}from"./ui/screens/HomeScreen.js?v=0.9.14-alpha.1";
import{MonsterListScreen}from"./ui/screens/MonsterListScreen.js?v=0.9.14-alpha.1";
import{MonsterDetailScreen}from"./ui/screens/MonsterDetailScreen.js?v=0.9.14-alpha.1";
import{SettingsScreen}from"./ui/screens/SettingsScreen.js?v=0.9.14-alpha.1";
import{ExploreScreen}from"./ui/screens/ExploreScreen.js?v=0.9.14-alpha.1";
import{BattleScreen}from"./ui/screens/BattleScreen.js?v=0.9.14-alpha.1";
import{Modal}from"./ui/components/Modal.js?v=0.9.14-alpha.1";
import{createMonster,displayName,calculatedStats,TRAITS,expNeedFor,limitBreakGrowth,affectionBonuses}from"./models/Monster.js?v=0.9.14-alpha.1";
import{createEquipment,equipmentPower}from"./models/Equipment.js?v=0.9.14-alpha.1";
import{receiveEquipment,takeFromStorage,equipmentSellPrice,slotLabel}from"./services/EquipmentStorage.js?v=0.9.14-alpha.1";
import{RARITY_ORDER,equipmentStatLabel}from"./data/equipment.js?v=0.9.14-alpha.1";
import{EquipmentScreen}from"./ui/screens/EquipmentScreen.js?v=0.9.14-alpha.1";
import{ShopScreen}from"./ui/screens/ShopScreen.js?v=0.9.14-alpha.1";
import{maxMp,learnedSkills,skillById,canUseSkill,skillDamage}from"./battle/SkillSystem.js?v=0.9.14-alpha.1";
import{ENEMY_ACTIONS,createEnemyBattleState,chooseEnemyAction,enemyDamageMultiplier,enemyHealAmount,enemyAttackMultiplier}from"./battle/EnemyAI.js?v=0.9.14-alpha.1";
import{createBattleRulesState,cooldownRemaining,setSkillCooldown,tickCooldowns,addBattleLog,applyEnemyStatus,processEnemyStatuses}from"./battle/BattleRules.js?v=0.9.14-alpha.1";
import{buildTurnQueue,currentTurnEntry,currentAlly,currentEnemy,aliveEnemies,selectedEnemy,advanceQueue,queueFinished,skipInvalidEntries}from"./battle/TurnSystem.js?v=0.9.14-alpha.1";
import{dangerConfig}from"./core/DangerSystem.js?v=0.9.14-alpha.1";
import{biomeForFloor,biomeProgress,recordBiomeFloor,recordBiomeEncounter,recordBiomeChest,recordBiomeBoss}from"./data/biomes.js?v=0.9.14-alpha.1";
import{WORLD_MAX_FLOOR,TEAM_BATTLE_UNLOCK_FLOOR,ENDGAME_BOSSES,normalizeEndgameState,dailyTeamAttempts,createTeamBattleEncounter,shouldTriggerEmergency,createEmergencyEncounter,recordEmergencyResult}from"./core/EndgameSystem.js?v=0.9.14-alpha.1";
...
function finishSpecialBattle(won){const type=battle.specialBattleType,prior=battle.priorVitals,bossId=battle.specialBossId;if(type==="team"){const team=dailyTeamAttempts(save.state);won?(team.totalWins++,team.stage++):team.totalLosses++}if(type==="emergency")recordEmergencyResult(save.state,battle,won);restorePartyVitals(prior);clearPartySynergy();clearBattleCheckpoint();document.querySelector(".battle-screen")?.remove();const boss=bossId?ENDGAME_BOSSES[bossId]:null;const title=won?"SPECIAL VICTORY":"DEFEAT";const body=won?`<div class="special-result win"><h2>${boss?boss.name:"チームバトル"}を撃退！</h2><p>${type==="team"?"次の試練が解放されました。":"世界異変を退けました。"}</p></div>`:`<div class="special-result lose"><h2>${boss?boss.name:"第試練"}には届かなかった…</h2><p>所持品・階層・仲間へのペナルティはありません。</p></div>`;save.save();battle=null;activeEnemy=null;app.insertAdjacentHTML("beforeend",Modal(title,body,type==="team"?"拠点へ戻る":"探索へ戻る"));const modal=topModal();modal.querySelector("#closeGameModal").onclick=()=>{modal.remove();if(type==="team"){snapshot=null;go("home")}else{screen="explore";render()}}}

function bindHome(){document.getElementById("openMonsters").onclick=()=>go("monsters");document.getElementById("openTeamBattle")?.addEventListener("click",openTeamBattle);document.getElementById("editHomeParty")?.addEventListener("click",openHomePartyEditor);document.getElementById("openRest")?.addEventListener("click",openRest);document.getElementById("openGacha")?.addEventListener("click",openGacha);document.getElementById("openDeepGacha")?.addEventListener("click",openDeepGacha);document.getElementById("openCodexHub")?.addEventListener("click",openCodexHub);document.getElementById("openMonsterCodex")?.addEventListener("click",()=>openCodex("monster"));document.getElementById("openEquipmentCodex")?.addEventListener("click",()=>openCodex("equipment"));document.getElementById("openSettings").onclick=()=>go("settings");document.getElementById("openExplore").onclick=()=>{const max=Math.min(WORLD_MAX_FLOOR,save.state.player.maxFloor);app.insertAdjacentHTML("beforeend",Modal("探索開始",`<p>再開する階層を選択</p><input id="floorSelect" type="number" min="1" max="${max}" value="${max}"><p class="muted">1〜${max}階。10000階が現在の最終到達地点です。</p>`,`出発`));const modal=topModal(),button=modal.querySelector("#closeGameModal");button.onclick=()=>{const f=Math.max(1,Math.min(max,Number(modal.querySelector("#floorSelect").value)||max));save.state.player.currentFloor=f;save.state.player.inRun=true;save.save();snapshot=null;modal.remove();go("explore")}};document.getElementById("openEquipment").onclick=()=>{equipmentTarget=save.state.party[0]??save.state.monsters[0]?.id;navigationOrigin="home";go("equipment")};detailButtons()}
function bindList(){
...
function monsterVisibleRarity(monster){const rarity=monster.summonTier??monster.summonRarity??SPECIES[monster.speciesId]?.rarity??"N";return save.state.player.maxFloor<1000&&["深淵","十神"].includes(rarity)?"？？？":rarity}
...
function rarityGuideHtml(){const unlocked=save.state.player.maxFloor>=1000,visible=SUMMON_RARITY_INFO.filter(r=>unlocked||!["深淵","十神"].includes(r.id));return`<div class="rarity-guide">${visible.map((r,i)=>`<div class="rarity-guide-row rarity-guide-${r.id}"><span>${i+1}</span><b>${r.id}</b><strong>${r.name}</strong><small>${r.note}</small></div>`).join("")}</div>${unlocked?'<p class="rarity-guide-note">下に行くほど上位。十神はすべてのガチャから排出されません。</p>':'<p class="rarity-guide-note">下に行くほど上位です。</p>'}` }
...
function codexVisibleRarity(rarity){return save.state.player.maxFloor<1000&&["深淵","十神"].includes(rarity)?"？？？":rarity}
...
function enhanceEquipment(id){const item=save.state.equipment.find(i=>i.id===id);if(!item||item.plus>=10)return;const rarityCost={N:80,R:140,SR:260,SSR:480,LR:900}[item.rarity],cost=rarityCost*(item.plus+1);if(save.state.player.gold<cost)return ensureGold(cost,()=>enhanceEquipment(id));if(!confirm(`${item.name}を +${item.plus+1}へ強化する？\n費用 ${cost}G`))return;save.state.player.gold-=cost;item.plus++;save.save();render()}
...
function win(caught,m){
 ...
 const victorySubtitle=boss?`👑 ${String(boss.name??SPECIES[boss.speciesId]?.name??"BOSS").replace(/^⚔️\s*/,"")}撃破！`:caught?`${m.nickname}を捕獲！`:"";const result=`<div class="battle-result-cinematic ${boss?"boss-clear":""} ${caught?"capture-clear":""}"><div class="victory-rays"></div><div class="victory-title">${boss?"BOSS DEFEATED":caught?"CAPTURE SUCCESS":"VICTORY"}</div>${victorySubtitle?`<div class="victory-subtitle">${victorySubtitle}</div>`:""}</div><div class="reward-summary cinematic-rewards"><b>🪙 +${gold}G</b>${crystalRoll?`<b>💎 +${crystalRoll}</b>`:""}<small>総EXP ${totalExp} / 生存 ${survivors.length}体で分配${firstBoss?"・初回ボス撃破ボーナス":""}</small>${drop?`<b>[${drop.rarity}] ${drop.name}（${slotLabel(drop.slot)}）</b><small>${dropReceipt.message}</small>`:""}${keyDrop?`<b>🗝️ 深淵の鍵を獲得</b>`:""}${caught?`<b>${m.nickname}を捕獲！</b>`:""}</div><div class="exp-results compact">${progress.map(p=>{const hpMax=p.afterStats.hp,mpMax=maxMp(p.x),remaining=Math.max(0,p.need-p.x.exp),diff=k=>p.afterStats[k]-(p.before.stats[k]??0);return`<div class="${p.alive?"":"exp-defeated"} ${p.levels?"level-up-card level-up-reveal":""}"><span>${SPECIES[p.x.speciesId].emoji}</span><section><b>${displayName(p.x)} ${p.levels?`Lv.${p.before.level} → Lv.${p.x.level}`:`Lv.${p.x.level}`}</b><div class="result-vitals"><small>HP ${p.x.currentHp}/${hpMax}</small><small>MP ${p.x.currentMp}/${mpMax}</small><small>${p.alive?`次まであと${remaining}EXP`:"戦闘不能：EXP 0"}</small></div><i class="result-exp"><u style="width:${Math.min(100,p.x.exp/p.need*100)}%"></u></i>${p.levels?`<div class="level-gains"><span>HP ${p.before.stats.hp} → ${p.afterStats.hp} <strong>+${diff("hp")}</strong></span><span>ATK ${p.before.stats.atk} → ${p.afterStats.atk} <strong>+${diff("atk")}</strong></span><span>DEF ${p.before.stats.def} → ${p.afterStats.def} <strong>+${diff("def")}</strong></span><span>SPD ${p.before.stats.spd} → ${p.afterStats.spd} <strong>+${diff("spd")}</strong></span></div>`:""}</section></div>`}).join("")}</div>`;

 if(boss){battle.enemy=boss;save.state.player.bossKills[floor]=(save.state.player.bossKills[floor]??0)+1;recordBiomeBoss(save.state,floor);if(snapshot?.world)snapshot.world.boss=null;if(firstBoss)return showBossRewards(result)}
