import{SaveService}from"./services/SaveService.js?v=2.11.21-build185";
import{CONTENT_TEST_MODE,BATTLE_SPEED_OPTIONS,CAMERA_DRAG_THRESHOLD_PX,WATER_RULES,MONSTER_STAR_MAX,MONSTER_STORAGE_CAP,ENDGAME_MAX_LEVEL,premiumCrystalCost,normalizeBattleSpeed,contentUnlockFloor,isContentUnlocked}from"./core/config.js?v=2.11.21-build185";
...
import{orderedMonsterSpecies}from"./data/monsterCatalog.js?v=2.11.11-build175";
import{HomeScreen,homePartySlots}from"./ui/screens/HomeScreen.js?v=2.11.21-build185";
import{FormationScreen}from"./ui/screens/FormationScreen.js?v=2.11.2-build166";
import{OnlinePartyScreen}from"./ui/screens/OnlinePartyScreen.js?v=2.11.2-build166";
import{OnlinePartyController}from"./online/OnlinePartyClient.js?v=2.11.2-build166";
import{MonsterListScreen}from"./ui/screens/MonsterListScreen.js?v=2.11.2-build166";
import{MonsterDetailScreen}from"./ui/screens/MonsterDetailScreen.js?v=2.11.2-build166";
import{SettingsScreen}from"./ui/screens/SettingsScreen.js?v=2.11.21-build185";
import{ExploreScreen}from"./ui/screens/ExploreScreen.js?v=2.11.21-build185";
import{GauntletScreen}from"./ui/screens/GauntletScreen.js?v=2.11.21-build185";
import{BattleScreen}from"./ui/screens/BattleScreen.js?v=2.11.21-build185";
...
import{createMonster,displayName,calculatedStats,TRAITS,expNeedFor,experienceCrystalValue,limitBreakGrowth,affectionBonuses,totalExperience,applyTotalExperience}from"./models/Monster.js?v=2.11.2-build166";
import{EXPERIENCE_PACK_TYPES,experiencePackType,availableExperiencePackTypes,consumeExperiencePacks,experiencePackCapacity,previewExperiencePacks}from"./core/ExperiencePackSystem.js?v=2.11.2-build166";
...
import{normalizeSeriesMastery,recordSeriesBattle,seriesMasteryBonusForMonster,seriesMasterySummary}from"./services/SeriesMastery.js?v=2.11.2-build166";
import{receiveEquipment,takeFromStorage,equipmentSellPrice,slotLabel}from"./services/EquipmentStorage.js?v=2.11.2-build166";
import{RARITY_ORDER,EQUIPMENT_BASES,equipmentDisplayRarity,equipmentRarityColor,equipmentStatLabel,equipmentSubslotLabel,compatibleSubslots,SLOT_UNLOCK_LEVEL}from"./data/equipment.js?v=2.11.2-build166";
import{EQUIPMENT_SERIES,aggregateSeriesEffects}from"./data/equipmentSeries.js?v=2.11.2-build166";
...
import{EquipmentScreen}from"./ui/screens/EquipmentScreen.js?v=2.11.2-build166";
...
import{assignEquipmentToSubslot,canEquipInSubslot,emptyEquipmentLoadout,normalizeEquipmentLoadouts}from"./services/EquipmentLoadoutSystem.js?v=2.11.2-build166";
import{ShopScreen}from"./ui/screens/ShopScreen.js?v=2.11.2-build166";
import{SkillScreen}from"./ui/screens/SkillScreen.js?v=2.11.21-build185";
import{AbyssSkillTreeScreen}from"./ui/screens/AbyssSkillTreeScreen.js?v=2.11.2-build166";
import{InventoryScreen,ArmoryScreen}from"./ui/screens/InventoryScreen.js?v=2.11.2-build166";
...
import{worldPresentationForFloor,shouldPlaySecondWorldIntro,markSecondWorldEntered}from"./core/WorldSystem.js?v=2.11.2-build166";
import{randomEventForFloor,markRandomEventResolved,randomEventCosts}from"./core/SecondWorldEventSystem.js?v=2.11.2-build166";
...
import{maxMp,learnedSkills,allLearnedSkills,equipSkill,skillById,skillElementLabel,canUseSkill,effectiveSkillMpCost,skillDamage,affixOutgoingDamageMultiplier,chooseAutoSkill,skillProgressFor,recordSkillUse,skillEffectDetails,skillEffectSummary}from"./battle/SkillSystem.js?v=2.11.21-build185";
import{ENEMY_ACTIONS,createEnemyBattleState,chooseEnemyAction,enemyActionMpCost,enemyDamageMultiplier,enemyHealAmount,enemyAttackMultiplier,specialActionMultiplier,specialActionInfo}from"./battle/EnemyAI.js?v=2.11.21-build185";
import{createBattleRulesState,cooldownRemaining,setSkillCooldown,tickCooldowns,addBattleLog,applyEnemyStatus,applyEnemyDamage,processEnemyStatuses,applyBattleEffect,effectStackBreakdown,effectValue,hasEffect,clearNegativeAllyEffects,clearPersistentAilments,syncPersistentAilments,tickBattleEffects,processAllyEffects}from"./battle/BattleRules.js?v=2.11.21-build185";
import{attackHits}from"./battle/HitSystem.js?v=2.11.2-build166";
import{buildTurnQueue,currentTurnEntry,currentAlly,currentEnemy,aliveEnemies,selectedEnemy,advanceQueue,queueFinished,skipInvalidEntries}from"./battle/TurnSystem.js?v=2.11.21-build185";
...
import{monsterCombatPower,partyCombatPower,partyCombatPowerBreakdown,formatCombatPower,recordPartyCombatPower}from"./core/CombatPower.js?v=2.11.2-build166";
import{beginSecretRoomExpedition,ensureSecretRoomExpedition,secretRoomPlan,enterSecretRoom,activeSecretRoom,spinSecretRoomCasino,useSecretRoomInn,buyDarkMarketOffer,buyDarkMarketRecovery,isDarkMarketBargain,SECRET_ROOM_RECOVERY_ITEMS,DARK_MARKET_ITEM_LIMIT,CASINO_CRYSTAL_COST,CASINO_MULTIPLIER_RATES}from"./core/SecretRoomSystem.js?v=2.11.2-build166";
import{applyGameMasterReward,applySerialReward,commitSerialRedemption,validateGameMasterCode,validateSerialCode}from"./core/SerialCodeSystem.js?v=2.11.2-build166";
import{NOTICE_DEFINITIONS,DAILY_NOTICE_GIFT,markNoticeRead,normalizeNoticeState,dailyNoticeGiftStatus,claimDailyNoticeGift,noticeAttentionCount}from"./core/NoticeSystem.js?v=2.11.2-build166";
import{weekdayGachaSchedule,weekdayGachaCost,WEEKDAY_GACHA_CALENDAR,WEEKDAY_ENDGAME_RATE,rollWeekdayEndgameHit}from"./core/WeekdayGachaSystem.js?v=2.11.2-build166";
import{bossExperiencePackReward}from"./core/BossRewardSystem.js?v=2.11.2-build166";
import{enemyExperienceReward}from"./core/ProgressionSystem.js?v=2.11.2-build166";
import{floorBossDefinitionForFloor,floorBossDefinitionById,milestoneBossIdsForFloor}from"./data/floorBosses.js?v=2.11.21-build185";
import{equipmentDropLevelForFloor}from"./core/EquipmentDropSystem.js?v=2.11.2-build166";
import{monsterSpriteUrl,monsterVisual,setMonsterVisualFrame}from"./ui/MonsterVisual.js?v=2.11.11-build175";
...
function normalizeFloorBossDedicatedItem(item){
 const definition=item?.ruleOverrides?.floorBossDedicated?floorBossDefinitionById(item.ruleOverrides.bossCatalogId):null,design=definition?.dedicatedWeapon;if(!design)return null;
 if(design.effect){item.fixedEffects={...design.effect.fixedEffects,...(item.fixedEffects??{})};item.floorBossWeaponEffectId=design.effect.id}
 if(design.skill)item.grantedSkillId=design.skill.id;
 item.fixedEffectText=`${design.effect?.name??"階層固有武装"}：${design.effect?.description??`${definition.element}属性と${definition.role}戦術へ最適化`}${design.skill?`／装備中限定技「${design.skill.name}」` :""}`;
 return design;
}
function normalizeEquipmentState(){
 ...
 for(const item of[...save.state.equipment,...save.state.reserveEquipment,...save.state.bossEquipmentVault])normalizeFloorBossDedicatedItem(item);
 ...
 save.state.monsters.forEach(m=>{
  m.traitId??="steady";
  const counts={},stats={},equippedItems=[],equipmentSkills=[];Object.values(m.equipment).forEach(id=>{const item=byId.get(id);if(!item)return;equippedItems.push(item);const design=normalizeFloorBossDedicatedItem(item);if(design?.skill)equipmentSkills.push(design.skill);const mult=equipmentStatMultiplier(item);Object.entries(item.stats??{}).forEach(([k,v])=>stats[k]=(stats[k]??0)+Math.round(v*mult));if(item.series)counts[item.series]=(counts[item.series]??0)+1});
...
function workshopMonsterCard(monster,{selected=false,targetCandidate=false,release=false}={}){
 ...
 return`<article class="monster-workshop-card rarity-${rarity} ${selected?"selected":""} ${workshopProtected(monster)?"protected":""}" data-workshop-monster="${monster.id}" ${targetCandidate?'data-workshop-drag-target="1"':""}>
  ...
  <div><b>${displayName(monster)}＋${monster.plus??0} <i>Lv.${monster.level}</i></b><small>旧★記録 ${stars<=6?"★".repeat(stars):`★${stars}`}（能力補正なし）　❤️${monster.affection??0}</small><small>HP ${stats.hp.toLocaleString()} / ATK ${stats.atk.toLocaleString()} / DEF ${stats.def.toLocaleString()} / SPD ${stats.spd.toLocaleString()}</small></div>
...
function confirmFormationReplacement(outgoingId,incomingId){
 ...
 app.insertAdjacentHTML("beforeend",Modal("部隊メンバーを交代",`<div class="formation-transfer">
  ...
  <section><b>装備を引き継ぐ</b><p>${gearCount?`${gearCount}個の装備をそのまま移動します。`:"引き継ぐ装備はありません。"}</p><small>旧★記録（能力補正なし）・限界突破＋・なつき度・個別スキルは各個体のままです。</small></section>
...
function claimDefeatTutorial(){
 ...
 return`<section class="defeat-tutorial"><small>DEFEAT GUIDE 2/2</small><h3>育成を組み直して再挑戦</h3><ul><li>装備強化：不要装備を素材にLvと能力を上げる</li><li>同種合成：同名2体で＋1。旧★記録は能力へ影響せず、＋が合成成長</li><li>種族特性とスキル熟練度を見て、長期戦用の構成へ調整</li><li>お気に入り／ロックで素材事故を防止</li><li>レベルで装備枠が解放。ランダムオプションはGOLDで厳選</li><li>一括自動合成を使い、敵階層の属性に合わせて編成を再構築</li></ul></section>`;
}
...
function reviveBattleMonster(target,hpRate=.35,mpRate=.25,source=null,{transferRate=0}={}){
 ...
 const row=battleContribution(source);if(row)row.revives++;syncInvincibleAllianceState();return true;
}
...
function allyAilment(target,statusId=null){
 ...
 return ailments[0]??effects.find(effect=>effect.sourceStatusId||["stun","spdDown","atkDown","defDown","evasionDown","accuracyDown","vulnerable"].includes(effect.kind))??null;
}
...
const POSITIVE_ENEMY_EFFECTS=new Set(["atkUp","defUp","spdUp","evasionUp","accuracyUp","regen","taunt","guard","counter","lifeSteal","magicToPhysical"]);
const INVERTED_BATTLE_EFFECTS=Object.freeze({atkUp:"atkDown",defUp:"defDown",spdUp:"spdDown",evasionUp:"evasionDown",accuracyUp:"accuracyDown",regen:"vulnerable",taunt:"vulnerable",guard:"defDown",counter:"atkDown",lifeSteal:"atkDown",magicToPhysical:"atkDown"});
...
async function command(type,skillId=null){
 ...
 if(type==="attack"){
  ...
  if(!attackHits({accuracy:s.accuracy??100,accuracyUp:effectValue(battle,a.id,"accuracyUp"),accuracyDown:effectValue(battle,a.id,"accuracyDown"),evasion:e.evasion??0,evasionUp:effectValue(battle,e.id,"evasionUp","enemy"),evasionDown:effectValue(battle,e.id,"evasionDown","enemy")})){registerFloorBossDodge(e);addBattleLog(battle,`${e.name}が攻撃を回避した`);await floatText("MISS / 回避",e.id,"miss");}
  else{
   const combatStats=convertedAttackStats(s,a.id),conversionActive=effectValue(battle,a.id,"magicToPhysical")>0,critical=Math.random()<Math.min(.95,affixCriticalChance(combatStats,Math.min(.35,.08+(combatStats.spd??0)*.005))+magicCircleCriticalBonus(a,1)+signatureBonus.critBonus),weapon=save.state.equipment.find(item=>item.id===a.equipment?.weaponRight),magicWeapon=!conversionActive&&(weapon?.weaponType==="staff"||(weapon?.stats?.matk??0)>(weapon?.stats?.atk??0)),formationMultiplier=1,attackStat=(magicWeapon?(combatStats.matk??combatStats.atk):combatStats.atk)*allyAttackFactor(a.id),defenseStat=magicWeapon?(e.mdef??e.def):e.def;
 ...
 if(type==="skill"&&skillId){
  ...
  if(skill.type==="selfHeal"||skill.type==="stance"&&skill.heal){
   ...
   if(skill.revive){const target=battle.party.filter(m=>m.currentHp<=0).sort((x,y)=>calculatedStats(y).hp-calculatedStats(x).hp)[0];if(target&&reviveBattleMonster(target,skill.revive,skill.reviveMp??.25,a)){applyRevivedSkillEffects(skill,target,a);healed.push(target.currentHp)}}
   ...
   await animateAttack(a.id,true);const amount=Math.max(1,Math.floor(a.currentHp*Math.min(2,Number(skill.selfSacrificeHpDamage)))),applied=applyEnemyDamage(battle,e,amount,{sourceId:a.id,bypassMimicArmor:true,element:skill.element??a.attribute??SPECIES[a.speciesId]?.element??"neutral",damageClass:skill.damageClass??"physical"});recordBattleDamage(a,applied.damage);registerWeaponFinisher(a,e,applied.beforeHp);a.currentHp=0;syncInvincibleAllianceState();addBattleLog(battle,`${displayName(a)}は命を代価に${applied.damage.toLocaleString()}ダメージ`);await animateHit(e.id,true);await floatText(`生命 -${applied.damage}`,e.id,"critical");applySkillEffects(skill,a,e);
  }else{
   ...
   for(const targetEnemy of skillTargets){const e=targetEnemy;let targetTotal=0;for(let i=0;i<hits&&e.hp>0;i++){
    if(!attackHits({accuracy:s.accuracy??100,accuracyUp:effectValue(battle,a.id,"accuracyUp"),accuracyDown:effectValue(battle,a.id,"accuracyDown"),evasion:e.evasion??0,evasionUp:effectValue(battle,e.id,"evasionUp","enemy"),evasionDown:effectValue(battle,e.id,"evasionDown","enemy"),guaranteedHit:Boolean(skill.guaranteedHit)})){registerFloorBossDodge(e);addBattleLog(battle,`${e.name}が${skill.name}を回避した`);await floatText("MISS / 回避",e.id,"miss");continue}
    const converted=convertedAttackStats(s,a.id),critical=Boolean(skill.guaranteedCritical)||Math.random()<Math.min(.95,affixCriticalChance(converted,Math.min(.65,.1+(skill.critBonus??0)+(converted.spd??0)*.004))+magicCircleCriticalBonus(a,skill.power??1)+signatureBonus.critBonus),ignore=Math.max(0,Math.min(.9,Number(skill.defenseIgnore)||0)),boosted={...converted,atk:converted.atk*allyAttackFactor(a.id),matk:(converted.matk??converted.atk)*allyAttackFactor(a.id),_currentHpRatio:a.currentHp/Math.max(1,converted.hp)},execute=(skill.execute&&e.hp/e.maxHp<=skill.execute)?2:1,statusBonus=skill.bonusVsStatus?.id&&(battle.enemyStatuses?.[e.id]??[]).some(status=>status.id===skill.bonusVsStatus.id)?Math.max(1,Number(skill.bonusVsStatus.multiplier)||1):1,effectBonus=skill.bonusVsEffect?.kind&&hasEffect(battle,e.id,skill.bonusVsEffect.kind,"enemy")?Math.max(1,Number(skill.bonusVsEffect.multiplier)||1):1,enemyBuffBonus=skill.bonusVsEnemyBuff&&(battle.enemyEffects?.[e.id]??[]).some(effect=>POSITIVE_ENEMY_EFFECTS.has(effect.kind))?Math.max(1,Number(skill.bonusVsEnemyBuff.multiplier)||1):1,raw=skillDamage(boosted,{...e,def:e.def*enemyDefenseFactor(e.id)*(1-ignore),mdef:(e.mdef??e.def)*enemyDefenseFactor(e.id)*(1-ignore)},skill,critical)*execute*statusBonus*effectBonus*enemyBuffBonus*affixExecutionMultiplier(a,e)*(1+affixValue(a,"skillPower",200)/100)*signatureBonus.damageMultiplier,d=Math.max(1,Math.floor(raw*attributeDamageMultiplier(skill.element,SPECIES[e.speciesId]?.element??"neutral")*abyssBattleMultiplier(a,"partyDamageRate")*enemyDamageMultiplier(e)*(e.hiddenDamageTaken??1)*endgameIncomingDamageMultiplier(e,skill.element)*weaponMasteryDamageMultiplier(save.state,a,e)*magicCircleDamageMultiplier(a))),applied=applyEnemyDamage(battle,e,d,{sourceId:a.id,element:skill.element??a.attribute??SPECIES[a.speciesId]?.element??"neutral",damageClass:skill.damageClass??"physical"});
...
async function dealEnemyHit(e,target,multiplier=1,label="",criticalChance=.08,element=null,rules={}){
 const st=calculatedStats(target),guard=Boolean(battle.guards[target.id]),endgameNoCrit=hasCircleEffect(target,"endgameNoCrit")&&Boolean(e.endgameBossId||["abyss","tenGod"].includes(e.faction)),criticalStatus=e.floorBossPassive?.criticalAgainstStatus,criticalEffects=e.floorBossPassive?.criticalAgainstEffects,critical=!endgameNoCrit&&(Boolean(rules.guaranteedCritical)||Boolean(e._floorBossForceCritical)||Boolean(criticalStatus&&allyAilment(target,criticalStatus))||Boolean(Array.isArray(criticalEffects)&&criticalEffects.length&&criticalEffects.every(kind=>hasEffect(battle,target.id,kind)))||Math.random()<Math.min(.9,criticalChance+(e.crit??0)));
 ...
 const guaranteedStatus=e.floorBossPassive?.guaranteedHitAgainstStatus,guaranteedBuff=e.floorBossPassive?.guaranteedHitAgainstBuff&&(battle.allyEffects?.[target.id]??[]).some(effect=>POSITIVE_ENEMY_EFFECTS.has(effect.kind));if(!attackHits({accuracy:e.accuracy??100,accuracyUp:effectValue(battle,e.id,"accuracyUp","enemy"),accuracyDown:effectValue(battle,e.id,"accuracyDown","enemy"),evasion:st.evasion??0,evasionUp:effectValue(battle,target.id,"evasionUp"),evasionDown:effectValue(battle,target.id,"evasionDown"),guaranteedHit:Boolean(rules.guaranteedHit||e.endgameUnavoidable||guaranteedBuff||(guaranteedStatus&&allyAilment(target,guaranteedStatus)))})){addBattleLog(battle,`${displayName(target)}が攻撃を回避した`);await floatText("MISS / 回避",target.id,"miss");return 0}
...
async function resolveEnemySpecialAction(e,action){
 ...
 if(info.utility){
  ...
  if(info.revive&&canBattleRevive()){
   const fallen=battle.enemies.find(enemy=>enemy.hp<=0);
	   if(fallen){const before=fallen.hp,transfer=info.reviveTransferRate?Math.floor(e.hp*Math.min(.9,info.reviveTransferRate)):0;if(transfer>0){e.hp=Math.max(1,e.hp-transfer);storeFloorBossSacrifice(e,transfer);fallen.hp=Math.max(1,Math.min(fallen.maxHp,transfer));addBattleLog(battle,`${e.name}はHP${transfer.toLocaleString()}を${fallen.name}へ分与`)}else fallen.hp=Math.max(1,Math.floor(fallen.maxHp*info.revive));battle.reviveCount++;queueBattleRecovery(fallen,"hp",before,fallen.hp);for(const effect of info.reviveEffects??[])applyBattleEffect(battle,fallen.id,{...effect,sourceKey:`${e.id}:${action}:revive`,sourceSkillName:info.label},"enemy");if(e.floorBossDomain?.effect==="healingFlare")e._floorBossHealedPowerReady=true;await flushBattleRecoveries()}
  ...
  for(const effect of info.effects??[])for(const ally of(effect.allies?allies:[e])){if(e.floorBossCatalogId)applyBattleEffect(battle,ally.id,{...effect,sourceKey:`${e.id}:${action}`,sourceSkillName:info.label},"enemy");else{if(effect.kind==="atkUp")ally.atk=Math.floor(ally.atk*(1+(effect.value??.2)));if(effect.kind==="defUp")ally.def=Math.floor(ally.def*(1+(effect.value??.2)));if(effect.kind==="spdUp")ally.spd=Math.floor(ally.spd*(1+(effect.value??.2)))}if(effect.kind==="guard")ally.divineBarrier=Math.max(ally.divineBarrier??0,Math.max(1,effect.turns??2));if(effect.kind==="regen")ally.eliteRegen=Math.max(ally.eliteRegen??0,effect.value??.1);if(effect.kind==="magicToPhysical"&&!ally._floorBossActionConversion){ally._floorBossActionConversion=true;const rate=Math.max(0,Math.min(1,Number(effect.value)||0)),magic=Math.max(0,Number(ally.matk)||0);ally.atk=Math.max(1,Math.floor(ally.atk+magic*rate));ally.matk=Math.max(1,Math.floor(magic*(1-rate)))}}
 ...
 const elements={inferno:"fire",tidal:"water",thunderstorm:"lightning",tempest:"wind",quake:"earth",radiance:"light",eclipse:"dark",absoluteZero:"ice",timeStop:"light",starfall:"wind"};
	 let multiplier=specialActionMultiplier(action)*(e.enraged?1.25:1)*Math.max(.5,Number(e._circleActionMultiplier)||1)*floorBossDomainActionMultiplier(e,info,action),totalDamage=0,totalMpDrained=0,consumedAilments=0;
 if(info.copyAtk){
...
function dedicatedFloorBossWeapon(floor,boss){
 const definition=floorBossDefinitionForFloor(floor);if(!definition||boss?.floorBossCatalogId!==definition.id)return null;
 const design=definition.dedicatedWeapon,pool=EQUIPMENT_BASES.weapon.filter(base=>design.weaponType?base.weaponType===design.weaponType:true),base=randomFrom(pool.length?pool:EQUIPMENT_BASES.weapon),item=createEquipment("weapon",{rarity:design.rarity,base,weaponType:design.weaponType,ruleOverrides:{floorBossDedicated:true,bossCatalogId:definition.id,cycleFloor:definition.cycleFloor}});
 ...
 item.level=equipmentDropLevelForFloor(floor,{boss:true});item.plus=Math.min(10,Math.floor(floor/1000));item.obtainedFloor=floor;item.obtainedMethod="floorBossDedicated";normalizeFloorBossDedicatedItem(item);
 return item
}
function createBossRewardOptions(floor,boss){
 ...
 const dedicated=dedicatedFloorBossWeapon(floor,boss);
 if(dedicated){const resources=pool.map(value=>({value,sort:Math.random()})).sort((a,b)=>a.sort-b.sort).slice(0,2).map(entry=>entry.value),weapon={id:`dedicated-${floor}`,type:"equipment",icon:"equipment",item:dedicated,title:`専用武器 [${dedicated.rarity}] ${dedicated.name}`,desc:`${boss.name.replace(/^⚔️\s*/,"")}の固有武装・装備Lv.${dedicated.level}`};return[weapon,...resources]}
