...
import{aggregateAffixes}from"./data/equipmentAffixes.js?v=0.9.15-alpha.27-phase10-5-affix-runtime";
...
import{maxMp,learnedSkills,allLearnedSkills,equipSkill,skillById,canUseSkill,effectiveSkillMpCost,skillDamage,chooseAutoSkill,skillProgressFor,recordSkillUse}from"./battle/SkillSystem.js?v=0.9.15-alpha.27-phase10-5-affix-runtime";
...
import{createBattleRulesState,cooldownRemaining,setSkillCooldown,tickCooldowns,addBattleLog,applyEnemyStatus,processEnemyStatuses,applyBattleEffect,effectValue,hasEffect,clearNegativeAllyEffects,tickBattleEffects,processAllyEffects}from"./battle/BattleRules.js?v=0.9.15-alpha.27-phase10-5-affix-runtime";
...
async function command(type,skillId=null){
 ...
 if(type==="attack"){
  ...
  if(Math.random()<.06)await floatText("MISS",e.id,"miss");
   ...
   const critMult=1.7+affixValue(a,"critDamage",150)/100,raw=(critical?Math.floor(base*critMult):base)*affixExecutionMultiplier(a,e),beforeHp=e.hp,d=Math.max(1,Math.floor(raw*enemyDamageMultiplier(e)*weaponMasteryDamageMultiplier(save.state,a,e)));e.hp=Math.max(0,e.hp-d);registerWeaponFinisher(a,e,beforeHp);const steal=outgoingLifeSteal(a);if(steal){const h=Math.max(1,Math.floor(d*steal));a.currentHp=Math.min(s.hp,a.currentHp+h)}
