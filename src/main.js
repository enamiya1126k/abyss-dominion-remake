import{SaveService}from"./services/SaveService.js?v=2.6.1";
import{CONTENT_TEST_MODE,BATTLE_SPEED_OPTIONS,CAMERA_DRAG_THRESHOLD_PX,WATER_RULES,MONSTER_STAR_MAX,MONSTER_STORAGE_CAP,ENDGAME_MAX_LEVEL,premiumCrystalCost,normalizeBattleSpeed,contentUnlockFloor,isContentUnlocked}from"./core/config.js?v=2.6.1";
import{AudioSystem}from"./core/AudioSystem.js?v=2.6.1";
import{endgameCharacter}from"./data/endgameCharacters.js?v=2.6.1";
import{SPECIES}from"./data/species.js?v=2.6.1";
import{captureStatusBonus,normalizePersistentAilments}from"./data/statusEffects.js?v=2.6.1";
import{attributeDamageMultiplier,attributeGuideRows,ATTRIBUTES}from"./data/attributes.js?v=2.6.1";
import{orderedMonsterSpecies}from"./data/monsterCatalog.js?v=2.6.1";
import{HomeScreen,homePartySlots}from"./ui/screens/HomeScreen.js?v=2.6.1";
import{FormationScreen}from"./ui/screens/FormationScreen.js?v=2.6.1";
import{MonsterListScreen}from"./ui/screens/MonsterListScreen.js?v=2.6.1";
import{MonsterDetailScreen}from"./ui/screens/MonsterDetailScreen.js?v=2.6.1";
import{SettingsScreen}from"./ui/screens/SettingsScreen.js?v=2.6.1";
import{ExploreScreen}from"./ui/screens/ExploreScreen.js?v=2.6.1";
import{GauntletScreen}from"./ui/screens/GauntletScreen.js?v=2.6.1";
import{BattleScreen}from"./ui/screens/BattleScreen.js?v=2.6.1";
import{Modal}from"./ui/components/Modal.js?v=2.6.1";
import{pixelIcon}from"./ui/components/GameChrome.js?v=2.6.1";
import{equipmentVisual}from"./ui/components/EquipmentVisual.js?v=2.6.1";
import{attributeVisual}from"./ui/components/AttributeVisual.js?v=2.6.1";
import{createMonster,displayName,calculatedStats,TRAITS,expNeedFor,experienceCrystalValue,limitBreakGrowth,affectionBonuses,totalExperience,applyTotalExperience}from"./models/Monster.js?v=2.6.1";
import{createEquipment,equipmentPower,equipmentStatMultiplier}from"./models/Equipment.js?v=2.6.1";
import{equipmentExpNeed,equipmentMaterialExp,enhancementMaterialCandidates,consumeEquipmentMaterials,projectEquipmentGrowth}from"./services/EquipmentEnhancement.js?v=2.6.1";
import{recordWeaponKill,weaponMasteryDamageMultiplier,weaponMasterySummary}from"./services/WeaponMastery.js?v=2.6.1";
import{normalizeSeriesMastery,recordSeriesBattle,seriesMasteryBonusForMonster,seriesMasterySummary}from"./services/SeriesMastery.js?v=2.6.1";
import{receiveEquipment,takeFromStorage,equipmentSellPrice,slotLabel}from"./services/EquipmentStorage.js?v=2.6.1";
import{RARITY_ORDER,EQUIPMENT_BASES,equipmentDisplayRarity,equipmentRarityColor,equipmentStatLabel,equipmentSubslotLabel,compatibleSubslots,SLOT_UNLOCK_LEVEL}from"./data/equipment.js?v=2.6.1";
import{EQUIPMENT_SERIES,aggregateSeriesEffects}from"./data/equipmentSeries.js?v=2.6.1";
import{AFFIX_QUALITY,aggregateAffixes,affixQuality,formatAffix,affixDefinition}from"./data/equipmentAffixes.js?v=2.6.1";
import{EquipmentScreen}from"./ui/screens/EquipmentScreen.js?v=2.6.1";
import{lockedAffixCount,maxLockableAffixes,normalizeEquipmentAffixLocks,rerollGoldCost,rerollUnlockedAffixes,toggleAffixLock}from"./services/EquipmentAffixCrafting.js?v=2.6.1";
import{assignEquipmentToSubslot,canEquipInSubslot,emptyEquipmentLoadout,normalizeEquipmentLoadouts}from"./services/EquipmentLoadoutSystem.js?v=2.6.1";
import{ShopScreen}from"./ui/screens/ShopScreen.js?v=2.6.1";
import{SkillScreen}from"./ui/screens/SkillScreen.js?v=2.6.1";
import{AbyssSkillTreeScreen}from"./ui/screens/AbyssSkillTreeScreen.js?v=2.6.1";
import{InventoryScreen,ArmoryScreen}from"./ui/screens/InventoryScreen.js?v=2.6.1";
import{abyssEquipmentRarityBonus,abyssExplorationChance,abyssSkillEffectTotal,abyssSkillEffects,abyssSkillMultiplier,abyssSkillNodeById,abyssSkillTreeSummary,learnAbyssSkill}from"./core/AbyssSkillTreeSystem.js?v=2.6.1";
import{Ending1000Screen}from"./ui/screens/Ending1000Screen.js?v=2.6.1";
import{Ending10000Screen}from"./ui/screens/Ending10000Screen.js?v=2.6.1";
import{SecondWorldIntroScreen}from"./ui/screens/SecondWorldIntroScreen.js?v=2.6.1";
import{worldPresentationForFloor,shouldPlaySecondWorldIntro,markSecondWorldEntered}from"./core/WorldSystem.js?v=2.6.1";
import{randomEventForFloor,markRandomEventResolved,randomEventCosts}from"./core/SecondWorldEventSystem.js?v=2.6.1";
import{shouldSpawnSecondWorldElite,createEliteEncounter,applyEliteModifiers,recordEliteEncounter,recordEliteDefeat,eliteRewards}from"./core/SecondWorldEliteSystem.js?v=2.6.1";
import{shouldPlayTenGodFirstContact,tenGodContactChoices,resolveTenGodFirstContact}from"./core/TenGodContactSystem.js?v=2.6.1";
import{TenGodContactScreen}from"./ui/screens/TenGodContactScreen.js?v=2.6.1";
import{maxMp,learnedSkills,allLearnedSkills,equipSkill,skillById,skillElementLabel,canUseSkill,effectiveSkillMpCost,skillDamage,affixOutgoingDamageMultiplier,chooseAutoSkill,skillProgressFor,recordSkillUse}from"./battle/SkillSystem.js?v=2.6.1";
import{ENEMY_ACTIONS,createEnemyBattleState,chooseEnemyAction,enemyActionMpCost,enemyDamageMultiplier,enemyHealAmount,enemyAttackMultiplier,specialActionMultiplier,specialActionInfo}from"./battle/EnemyAI.js?v=2.6.1";
import{createBattleRulesState,cooldownRemaining,setSkillCooldown,tickCooldowns,addBattleLog,applyEnemyStatus,processEnemyStatuses,applyBattleEffect,effectValue,hasEffect,clearNegativeAllyEffects,clearPersistentAilments,syncPersistentAilments,tickBattleEffects,processAllyEffects}from"./battle/BattleRules.js?v=2.6.1";
import{buildTurnQueue,currentTurnEntry,currentAlly,currentEnemy,aliveEnemies,selectedEnemy,advanceQueue,queueFinished,skipInvalidEntries}from"./battle/TurnSystem.js?v=2.6.1";
import{dangerConfig}from"./core/DangerSystem.js?v=2.6.1";
import{bossLevelForFloor,enemyLevelForFloor as scaledEnemyLevelForFloor,enemyHiddenProfileForFloor}from"./core/EnemyScalingSystem.js?v=2.6.1";
import{MAGIC_CIRCLES,equippedMagicCircle,magicCircleLevel,magicCirclePrice,magicCircleNextEffect,buyOrUpgradeMagicCircle,equipMagicCircle,magicCircleMarkup,slotDamageMultiplier}from"./core/MagicCircleSystem.js?v=2.6.1";
import{biomeForFloor,biomeProgress,recordBiomeFloor,recordBiomeEncounter,recordBiomeChest,recordBiomeBoss}from"./data/biomes.js?v=2.6.1";
...
import{beginManualExpedition,recordManualFloorClear,claimManualReturn,abandonManualExpedition,idleReturnPreview,claimIdleReturn,returnRarityRates,returnRewardGrade,goldForClearedFloor}from"./core/ReturnRewardSystem.js?v=2.6.1";
import{modifiedGoldReward}from"./core/GoldRewardSystem.js?v=2.6.1";
import{battleGoldBase,chestGoldBase,secondWorldEventGoldBase,specialBattleGoldBase}from"./core/GoldEconomySystem.js?v=2.6.1";
import{monsterCombatPower,partyCombatPower,formatCombatPower,recordPartyCombatPower}from"./core/CombatPower.js?v=2.6.1";
import{beginSecretRoomExpedition,ensureSecretRoomExpedition,secretRoomPlan,enterSecretRoom,activeSecretRoom,spinSecretRoomCasino,useSecretRoomInn,buyDarkMarketOffer,buyDarkMarketRecovery,SECRET_ROOM_RECOVERY_ITEMS,DARK_MARKET_ITEM_LIMIT,CASINO_CRYSTAL_COST,CASINO_MULTIPLIER_RATES}from"./core/SecretRoomSystem.js?v=2.6.1";
import{applyGameMasterReward,applySerialReward,commitSerialRedemption,validateGameMasterCode,validateSerialCode}from"./core/SerialCodeSystem.js?v=2.6.1";
import{NOTICE_DEFINITIONS,markAllNoticesRead,normalizeNoticeState}from"./core/NoticeSystem.js?v=2.6.1";
import{monsterSpriteUrl,monsterVisual,setMonsterVisualFrame}from"./ui/MonsterVisual.js?v=2.6.1";
...
function openTeamBattle(){
 ...
 app.insertAdjacentHTML("beforeend",Modal("4対4",`<div class="team-battle-intro"><small>深淵闘技場${CONTENT_TEST_MODE?"・試遊解放":""}</small><h2>第${team.stage}試練</h2><p>4体対4体。10戦ごとに敵編成と補正が跳ね上がり、第50試練は役割・属性・装備構成が必須です。</p><div class="team-reward-preview"><b>勝利GOLD ×${reward.goldMultiplier}</b><span>${pixelIcon("crystal")} ${reward.crystals}個</span>${reward.guaranteedRarity?`<em>${reward.guaranteedRarity}装備確定</em>`:""}</div><div class="daily-attempt-plaque"><b>本日 残り${team.remaining}回</b><small>${team.dailyAttempts}/${team.limit}・日本時間0時更新</small></div></div>`,`挑戦する`));
...
function triggerEmergencyEncounter(forcedId=null,{testPreview=false,returnScreen=null,manual=false}={}){
 ...
 if(wasExploring&&!manual&&!testPreview){
  ...
  startSpecialBattle(event.enemies,{type:"emergency",title:event.boss.name,subtitle:`${manifestation.label} / ${manifestation.percent}%`,priorVitals:prior,bossId:event.boss.id,powerPercent:manifestation.percent,fragmentReward,returnScreen:returnScreen??"explore"});return;
 }
 ...
 modal.querySelectorAll("[data-endgame-prelude]").forEach(button=>button.onclick=()=>{const prelude=resolveEndgamePrelude(save.state,event.boss.id,button.dataset.endgamePrelude);if(manual){const consumed=consumeManualEndgameChallenge(save.state,event.boss.id,prelude.id);if(!consumed.ok)return showToast(consumed.message)}applyPreludeToEncounter(event,prelude);if(emergencyState.pendingEncounter?.bossId===event.boss.id)emergencyState.pendingEncounter=null;fullyRecoverParty();save.save();modal.remove();if(wasExploring){snapshot=currentSnapshot();stopGame()}startSpecialBattle(event.enemies,{type:"emergency",title:event.boss.name,subtitle:prelude.title,priorVitals:prior,bossId:event.boss.id,powerPercent:prelude.percent,fragmentReward:prelude.fragmentReward,manualChallenge:manual,preludeChoiceId:prelude.id,preludeResultText:prelude.resultText,returnScreen:returnScreen??(wasExploring?"explore":"home")})});
}
...
function finishSpecialBattle(won){
 ...
 if(type==="gauntlet"&&!won){fullyRecoverParty();save.save();activeEnemy=null;openBattleContributionReport(contributionSnapshot,()=>{battle=null;showGauntletSettlement(gauntletSettlement??normalizeEndgameState(save.state).trials.lastSettlement)});return}
 const trialName=ENDGAME_TRIALS[trialNumber-1]?.name??`第${trialNumber}戦`,subject=type==="gauntlet"?`奈落回廊 第${trialNumber}戦`:boss?.name??"チームバトル",progressText=type==="gauntlet"?(trialProgress?.loopCompleted?`22戦を踏破。精算せず${trialProgress.loop}周目へ進めます。`:`次は第${trialProgress?.battle??trialNumber}戦「${ENDGAME_TRIALS[(trialProgress?.battle??trialNumber)-1]?.name??trialName}」。`):type==="team"?"次の試練が解放されました。":"世界異変を退けました。";
...
function openExploreFloorSelector(){
 ...
 const partyCards=party.map(monster=>{const species=SPECIES[monster.speciesId]??{};return`<article class="departure-party-card">${monsterVisual(monster,species.emoji??"👹",{className:"departure-monster-visual"})}<b>${displayName(monster)}</b><small>Lv.${monster.level}</small><span>★${Math.max(1,Math.min(MONSTER_STAR_MAX,monster.stars??1))}</span></article>`}).join("");
...
function settleGauntletRun(reason="return"){
 ...
 const victories=Math.max(0,Math.floor(Number(run.victories)||0)),score=Math.max(0,Math.floor(Number(run.score)||0)),maxFloor=Math.max(1,Number(save.state.player.maxFloor)||1),bossIds=(run.defeatedBossIds??[]).filter(id=>ENDGAME_BOSSES[id]);
 ...
 const settlement={reason,victories,score,gold,fragments:fragmentMap,equipment,equipmentReceipt:equipmentReceiptResult,maxLoop:Math.max(1,Number(run.maxLoop)||Number(run.loop)||1),endedAt:new Date().toISOString()};
...
function showGauntletSettlement(settlement){
 ...
 const fragmentRows=Object.entries(settlement.fragments??{}).map(([id,count])=>{const boss=ENDGAME_BOSSES[id];return`<article>${monsterVisual(id,boss?.icon??"MONSTER",{className:"gauntlet-settlement-fragment"})}<span><b>${boss?.name??id}の欠片 ×${count}</b><small>欠片祭壇へ追加</small></span></article>`}).join("");
...
function beginGauntletEncounter(){
 ...
 const encounter=createEndgameTrialEncounter(save.state,run.battle),trial=encounter.trial;run.fights=Math.max(0,Number(run.fights)||0)+1;save.save();
...
function bindSkills(){
 ...
 document.querySelector("[data-open-abyss-skill-tree]")?.addEventListener("click",()=>go("abyssSkills"));
...
function workshopMonsterCard(monster,{selected=false,targetCandidate=false,release=false}={}){
 const species=SPECIES[monster.speciesId]??{},rarity=workshopRarity(monster),stats=calculatedStats(monster),stars=Math.max(1,Math.min(MONSTER_STAR_MAX,monster.stars??1));
 return`<article class="monster-workshop-card rarity-${rarity} ${selected?"selected":""} ${workshopProtected(monster)?"protected":""}" data-workshop-monster="${monster.id}" ${targetCandidate?'data-workshop-drag-target="1"':""}>
  ...
  <div><b>${displayName(monster)}＋${monster.plus??0} <i>Lv.${monster.level}</i></b><small>固有値 ${stars<=6?"★".repeat(stars):`★${stars}`}　❤️${monster.affection??0}</small><small>HP ${stats.hp.toLocaleString()} / ATK ${stats.atk.toLocaleString()} / DEF ${stats.def.toLocaleString()} / SPD ${stats.spd.toLocaleString()}</small></div>
...
function bulkSynthesizeMonsters(maxRarity="SSR"){
 ...
 for(const group of groups.values()){
  ...
  const ranked=[...group].sort((a,b)=>(b.level??1)-(a.level??1)||(b.stars??1)-(a.stars??1)||(b.plus??0)-(a.plus??0)||(b.affection??0)-(a.affection??0)),target=ranked[0];
...
function openEquipmentSlotPicker(subslot){
 ...
 const candidates=save.state.equipment.filter(item=>canEquipInSubslot(item,target,subslot)&&(!item.equippedBy||item.equippedBy===target.id)).sort((a,b)=>(RARITY_ORDER[equipmentDisplayRarity(b)]??0)-(RARITY_ORDER[equipmentDisplayRarity(a)]??0)||equipmentPower(b)-equipmentPower(a));
 const rows=candidates.map(item=>{
  ...
  return`<button type="button" class="equipment-slot-picker-row rarity-${rarityCssClass(rarity)} ${item.id===currentId?"current":""}" data-equipment-picker-item="${item.id}"><span class="equipment-picker-visual">${equipmentVisual(item,{className:"equipment-picker-art"})}</span><div><small>${rarity}・Lv.${item.level??1}</small><b>${item.name}</b><span>${stats||"能力補正なし"}</span><em>${diamonds}</em></div><strong>${item.id===currentId?"装備中":"装備"}</strong></button>`;
 }).join("");
...
const INVENTORY_STACK_INFO={
 ...
 experienceItems:["🔶","EXP結晶","通常種は50個でLv.10,000相当。深淵・十神は必要EXPが5倍です。"],
...
function inventoryStackContext(id){
 ...
 const usable=price>0||id==="experienceItems";
...
const NORMAL_SUMMON_RATES=Object.freeze({N:50,R:32,SR:13,SSR:4,UR:.8,LR:.18,"神話":.02,"深淵":0,"十神":0});
const GUERRILLA_SUMMON_RATES=Object.freeze({...NORMAL_SUMMON_RATES,N:49.966,"深淵":.018,"十神":.016});
...
function currentGachaCampaigns(){
 const guerrilla=guerrillaGachaStatus();return[
  ...
  {id:"guerrilla",badge:guerrilla.active?"本日限定・0:00まで":"ゲリラ待機",title:"深淵・十神　天変召喚",copy:guerrilla.active?"神話0.020%・深淵0.018%・十神0.016%":"7日ごとにランダムな1日だけ出現。開催日は事前固定されません。",tone:"abyss",mode:"guerrilla",disabled:!guerrilla.active},
...
const SUMMON_RARITY_INFO=[
 {id:"N",name:"ノーマル",note:"通常時 50.000%"},
 ...
 {id:"神話",name:"神話級",note:"通常時 0.020%"},
 {id:"深淵",name:"深淵級",note:"通常時 0%・ゲリラ時 0.018%"},
 {id:"十神",name:"十神",note:"通常時 0%・ゲリラ時 0.016%"}
];
...
function guerrillaRarityRoll(){const roll=Math.random()*100;if(roll<.016)return"十神";if(roll<.034)return"深淵";if(roll<.054)return"神話";if(roll<.234)return"LR";if(roll<1.034)return"UR";if(roll<5.034)return"SSR";if(roll<18.034)return"SR";if(roll<50.034)return"R";return"N"}
function summonGuerrillaOne({guaranteedRare=false}={}){let rarity=guerrillaRarityRoll();if(guaranteedRare&&(rarity==="N"||rarity==="R"))rarity=rarityRoll("guaranteed");if(rarity==="十神")return summonEndgameGacha("tenGod");if(rarity==="深淵")return summonEndgameGacha("abyss");return summonOne({mode:"monster",guaranteedMonster:true,forcedRarity:rarity})}
...
function openGacha(){
 const campaigns=currentGachaCampaigns(),guerrilla=guerrillaGachaStatus(),tutorialFree=Math.max(0,Number(save.state.gacha?.tutorialFreeSummons)||0);save.save();const body=`<div class="gacha-festival-v3">
  ...
  <section class="gacha-event-list"><h3>イベント召喚</h3><button type="button" class="guerrilla-entry ${guerrilla.active?"active":"sleeping"}" data-open-guerrilla ${guerrilla.active?"":"disabled"}><span class="gacha-event-mark"></span><b>深淵・十神　天変召喚</b><small>${guerrilla.active?"本日0時まで開催中":"7日間に1日だけ、予告なく出現"}</small><em>${guerrilla.active?"開催中":"沈黙"}</em></button><button type="button" class="event-series-preview" disabled><span class="gacha-category-art equipment-art"></span><b>限定・装備シリーズ召喚</b><small>イベントデータ追加で自動表示できる枠です</small><em>準備中</em></button></section>
 ...
 modal.querySelectorAll("[data-gacha-campaign]").forEach(button=>button.onclick=()=>{const campaign=button.dataset.gachaCampaign;if(campaign==="beginner")return performGachaBatch("monster",10,{campaign:"beginner",cost:0});if(campaign==="event-preview")return;openGachaCountPicker(campaigns.find(entry=>entry.id===campaign)?.mode??"mixed",campaign)});
 modal.querySelector("[data-open-guerrilla]")?.addEventListener("click",()=>openGachaCountPicker("guerrilla","guerrilla"));
...
function showSummonResults(results,deep=false,{campaign="standard"}={}){
 ...
 const visual=result=>result.type==="monster"?monsterVisual(result.item??result,result.icon,{className:"gacha-result-monster-visual"}):result.type==="gold"?pixelIcon("coin","gacha-result-gold-art"):equipmentVisual(result.item,{className:"gacha-result-equipment-art"});
 const row=(result,index)=>{const rarity=result.displayRarity??result.rarity,key=rarityCssClass(rarity);return`<article class="gacha-result-card rarity-${key}" style="--reveal-index:${index}"><span>${visual(result)}</span><div><small>${result.type==="monster"?"魔物":result.type==="gold"?"GOLD":"装備"} ${String(index+1).padStart(2,"0")}</small><b class="rarity-name-${key}">[${rarity}] ${result.name}</b><em>${result.isNew?"新規":result.type==="gold"?"獲得":"重複"}</em></div></article>`};
 ...
 const returnToSummonTop=()=>{if(!completed)return;modal.remove();document.querySelectorAll(".gacha-modal-v2,.gacha-count-modal").forEach(entry=>entry.remove());openGacha();requestAnimationFrame(()=>window.scrollTo({top:0,behavior:"auto"}))};
...
function attributeSynergyFor(elements=[]){
 const counts={};elements.filter(Boolean).forEach(element=>{const key=element==="thunder"?"lightning":element;counts[key]=(counts[key]??0)+1});const [element,count]=Object.entries(counts).sort((a,b)=>b[1]-a[1])[0]??[null,0];if(count<2)return null;
 const rate=count>=4?.28:count===3?.16:.08,names={neutral:"無",fire:"火",water:"水",ice:"氷",lightning:"雷",earth:"土",wind:"風",light:"光",dark:"闇",poison:"毒",nature:"自然",chaos:"混沌"},effects={neutral:{atk:rate*.7,def:rate*.7,hp:rate*.7},fire:{atk:rate},water:{def:rate,hp:rate*.7},ice:{def:rate,spd:rate*.35},lightning:{spd:rate,crit:count>=4?18:count===3?10:5},earth:{def:rate,hp:rate},wind:{spd:rate,evasion:count>=4?14:count===3?8:4},light:{def:rate*.75,hp:rate*.75},dark:{atk:rate*.8,crit:count>=4?16:count===3?9:4},poison:{atk:rate*.8,spd:rate*.45},nature:{hp:rate,def:rate*.6},chaos:{atk:rate*.75,def:rate*.75,spd:rate*.75}};
...
function normalizedElement(element){return element==="thunder"?"lightning":element||"neutral"}
function battleBiomeForFloor(floor){
 const biome=biomeForFloor(floor),favorable=[...new Set((biome.elements??[]).map(normalizedElement))];
 const adverseByTheme={
  fire:["nature","ice"],lava:["nature","ice"],ice:["fire","nature"],poison:["light","water"],lightning:["water"],earth:["lightning","wind"],wind:["earth"],light:["dark","poison"],dark:["light"],water:["fire"],chaos:[]
 };
 return{id:biome.id,name:biome.name,theme:biome.theme,accent:biome.accent,favorable,adverse:(adverseByTheme[biome.theme]??[]).map(normalizedElement),boost:1.22,penalty:.84};
}
...
function equipmentReceipt(item,options={}){
 if(options.scaleToFloor!==false&&(Number(item.level)||1)<=1&&!item.endgameBossId&&!item.ruleOverrides?.fixedLevel){
  ...
  item.level=Math.max(1,Math.min(ENDGAME_MAX_LEVEL,Math.round(floor*(.5+Math.random()))));
...
function applyExploreAutoPath(){
 ...
 if(mode==="items"){
  (game.world.chests??[]).filter(entry=>!entry.open).forEach(entry=>targets.push(entry));
 ...
 const best=nearestAutoTarget(targets.filter(Boolean));if(best)game.player.setPath(best.route);else{save.state.settings.exploreAutoMode="off";save.save();showExploreNotice("自動探索を停止しました")}
}
...
const FLOOR_TUTORIALS={
 1:{title:"歩く・拾う・次の階へ",summary:"移動、敵との遭遇、戦利品、階段を覚える。",sections:["床をタップすると部隊が移動します。歩いていると敵に遭遇し、勝利すればGOLD・EXP・装備などを獲得します。","宝箱や樽、木箱、骨、魔晶石、水場は近づいて調べられます。階段へ着くと次の階へ進みます。"]},
 2:{title:"戦闘画面と隊列",summary:"HP・MP・CT、前衛／後衛、AUTOと速度を確認。",sections:["HPが0になると戦闘不能、MPはスキルで消費します。CT（クールダウン）中のスキルは再使用まで待つ必要があります。","前2枠は前衛、後ろ2枠は後衛です。AUTOで自動戦闘、×0.5／×1／×2／×4で演出速度を変更できます。"]},
 3:{title:"消耗・回復・捕獲",summary:"探索中のHP／MP持越し、状態異常、捕獲、帰還。",sections:["戦闘後もHP・MPと状態異常は残ります。回復アイテム、拠点の寝台、治療スキルを使って立て直しましょう。","敵を弱らせ、捕獲結晶を使うと仲間にできます。危険なら帰還して戦利品を確定し、準備を整えて再出発できます。"]},
 4:{title:"属性と同属性編成",summary:"無属性を含む相性、属性フロア、同属性パーティー。",sections:["無・火・水・氷・雷・地・風・光・闇・毒・自然の属性があります。有利は1.25倍、不利は0.8倍。無属性は基本的に等倍です。","階層ごとに適性属性があり、同属性を揃えた編成では共鳴ボーナスが発動します。敵と地形に合わせて組み替えましょう。"]},
 5:{title:"探索を自動化する",summary:"ミニマップ、3種AUTO、調査物、秘密入口、ボスと1000階目標。",sections:["ミニマップは表示・移動が可能です。自動探索は「階層攻略」「回収優先」「経験値優先」の3モード。メニューを閉じても選んだ方針は継続します。","調べられる物や壁の秘密入口を見逃さず進みましょう。10階ごとにボスが待ち、最初の大目標は1000階です。"]}
};
function tutorialBody(tutorial){return`<div class="tutorial-lesson"><p class="tutorial-summary"><b>${tutorial.summary}</b></p>${tutorial.sections.map((section,index)=>`<section><small>STEP ${index+1}</small><p>${section}</p></section>`).join("")}</div>`}
function showFloorTutorial(){
 ...
 game.paused=true;setTimeout(()=>{
  if(!game?.running)return;
  app.insertAdjacentHTML("beforeend",Modal(`${floor}階チュートリアル：${tutorial.title}`,`${tutorialBody(tutorial)}<p class="muted">自動表示は各階1回です。設定の指南書からいつでも再生できます。</p><button type="button" class="secondary tutorial-skip-all" data-tutorial-skip-all>以後の序盤チュートリアルをスキップ</button>`,`探索開始`));
  const modal=topModal(),close=()=>{modal?.remove();if(game)game.paused=false};modal.querySelector("[data-tutorial-skip-all]")?.addEventListener("click",()=>{for(let level=1;level<=5;level++)save.state.settings.tutorialSeen[level]=true;save.save();showToast("序盤チュートリアルをスキップしました");close()});modal._onDismiss=close;modal.querySelector("[data-modal-primary]").onclick=close
 },120)
}
function openTutorialBook(){
 const rows=Object.entries(FLOOR_TUTORIALS).map(([floor,t])=>`<button class="tutorial-book-row" data-tutorial-floor="${floor}"><span>${floor}F</span><div><b>${t.title}</b><small>${t.summary}</small></div><em>›</em></button>`).join("");
 ...
 const modal=topModal();modal.querySelectorAll("[data-tutorial-floor]").forEach(button=>button.onclick=()=>{const floor=button.dataset.tutorialFloor,t=FLOOR_TUTORIALS[floor];app.insertAdjacentHTML("beforeend",Modal(`${floor}階：${t.title}`,tutorialBody(t),`一覧へ戻る`));topModalButton().onclick=closeTopModal});modal.querySelector("[data-modal-primary]").onclick=closeTopModal
}
function claimDefeatTutorial(){
 const seen=Math.max(0,Math.min(2,Number(save.state.settings?.tutorialDefeatsSeen)||0));if(seen>=2)return"";
 save.state.settings.tutorialDefeatsSeen=seen+1;save.state.gacha??={};
 if(seen===0){save.state.gacha.tutorialFreeSummons=Math.max(1,Number(save.state.gacha.tutorialFreeSummons)||0);return`<section class="defeat-tutorial"><small>DEFEAT GUIDE 1/2</small><h3>敗北は編成を作る入口</h3><p><b>無料の初回チュートリアル召喚×1</b>を召喚の祭壇へ追加しました。</p><ul><li>新しい仲間を「編成」で出撃枠へ入れる</li><li>武器・防具・アクセを「装備」で割り当てる</li><li>寝台でHP・MP・状態異常を回復する</li><li>スキル画面で4枠と消費MPを整える</li></ul></section>`}
 return`<section class="defeat-tutorial"><small>DEFEAT GUIDE 2/2</small><h3>育成を組み直して再挑戦</h3><ul><li>装備強化：不要装備を素材にLvと能力を上げる</li><li>同種合成：同名2体で＋1。★は生まれつきの固有値、＋は合成成長</li><li>スキル熟練度とMP消費を見て、長期戦用の構成へ調整</li><li>お気に入り／ロックで素材事故を防止</li><li>レベルで装備枠が解放。ランダムオプションはGOLDで厳選</li><li>一括自動合成を使い、敵階層の属性に合わせて編成を再構築</li></ul></section>`;
}
...
function speciesPoolForFloor(floor){
 ...
 const unlocked=Object.values(SPECIES).filter(species=>species.fieldEncounter!==false&&!species.ultraRareEncounter&&(species.minFloor??1)<=floor).sort((a,b)=>(a.minFloor??1)-(b.minFloor??1));
...
function randomEnemy(){const floor=save.state.player.currentFloor;if(floor===1)return{speciesId:"slime",level:1,boss:false,equipped:false,gear:null,enemyFloor:floor};const rareEncounterRate=(save.state.party??[]).map(id=>save.state.monsters.find(monster=>monster.id===id)).filter(Boolean).reduce((sum,monster)=>sum+seriesEffectValue(monster,"rareEncounter",.5),0);if(floor>=777&&Math.random()<Math.min(.0012,.00025*(1+rareEncounterRate)))return{speciesId:"ochuki",level:Math.max(1,enemyLevelForFloor(floor)),boss:false,equipped:false,gear:null,rareExp:true,fleeAfterTurns:2+Math.floor(Math.random()*3),uncapturable:false,enemyFloor:floor};if(floor>=2&&Math.random()<Math.min(.03,.006*(1+rareEncounterRate)))return{speciesId:"baby_slime",level:Math.max(1,enemyLevelForFloor(floor)),boss:false,equipped:false,gear:null,rareExp:true,enemyFloor:floor};const pool=speciesPoolForFloor(floor).filter(s=>s.id!=="baby_slime"),picked=pool[Math.floor(Math.random()*pool.length)],speciesId=picked.id,equipped=floor>=6&&Math.random()<.11,gear=equipped?createEquipment(["weapon","armor","accessory"][Math.floor(Math.random()*3)]):null;return{speciesId,level:enemyLevelForFloor(floor),boss:false,equipped,gear,enemyFloor:floor}}
function randomEnemyGroup(){
 const floor=save.state.player.currentFloor;if(floor<=4)return[randomEnemy()];
 if(floor>1000){
  const endgameRate=floor>=3000?.82:floor>=1500?.68:.52;
  if(Math.random()<endgameRate){
   const bosses=Object.values(ENDGAME_BOSSES),countRoll=Math.random(),count=countRoll<.25?2:countRoll<.62?3:4;
   const level=enemyLevelForFloor(floor),depthMultiplier=1.5+Math.max(0,floor-1000)/500;
   return Array.from({length:count},(_,index)=>{
    const boss=bosses[Math.floor(Math.random()*bosses.length)];
    return{speciesId:boss.speciesId,level:Math.max(1,level-index*17),boss:false,endgameBossId:boss.id,visualSpeciesId:boss.id,faction:boss.faction,nameOverride:boss.name,statMultiplier:depthMultiplier*(1+index*.08),uncapturable:true,endgameRoaming:true,bossPassive:boss.passive,elementMultipliers:boss.elementMultipliers,statusProfile:boss.statusProfile,enemyFloor:floor};
   });
  }
 }
 ...
 else if(floor<150)count=roll<.08?4:roll<.24?3:roll<.58?2:1;
 else if(floor<500)count=roll<.18?4:roll<.48?3:roll<.78?2:1;
 else count=roll<.34?4:roll<.68?3:roll<.9?2:1;
...
function floorBossParty(bossInfo,floor){
 if(bossInfo.endgameBossId)return[bossInfo];
 const count=floor>=500?4:floor>=100?3:floor>=30?2:1;if(count===1)return[bossInfo];
 const random=seeded(floorSeed(floor)+1709),pool=speciesPoolForFloor(Math.max(10,floor)).filter(species=>species.id!==bossInfo.speciesId);
 const supports=Array.from({length:count-1},(_,index)=>{
  const species=pool[Math.floor(random()*pool.length)]??SPECIES.slime;
  return{speciesId:species.id,level:Math.max(1,bossInfo.level-4-index*3),boss:false,uncapturable:true,endgameSupport:true,nameOverride:`階層眷属・${species.name}`,statMultiplier:1.85+Math.min(3.2,floor/260)+index*.16};
 });
 return[{...bossInfo,statMultiplier:(bossInfo.statMultiplier??1)*(1.25+Math.min(2.4,floor/520))},...supports]
}
function openFloorBossChallenge(bossInfo,floor){
 ...
 app.insertAdjacentHTML("beforeend",Modal("階層支配者",`<div class="floor-boss-challenge-v3 tone-${tone}"><div class="boss-chain-frame" aria-hidden="true"><i></i><i></i></div><div class="boss-crest-v3"><span>階層</span><strong>${floor}</strong><em>支配者</em></div><div class="boss-visual-stage-v3"><div class="boss-fog-v3"></div>${monsterVisual({...bossInfo,visualSpeciesId:bossInfo.endgameBossId},species.emoji??"BOSS",{className:"floor-boss-monster-visual-v3"})}</div><small>${endgame?.title??`第${floor}階層の支配者`}</small><h2>${name}</h2><b>Lv.${bossInfo.level}</b><blockquote>${quote}</blockquote><div class="boss-power-versus-v3"><span><small>部隊戦力</small><b>${formatCombatPower(partyPower)}</b></span><i>対</i><span><small>ボス戦力</small><b>${formatCombatPower(bossPower)}</b></span></div><button type="button" class="boss-retreat-v3" data-boss-retreat>いったん退く</button></div>`,"支配者へ挑む"));
...
const EXPLORE_TEXTURE_URLS={
 floor:"assets/ui/explore/dungeon-floor.png?v=2.6.1",
 wall:"assets/ui/explore/dungeon-wall.png?v=2.6.1",
 stairs:"assets/ui/explore/dungeon-stairs-arch.png?v=2.6.1",
 props:"assets/ui/explore/dungeon-props-atlas.png?v=2.6.1",
 usedWater:"assets/ui/explore/empty-water-basin.png?v=2.6.1"
};
...
async function battleIntro(enemies){
 ...
 else if(battle?.specialBattle){const isEmergency=battle.specialBattleType==="emergency",isGauntlet=battle.specialBattleType==="gauntlet";battleFlash(isEmergency?"boss":"hit");await battleBanner(isEmergency?"世界異変":isGauntlet?"深淵の試練":"部隊戦",battle.specialTitle??(isGauntlet?"奈落回廊":"4対4"),isEmergency?"boss":"encounter",1100)}
...
function makeBattleEnemy(e,index=0){
 const sp=SPECIES[e.speciesId],scaled={...e,level:Math.max(1,e.level??1)},enemy=createEnemyBattleState(sp,scaled,save.state.player.currentFloor),profile=e.endgameBossId?endgameCharacter(e.endgameBossId):null,endgameBase=endgameFactionStatMultiplier(e.faction??profile?.faction),mult=(Number(e.statMultiplier)||1)*endgameBase;
 enemy.dangerLevel=e.boss?5:e.speciesId==="mimic"?3:e.equipped?3:((e.level??1)>save.state.player.currentFloor+4?2:1);
 if(e.nameOverride)enemy.name=e.nameOverride;
 applyEnemyMultiplier(enemy,mult);
 enemy.endgameBossId=e.endgameBossId??null;enemy.faction=e.faction??profile?.faction??null;enemy.powerRate=e.powerRate??null;enemy.manifestationLabel=e.manifestationLabel??null;enemy.endgameSupport=Boolean(e.endgameSupport);enemy.uncapturable=Boolean(e.uncapturable);enemy.trialElement=normalizedElement(e.trialElement??e.attribute??profile?.element??sp?.element);enemy.id=`enemy-${Date.now()}-${index}-${Math.random().toString(36).slice(2,7)}`;
 applyEliteModifiers(enemy,e);hydrateEndgameEnemy(enemy,e);
 const hiddenFloor=Math.max(1,Number(e.enemyFloor??(e.memorySourceFloor||save.state.player.currentFloor))||1),hidden=enemyHiddenProfileForFloor(hiddenFloor,{rank:e.faction??sp?.rarity??"N",faction:e.faction,boss:Boolean(e.boss)});enemy.hiddenProfile=hidden;enemy.hiddenDamageTaken=hidden.damageTaken??1;enemy.hiddenStatusResist=hidden.statusResist??0;enemy.hiddenCapturePressure=hidden.capturePressure??1;enemy.hiddenAi=hidden.ai??0;
...
function saveBattleCheckpoint(){
 ...
 save.state.activeBattle={
  ...
  ,reviveCount:battle.reviveCount??0,performance:battle.performance??{},circleTurnMultipliers:battle.circleTurnMultipliers??{},circleTurnKeys:battle.circleTurnKeys??{},circleShields:battle.circleShields??{},openingCircleBuff:Boolean(battle.openingCircleBuff)
 };
 save.save()
}
...
function resumeSavedBattle(){
 ...
 battle={...data,battleId:data.battleId??crypto.randomUUID?.()??`${Date.now()}-${Math.random()}`,party,species:SPECIES,busy:false,skillMenu:false,itemMenu:false,enemy:data.enemies[0],auto:explorationAuto||data.auto,explorationAuto,reviveCount:data.reviveCount??0,performance:data.performance??Object.fromEntries(party.map(monster=>[monster.id,{damage:0,taken:0,healing:0,revives:0,kills:0}])),circleTurnMultipliers:data.circleTurnMultipliers??{},circleTurnKeys:data.circleTurnKeys??{},circleShields:data.circleShields??{},magicCircleArt:Object.fromEntries(party.map(monster=>[monster.id,magicCircleMarkup(monster,save.state,{className:"battle-magic-circle"})])),openingCircleBuff:Boolean(data.openingCircleBuff),...createBattleRulesState(party),cooldowns:data.cooldowns??{},enemyStatuses:data.enemyStatuses??{},allyAilments:data.allyAilments??Object.fromEntries(party.map(monster=>[monster.id,normalizePersistentAilments(monster.ailments)])),allyEffects:data.allyEffects??{},enemyEffects:data.enemyEffects??{},lastStatusTurn:data.lastStatusTurn??0,log:data.log??[]};
 battle.hpDisplayRates={};battle.hpTrails={};battle.invincibleAlliance=invincibleAllianceReady();
...
function circleInfo(monster){return monster?equippedMagicCircle(monster,save.state):null}
...
function magicCircleDamageMultiplier(monster){
 ...
 if(hasCircleEffect(monster,"goldPower")){const gold=Math.max(0,Number(save.state.player.gold)||0);value*=1+Math.min(1,Math.log10(gold+1)/12)}
 return value;
}
...
function consumeMagicCircleActionCost(monster){if(!hasCircleEffect(monster,"goldPower"))return;const cost=Math.min(Math.max(1000,Math.floor((Number(save.state.player.gold)||0)*.00001)),1_000_000);save.state.player.gold=Math.max(0,Math.floor(save.state.player.gold-cost));addBattleLog(battle,`${displayName(monster)}：黄金換力 -${cost.toLocaleString()}G`)}
function absorbMagicCircleShield(monster,damage){const available=Math.max(0,Math.floor(battle?.circleShields?.[monster.id]||0));if(!available)return damage;const absorbed=Math.min(available,damage);battle.circleShields[monster.id]-=absorbed;if(absorbed)addBattleLog(battle,`${displayName(monster)}の魔法陣障壁が${absorbed}吸収`);return Math.max(0,damage-absorbed)}
function handleMagicCircleDeath(monster){
 if(!monster||monster.currentHp>0||monster._circleDeathHandled)return;monster._circleDeathHandled=true;
 if(hasCircleEffect(monster,"inheritance"))for(const ally of battle.party.filter(member=>member.currentHp>0)){applyBattleEffect(battle,ally.id,{kind:"atkUp",value:.3,turns:5},"ally");applyBattleEffect(battle,ally.id,{kind:"defUp",value:.3,turns:5},"ally");applyBattleEffect(battle,ally.id,{kind:"spdUp",value:.2,turns:5},"ally")}
 if(hasCircleEffect(monster,"deathDrain"))for(const enemy of aliveEnemies(battle))enemy.currentMp=Math.max(0,(enemy.currentMp??0)-Math.floor((enemy.maxMp??0)*.65));
}
function magicCircleInstantDeath(target,source=null){
 ...
 if(ally&&hasCircleEffect(target,"deathMirror")&&!target._circleDeathMirrorUsed){
  ...
  if(source){if(Object.prototype.hasOwnProperty.call(source,"currentHp")){source.currentHp=0;handleMagicCircleDeath(source)}else if(Object.prototype.hasOwnProperty.call(source,"hp"))source.hp=0}
  addBattleLog(battle,`${displayName(target)}の死返しが即死を反射`);return true;
 }
 if(ally){target.currentHp=0;handleMagicCircleDeath(target)}else target.hp=0;return false;
}
...
async function trySeriesChainAttack(monster,enemy,damage){
 ...
 recordBattleDamage(monster,Math.min(beforeHp,follow));
...
async function applyOpeningMagicCircles(){
 if(!battle)return;
 const shieldOwner=battle.party.find(monster=>(battle.circleShields?.[monster.id]??0)>0);if(shieldOwner)await battleBanner("半月障壁",`最大HP50%の障壁を展開`,"skill",560,shieldOwner);
 const opener=battle.party.find(monster=>hasCircleEffect(monster,"openingBuff"));if(opener)await battleBanner("開戦共鳴",`味方全体の最終ダメージ・会心率+20%`,"skill",620,opener);
 for(const owner of battle.party.filter(monster=>monster.currentHp>0&&hasCircleEffect(monster,"sacrifice"))){
  ...
  const victim=allies[Math.floor(Math.random()*allies.length)],foe=foes[Math.floor(Math.random()*foes.length)];magicCircleInstantDeath(victim,owner);magicCircleInstantDeath(foe,owner);registerWeaponFinisher(owner,foe,1);await battleBanner("等価滅殺",`${displayName(victim)}と${foe.name}へ即死判定`,"boss",800,owner);
 }
...
async function prepareMagicCircleTurn(monster){
 if(!battle||!monster||!hasCircleEffect(monster,"slot"))return false;
 const key=`${battle.turn}:${battle.queueIndex}:${monster.id}`;if(battle.circleTurnKeys?.[monster.id]===key)return false;
 battle.circleTurnKeys[monster.id]=key;const roll=Math.floor(Math.random()*1000),multiplier=slotDamageMultiplier(roll);battle.circleTurnMultipliers[monster.id]=multiplier;battle.busy=true;renderBattle();
 const digits=String(roll).padStart(3,"0");await battleBanner(digits,multiplier===0?"このターンは行動不能":`最終ダメージ ×${multiplier.toFixed(2)}`,multiplier===0?"boss":"slot",720,monster);addBattleLog(battle,`${displayName(monster)}：運命の三桁環 ${digits}${multiplier?`（×${multiplier.toFixed(2)}）`:"（行動休止）"}`);battle.busy=false;
...
function startBattle(encounter,options={}){
 ...
 const explorationAuto=save.state.settings.exploreAutoMode!=="off"&&!options.specialBattle&&!options.memoryBattle;
 battle={battleId:crypto.randomUUID?.()??`${Date.now()}-${Math.random().toString(16).slice(2)}`,enemies,enemy:enemies[0],targetEnemyId:enemies[0]?.id,party,species:SPECIES,turn:1,busy:false,auto:explorationAuto||save.state.settings.autoBattle,explorationAuto,guards:{},escapePending:false,actionCommitted:false,skillMenu:false,itemMenu:false,allySynergy:synergy,enemySynergy,biomeBattle,battleTheme,reviveCount:0,performance:Object.fromEntries(party.map(monster=>[monster.id,{damage:0,taken:0,healing:0,revives:0,kills:0}])),circleTurnMultipliers:{},circleTurnKeys:{},circleShields:{},hpDisplayRates:{},hpTrails:{},magicCircleArt:Object.fromEntries(party.map(monster=>[monster.id,magicCircleMarkup(monster,save.state,{className:"battle-magic-circle"})])),openingCircleBuff:party.some(monster=>hasCircleEffect(monster,"openingBuff")),...createBattleRulesState(party),...options};
 battle.invincibleAlliance=invincibleAllianceReady();
 ...
 if(battle.invincibleAlliance)addBattleLog(battle,"無敵・四神話連携が発動！");
 addBattleLog(battle,`行動順：${battle.turnQueue.map(entry=>entry.name).join(" → ")}`);
 saveBattleCheckpoint();renderBattle();setTimeout(async()=>{await battleIntro(enemies);if(battle?.invincibleAlliance)await battleBanner("無敵","えなみ・りおん・より・ひで　四神話連携","synergy invincible",900,party[0]);await applyOpeningMagicCircles();continueBattleFlow()},scaledBattleDelay(120))
}
...
function refreshBattleHpTrails(){
 ...
 for(const [id,rate] of Object.entries(rates)){
  ...
  if(Number.isFinite(previous)&&rate<previous-.01)battle.hpTrails[id]={from:previous,to:rate,startedAt:now,duration:1400};
  else if(Number.isFinite(previous)&&rate>previous+.01)delete battle.hpTrails[id];
  battle.hpDisplayRates[id]=rate;
 }
 for(const [id,trail] of Object.entries(battle.hpTrails))if(now-(Number(trail.startedAt)||0)>1700)delete battle.hpTrails[id];
}
function renderBattle(){
 ...
 document.querySelectorAll("[data-status-detail]").forEach(button=>button.onclick=event=>{event.stopPropagation();const labels=[...button.querySelectorAll(".status-chip")].map(chip=>chip.textContent.trim()).filter(Boolean);app.insertAdjacentHTML("beforeend",Modal("状態効果",`<div class="battle-status-detail-list">${labels.map(label=>`<p>${label}</p>`).join("")||"<p>状態効果はありません</p>"}</div>`,"閉じる"));topModalButton().onclick=closeTopModal});
 ...
 document.getElementById("toggleBattleAuto").onclick=()=>{
  if(battle.explorationAuto)return showToast("探索AUTO中");
  battle.auto=!battle.auto;save.state.settings.autoBattle=battle.auto;save.save();renderBattle();if(battle.auto&&!battle.busy)continueBattleFlow();
 };
...
function invincibleAllianceReady(){const ids=new Set((battle?.party??[]).map(monster=>monster.speciesId));return INVINCIBLE_ALLIANCE_IDS.every(id=>ids.has(id))}
...
async function performInvincibleAllianceSkill(member,skill){
 ...
 for(const enemy of targets){let dealt=0;for(let hit=0;hit<hits&&enemy.hp>0;hit++){const critical=Boolean(skill.guaranteedCritical)||Math.random()<Math.min(.82,.12+(skill.critBonus??0)+(stats.spd??0)*.003),ignore=Math.max(0,Math.min(.9,Number(skill.defenseIgnore)||0)),before=enemy.hp,raw=skillDamage({...stats,_currentHpRatio:member.currentHp/Math.max(1,stats.hp)},{...enemy,def:enemy.def*(1-ignore),mdef:(enemy.mdef??enemy.def)*(1-ignore)},skill,critical),damage=Math.max(1,Math.floor(raw*.72*attributeDamageMultiplier(skill.element??SPECIES[member.speciesId]?.element??"neutral",SPECIES[enemy.speciesId]?.element??"neutral")*enemyDamageMultiplier(enemy)*(enemy.hiddenDamageTaken??1)*magicCircleDamageMultiplier(member)));enemy.hp=Math.max(0,enemy.hp-damage);dealt+=damage;recordBattleDamage(member,damage);registerWeaponFinisher(member,enemy,before);await animateHit(enemy.id,critical);await floatText(`${critical?"会心 ":""}-${damage}`,enemy.id,critical?"critical":"skill")}if(dealt&&skill.status&&enemy.hp>0&&Math.random()<(skill.status.chance??0))applyEnemyStatus(battle,{...skill.status,sourceMonsterId:member.id},enemy.id)}
...
async function command(type,skillId=null){
 ...
 battle.busy=true;
 const s=calculatedStats(a),e=selectedEnemy(battle);if(!e){battle.busy=false;return win(false,null)};battle.enemy=e;let triggerAlliance=false;
 ...
 if(hasCircleEffect(a,"randomSkill")&&(type==="attack"||type==="skill"&&!skillId)){
  ...
  if(pool.length){const randomSkill=pool[Math.floor(Math.random()*pool.length)];type="skill";skillId=randomSkill.id;a._randomCircleSkill=true;await battleBanner("万象抽選",randomSkill.name,"skill",520,a)}
 }
 ...
 if(type==="attack"){
  ...
  if(Math.random()<.06)await floatText("回避",e.id,"miss");
   ...
   const attackElement=a.attribute??SPECIES[a.speciesId]?.element??"neutral",targetElement=SPECIES[e.speciesId]?.element??"neutral",critMult=1.7+affixValue(a,"critDamage",150)/100,damageStats={...s,_currentHpRatio:a.currentHp/Math.max(1,s.hp)},raw=(critical?Math.floor(base*critMult):base)*formationMultiplier*affixOutgoingDamageMultiplier(damageStats,e,attackElement)*affixExecutionMultiplier(a,e),beforeHp=e.hp,d=Math.max(1,Math.floor(raw*attributeDamageMultiplier(attackElement,targetElement)*abyssBattleMultiplier(a,"partyDamageRate")*enemyDamageMultiplier(e)*(e.hiddenDamageTaken??1)*endgameIncomingDamageMultiplier(e,attackElement)*weaponMasteryDamageMultiplier(save.state,a,e)*magicCircleDamageMultiplier(a)));e.hp=Math.max(0,e.hp-d);recordBattleDamage(a,d);registerWeaponFinisher(a,e,beforeHp);consumeMagicCircleActionCost(a);const steal=outgoingLifeSteal(a);if(steal){const h=Math.max(1,Math.floor(d*steal));const before=a.currentHp;a.currentHp=Math.min(s.hp,a.currentHp+h);recordBattleHealing(a,a.currentHp-before)}
   await animateHit(e.id,critical);if(critical){battleFlash("critical");burstParticles(e.id,"critical",16)}await floatText(`${critical?"会心 ":""}-${d}`,e.id,critical?"critical":"damage");await trySeriesChainAttack(a,e,d);
   const rageHits=hasCircleEffect(a,"rage")?(a._circleRage>=9?2:a._circleRage>=4?1:0):0;
   for(let hit=0;hit<rageHits&&e.hp>0;hit++){const before=e.hp,follow=Math.max(1,Math.floor(d*(hit?0.45:0.65)));e.hp=Math.max(0,e.hp-follow);recordBattleDamage(a,Math.min(before,follow));registerWeaponFinisher(a,e,before);await animateHit(e.id,false);await floatText(`連撃 -${follow}`,e.id,"skill")}
  }
 }
 ...
 if(type==="skill"&&skillId){
  const skill=skillById(skillId),cd=cooldownRemaining(battle,a.id,skillId),randomCircleSkill=Boolean(a._randomCircleSkill);
  ...
  if(skill.type==="selfHeal"||skill.type==="stance"&&skill.heal){
   ...
   for(const targetEnemy of skillTargets){const e=targetEnemy;let targetTotal=0;for(let i=0;i<hits&&e.hp>0;i++){
    const critical=Boolean(skill.guaranteedCritical)||Math.random()<Math.min(.95,affixCriticalChance(s,Math.min(.65,.1+(skill.critBonus??0)+(s.spd??0)*.004))+magicCircleCriticalBonus(a,skill.power??1)),ignore=Math.max(0,Math.min(.9,Number(skill.defenseIgnore)||0)),boosted={...s,atk:s.atk*allyAttackFactor(a.id),matk:(s.matk??s.atk)*allyAttackFactor(a.id),_currentHpRatio:a.currentHp/Math.max(1,s.hp)},execute=(skill.execute&&e.hp/e.maxHp<=skill.execute)?2:1,raw=skillDamage(boosted,{...e,def:e.def*enemyDefenseFactor(e.id)*(1-ignore),mdef:(e.mdef??e.def)*enemyDefenseFactor(e.id)*(1-ignore)},skill,critical)*execute*affixExecutionMultiplier(a,e)*(1+affixValue(a,"skillPower",200)/100),beforeHp=e.hp,d=Math.max(1,Math.floor(raw*attributeDamageMultiplier(skill.element,SPECIES[e.speciesId]?.element??"neutral")*abyssBattleMultiplier(a,"partyDamageRate")*enemyDamageMultiplier(e)*(e.hiddenDamageTaken??1)*endgameIncomingDamageMultiplier(e,skill.element)*weaponMasteryDamageMultiplier(save.state,a,e)*magicCircleDamageMultiplier(a)));
    e.hp=Math.max(0,e.hp-d);recordBattleDamage(a,d);registerWeaponFinisher(a,e,beforeHp);total+=d;targetTotal+=d;await animateHit(e.id,critical);if(critical){battleFlash("critical");burstParticles(e.id,"critical",14)}await floatText(`${critical?"会心 ":""}-${d}`,e.id,critical?"critical":"skill")
   }
 ...
 if(type==="capture"){
  ...
  if(e.uncapturable||e.endgameBossId||["abyss","tenGod"].includes(e.faction)){battle.busy=false;return alert("深淵・十神は捕獲できません。討伐時に欠片を落とすことがあります。")}
...
function chooseEnemyTarget(enemy=null,mode="normal"){
 ...
 const front=alive.filter(monster=>battle.party.indexOf(monster)<2),rear=alive.filter(monster=>battle.party.indexOf(monster)>=2),role=String(SPECIES[enemy?.speciesId]?.role??""),ranged=["magic","mage","healer","support","ranged","controller"].some(value=>role.includes(value));
...
async function dealEnemyHit(e,target,multiplier=1,label="",criticalChance=.08,element=null,rules={}){
 const st=calculatedStats(target),guard=Boolean(battle.guards[target.id]),endgameNoCrit=hasCircleEffect(target,"endgameNoCrit")&&Boolean(e.endgameBossId||["abyss","tenGod"].includes(e.faction)),critical=!endgameNoCrit&&(Boolean(rules.guaranteedCritical)||Math.random()<Math.min(.9,criticalChance+(e.crit??0)));
 if(!rules.guaranteedHit&&Math.random()<Math.min(.60,(st.evasion??0)/100)){addBattleLog(battle,`${displayName(target)}が回避した`);await floatText("回避",target.id,"miss");return 0}
 const ignore=Math.max(0,Math.min(.9,Number(rules.defenseIgnore)||0)),execute=rules.execute&&target.currentHp/Math.max(1,st.hp)<=rules.execute?2:1,guardFx=Math.min(.85,effectValue(battle,target.id,"guard")*(1+affixValue(target,"guardPower",100)/100)),vulnerable=effectValue(battle,target.id,"vulnerable"),reduction=Math.min(.75,affixValue(target,"damageReduction",75)/100),attackElement=element??SPECIES[e.speciesId]?.element??null,targetElement=target.attribute??SPECIES[target.speciesId]?.element??"neutral",resistance=elementalResistance(target,attackElement);let d=Math.max(1,Math.floor((e.atk*enemyAttackFactor(e.id)-st.def*(1-ignore)*allyDefenseFactor(target.id)*.45)*multiplier*attributeDamageMultiplier(attackElement,targetElement)*execute*(guard&&!rules.guaranteedHit?Math.max(.15,.45-affixValue(target,"guardPower",100)/200):1)*(1-guardFx)*(1+vulnerable)*(1-reduction)*(1-resistance)*abyssBattleMultiplier(target,"partyDamageTakenRate")));if(rules.currentHpDamage)d+=Math.max(1,Math.floor(target.currentHp*Math.min(.25,rules.currentHpDamage)));if(critical)d=Math.floor(d*1.55);if(hasCircleEffect(target,"soleSurvivor")&&battle.party.filter(member=>member.currentHp>0).length===1)d=Math.max(1,Math.floor(d*.6));d=absorbMagicCircleShield(target,d);
 target.currentHp=Math.max(0,target.currentHp-d);recordBattleTaken(target,d);if(hasCircleEffect(target,"rage")&&d>0)target._circleRage=Math.min(12,(target._circleRage??0)+1);if(target.currentHp<=0&&tryUnyielding(target)){addBattleLog(battle,`${displayName(target)}の復活・耐久効果が発動（${battle.reviveCount}/99）`);await floatText("復活",target.id,"guard")}else addBattleLog(battle,`${displayName(target)}に${d}ダメージ`);
 await animateHit(target.id,critical);if(critical){battleFlash("danger");burstParticles(target.id,"enemy",14)}await floatText(`${label}${critical?"会心 ":""}-${d}`,target.id,critical?"critical":"enemy");
 if(target.currentHp<=0){handleMagicCircleDeath(target);await animateDefeat(target.id)}else if(hasEffect(battle,target.id,"counter")){const cs=calculatedStats(target),counterBoost=1+affixValue(target,"counterDamage",150)/100,counter=Math.max(1,Math.floor((cs.atk*effectValue(battle,target.id,"counter")-e.def*.25)*counterBoost*abyssBattleMultiplier(target,"partyDamageRate")*magicCircleDamageMultiplier(target)));e.hp=Math.max(0,e.hp-counter);recordBattleDamage(target,counter);addBattleLog(battle,`${displayName(target)}が${counter}反撃ダメージ`);await floatText(`反撃 -${counter}`,e.id,"skill")}await tryGuardianPassive();return d;
}
...
async function finishCurrentAction(){
 ...
 const finished=currentTurnEntry(battle);if(finished?.type==="ally"&&battle.circleTurnMultipliers)delete battle.circleTurnMultipliers[finished.id];
 advanceQueue(battle);
...
async function continueBattleFlow(){
 ...
 if(entry?.type==="ally"){const current=currentAlly(battle);if(current&&await prepareMagicCircleTurn(current))return;if(battle.auto){await wait(220);const a=currentAlly(battle);if(a){a._maxHp=calculatedStats(a).hp;if(hasCircleEffect(a,"rage")&&(a._circleRage??0)>=4&&Math.random()<Math.min(.75,(a._circleRage??0)*.06))return command("attack");const skill=chooseAutoSkill(a,battle);if(skill)return command("skill",skill.id)}return command("attack")}}
}
...
function win(caught,m){
 ...
 const baseGain=defeated.reduce((sum,e)=>{
  ...
  if(e.rareExp)return sum+(e.speciesId==="ochuki"?Math.round(100000+e.level*500):Math.round(100+e.level*22));
 ...
 const fragmentDrops=[];
 for(const enemy of defeated.filter(entry=>entry.endgameBossId&&!entry.captured)){
...
function bossRewardEquipment(floor,boss,slot,weaponType=null){
 ...
 item.level=Math.max(1,Math.min(ENDGAME_MAX_LEVEL,Math.round(floor*(.5+Math.random()))));item.obtainedFloor=floor;item.obtainedMethod="bossReward";
 return item
}
function createBossRewardOptions(floor,boss){
 ...
 const heals=Math.max(5,4+Math.floor(floor/50)),expItems=Math.max(1,Math.min(12,1+Math.floor(floor/1000)));
 ...
 const utility=[
  ...
  {id:`supply-${floor}`,type:"supply",icon:"chest",expItems,heals,keys:1,title:"深淵遠征セット",desc:`EXP結晶×${expItems}・上級回復薬×${heals}・深淵の鍵×1`}
 ];
...
function lose(){
 ...
 clearPartySynergy();const lossCap=Math.max(100,goldForClearedFloor(save.state.player.currentFloor)),lost=Math.min(Math.floor(save.state.player.gold*.10),lossCap),defeatTutorial=claimDefeatTutorial();save.state.player.gold-=lost;save.state.player.currentFloor=save.state.player.checkpoint;save.state.player.inRun=false;abandonManualExpedition(save.state);
 syncPersistentAilments(battle);battle.party.forEach(m=>{m.currentHp=1;m.currentMp=0;m.affection=Math.max(0,(m.affection??m.bond??0)-1);m.bond=m.affection;m.history??={};m.history.defeats=(m.history.defeats??0)+1;m.history.consecutiveDeployments=0});clearExpeditionSnapshot();clearBattleCheckpoint();snapshot=null;document.querySelector(".battle-screen")?.remove();
 app.insertAdjacentHTML("beforeend",Modal("敗北",`<div class="defeat-cinematic"><div class="defeat-mark">☠</div><h2>深淵に敗れた…</h2><p><b>${lost}G</b>を失い、${save.state.player.checkpoint}Fの拠点へ帰還します。</p><small>仲間はHP1で救出され、なつき度が1下がりました。</small></div>${defeatTutorial}`,"拠点へ戻る"));
