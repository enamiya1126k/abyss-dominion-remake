...
import{aggregateAffixes}from"./data/equipmentAffixes.js?v=0.9.15-alpha.18-affix-runtime";
...
import{maxMp,learnedSkills,allLearnedSkills,equipSkill,skillById,canUseSkill,skillDamage,chooseAutoSkill,effectiveSkillMpCost,recordSkillUse}from"./battle/SkillSystem.js?v=0.9.15-alpha.18-affix-runtime";
...
function triggerEmergencyEncounter(){
 ...
 app.insertAdjacentHTML("beforeend",Modal(event.boss.faction==="tenGod"?"――神が降臨しました。":"――深淵反応を検知。",`<div class="emergency-warning ${event.boss.faction}"><div class="warning-icon">${event.boss.icon}</div><small>${event.manifestation.label} / ${event.manifestation.percent}%</small><h2>${event.boss.name}</h2><p>${event.boss.title}</p><p>味方は全回復。逃走不可。敗北ペナルティはありません。</p></div><div class="endgame-prelude-grid">${optionHtml}</div>`,`選択してください`));
 const modal=topModal(),primary=modal.querySelector("#closeGameModal");if(primary)primary.disabled=true;
...
function finishSpecialBattle(won){
 ...
 save.save();battle=null;activeEnemy=null;app.insertAdjacentHTML("beforeend",Modal(title,body,type==="team"?"拠点へ戻る":"探索へ戻る"));const modal=topModal();modal.querySelector("#closeGameModal").onclick=()=>{modal.remove();if(type==="team"){snapshot=null;go("home")}else{screen="explore";render()}}
}
...
function affixValue(monster,id){return Math.max(0,Number(monster?._equipmentAffixes?.[id]??0))}
function partyAffixValue(id,{aliveOnly=false,cap=Infinity}={}){const members=(battle?.party??save.state.party.map(mid=>save.state.monsters.find(m=>m.id===mid)).filter(Boolean)).filter(m=>!aliveOnly||m.currentHp>0);return Math.min(cap,members.reduce((sum,m)=>sum+affixValue(m,id),0))}
function outgoingAffixMultiplier(monster,enemy,{critical=false}={}){const a=monster?._equipmentAffixes??{},boss=enemy?.boss||enemy?.endgameBossId?a.bossDamage??0:a.normalDamage??0,ratio=(monster?.currentHp??1)/Math.max(1,calculatedStats(monster).hp),condition=(ratio<=.35?(a.lowHpDamage??0):0)+(ratio>=.999?(a.fullHpDamage??0):0),execution=enemy?.hp/Math.max(1,enemy?.maxHp??1)<=.25?(a.execution??0):0,crit=critical?(a.critDamage??0):0;return 1+(boss+condition+execution+crit)/100}
function healAffixMultiplier(monster){return 1+affixValue(monster,"healPower")/100}
function rollLootRarity(bonus=0){const r=Math.random(),boost=Math.min(.22,Math.max(0,bonus)/500);if(r<.01+boost*.18)return"LR";if(r<.07+boost*.48)return"SSR";if(r<.27+boost)return"SR";if(r<.82)return"R";return"N"}
...
async function command(type,skillId=null){
 ...
 if(type==="attack"){
  ...
  if(Math.random()<.06)await floatText("MISS",e.id,"miss");
   ...
   const critical=Math.random()<Math.min(.55,.08+(s.spd??0)*.005+(s.crit??0)/100);
   const base=Math.max(1,Math.floor(s.atk*(.9+Math.random()*.2)-e.def*.4));
   const raw=critical?Math.floor(base*1.7):base,beforeHp=e.hp,d=Math.max(1,Math.floor(raw*outgoingAffixMultiplier(a,e,{critical})*enemyDamageMultiplier(e)*weaponMasteryDamageMultiplier(save.state,a,e)));e.hp=Math.max(0,e.hp-d);registerWeaponFinisher(a,e,beforeHp);const steal=affixValue(a,"lifeSteal")/100;if(steal>0){const healed=Math.max(1,Math.floor(d*steal));a.currentHp=Math.min(s.hp,a.currentHp+healed);if(healed>0)await floatText(`+${healed}`,a.id,"heal")}
 ...
 if(type==="skill"&&skillId){
  ...
  skillMpCost=effectiveSkillMpCost(a,skill);a.currentMp-=skillMpCost;setSkillCooldown(battle,a.id,skill);recordSkillUse(a,skill.id,1+(affixValue(a,"skillMasteryGain")+affixValue(a,"abyssGrowth"))/100);battle.skillMenu=false;addBattleLog(battle,`${displayName(a)}：${skill.name}`);await battleBanner(skill.name,skill.description??"","skill",430);
  if(skill.type==="selfHeal"||skill.type==="stance"&&skill.heal){
   const h=Math.max(1,Math.floor(s.hp*(skill.heal??0)*healAffixMultiplier(a)));a.currentHp=Math.min(s.hp,a.currentHp+h);if(h>0)await floatText(`+${h}`,a.id,"heal");applySkillEffects(skill,a,e);
  }else if(skill.type==="allHeal"){
   const healed=[];battle.party.filter(m=>m.currentHp>0).forEach(m=>{const max=calculatedStats(m).hp,h=Math.max(1,Math.floor(max*skill.heal*healAffixMultiplier(a))),before=m.currentHp;m.currentHp=Math.min(max,m.currentHp+h);healed.push(m.currentHp-before)});
  ...
  }else if(skill.type==="buff"||skill.type==="stance"){applySkillEffects(skill,a,e);if(skill.heal){const targets=skill.target==="味方全体"?battle.party.filter(m=>m.currentHp>0):[a];targets.forEach(m=>{const mx=calculatedStats(m).hp;m.currentHp=Math.min(mx,m.currentHp+Math.floor(mx*skill.heal*healAffixMultiplier(a)))})}await floatText("EFFECT","party","guard");
  ...
  }else if(skill.type==="revive"){const target=battle.party.filter(m=>m.currentHp<=0).sort((x,y)=>calculatedStats(y).hp-calculatedStats(x).hp)[0];if(target){target.currentHp=Math.max(1,Math.floor(calculatedStats(target).hp*(skill.revive??.35)));await floatText("REVIVE",target.id,"heal")}else{a.currentMp+=skillMpCost;setSkillCooldown(battle,a.id,{...skill,cooldown:0})}
  }else{
   ...
   for(const targetEnemy of skillTargets){const e=targetEnemy;for(let i=0;i<hits&&e.hp>0;i++){
    const critical=Math.random()<Math.min(.75,.1+(skill.critBonus??0)+(s.spd??0)*.004+(s.crit??0)/100),boosted={...s,_currentHpRatio:a.currentHp/Math.max(1,s.hp),atk:s.atk*allyAttackFactor(a.id)},execute=(skill.execute&&e.hp/e.maxHp<=skill.execute)?2:1,raw=skillDamage(boosted,{...e,def:e.def*enemyDefenseFactor(e.id)},skill,critical)*execute*(1+(e.hp/e.maxHp<=.25?affixValue(a,"execution")/100:0)),beforeHp=e.hp,d=Math.max(1,Math.floor(raw*enemyDamageMultiplier(e)*weaponMasteryDamageMultiplier(save.state,a,e)));
    e.hp=Math.max(0,e.hp-d);registerWeaponFinisher(a,e,beforeHp);const steal=affixValue(a,"lifeSteal")/100;if(steal>0){const healed=Math.max(1,Math.floor(d*steal));a.currentHp=Math.min(s.hp,a.currentHp+healed);if(healed>0)await floatText(`+${healed}`,a.id,"heal")}total+=d;await animateHit(e.id,critical);if(critical){battleFlash("critical");burstParticles(e.id,"critical",14)}await floatText(`${critical?"CRITICAL ":""}-${d}`,e.id,critical?"critical":"skill")
   }
   }
   if(skill.type==="drain"||hasEffect(battle,a.id,"lifeSteal")||affixValue(a,"lifeSteal")>0){const rate=(skill.drain??0)+effectValue(battle,a.id,"lifeSteal")+affixValue(a,"lifeSteal")/100,h=Math.max(1,Math.floor(total*rate));a.currentHp=Math.min(s.hp,a.currentHp+h);await floatText(`+${h}`,a.id,"heal")}
   if(skill.status&&e.hp>0&&Math.random()<Math.min(.98,skill.status.chance*(1+affixValue(a,"statusChance")/100))){applyEnemyStatus(battle,{...skill.status,power:(skill.status.power??0)*(1+affixValue(a,"dotDamage")/100),sourceMonsterId:a.id},e.id);addBattleLog(battle,`${e.name}は${skill.status.name}状態になった`);await floatText(skill.status.name,e.id,skill.status.id)}applySkillEffects(skill,a,e)
  }
 }
 ...
 if(type==="capture"){
  ...
  const captureBonus=Math.min(.25,partyAffixValue("captureRate",{aliveOnly:true,cap:25})/100),chance=e.boss?Math.max(.01,Math.min(.20,(.01+(1-e.hp/e.maxHp)*.04)*currentDanger().bossCapture+captureBonus*.35)):Math.max(.08,Math.min(.95,.2+(1-e.hp/e.maxHp)*.55+(Math.max(...battle.party.map(m=>m.level+m.stars*2+m.plus))-e.level)*.012+captureBonus));
 ...
 if(skillId&&Math.random()<Math.min(.60,affixValue(a,"arcaneEcho")/100)){a.currentMp=Math.min(maxMp(a),a.currentMp+skillMpCost);addBattleLog(battle,`${displayName(a)}のMPが還元された`);await floatText(`MP +${skillMpCost}`,a.id,"heal")}
...
async function dealEnemyHit(e,target,multiplier=1,label="",criticalChance=.08){
 ...
 if(Math.random()<Math.min(.60,(st.evasion??0)/100)){addBattleLog(battle,`${displayName(target)}は攻撃を回避した`);await floatText("MISS",target.id,"miss");return 0}
 const guardFx=effectValue(battle,target.id,"guard"),vulnerable=effectValue(battle,target.id,"vulnerable"),reduction=Math.min(.60,affixValue(target,"damageReduction")/100),guardFactor=guard?Math.max(.15,.45*(1-affixValue(target,"guardPower")/100)):1;let d=Math.max(1,Math.floor((e.atk*enemyAttackFactor(e.id)-st.def*allyDefenseFactor(target.id)*.45)*multiplier*guardFactor*(1-guardFx)*(1+vulnerable)*(1-reduction)));if(critical)d=Math.floor(d*1.55);
 const beforeHp=target.currentHp,fatal=beforeHp-d<=0&&Math.random()<Math.min(.60,affixValue(target,"unyielding")/100);target.currentHp=fatal?1:Math.max(0,beforeHp-d);if(fatal){d=Math.max(0,beforeHp-1);addBattleLog(battle,`${displayName(target)}は致死攻撃を耐えた`)}addBattleLog(battle,`${displayName(target)}に${d}ダメージ`);
 ...
 if(target.currentHp<=0)await animateDefeat(target.id);else if(hasEffect(battle,target.id,"counter")){const cs=calculatedStats(target),counter=Math.max(1,Math.floor((cs.atk*effectValue(battle,target.id,"counter")-e.def*.25)*(1+affixValue(target,"counterDamage")/100)));e.hp=Math.max(0,e.hp-counter);addBattleLog(battle,`${displayName(target)}が${counter}反撃ダメージ`);await floatText(`COUNTER -${counter}`,e.id,"skill")}return d;
}
...
async function endRound(){
 ...
 const allyResults=processAllyEffects(battle,calculatedStats);for(const m of battle.party.filter(x=>x.currentHp>0)){const regen=affixValue(m,"regen");if(regen>0){const max=calculatedStats(m).hp,amount=Math.max(1,Math.floor(max*regen/100)),before=m.currentHp;m.currentHp=Math.min(max,m.currentHp+amount);if(m.currentHp>before){addBattleLog(battle,`${displayName(m)}は装備効果で${m.currentHp-before}回復`);await floatText(`+${m.currentHp-before}`,m.id,"heal")}}}
 for(const result of allyResults){addBattleLog(battle,`${displayName(result.monster)} ${result.kind==="heal"?"回復":"継続ダメージ"} ${result.amount}`);await floatText(`${result.kind==="heal"?"+":"-"}${result.amount}`,result.monster.id,result.kind==="heal"?"heal":result.kind)}
...
function win(caught,m){
 ...
 const rewardMult=eliteDefeated.length?1.65:1,goldBonus=partyAffixValue("goldGain",{aliveOnly:true,cap:200});const gold=Math.round(defeated.reduce((sum,e)=>sum+(e.boss?(firstBoss?80+e.level*14:28+e.level*7):16+e.level*5),0)*rewardMult*(1+goldBonus/100));
 ...
 const progress=battle.party.map(monster=>{
  ...
  const basePersonalGain=alive?share+(remainder-->0?1:0):0,personalExpBonus=Math.min(250,affixValue(monster,"expGain")+affixValue(monster,"abyssGrowth"));const gain=Math.round(basePersonalGain*(1+personalExpBonus/100));
  monster.exp+=gain;
  let levels=0;
 ...
 const geared=defeated.find(e=>e.gear),dropBonus=partyAffixValue("dropRate",{aliveOnly:true,cap:100}),rareBonus=partyAffixValue("treasureSense",{aliveOnly:true,cap:100});
 if(geared&&Math.random()<Math.min(.75,.18*(1+dropBonus/100))){drop={...geared.gear,id:crypto.randomUUID?.()??`${Date.now()}-${Math.random()}`,equippedBy:null,createdAt:new Date().toISOString()};dropReceipt=equipmentReceipt(drop)}else if(Math.random()<Math.min(.65,.12*(1+dropBonus/100))){drop=createEquipment(["weapon","armor","accessory"][Math.floor(Math.random()*3)],{rarity:rollLootRarity(rareBonus)});dropReceipt=equipmentReceipt(drop)}
