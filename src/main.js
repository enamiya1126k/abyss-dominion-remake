import{SaveService}from"./services/SaveService.js?v=0.7.0-alpha.1";
...
import{receiveEquipment,takeFromStorage,equipmentSellPrice,slotLabel}from"./services/EquipmentStorage.js?v=0.7.0-alpha.1";
import{RARITY_ORDER,equipmentStatLabel}from"./data/equipment.js";
import{EquipmentScreen}from"./ui/screens/EquipmentScreen.js?v=0.7.0-alpha.1";
...
function normalizeEquipmentState(){
 ...
 save.state.gacha??={firstTenUsed:false,lastDailyKey:null};
 save.state.rest??={lastFreeKey:null};
 save.state.monsters.forEach(m=>{
  ...
  const counts={},stats={};
  Object.values(m.equipment??{}).forEach(id=>{const item=save.state.equipment.find(i=>i.id===id);if(!item)return;const mult=1+(item.plus??0)*.08;Object.entries(item.stats??{}).forEach(([k,v])=>stats[k]=(stats[k]??0)+Math.round(v*mult));if(item.series)counts[item.series]=(counts[item.series]??0)+1});
  m._equipmentStats=stats;m._seriesCounts=counts
 });
}
...
function togglePartyMember(id){
 ...
 if(has){
  ...
  if(m){m.equipment={weapon:null,armor:null,accessory:null};preserveVitals(m,beforeStats,beforeMp)}
  save.state.party=save.state.party.filter(x=>x!==id)
 }else save.state.party.push(id);
 save.save()
}
function openHomePartyEditor(){
 const body=`<p class="muted">タップで出撃／控えを切り替え。出撃は1〜4体。</p><div class="party-editor">${save.state.monsters.map(m=>`<button data-home-party-toggle="${m.id}" class="${save.state.party.includes(m.id)?"selected":""}"><span>${SPECIES[m.speciesId].emoji}</span><b>${displayName(m)}</b><small>${save.state.party.includes(m.id)?"⚔️ 出撃":"控え"} Lv.${m.level}</small></button>`).join("")}</div>`;
 ...
 document.getElementById("closeGameModal").onclick=()=>{document.querySelector(".game-modal").remove();render()}
}
function equipItem(itemId,monsterId){
 ...
 const beforeStats=monster?calculatedStats(monster):null,beforeMp=monster?maxMp(monster):1;
 ...
 monster.equipment[item.slot]=item.id;item.equippedBy=monster.id;preserveVitals(monster,beforeStats,beforeMp);save.save();render();
}
function autoEquipMonster(monsterId){const monster=save.state.monsters.find(m=>m.id===monsterId);if(!monster||!save.state.party.includes(monsterId))return;const beforeStats=calculatedStats(monster),beforeMp=maxMp(monster);for(const slot of["weapon","armor","accessory"]){const current=save.state.equipment.find(i=>i.id===monster.equipment?.[slot]),candidates=save.state.equipment.filter(i=>i.slot===slot&&(!i.equippedBy||i.equippedBy===monsterId)),best=candidates.sort((a,b)=>equipmentPower(b)-equipmentPower(a))[0];if(!best)continue;if(current&&current.id!==best.id)current.equippedBy=null;monster.equipment[slot]=best.id;best.equippedBy=monsterId}preserveVitals(monster,beforeStats,beforeMp)}
function unequipItem(itemId){
 const item=save.state.equipment.find(i=>i.id===itemId);if(!item?.equippedBy)return;
 const monsterBefore=save.state.monsters.find(m=>m.id===item.equippedBy),beforeStats=monsterBefore?calculatedStats(monsterBefore):null,beforeMp=monsterBefore?maxMp(monsterBefore):1;
 const monster=save.state.monsters.find(m=>m.id===item.equippedBy);
 if(monster)monster.equipment[item.slot]=null;
 item.equippedBy=null;if(monster)preserveVitals(monster,beforeStats,beforeMp);save.save();render();
}
...
function openRest(){const key=localDayKey(),free=save.state.rest.lastFreeKey!==key,cost=50+save.state.player.maxFloor*5;app.insertAdjacentHTML("beforeend",Modal("🛏️ 深淵の休息",`<p>出撃メンバーのHP・MP・状態異常を完全回復します。</p><div class="rest-price"><b>${free?"本日1回 無料":`${cost}G`}</b><small>所持 ${save.state.player.gold}G</small></div>`,free?"無料で休む":`${cost}Gで休む`));document.getElementById("closeGameModal").onclick=()=>{if(!free&&save.state.player.gold<cost)return alert("GOLDが足りない");if(free)save.state.rest.lastFreeKey=key;else save.state.player.gold-=cost;healParty();save.save();document.querySelector(".game-modal").remove();render()}}
...
function openFieldItems(){
 ...
 const body=`<div class="modal-party-vitals">${targets.map(m=>{const s=calculatedStats(m);return`<button data-modal-monster-detail="${m.id}"><b>${displayName(m)} Lv.${m.level}</b><small>HP ${m.currentHp??s.hp}/${s.hp}　MP ${m.currentMp??maxMp(m)}/${maxMp(m)}</small></button>`}).join("")}</div><div class="field-item-target"><b>対象</b><select id="fieldItemTarget">${targets.map(m=>`<option value="${m.id}">${displayName(m)} HP ${m.currentHp??calculatedStats(m).hp}/${calculatedStats(m).hp}</option>`).join("")}</select></div><div class="field-item-grid">${items.filter(([id])=>itemCount(id)>0).map(([id,icon,name,desc])=>`<button data-field-item="${id}"><span>${icon}</span><b>${name}</b><small>${desc}</small><em>×${itemCount(id)}</em></button>`).join("")||'<div class="empty">使用できるアイテムを持っていません</div>'}</div>`;
 app.insertAdjacentHTML("beforeend",Modal("持ち物",body,"閉じる"));
 document.querySelectorAll("[data-modal-monster-detail]").forEach(b=>b.onclick=()=>exploreMonsterDetail(b.dataset.modalMonsterDetail));document.querySelectorAll("[data-field-item]").forEach(b=>b.onclick=()=>useFieldItem(b.dataset.fieldItem,document.getElementById("fieldItemTarget").value));
...
function renderBattle(){
 ...
 document.querySelectorAll("[data-enemy-target]").forEach(b=>b.onclick=()=>{if(battle.busy)return;battle.targetEnemyId=b.dataset.enemyTarget;renderBattle()});
...
function win(caught,m){
 ...
 clearPartySynergy();save.save();
 activeEnemy=null;
 document.querySelector(".battle-screen")?.remove();

 const result=`<div class="victory-title">VICTORY</div><div class="reward-summary"><b>+${gold}G</b><small>総EXP ${totalExp} / 生存 ${survivors.length}体で分配${firstBoss?"・初回ボス撃破ボーナス":""}</small>${drop?`<b>[${drop.rarity}] ${drop.name}（${slotLabel(drop.slot)}）</b><small>${dropReceipt.message}</small>`:""}${caught?`<b>${m.nickname}を捕獲！</b>`:""}</div><div class="exp-results compact">${progress.map(p=>{const hpMax=p.afterStats.hp,mpMax=maxMp(p.x),remaining=Math.max(0,p.need-p.x.exp),diff=k=>p.afterStats[k]-(p.before.stats[k]??0);return`<div class="${p.alive?"":"exp-defeated"} ${p.levels?"level-up-card":""}"><span>${SPECIES[p.x.speciesId].emoji}</span><section><b>${displayName(p.x)} ${p.levels?`Lv.${p.before.level} → Lv.${p.x.level} <em>LEVEL UP!</em>`:`Lv.${p.x.level}`}</b><div class="result-vitals"><small>HP ${p.x.currentHp}/${hpMax}</small><small>MP ${p.x.currentMp}/${mpMax}</small><small>${p.alive?`次まであと${remaining}EXP`:"戦闘不能：EXP 0"}</small></div><i class="result-exp"><u style="width:${Math.min(100,p.x.exp/p.need*100)}%"></u></i>${p.levels?`<div class="level-gains"><span>HP ${p.before.stats.hp} → ${p.afterStats.hp} <strong>+${diff("hp")}</strong></span><span>ATK ${p.before.stats.atk} → ${p.afterStats.atk} <strong>+${diff("atk")}</strong></span><span>DEF ${p.before.stats.def} → ${p.afterStats.def} <strong>+${diff("def")}</strong></span><span>SPD ${p.before.stats.spd} → ${p.afterStats.spd} <strong>+${diff("spd")}</strong></span></div>`:""}</section></div>`}).join("")}</div>`;
