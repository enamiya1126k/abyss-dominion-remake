...
import{dangerConfig}from"./core/DangerSystem.js";
import{biomeForFloor,biomeProgress}from"./data/biomes.js";
...
let partyEditorState={query:"",element:"all",sort:"rarity",direction:"desc"};
const PARTY_RARITY={N:0,R:1,SR:2,SSR:3,UR:4,LR:5,"神話":6,"深淵":7,"十神":8};
const ELEMENT_LABEL={all:"全属性",neutral:"無",fire:"火",water:"水",earth:"土",wind:"風",light:"光",dark:"闇",poison:"毒",nature:"自然"};
function openHomePartyEditor(){
 const current=save.state.party.map(id=>save.state.monsters.find(m=>m.id===id)).filter(Boolean);
 const elements=["all",...new Set(save.state.monsters.map(m=>SPECIES[m.speciesId]?.element??"neutral"))];
 const controls=`<div class="party-current"><b>現在の編成 ${current.length}/4</b><div>${[0,1,2,3].map(i=>{const m=current[i];return m?`<button data-home-party-toggle="${m.id}" title="タップで外す"><span>${SPECIES[m.speciesId].emoji}</span><small>${i+1}. ${displayName(m)}</small></button>`:`<i><span>＋</span><small>${i+1}. 空き</small></i>`}).join("")}</div></div><div class="party-tools"><input id="partySearch" placeholder="名前で検索" value="${partyEditorState.query}"><select id="partyElement">${elements.map(e=>`<option value="${e}" ${partyEditorState.element===e?"selected":""}>${ELEMENT_LABEL[e]??e}</option>`).join("")}</select><select id="partySort"><option value="rarity">レア度</option><option value="level">レベル</option><option value="stars">★強化</option><option value="bond">なつき度</option><option value="hp">HP</option><option value="atk">ATK</option><option value="def">DEF</option><option value="spd">SPD</option><option value="newest">入手順</option><option value="name">名前</option></select><button type="button" id="partyDirection">${partyEditorState.direction==="desc"?"高い順 ↓":"低い順 ↑"}</button></div><div id="partyRoster" class="party-roster"></div>`;
 app.insertAdjacentHTML("beforeend",Modal("パーティー編成",controls,"確定"));
 const modal=topModal();modal.querySelector("#partySort").value=partyEditorState.sort;
 const renderRoster=()=>{const q=partyEditorState.query.trim().toLowerCase();const sorted=save.state.monsters.filter(m=>{const sp=SPECIES[m.speciesId];return(!q||displayName(m).toLowerCase().includes(q)||sp.name.toLowerCase().includes(q))&&(partyEditorState.element==="all"||(sp.element??"neutral")===partyEditorState.element)}).sort((a,b)=>{const sa=calculatedStats(a),sb=calculatedStats(b),spa=SPECIES[a.speciesId],spb=SPECIES[b.speciesId];const val=m=>({rarity:PARTY_RARITY[SPECIES[m.speciesId].rarity]??0,level:m.level,stars:m.stars??1,bond:m.bond??m.affection??0,hp:calculatedStats(m).hp,atk:calculatedStats(m).atk,def:calculatedStats(m).def,spd:calculatedStats(m).spd,newest:Number(new Date(m.createdAt??0))||0,name:displayName(m)}[partyEditorState.sort]);const av=val(a),bv=val(b);const cmp=typeof av==="string"?av.localeCompare(bv,"ja"):av-bv;return partyEditorState.direction==="desc"?-cmp:cmp});modal.querySelector("#partyRoster").innerHTML=sorted.map(m=>{const sp=SPECIES[m.speciesId],st=calculatedStats(m),active=save.state.party.includes(m.id),bond=m.bond??m.affection??0;return`<article class="party-roster-card ${active?"selected":""}"><button class="party-pick" data-home-party-toggle="${m.id}"><span class="party-emoji">${sp.emoji}</span><section><b>${active?"🟢 出撃 ":""}${displayName(m)}</b><small>${sp.rarity} / ${ELEMENT_LABEL[sp.element]??sp.element} / Lv.${m.level} / +${m.plus??0}</small><small>★${m.stars??1}　なつき ${bond}</small><em>HP ${st.hp}　ATK ${st.atk}　DEF ${st.def}　SPD ${st.spd}</em></section></button><button class="party-detail" data-party-detail="${m.id}">詳細</button></article>`}).join("")||'<p class="empty">条件に合うモンスターがいません</p>';modal.querySelectorAll("[data-home-party-toggle]").forEach(b=>b.onclick=()=>{togglePartyMember(b.dataset.homePartyToggle);renderRoster()});modal.querySelectorAll("[data-party-detail]").forEach(b=>b.onclick=()=>{selected=b.dataset.partyDetail;modal.remove();go("detail")})};
 modal.querySelector("#partySearch").oninput=e=>{partyEditorState.query=e.target.value;renderRoster()};modal.querySelector("#partyElement").onchange=e=>{partyEditorState.element=e.target.value;renderRoster()};modal.querySelector("#partySort").onchange=e=>{partyEditorState.sort=e.target.value;renderRoster()};modal.querySelector("#partyDirection").onclick=e=>{partyEditorState.direction=partyEditorState.direction==="desc"?"asc":"desc";e.currentTarget.textContent=partyEditorState.direction==="desc"?"高い順 ↓":"低い順 ↑";renderRoster()};
 renderRoster();topModalButton().onclick=()=>{modal.remove();render()}
}
...
function summonOne({mode="mixed",guaranteedMonster=false,guaranteedEquipment=false,guaranteedRare=false}={}){const isMonster=guaranteedMonster||(!guaranteedEquipment&&(mode==="monster"||Math.random()<.30)),rarity=rarityRoll(guaranteedRare?"guaranteed":"normal");if(isMonster){const pool=Object.values(SPECIES).filter(s=>s.rarity!=="LR"&&!(s.race==="dragon"&&rarity!=="SSR"&&rarity!=="LR")),speciesId=pool[Math.floor(Math.random()*pool.length)].id,stars={N:1,R:1,SR:2,SSR:3,LR:4}[rarity],m=createMonster(speciesId,{stars,nickname:SPECIES[speciesId].name});m.summonRarity=rarity;save.state.monsters.push(m);save.state.codex.captures[speciesId]=(save.state.codex.captures[speciesId]??0)+1;save.state.codex.encounters[speciesId]=(save.state.codex.encounters[speciesId]??0)+1;return{type:"monster",rarity,name:displayName(m),icon:SPECIES[speciesId].emoji,item:m}}const slot=["weapon","armor","accessory"][Math.floor(Math.random()*3)],item=createEquipment(slot,{rarity});receiveEquipment(save.state,item);save.state.codex.equipment[item.name]=(save.state.codex.equipment[item.name]??0)+1;return{type:"equipment",rarity,name:item.name,icon:{weapon:"⚔️",armor:"🛡️",accessory:"💍"}[slot],item}}
function openRarityGuide(){const rows=[["N","ノーマル"],["R","レア"],["SR","スーパーレア"],["SSR","スペシャルレア"],["UR","ウルトラレア"],["LR","レジェンドレア"],["神話","極めて希少"],["深淵","1000階以降の深淵召喚で解放"],["十神","ガチャ排出なし"]];app.insertAdjacentHTML("beforeend",Modal("レア度一覧",`<div class="rarity-guide">${rows.map(([r,d],i)=>`<div><b>${r}</b><span>${d}</span>${i<rows.length-1?"<i>↓</i>":""}</div>`).join("")}</div>`,"閉じる"));topModalButton().onclick=closeTopModal}
function openGacha(){const key=localDayKey(),daily=save.state.gacha.lastDailyKey!==key,first=!save.state.gacha.firstTenUsed;const abyssUnlocked=save.state.player.maxFloor>=1000;const body=`<div class="gacha-head"><b>魔晶石を捧げて召喚</b><span>💎 ${save.state.player.crystals}</span><button type="button" id="rarityGuide">？</button></div><div class="gacha-menu">${first?'<button data-gacha="first"><b>初回限定 無料10連</b><small>SR以上モンスター1体確定</small></button>':''}<button data-gacha="daily" ${daily?'':'disabled'}><b>1日1回 無料召喚</b><small>${daily?'モンスター・装備から抽選':'本日分は召喚済み'}</small></button><button data-gacha="monster-single"><b>🧌 モンスター召喚　💎5</b><small>モンスター100%</small></button><button data-gacha="monster-ten"><b>🧌 モンスター10連　💎45</b><small>最後の1体はSR以上</small></button><button data-gacha="equipment-single"><b>⚔️ 装備召喚　💎5</b><small>武器・防具・装飾品100%</small></button><button data-gacha="equipment-ten"><b>⚔️ 装備10連　💎45</b><small>最後の1枠はSR以上</small></button>${abyssUnlocked?'<button disabled><b>🌌 深淵召喚</b><small>深淵領域の専用召喚（準備中）</small></button>':''}</div>`;app.insertAdjacentHTML("beforeend",Modal("🔮 召喚の祭壇",body,"閉じる"));document.querySelectorAll("[data-gacha]").forEach(b=>b.onclick=()=>performGacha(b.dataset.gacha));document.getElementById("rarityGuide").onclick=openRarityGuide;topModalButton().onclick=closeTopModal}
function performGacha(type){let count=1,cost=0,mode="mixed";if(type==="first"){if(save.state.gacha.firstTenUsed)return;count=10;mode="monster";save.state.gacha.firstTenUsed=true}else if(type==="daily"){const key=localDayKey();if(save.state.gacha.lastDailyKey===key)return;save.state.gacha.lastDailyKey=key}else{mode=type.startsWith("monster")?"monster":"equipment";if(type.endsWith("single"))cost=5;else{cost=45;count=10}}if(save.state.player.crystals<cost)return alert("魔晶石が足りない");save.state.player.crystals-=cost;const results=[];for(let i=0;i<count;i++){const last=i===count-1;results.push(summonOne({mode,guaranteedMonster:mode==="monster",guaranteedEquipment:mode==="equipment",guaranteedRare:count===10&&last}))}save.save();document.querySelector(".game-modal")?.remove();app.insertAdjacentHTML("beforeend",Modal("召喚結果",`<div class="gacha-results">${results.map(r=>`<div class="rarity-${r.rarity}"><span>${r.icon}</span><b>[${r.rarity}] ${r.name}</b><small>${r.type==="monster"?"モンスター":"装備"} NEW</small></div>`).join("")}</div><p class="muted">残り 💎${save.state.player.crystals}</p>`,"ホームへ"));topModalButton().onclick=()=>{document.querySelector(".game-modal").remove();render()}}
function openCodex(type){if(type==="monster"){const owned=new Set(save.state.monsters.map(m=>m.speciesId)),rows=Object.values(SPECIES).sort((a,b)=>(a.minFloor??0)-(b.minFloor??0)).map((sp,i)=>{const seen=(save.state.codex.encounters[sp.id]??0)>0||owned.has(sp.id),captured=save.state.codex.captures[sp.id]??save.state.monsters.filter(m=>m.speciesId===sp.id).length;return`<div class="codex-row ${seen?'':'unknown'}"><span>${seen?sp.emoji:'❔'}</span><b>No.${String(i+1).padStart(3,'0')} ${seen?sp.name:'？？？？？'}</b><small>${seen?`${sp.rarity} / ${sp.element} / 遭遇 ${save.state.codex.encounters[sp.id]??0} / 捕獲 ${captured}`:'未遭遇'}</small></div>`}).join('');app.insertAdjacentHTML("beforeend",Modal("📖 モンスター図鑑",`<div class="codex-summary">発見 ${Object.values(SPECIES).filter(sp=>(save.state.codex.encounters[sp.id]??0)>0||owned.has(sp.id)).length} / ${Object.keys(SPECIES).length}</div><div class="codex-list">${rows}</div>`,"閉じる"))}else{const all=[...save.state.equipment,...save.state.reserveEquipment,...save.state.bossEquipmentVault],names=[...new Set(all.map(i=>i.name))],rows=names.length?names.map(name=>{const items=all.filter(i=>i.name===name),best=items.sort((a,b)=>(RARITY_ORDER[b.rarity]??0)-(RARITY_ORDER[a.rarity]??0))[0];return`<div class="codex-row"><span>${best.slot==="weapon"?'⚔️':best.slot==="armor"?'🛡️':'💍'}</span><b>[${best.rarity}] ${name}</b><small>${slotLabel(best.slot)} / 所持 ${items.length}</small></div>`}).join(''):'<div class="empty">まだ装備を発見していません</div>';app.insertAdjacentHTML("beforeend",Modal("🗡️ 装備図鑑",`<div class="codex-summary">発見 ${names.length}種</div><div class="codex-list">${rows}</div>`,"閉じる"))}topModalButton().onclick=closeTopModal}
...
function bindExplore(){
 ...
 game.input=createInputState();game.player??=new Entity(game.world.start.x,game.world.start.y);game.trail=snapshot?.trail??game.trail??[];game.world.encountering=false;game.player.path=[];game.player.p=0;
 ...
 game.player.rx=game.player.x;game.player.ry=game.player.y;if(!game.trail.length)game.trail=Array.from({length:4},()=>({x:game.player.x,y:game.player.y}));save.state.player.visitedFloors??=[];if(!save.state.player.visitedFloors.includes(save.state.player.currentFloor)){save.state.player.visitedFloors.push(save.state.player.currentFloor);save.save()}game.camera=new Camera(canvas);
...
function update(dt){
 ...
 if(game.player.move(dt,7.5)){
  game.trail.unshift(previous);game.trail=game.trail.slice(0,4);
  game.world.steps++;
...
function draw(){const c=game.ctx,w=game.world;c.fillStyle="#120c18";c.fillRect(0,0,game.canvas.width,game.canvas.height);for(let y=0;y<w.rows;y++)for(let x=0;x<w.cols;x++){const p=game.camera.world(x*TILE,y*TILE),s=TILE*game.camera.z;c.fillStyle=w.tiles[y][x]?"#21182a":"#6a4a7f";c.fillRect(p.x,p.y,s+1,s+1)}emoji(w.exit,"🕳️");if(w.shop)emoji(w.shop,"🚪");if(w.boss)emoji(w.boss,"👹",true);w.chests.forEach(x=>!x.open&&emoji(x,x.emoji,x.kind==="radiant"));const party=save.state.party.map(id=>save.state.monsters.find(m=>m.id===id)).filter(Boolean);for(let i=party.length-1;i>=0;i--){const pos=i===0?{x:game.player.rx,y:game.player.ry}:(game.trail[i-1]??{x:game.player.rx,y:game.player.ry});emoji(pos,SPECIES[party[i].speciesId].emoji,i===0)}drawMini()}
...
function makeBattleEnemy(e,index=0){const sp=SPECIES[e.speciesId],danger={stats:1},scaled={...e,level:Math.max(1,e.level??1)},enemy=createEnemyBattleState(sp,scaled,save.state.player.currentFloor);enemy.dangerLevel=e.boss?5:e.speciesId==="mimic"?3:e.equipped?3:((e.level??1)>save.state.player.currentFloor+4?2:1);enemy.id=`enemy-${Date.now()}-${index}-${Math.random().toString(36).slice(2,7)}`;enemy.maxHp=Math.max(1,Math.round(enemy.maxHp*danger.stats));enemy.hp=enemy.maxHp;enemy.atk=Math.max(1,Math.round(enemy.atk*danger.stats));enemy.def=Math.max(0,Math.round(enemy.def*danger.stats));enemy.spd=Math.max(1,Math.round(enemy.spd*(1+(danger.stats-1)*.35)));if(e.equipped&&e.gear){enemy.gear=e.gear;enemy.name=`⚔️ ${enemy.name}`;enemy.atk+=e.gear.stats.atk??0;enemy.def+=e.gear.stats.def??0;enemy.spd+=e.gear.stats.spd??0;enemy.maxHp+=e.gear.stats.hp??0;enemy.hp=enemy.maxHp}return enemy}
...
function win(caught,m){
 ...
 const rewardMult=1;const gold=Math.round(defeated.reduce((sum,e)=>sum+(e.boss?(firstBoss?80+e.level*14:28+e.level*7):16+e.level*5),0)*rewardMult);
 ...
 const keyDrop=defeated.some(e=>!e.boss&&Math.random()<.002*currentDanger().keyRate)||(firstBoss&&floor%50===0);
 ...
 const result=`<div class="victory-title">VICTORY</div><div class="reward-summary"><b>+${gold}G</b>${crystalRoll?`<b>💎 +${crystalRoll}</b>`:""}<small>総EXP ${totalExp} / 生存 ${survivors.length}体で分配${firstBoss?"・初回ボス撃破ボーナス":""}</small>${drop?`<b>[${drop.rarity}] ${drop.name}（${slotLabel(drop.slot)}）</b><small>${dropReceipt.message}</small>`:""}${keyDrop?`<b>🗝️ 深淵の鍵を獲得</b>`:""}${caught?`<b>${m.nickname}を捕獲！</b>`:""}</div><div class="exp-results compact">${progress.map(p=>{const hpMax=p.afterStats.hp,mpMax=maxMp(p.x),remaining=Math.max(0,p.need-p.x.exp),diff=k=>p.afterStats[k]-(p.before.stats[k]??0);return`<div class="${p.alive?"":"exp-defeated"} ${p.levels?"level-up-card":""}"><span>${SPECIES[p.x.speciesId].emoji}</span><section><b>${displayName(p.x)} ${p.levels?`Lv.${p.before.level} → Lv.${p.x.level} <em>LEVEL UP!</em>`:`Lv.${p.x.level}`}</b><div class="result-vitals"><small>HP ${p.x.currentHp}/${hpMax}</small><small>MP ${p.x.currentMp}/${mpMax}</small><small>${p.alive?`次まであと${remaining}EXP`:"戦闘不能：EXP 0"}</small></div><i class="result-exp"><u style="width:${Math.min(100,p.x.exp/p.need*100)}%"></u></i>${p.levels?`<div class="level-gains"><span>HP ${p.before.stats.hp} → ${p.afterStats.hp} <strong>+${diff("hp")}</strong></span><span>ATK ${p.before.stats.atk} → ${p.afterStats.atk} <strong>+${diff("atk")}</strong></span><span>DEF ${p.before.stats.def} → ${p.afterStats.def} <strong>+${diff("def")}</strong></span><span>SPD ${p.before.stats.spd} → ${p.afterStats.spd} <strong>+${diff("spd")}</strong></span></div>`:""}</section></div>`}).join("")}</div>`;
 ...
 const resultModal=topModal(),restoreExplore=()=>{if(resultModal.dataset.closed)return;resultModal.dataset.closed="1";resultModal.remove();screen="explore";render()};
 resultModal.querySelector("#closeGameModal").onclick=restoreExplore;resultModal.querySelector(".modal-x").onclick=restoreExplore;
}
