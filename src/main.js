import{SaveService}from"./services/SaveService.js?v=0.9.3-alpha.1";
import{SPECIES}from"./data/species.js?v=0.9.3-alpha.1";
import{HomeScreen}from"./ui/screens/HomeScreen.js?v=0.9.3-alpha.1";
import{MonsterListScreen}from"./ui/screens/MonsterListScreen.js?v=0.9.3-alpha.1";
import{MonsterDetailScreen}from"./ui/screens/MonsterDetailScreen.js?v=0.9.3-alpha.1";
import{SettingsScreen}from"./ui/screens/SettingsScreen.js?v=0.9.3-alpha.1";
import{ExploreScreen}from"./ui/screens/ExploreScreen.js?v=0.9.3-alpha.1";
import{BattleScreen}from"./ui/screens/BattleScreen.js?v=0.9.3-alpha.1";
import{Modal}from"./ui/components/Modal.js?v=0.9.3-alpha.1";
import{createMonster,displayName,calculatedStats,TRAITS,expNeedFor}from"./models/Monster.js?v=0.9.3-alpha.1";
import{createEquipment,equipmentPower}from"./models/Equipment.js?v=0.9.3-alpha.1";
import{receiveEquipment,takeFromStorage,equipmentSellPrice,slotLabel}from"./services/EquipmentStorage.js?v=0.9.3-alpha.1";
import{RARITY_ORDER,equipmentStatLabel}from"./data/equipment.js?v=0.9.3-alpha.1";
import{EquipmentScreen}from"./ui/screens/EquipmentScreen.js?v=0.9.3-alpha.1";
import{ShopScreen}from"./ui/screens/ShopScreen.js?v=0.9.3-alpha.1";
import{maxMp,learnedSkills,skillById,canUseSkill,skillDamage}from"./battle/SkillSystem.js?v=0.9.3-alpha.1";
import{ENEMY_ACTIONS,createEnemyBattleState,chooseEnemyAction,enemyDamageMultiplier,enemyHealAmount,enemyAttackMultiplier}from"./battle/EnemyAI.js?v=0.9.3-alpha.1";
import{createBattleRulesState,cooldownRemaining,setSkillCooldown,tickCooldowns,addBattleLog,applyEnemyStatus,processEnemyStatuses}from"./battle/BattleRules.js?v=0.9.3-alpha.1";
import{buildTurnQueue,currentTurnEntry,currentAlly,currentEnemy,aliveEnemies,selectedEnemy,advanceQueue,queueFinished,skipInvalidEntries}from"./battle/TurnSystem.js?v=0.9.3-alpha.1";
import{dangerConfig}from"./core/DangerSystem.js?v=0.9.3-alpha.1";
import{biomeForFloor,biomeProgress,recordBiomeFloor,recordBiomeEncounter,recordBiomeChest,recordBiomeBoss}from"./data/biomes.js?v=0.9.3-alpha.1";
...
function openPartyMonsterDetail(id){const m=save.state.monsters.find(x=>x.id===id);if(!m)return;const sp=SPECIES[m.speciesId],st=calculatedStats(m);app.insertAdjacentHTML("beforeend",Modal(`${sp.emoji} ${displayName(m)}`,`<div class="codex-detail"><p><b>${monsterVisibleRarity(m)} / ${elementLabel(sp.element)} / ${sp.role??"不明"}</b></p><div class="detail-stat-grid"><span>Lv.${m.level}</span><span>★${m.stars??1}</span><span>+${m.plus??0}</span><span>なつき ${m.affection??0}</span><span>HP ${st.hp}</span><span>ATK ${st.atk}</span><span>DEF ${st.def}</span><span>SPD ${st.spd}</span></div><p class="muted">種族 ${sp.race??"不明"}<br>特性 ${m.traitId??"なし"}</p></div>`,"戻る"));topModalButton().onclick=closeTopModal}
