import{SaveService}from"./services/SaveService.js?v=2.10.0";
...
import{OnlinePartyScreen}from"./ui/screens/OnlinePartyScreen.js?v=2.10.0-build148";
import{OnlinePartyController}from"./online/OnlinePartyClient.js?v=2.10.0-build148";
...
import{attributeVisual}from"./ui/components/AttributeVisual.js?v=2.10.0";
...
import{maxMp,learnedSkills,allLearnedSkills,equipSkill,skillById,skillElementLabel,canUseSkill,effectiveSkillMpCost,skillDamage,affixOutgoingDamageMultiplier,chooseAutoSkill,skillProgressFor,recordSkillUse,skillEffectDetails,skillEffectSummary}from"./battle/SkillSystem.js?v=2.10.0";
...
import{monsterSpriteUrl,monsterVisual,setMonsterVisualFrame}from"./ui/MonsterVisual.js?v=2.10.0";
...
function claimOnlinePartyReward({rewardId,reward={},source={}}={}){
 const id=String(rewardId??"").slice(0,160);if(!id)return{ok:false};const online=save.state.onlineParty??={claimedRewards:[],totalGold:0,totalCaptureCrystals:0,expeditionsCompleted:0,battlesWon:0,captures:0};online.claimedRewards=Array.isArray(online.claimedRewards)?online.claimedRewards:[];if(online.claimedRewards.includes(id))return{ok:true,duplicate:true};
 const gold=Math.max(0,Math.min(5_000_000,Math.floor(Number(reward.gold)||0))),captureCrystals=Math.max(0,Math.min(3,Math.floor(Number(reward.captureCrystals)||0))),battleCapture=source.kind==="battleCapture"&&Boolean(reward.captureAttempted),captureCost=battleCapture?1:0;let captureSuccess=false,captureStorageFull=false,captureNoCrystal=false,captureName="";
