...
import{normalizeSeriesMastery,recordSeriesBattle,seriesMasteryBonusForMonster}from"./services/SeriesMastery.js?v=0.9.15-alpha.5-part5";
...
function equipmentEnhancementBody(item){
 ...
 return`<div class="equipment-enhancement"><div class="enhancement-target"><small>ENHANCEMENT</small><h3>[${item.rarity}] ${item.name}</h3><b>Lv.${level} / ${cap}${atCap?" MAX":""}</b><div class="limit-break-pips">${Array.from({length:EQUIPMENT_LIMIT_BREAK_MAX},(_,i)=>`<i class="${i<breaks?"active":""}"></i>`).join("")}<small>限界突破 ${breaks}/${EQUIPMENT_LIMIT_BREAK_MAX}</small></div><div class="equipment-exp large"><i style="width:${progress}%"></i></div><p>${atCap?maxBreak?"最終レベル上限です":"同名装備で限界突破できます":`EXP ${(item.exp??0).toLocaleString()} / ${need.toLocaleString()}`}</p></div>${atCap&&!maxBreak?`<section class="limit-break-panel"><div class="spread"><b>◆ 限界突破</b><span>Lv.${cap} → ${cap+10}</span></div><p class="muted">同名装備を1個消費して、レベル上限を10上げます。</p><div class="equipment-break-list">${breakRows}</div><button id="executeEquipmentLimitBreak" class="primary" ${breakMaterials.length?"":"disabled"}>同名装備で限界突破</button></section>`:""}${!atCap?`<div class="enhancement-tools"><button data-material-preset="same">同名だけ</button><button data-material-preset="low">N・R</button><button data-material-preset="all">全部</button><button data-material-preset="none">解除</button></div><div class="enhancement-preview" id="enhancementPreview"><b>素材 0個</b><span>獲得EXP 0</span><small>強化後 Lv.${level} / ${cap}</small></div><p class="muted">不要装備をまとめて合成できます。同名装備は獲得EXPが5倍です。</p><div class="equipment-material-list">${rows}</div><button id="executeEquipmentEnhancement" class="primary" disabled>選択した装備で強化</button>`:""}</div>`
}
