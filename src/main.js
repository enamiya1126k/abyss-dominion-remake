import{chapterTwoUnlocked,chapterTwoState,beginChapterTwoRun,moveChapterTwoRoom,beginChapterTwoEncounter,settleChapterTwoEncounter,openChapterTwoChest,chapterTwoEnemyEntries,tuneChapterTwoEnemy,chapterTwoNeedsIntroduction,chapterTwoObjective,ROOMS as CHAPTER_TWO_ROOMS,ENCOUNTERS as CHAPTER_TWO_ENCOUNTERS}from"./chapterTwo/ChapterTwoSystem.js?v=3.1.57-build377";
import{ChapterTwoScreen,chapterTwoDestinations,chapterTwoHelp,chapterTwoMapMarkup}from"./chapterTwo/ChapterTwoScreen.js?v=3.1.57-build377";
import{mountChapterTwoField}from"./chapterTwo/ChapterTwoField.js?v=3.1.57-build377";
import{CHAPTER_TWO_INTRO,CHAPTER_TWO_EPILOGUE,chapterTwoStoryScene}from"./chapterTwo/ChapterTwoStory.js?v=3.1.57-build377";
...
import{royalRevengeRewards,grantRoyalRevengeRewards}from'./core/RoyalRevengeRewards.js?v=3.1.57-build377';
...
import{royalState,beginRoyalAttempt,settleRoyalAttempt,rewindRoyalAttempt,abandonRoyalAttempt}from"./core/RoyalChamberSystem.js?v=3.1.41-build361";
import{mountRoyalChamber}from"./ui/RoyalChamberField.js?v=3.1.41-build361";
...
import{prepareUltimateBattle,beginUltimateAction,finishUltimateAction,ultimateAvailability,chooseEndgameUltimate,castEndgameUltimate,beforeUltimateAction,afterUltimateOrdinary,endUltimateRound,cleanupUltimateBattle,drainUltimateEvents,ultimateIsolated,ultimateExtraBlocked,ultimateBasicOnly,isEndgameUltimate,ultimateCircle,ultimateLabels}from"./core/EndgameUltimateSystem.js?v=3.1.41-build361";
import{equipmentAffixesWithSeries}from"./core/EquipmentAffixSystem.js?v=3.1.55-build375";
import{applyCampaignHeroLoadout}from"./core/CampaignHeroLoadoutSystem.js?v=3.1.57-build377";
import{heroId,heroHp,heroEffects,heroEffect,heroCooldowns,heroAllianceState,heroAuthoredSkills,reserveHeroAction,chooseHeroAllianceSkill,runHeroAllianceAction,triggerHeroAlliance,drainHeroReactions,mitigateHeroDamage,tryHeroLastStand,heroOverheal}from"./core/HeroAllianceSystem.js?v=3.1.41-build361";
import{commitCampaignHeroSkill}from'./battle/CampaignHeroSkillSystem.js?v=3.1.39-build359';
import{SaveService,normalizeRaidJuvenileContract}from"./services/SaveService.js?v=3.1.57-build377";
import{CONTENT_TEST_MODE,BATTLE_SPEED_OPTIONS,CAMERA_DRAG_THRESHOLD_PX,WATER_RULES,MONSTER_STAR_MAX,MONSTER_STORAGE_CAP,ENDGAME_MAX_LEVEL,premiumCrystalCost,normalizeBattleSpeed,contentUnlockFloor,isContentUnlocked}from"./core/config.js?v=3.1.57-build377";
...
import{heroResonanceMembers,heroResonanceProfile,isHeroResonanceSpecies,scaleHeroResonanceSkill,heroPersonalPressure,HERO_INVINCIBLE_PRESSURE}from"./core/HeroResonanceSystem.js?v=3.1.39-build359";
...
import{HomeScreen,homePartySlots}from"./ui/screens/HomeScreen.js?v=3.1.57-build377";
...
import{FormationScreen}from"./ui/screens/FormationScreen.js?v=3.1.57-build377";
import{OnlinePartyScreen,ONLINE_STORAGE_KEYS}from"./ui/screens/OnlinePartyScreen.js?v=3.1.57-build377";
import{OnlinePartyController,resetCurrentWeeklyRaidForFullReset}from"./online/OnlinePartyClient.js?v=3.1.57-build377";
...
import{MonsterListScreen}from"./ui/screens/MonsterListScreen.js?v=3.1.57-build377";
import{MonsterDetailScreen}from"./ui/screens/MonsterDetailScreen.js?v=3.1.57-build377";
import{SettingsScreen}from"./ui/screens/SettingsScreen.js?v=3.1.57-build377";
...
import{CampaignFinalFloorScreen,finalAudienceDialogue}from"./ui/screens/CampaignFinalFloorScreen.js?v=3.1.57-build377";
import{GauntletScreen}from"./ui/screens/GauntletScreen.js?v=3.1.57-build377";
...
import{createMonster,displayName,calculatedStats,TRAITS,expNeedFor,experienceCrystalValue,limitBreakGrowth,affectionBonuses,totalExperience,applyTotalExperience}from"./models/Monster.js?v=3.1.57-build377";
import{EXPERIENCE_PACK_TYPES,experiencePackType,availableExperiencePackTypes,consumeExperiencePacks,experiencePackCapacity,previewExperiencePacks}from"./core/ExperiencePackSystem.js?v=3.1.57-build377";
...
import{receiveEquipment,takeFromStorage,equipmentSellPrice,slotLabel}from"./services/EquipmentStorage.js?v=3.1.55-build375";
...
import{EquipmentScreen}from"./ui/screens/EquipmentScreen.js?v=3.1.57-build377";
...
import{assignEquipmentToSubslot,canEquipInSubslot,emptyEquipmentLoadout,normalizeEquipmentLoadouts}from"./services/EquipmentLoadoutSystem.js?v=3.1.55-build375";
import{ShopScreen}from"./ui/screens/ShopScreen.js?v=3.1.57-build377";
import{SkillScreen}from"./ui/screens/SkillScreen.js?v=3.1.57-build377";
import{AbyssSkillTreeScreen}from"./ui/screens/AbyssSkillTreeScreen.js?v=3.1.41-build361";
import{InventoryScreen,ArmoryScreen}from"./ui/screens/InventoryScreen.js?v=3.1.55-build375";
...
import{worldPresentationForFloor,shouldPlaySecondWorldIntro,markSecondWorldEntered}from"./core/WorldSystem.js?v=3.1.55-build375";
import{randomEventForFloor,markRandomEventResolved,randomEventCosts}from"./core/SecondWorldEventSystem.js?v=3.1.57-build377";
...
import{ENEMY_ACTIONS,createEnemyBattleState,chooseEnemyAction,enemyActionMpCost,enemyDamageMultiplier,enemyDamageAfterDefense,enemyHealAmount,enemyAttackMultiplier,specialActionMultiplier,specialActionInfo}from"./battle/EnemyAI.js?v=3.1.57-build377";
import{createBattleRulesState,cooldownRemaining,setSkillCooldown,tickCooldowns,addBattleLog,applyEnemyStatus,applyEnemyDamage,processEnemyStatuses,applyBattleEffect,effectStackBreakdown,effectValue,hasEffect,clearNegativeAllyEffects,clearPersistentAilments,syncPersistentAilments,tickBattleEffects,processAllyEffects}from"./battle/BattleRules.js?v=3.1.41-build361";
import{attackHits}from"./battle/HitSystem.js?v=3.1.1-build311";
import{buildTurnQueue,currentTurnEntry,currentAlly,currentEnemy,aliveEnemies,selectedEnemy,advanceQueue,queueFinished,skipInvalidEntries}from"./battle/TurnSystem.js?v=3.1.57-build377";
...
import{generateSectionDungeon,sectionIdAt,sectionRoute,portalTowardSection,portalTapDestination,sectionBounds,safeSectionExitCandidates,chooseSafeSectionExitCell}from"./core/DungeonSectionSystem.js?v=3.1.57-build377";
import{createCampaignDungeonLayout}from"./core/CampaignDungeonLayoutSystem.js?v=3.1.57-build377";
import{requiredCampaignBossSectionCount,shouldRegenerateCampaignBossSnapshot}from"./core/CampaignBossWorldSystem.js?v=3.1.57-build377";
...
import{monsterCombatPower,partyCombatPower,partyCombatPowerBreakdown,formatCombatPower,recordPartyCombatPower}from"./core/CombatPower.js?v=3.1.57-build377";
import{beginSecretRoomExpedition,ensureSecretRoomExpedition,secretRoomPlan,enterSecretRoom,activeSecretRoom,spinSecretRoomCasino,casinoBetLimit,useSecretRoomInn,buyDarkMarketOffer,buyDarkMarketRecovery,isDarkMarketBargain,darkMarketPlayerMaxLevel,SECRET_ROOM_RECOVERY_ITEMS,DARK_MARKET_ITEM_LIMIT,CASINO_CRYSTAL_COST,CASINO_MULTIPLIER_RATES}from"./core/SecretRoomSystem.js?v=3.1.57-build377";
import{applyGameMasterReward,applySerialReward,commitSerialRedemption,validateGameMasterCode,validateSerialCode}from"./core/SerialCodeSystem.js?v=3.1.57-build377";
...
import{COMPLETE_MONSTER_CODEX,codexCollectionSummary,syncCollectionRewardInbox,rewardDescription}from"./core/CollectionRewardSystem.js?v=3.1.55-build375";
import{achievementSummary,syncAchievementRewardInbox,achievementIconKeyForId}from"./core/AchievementRewardSystem.js?v=3.1.55-build375";
...
import{enemyExperienceReward}from"./core/ProgressionSystem.js?v=3.1.57-build377";
...
import{equipmentDropLevelForFloor}from"./core/EquipmentDropSystem.js?v=3.1.57-build377";
...
function scheduleBattleContextGuide(){
 ...
 if(battle.tutorialCaptureEligible){
  if(!contextGuideDone("battle_attack"))return showContextGuide({id:"battle_attack",title:"傷を負ったSRの狼と遭遇！",text:"月喰狼ディアヴォルグが道をふさいでいる。まずは「たたかう」で攻撃してみよう。",target:'[data-command="attack"]',placement:"top"});
  ...
  if(!contextGuideDone("battle_skill_use")){
   const personal=battle.skillMenu?document.querySelector(`[data-skill-id="${LIONEL_STARTER_SKILL.id}"]:not([disabled])`):null;
   const target=personal??(battle.skillMenu?document.querySelector("[data-skill-id]:not([disabled])"):document.querySelector('[data-command="skill"]'));
   return showContextGuide({id:"battle_skill_use",title:personal?"リオネル専用・予見の一撃":battle.skillMenu?"使うスキルを1つ選ぼう":"スキル一覧をもう一度開こう",text:personal?"敵の動く先を読む必中の一撃。MP4を使って、預言者の力を試そう。":battle.skillMenu?"光っている技を押して、実際に発動してみよう。":"スキル一覧から使える技を選ぼう。",target,placement:"top"});
  }
  if(!contextGuideDone("battle_capture"))return showContextGuide({id:"battle_capture",title:"捕獲結晶を使ってみよう",text:"今が契約の好機！初回捕獲は必ず成功します。「捕獲」でSRの狼を仲間にしよう。",target:'[data-command="capture"]',placement:"top"});
 }
...
class Entity{constructor(x,y,facing="right"){this.x=x;this.y=y;this.rx=x;this.ry=y;this.facing=facing==="left"?"left":"right";this.path=[];this.p=0}setPath(p){this.path=p;this.p=0}move(dt,s){if(!this.path.length)return false;const t=this.path[0];if(dt*s>0&&t.x!==this.x)this.facing=t.x<this.x?"left":"right";this.p+=dt*s;const n=Math.min(1,this.p);this.rx=this.x+(t.x-this.x)*n;this.ry=this.y+(t.y-this.y)*n;if(this.p>=1){this.x=t.x;this.y=t.y;this.rx=this.x;this.ry=this.y;this.path.shift();this.p=0;return true}return false}}
...
function normalizeEquipmentState(){
 const cycleRate=syncCycleEquipment(save.state);
 ...
 save.state.gacha??={firstTenUsed:false,lastDailyKey:null};save.state.codex??={encounters:{},captures:{},equipment:{}};save.state.codex.encounters??={};save.state.codex.captures??={};save.state.codex.equipment??={};save.state.rest??={lastFreeKey:null};
  for(const item of[...save.state.equipment,...save.state.reserveEquipment,...save.state.bossEquipmentVault]){if(item?.slot==="weapon")item.handedness="either";normalizeFloorBossDedicatedItem(item);normalizeSignatureWeaponItem(item);normalizeVajraRaidWeapon(item)}
 ...
 save.state.monsters.forEach(m=>{
  ...
  if(cycleRate<1){for(const key of ["_equipmentAffixes","_seriesEffects","_seriesMasteryBonus","_signatureBonuses"]){const values=m[key];if(values&&typeof values==="object")m[key]=Object.fromEntries(Object.entries(values).map(([k,v])=>[k,typeof v==="number"?v*cycleRate:v]));}}
...
function campaignHeroIntelPresentation(model){
 return Object.fromEntries((model?.heroes??[]).map(hero=>{
  const species=SPECIES[hero.id]??{},skills=(species.authoredSkills??species.skills??[]).filter(Boolean).slice(0,6).map(skill=>({
...
function render(){
 ...
 if(!battle)audio.setScene(["explore","gauntlet","chapterTwoField"].includes(screen)?"explore":screen==="campaignFinalFloor"?"divine":"home");
 if(screen==="home"){app.innerHTML=HomeScreen(save.state,{serverStatus:homeServerStatus});bindHome()}
 else if(screen==="chapterTwo"||screen==="chapterTwoField"){renderChapterTwo(screen==="chapterTwoField")}
 ...
 if(!legacyPromptOpen){requestAnimationFrame(scheduleContextGuide);queueCampaignStoryScenes({delay:220});maybeStartChapterTwoIntro()}
}
...
function campaignHeroBattleEntry(heroId,{encounterId=null,carryHpRate=1,final=false}={}){
 return prepareEnemyEntry({speciesId:heroId,visualSpeciesId:heroId,nameOverride:campaignHeroName(heroId),combatRarity:"神話",level:CAMPAIGN_HERO_PROFILES[heroId]?.fixedLevel??1000,boss:true,uncapturable:true,noItemDrops:true,statMultiplier:1,fixedTrialScaling:true,fixedTrialHpMultiplier:CAMPAIGN_HERO_HP_MULTIPLIER,carryHpRate,campaignHeroId:heroId,campaignHeroEncounterId:encounterId,campaignHeroFinal:Boolean(final),enemyLoadoutVersion:5,equipped:false,enemyGear:[],enemyMagicCircle:null},CAMPAIGN_MAX_FLOOR,{economyFloor:campaignFloorToLegacyFloor(CAMPAIGN_MAX_FLOOR)})
}
...
function showCampaignEnding(ending,{variant=null,resultId=`campaign-final:${Date.now()}`}={}){
 ...
 let persisted=Boolean(save.save());app.insertAdjacentHTML("beforeend",Modal(outcome.victorious?"予言の十日間・完結":`予言の十日間・${copy.title}`,`<div class="campaign-ending ending-${canonical}${special?" ending-preemptive":""}"><div class="campaign-clear-seal361"><span>百階を越え、予言を打ち破った</span><strong>予言の十日間・完結</strong><small>あなたと四体が守った世界</small></div><small>ENDING ${canonical==="complete"?"I":canonical==="narrow"?"II":"III"}・予言10日目</small><h2>${copy.title}</h2><div class="clear-party361">${(save.state.party??[]).map(id=>save.state.monsters.find(m=>m.id===id)).filter(Boolean).map(m=>`<figure>${monsterVisual(m,"",{className:"clear-monster361"})}<figcaption>${escapeAttribute(displayName(m))}</figcaption></figure>`).join("")}</div><p>${copy.lead}</p><p>${copy.detail}</p>${voices}${epilogue351?'<button type="button" data-campaign-ending-story>魔王側の後日譚を読む</button>':""}<b>${persisted?`${outcome.victorious?"勇者の進軍は止まり、予言は終わりました。踏破済み階層で探索・育成を続け、玉座では全快した四勇とのリベンジに挑めます。第二章では、育てた部隊で新しい物語へ進めます。":"敗北記録を保存しました。強制巻き戻しはありません。"}`:"記録をまだ保存できていません。空き容量を確認して再試行してください。"}</b></div>`,persisted?"魔王城へ戻る":"保存して魔王城へ戻る"));const modal=configureCampaignOutcomeModal(topModal(),"campaignEndingTitle"),finish=()=>{if(!persisted){persisted=Boolean(save.save());if(!persisted){showToast("結末を保存できませんでした。容量を確認してください");return}}modal?.remove();go("home")};modal._onDismiss=finish;modal.querySelector("[data-modal-primary]").onclick=finish;modal.querySelector("[data-campaign-ending-story]")?.addEventListener("click",()=>showCampaignStoryReplaySequence([epilogue351]))
}
...
function finishCampaignFinalBattle(won){
 ...
 cleanupUltimateBattle(current);restoreCampaignFinalParty();
 if(result.memory&&won&&!result.duplicate){result.rewards=grantRoyalRevengeRewards(save.state,result,resultId);}
 fullyRecoverParty();clearPartySynergy();delete save.state.activeBattle;
...
function showRoyalRevengeRewards365(receipt){
 ...
 app.insertAdjacentHTML('beforeend',Modal(`第${number(receipt.stage)}次リベンジ・勝利報酬`,`<div class="royal-reward365"><small>VICTORY REWARD</small><h3>四勇の記憶を越えた</h3><div class="reward-currency365"><div><small>GOLD</small><b>+${number(receipt.gold)}G</b></div><div><small>魔晶石</small><b>+${number(receipt.crystals)}</b></div></div><div class="reward-members365">${members}</div>${receipt.titleAwarded?'<p>称号「四勇を越えし者」を獲得</p>':''}<p>報酬は受取済みです。次のリベンジでは勇者も報酬も強化されます。</p></div>`,'王室へ戻る'));
...
function openRoyalThrone360(){
 const room=royalState(save.state);if(room.phase==='victory'){
  const pending=room.pendingEnding??{ending:'complete',variant:'all-preempted',resultId:`royal-empty:${room.cycle}`};room.phase='cleared';room.attempt=null;room.pendingEnding=null;stopGame();audio.setScene('victory');audio.sfx('victory');showCampaignEnding(pending.ending,pending);return;
 }
 if(room.phase!=='cleared')return;
 app.insertAdjacentHTML('beforeend',Modal('予言を越えた玉座',`<div class="royal-postgame"><p>この世界を歩き続けるか。記憶の中で四勇に挑むか。勝利の先の物語へ進むか。</p>${save.state.campaign100.royalMemoryChampion360?'<b>称号：四勇を越えし者</b>':''}${room.cycle>0?`<b>輪廻 ${room.cycle}・予言の踏破者</b>`:''}<p>第${revengeStage(save.state)}次リベンジ：勇者4人・全員健在。今周回 ${room.memoryWins??0}勝／歴代最高 ${save.state.campaign100.revengeBest361??0}段階</p><p>勝利で次の強さへ。各員EXP +${revengeExperience(revengeStage(save.state)).toLocaleString()}・GOLD +${royalRevengeRewards(revengeStage(save.state)).gold.toLocaleString()}・魔晶石 +${royalRevengeRewards(revengeStage(save.state)).crystals.toLocaleString()}。敗北・撤退では報酬なし・段階は進みません。使用物資は戻ります。</p>${room.rewardReceipt365?'<button type="button" data-royal-last-reward>前回の勝利報酬を見る</button>':''}<button type="button" data-royal-memory>決戦の記憶へ挑む</button><button type="button" data-royal-cycle>第二章へ</button><button type="button" data-royal-free>拠点へ・自由探索を続ける</button></div>`,'王室へ戻る'));
 const modal=topModal();modal.querySelector('[data-royal-last-reward]')?.addEventListener('click',()=>showRoyalRevengeRewards365(room.rewardReceipt365));modal.querySelector('[data-royal-memory]').onclick=()=>{modal.remove();openCampaignFinalPreparation({memory:true})};modal.querySelector('[data-royal-cycle]').onclick=()=>{modal.remove();openCampaignReincarnationDialog()};modal.querySelector('[data-royal-free]').onclick=()=>{modal.remove();stopGame();go('home')};
}
...
function renderCampaignFinalFloor(){
 ...
 stopGame();const room=royalState(save.state),party=(save.state.party??[]).map(id=>save.state.monsters.find(m=>m.id===id)).filter(Boolean).slice(0,4),heroes=royalHeroes360().map(h=>room.phase==='cleared'?{...h,defeated:false,remainingHpRate:1}:h);
...
function openCampaignFinalPreparation({memory=false}={}){
 ...
 startSpecialBattle(remaining.map(e=>campaignHeroBattleEntry(e.heroId,{carryHpRate:e.carryHpRate,final:true})),{type:'campaignFinal',campaignStage:'party',title:memory?`第${room.attempt.stage}次リベンジ・四勇共鳴`:'魔王城-王室・勇者軍最終決戦',subtitle:memory?'全員健在の勇者4人との再戦':`魔王軍4体 対 残った勇者${remaining.length}人`,returnScreen:'campaignFinalFloor'});
}
...
function powerRankingDisplayName(){
 return readPlayerName(onlinePartyController?.profile?.displayName||"冒険者")
}
function powerRankingPublicSnapshot(){
 const allEquipment=[...(save.state.equipment??[]),...(save.state.reserveEquipment??[]),...(save.state.bossEquipmentVault??[])],byId=new Map(allEquipment.filter(Boolean).map(item=>[String(item.id),item]));
...
function schedulePowerRankingPublish({initial=false}={}){
 ...
 if(powerRankingPublishTimer&&!initial)return; // Publish the latest state within 30s even while equipment keeps changing.
...
function handlePowerRankingState(result){
 ...
 state._receivedAt=Date.now();state._cached=false;writePowerRankingCache(state);powerRankingUi.state=state;const modal=document.querySelector("[data-power-record-modal]");
...
function combatPowerRankingMarkup(){
 ...
 if(!connected&&!state&&homeServerStatus.state==="offline")return`<div class="power-ranking-unavailable is-connecting"><span>${pixelIcon("notice")}</span><h3>サーバーオフライン</h3><p>保存されたランキング情報はありません。オンライン時に一度取得すると、次回から前回の記録を表示します。</p><button type="button" data-power-ranking-retry>再接続する</button></div>`;
 ...
 const cached=Boolean(state?._cached||!connected&&state),source=Array.isArray(state?.entries)?state.entries:[],entries=source.slice(0,100).map(entry=>cached?{...entry,online:false}:entry),selfEntry=state?.self?(cached?{...state.self,online:false}:state.self):null,ids=new Set(entries.map(entry=>String(entry?.playerId??""))),presenceOptions={serverNow:state?.serverNow,receivedAt:state?._receivedAt??state?._cachedAt,presenceOnlineMs:state?.presenceOnlineMs},rows=entries.map(entry=>powerRankingEntryMarkup(entry,{self:String(entry?.playerId??"")===String(controller?.selfId??""),...presenceOptions})).join(""),selfRow=selfEntry&&!ids.has(String(selfEntry.playerId??""))?`<div class="power-ranking-self-divider"><span>あなたの順位</span></div>${powerRankingEntryMarkup(selfEntry,{self:true,...presenceOptions})}`:"";
 ...
 const cachedAt=Number(state?._cachedAt??state?._receivedAt)||0,cachedTime=cachedAt?new Date(cachedAt).toLocaleString("ja-JP",{timeZone:"Asia/Tokyo",month:"numeric",day:"numeric",hour:"2-digit",minute:"2-digit"}):"時刻不明",note=cached?`サーバーオフライン・前回の記録（${cachedTime}）`:powerRankingUi.listTimedOut?"更新できませんでした。表示中の順位は前回取得時点です。":`直近30日以内に更新したプレイヤー・全${Math.max(entries.length,Number(state?.total)||0).toLocaleString()}人`,empty=cached?'保存されたランキング情報はありません。':'ランキング登録者はまだいません。';
 return`<section class="power-ranking-board ${cached?"is-cached":""}"><header><div><small>ALL PLAYERS・TOP 100</small><h3>全体戦力ランキング</h3></div><button type="button" data-power-ranking-refresh ${powerRankingUi.loadingList?"disabled":""}>${powerRankingUi.loadingList?"更新中…":powerRankingUi.listTimedOut?"再試行":"更新"}</button></header><div class="power-ranking-season"><span>${pixelIcon("event")}</span><div><b>TOP 100 週間報酬</b><small>${settlement} JSTに順位確定・お知らせへ配布</small></div></div><p class="power-ranking-note ${powerRankingUi.listTimedOut||cached?"is-error":""}">${note}</p><div class="power-ranking-list">${rows||`<p class="muted">${empty}</p>`}${selfRow}</div></section>`
}
...
function renderCombatPowerRecordModal(modal,tab="own"){
 ...
 body.innerHTML=`<div class="power-record-tabs" role="tablist" aria-label="戦力記録の表示"><button type="button" role="tab" data-power-record-tab="own" aria-selected="${tab==="own"}">自分の記録</button><button type="button" role="tab" data-power-record-tab="ranking" aria-selected="${tab==="ranking"}">全体ランキング</button></div><div class="power-record-panel">${tab==="ranking"?`<div class="player-name-shortcut363"><div><small>あなたのプレイヤー名</small><b>${escapeAttribute(powerRankingDisplayName())}</b></div><button type="button" data-player-name-edit>名前を変更</button></div>${combatPowerRankingMarkup()}`:combatPowerOwnMarkup()}</div>`;
 ...
 body.querySelector("[data-player-name-edit]")?.addEventListener("click",()=>openPlayerNameEditor(modal));
...
function openIdleReturnPreview(){
 ...
 primary.innerHTML=preview.available
  ?`<i class="claim-chest">🎁</i><span><strong>報酬をすべて受け取る</strong><small>${preview.gold.toLocaleString()}G ＋ 💎${preview.crystals} ＋ 装備${preview.equipmentCount}個</small></span><em>${preview.capped&&preview.equipmentCapped?"MAX報酬":"受取可能"}</em>`
...
function openCampaignReincarnationDialog(){openChapterTwo()}
function chapterTwoCommit(change){
 const backup=JSON.parse(JSON.stringify(save.state));
 const result=change();
 if(result?.ok===false)return result;
 if(save.save())return result??{ok:true};
 save.state=backup;if(game?.chapterTwo){game.discardChapterTwoSave=true;game.paused=true;requestAnimationFrame(()=>{if(screen==='chapterTwoField')render()})}showToast('保存できませんでした。空き容量を確認して、もう一度お試しください。');return{ok:false,saveFailed:true};
}
let chapterTwoIntroTimer=null,chapterTwoIntroDismissed=false;
function maybeStartChapterTwoIntro(){
 clearTimeout(chapterTwoIntroTimer);
 if(screen!=='home'||!chapterTwoNeedsIntroduction(save.state)||chapterTwoIntroDismissed)return;
 chapterTwoIntroTimer=setTimeout(()=>{
  if(screen!=='home'||battle||save.state.activeBattle||save.state.player.inRun)return;
  if(campaignStoryPresenting||document.querySelector('.game-modal')){maybeStartChapterTwoIntro();return}
  showChapterTwoTitle();
 },450);
}
function showChapterTwoTitle(){
 if(!chapterTwoUnlocked(save.state)||battle||save.state.player.inRun||campaignStoryPresenting)return;
 chapterTwoIntroDismissed=true;
 app.insertAdjacentHTML('beforeend',Modal('第二章', '<div class="chapter-two-title-card"><small>ABYSS DOMINION</small><strong>〜 第二章 〜</strong><p>理の外に生きる者たち</p></div>','物語へ'));
 const modal=topModal();modal.classList.add('chapter-two-title-modal');let advanced=false;
 const advance=()=>{if(advanced||!modal.isConnected)return;advanced=true;clearTimeout(timer);modal.remove();showChapterTwoDialogue('intro')},timer=setTimeout(advance,1800);
 modal.querySelector('[data-modal-primary]').onclick=advance;modal._onDismiss=()=>{advanced=true;clearTimeout(timer);modal.remove()};
}
function openChapterTwo(){
 if(!chapterTwoUnlocked(save.state))return showToast('100階を突破し、勇者一行に勝利すると解放されます。');
 if(battle||save.state.activeBattle)return showToast('戦闘を終えてから進んでください。');
 if(save.state.player.inRun)return showToast('探索から帰還してから進んでください。');
 if(chapterTwoNeedsIntroduction(save.state))return showChapterTwoTitle();
 openChapterTwoDestinations();
}
function showChapterTwoDialogue(kind='intro'){
 const p=chapterTwoState(save.state);if(!p||campaignStoryPresenting||kind==='epilogue'&&!p.clears)return;
 const scene=chapterTwoStoryScene(kind,CAMPAIGN_STORY_CHARACTERS),lines=scene.dialogue,key=kind==='intro'?'introIndex':'epilogueIndex',done=kind==='intro'?'introComplete':'epilogueComplete',replay=p[done];
 let index=replay?0:Math.min(lines.length-1,p[key]||0),closed=false;
 campaignStoryPresenting=true;if(game?.running)game.paused=true;
 app.insertAdjacentHTML('beforeend',Modal(kind==='intro'?'第二章・序章':'境界の森・結末',campaignStoryPresentationBody(scene),'次の会話'));
 ...
 const finish=()=>{if(closed)return;closed=true;modal.remove();campaignStoryPresenting=false;stopGame();snapshot=null;go('home');};
 const renderLine=()=>{const line=lines[index],speaker=characters.find(c=>c.id===line.speakerId),box=modal.querySelector('[data-story-dialogue]'),speakerIndex=characters.indexOf(speaker);box.className=`campaign-story-dialogue${speaker?'':' is-narration'}`;if(speaker)box.style.setProperty('--story-speaker-x',`${(speakerIndex+.5)*100/characters.length}%`);else box.style.removeProperty('--story-speaker-x');box.querySelector('[data-story-speaker-title]').textContent=speaker?.title??'語り';box.querySelector('[data-story-speaker]').textContent=speaker?.name??'';box.querySelector('[data-story-text]').textContent=line.text;modal.querySelectorAll('[data-story-character-id]').forEach(n=>n.classList.toggle('is-speaking',n.dataset.storyCharacterId===line.speakerId));modal.querySelector('[data-story-line-progress]').innerHTML=lines.map((_,n)=>`<i class="${n===index?'current':n<index?'passed':''}"></i>`).join('');primary.textContent=index===lines.length-1?'ホームへ戻る':'次の会話'};
 const advance=()=>{if(closed)return;const last=index===lines.length-1;
  if(!replay&&!chapterTwoCommit(()=>{const fresh=chapterTwoState(save.state);fresh[key]=index+1;if(last){fresh[done]=true;if(kind==='intro')fresh.dungeonHint377=true}return{ok:true}}).ok)return;
  if(last)finish();else{index++;renderLine()}
 };
 modal._onDismiss=finish;primary.onclick=advance;modal.addEventListener('keydown',e=>{if(e.target===modal&&['Enter',' ','ArrowRight'].includes(e.key)){e.preventDefault();advance()}});renderLine();requestAnimationFrame(()=>primary.focus());
}
function enterChapterTwoForest({restart=false}={}){
 const p=chapterTwoState(save.state);if(!p||battle||save.state.activeBattle||save.state.player.inRun)return;
 if(!p.introComplete)return showChapterTwoTitle();
 if(!save.state.party.some(id=>save.state.monsters.some(m=>m.id===id)))return showToast('部隊に仲間を編成してください。');
 if(restart&&!confirm('境界の森を最初から再探索しますか？\nこの森の敵・宝箱・封印を再配置します。育成と物語の記録は残ります。'))return;
 if((!p.run||restart)&&!chapterTwoCommit(()=>beginChapterTwoRun(save.state)).ok)return;
 if(!chapterTwoCommit(()=>{chapterTwoState(save.state).dungeonHint377=false;return{ok:true}}).ok)return;
 stopGame();snapshot=null;go('chapterTwoField');
}
function openChapterTwoDestinations(){
 if(!chapterTwoUnlocked(save.state)||battle)return;
 app.insertAdjacentHTML('beforeend',Modal('ダンジョン・行き先を選択',chapterTwoDestinations(save.state),'閉じる'));
 const modal=topModal();modal.classList.add('chapter-two-destination-modal');modal.querySelector('[data-modal-primary]').onclick=()=>modal.remove();
 modal.querySelector('[data-ch2-old-floors]').onclick=()=>{modal.remove();openExploreFloorSelector({chapterOneOnly:true})};
 modal.querySelector('[data-ch2-royal]').onclick=()=>{modal.remove();enterCampaignFinalFloor()};
 modal.querySelector('[data-ch2-enter]').onclick=()=>{modal.remove();enterChapterTwoForest()};
 modal.querySelector('[data-ch2-new]')?.addEventListener('click',()=>{modal.remove();enterChapterTwoForest({restart:true})});
 modal.querySelectorAll('[data-ch2-story]').forEach(b=>b.onclick=()=>{modal.remove();showChapterTwoDialogue(b.dataset.ch2Story)});
}
function showChapterTwoHelp(){
 if(!game?.chapterTwo)return;
 app.insertAdjacentHTML('beforeend',Modal('境界の森・目的と遊び方',chapterTwoHelp(chapterTwoState(save.state).run),'探索へ戻る'));const modal=topModal();modal.classList.add('chapter-two-help-modal');modal.querySelector('[data-modal-primary]').onclick=()=>modal.remove();
}
function showChapterTwoMap(){
 if(!game?.chapterTwo)return;game.saveChapterTwo();
 app.insertAdjacentHTML('beforeend',Modal('境界の森・マップ',chapterTwoMapMarkup(game.world,chapterTwoState(save.state).run),'探索へ戻る'));const modal=topModal();modal.classList.add('chapter-two-help-modal');modal.querySelector('[data-modal-primary]').onclick=()=>modal.remove();
 modal.querySelectorAll('[data-ch2-map-room]').forEach(b=>b.onclick=()=>{modal.remove();const label=game?.chapterTwoNavigate?.(Number(b.dataset.ch2MapRoom));showToast(label?`${label}へ向かう入口まで移動します。`:'現在いる区画です。目的・遊び方から次の行動を確認できます。')});
}
function renderChapterTwo(field=false){
 stopGame();
 const p=chapterTwoState(save.state);if(!p||save.state.player.inRun){screen='home';app.innerHTML=HomeScreen(save.state,{serverStatus:homeServerStatus});bindHome();return}
 if(!field||!p.run||!p.introComplete){screen='home';app.innerHTML=HomeScreen(save.state,{serverStatus:homeServerStatus});bindHome();requestAnimationFrame(openChapterTwo);return}
 app.innerHTML=ChapterTwoScreen(save.state);
 const party=save.state.party.map(id=>save.state.monsters.find(m=>m.id===id)).filter(Boolean);
 const syncAuto=on=>{const b=document.getElementById('exploreAutoToggle');b?.classList.toggle('active',on);b?.setAttribute('aria-pressed',String(on));if(b)b.querySelector('small').textContent=on?'ON':'OFF';save.save()};
 game={};mountChapterTwoField(game,{canvas:document.getElementById('gameCanvas'),Entity,Camera,findPath:path,drawScene:draw,bindInput,updateTrail:updateExplorationPartyTrail,TILE,run:p.run,party,onSave:()=>save.save(),onAutoChange:syncAuto,blocked:()=>Boolean(battle||document.querySelector('.game-modal')),onContact:object=>{
  if(object.type==='door'){const result=chapterTwoCommit(()=>moveChapterTwoRoom(save.state,object.id));if(result.ok){render();const curtain=document.createElement('div');curtain.className='section-passage-fade';curtain.innerHTML=`<span><small>区画移動</small><b>${CHAPTER_TWO_ROOMS[p.run.room].name}</b></span>`;document.querySelector('.explore-stage')?.append(curtain);setTimeout(()=>curtain.remove(),720)}else if(result.message)showToast(result.message);return}
  if(object.type==='chest'){const result=chapterTwoCommit(()=>openChapterTwoChest(save.state));if(result.ok){render();app.insertAdjacentHTML('beforeend',Modal('宝箱を開いた',`<div class="chapter-two-reward">${pixelIcon('chest')}<h3>GOLD +${result.gold.toLocaleString()}</h3><p>森の探索で見つけた育成資金を獲得した。</p></div>`,'探索へ戻る'));topModal().querySelector('[data-modal-primary]').onclick=closeTopModal}return}
  if(object.type==='spring'){if(chapterTwoCommit(()=>{fullyRecoverParty();return{ok:true}}).ok){const point={...object};render();game.hotSpringRecoveryFx={...point,startedAt:performance.now(),until:performance.now()+1800};showToast('泉の光で部隊のHP・MP・状態異常が回復した')}return}
  if(object.type==='enemy')openChapterTwoBattle(object.id);
 }});
 document.getElementById('centerCamera').onclick=()=>game?.centerChapterTwo?.();
 document.getElementById('resourceHelp').onclick=showChapterTwoHelp;
 document.querySelector('[data-ch2-help]').onclick=showChapterTwoHelp;
 document.querySelector('.explore-stage').append(document.getElementById('miniMapToggle'));
 document.getElementById('miniMapToggle').onclick=showChapterTwoMap;
 const mini=document.getElementById('miniMap');mini.onclick=showChapterTwoMap;mini.onkeydown=e=>{if(['Enter',' '].includes(e.key)){e.preventDefault();showChapterTwoMap()}};
 document.getElementById('exploreAutoToggle').onclick=()=>game?.chapterTwoToggleAuto?.();
 const menu=target=>{stopGame();snapshot=null;if(target==='formation')formationOrigin='chapterTwoField';else if(target==='equipment')navigationOrigin='chapterTwoField';else inventoryNavigationOrigin='chapterTwoField';go(target)};
 document.getElementById('pauseParty').onclick=()=>menu('formation');document.getElementById('fieldEquipment').onclick=()=>menu('equipment');document.getElementById('pauseItems').onclick=()=>{inventoryCategory='consumable';menu('inventory')};
 document.getElementById('returnHome').onclick=()=>{stopGame();snapshot=null;go('home');if(p.clears&&!p.epilogueComplete)showChapterTwoDialogue('epilogue')};
 document.getElementById('toggleExplorePartyHud').onclick=()=>{save.state.settings.explorePartyHudCollapsed=!save.state.settings.explorePartyHudCollapsed;save.save();render()};
 document.querySelectorAll('[data-explore-monster]').forEach(b=>b.onclick=()=>{equipmentTarget=b.dataset.exploreMonster;menu('equipment')});
 if(!p.guideSeen377){requestAnimationFrame(()=>{if(screen!=='chapterTwoField'||battle||document.querySelector('.game-modal'))return;showChapterTwoHelp();chapterTwoCommit(()=>{chapterTwoState(save.state).guideSeen377=true;return{ok:true}})})}
}
function openChapterTwoBattle(id){
 if(battle||!chapterTwoUnlocked(save.state))return;
 const party=save.state.party.map(id=>save.state.monsters.find(m=>m.id===id)).filter(Boolean);
 if(!party.some(m=>m.currentHp>0))return showToast('戦える仲間がいません。泉か拠点で回復してください。');
 const encounter=CHAPTER_TWO_ENCOUNTERS[id];if(!encounter)return;
 const attempt=chapterTwoCommit(()=>beginChapterTwoEncounter(save.state,id));if(!attempt.ok)return;
 const priorVitals=capturePartyVitals();stopGame();
 startBattle(chapterTwoEnemyEntries(id),{specialBattle:true,specialBattleType:'chapterTwo',chapterTwoToken:attempt.token,chapterTwoEncounter:id,specialTitle:`第二章・${encounter.name}`,specialSubtitle:'境界の森・侵食調査',specialReturnScreen:'chapterTwoField',priorVitals,explorationAuto:false});
}
function finishChapterTwoBattle(won,{retreated=false}={}){
 const current=battle;if(!current||current.resultSettled||current.specialBattleType!=='chapterTwo')return;
 current.busy=true;
 const backup=JSON.parse(JSON.stringify(save.state));
 syncPersistentAilments(current);clearPartySynergy();
 if(!won)restorePartyVitals(current.priorVitals);
 const result=settleChapterTwoEncounter(save.state,current.chapterTwoToken,{won});
 delete save.state.activeBattle;
 if(!save.save()){
  save.state=backup;current.party=save.state.party.map(id=>save.state.monsters.find(m=>m.id===id)).filter(Boolean);
  app.insertAdjacentHTML('beforeend',Modal('結果の保存を再試行', '<p>保存できなかったため、結果をまだ確定していません。空き容量を確認して再試行してください。</p>', '保存して結果を確定'));
  const retry=topModal();retry.classList.add('chapter-two-modal');retry._onDismiss=()=>{retry.remove();finishChapterTwoBattle(won,{retreated})};retry.querySelector('[data-modal-primary]').onclick=()=>{retry.remove();finishChapterTwoBattle(won,{retreated})};return;
 }
 current.resultSettled=true;cleanupUltimateBattle(current);document.querySelector('.battle-screen')?.remove();activeEnemy=null;battle=null;snapshot=null;screen='chapterTwoField';render();
 audio.sfx(won?'victory':'defeat');
 const title=won?'境界の森・戦闘勝利':retreated?'部隊を退かせた':'体勢を立て直そう';
 const body=result.ok&&won?`<div class="chapter-two-dialogue"><h3>${CHAPTER_TWO_ENCOUNTERS[result.encounter].name}を突破</h3><p>各員 EXP +${result.experience.toLocaleString()}<br>GOLD +${result.gold.toLocaleString()}${result.crystals?`<br>初回踏破：魔晶石 +${result.crystals}`:''}</p><p>${result.encounter==='heart'?'森の侵食を鎮めた。報酬を確認して、物語の続きへ。':CHAPTER_TWO_ENCOUNTERS[result.encounter].seal?(chapterTwoObjective(chapterTwoState(save.state).run).seals===2?'封印をすべて解放した！東の封印樹から北へ、最深部への道が開いた。':'封印を1つ解放した！残る封印樹をマップで確認しよう。'):chapterTwoObjective(chapterTwoState(save.state).run).detail}</p></div>`:result.ok?'<p>この戦闘の経験値・GOLD報酬はありません。HP・MPは戦闘前の状態に戻りました。使用した消耗品は消費されます。</p>':'<p>この戦闘の結果は受領済みか、探索記録が見つかりません。報酬の重複付与は行いません。</p>';
 app.insertAdjacentHTML('beforeend',Modal(title,body,won&&result.encounter==='heart'?'森の結末へ':'探索へ戻る'));const modal=topModal();modal.classList.add('chapter-two-reward-modal');const finish=()=>{modal.remove();if(won&&result.encounter==='heart')showChapterTwoDialogue('epilogue')};modal._onDismiss=finish;modal.querySelector('[data-modal-primary]').onclick=finish;
}

function openExploreFloorSelector({chapterOneOnly=false}={}){
 if(!chapterOneOnly&&chapterTwoUnlocked(save.state))return openChapterTwo();
...
function claimRewardInboxEntry(id){
 ...
 if(!result.ok){save.state=backup;return result}if(!save.save()){save.state=backup;return{ok:false,reason:"save"}}refreshExploreResourceHud();return result;
}
...
function normalizedMemoryEncounterEntry(entry){
 ...
 for(const key of["id","hp","maxHp","currentHp","currentMp","status","statuses","ailments","captured","defeated","intent","turnState","firstCaptureProfileVersion"])delete memory[key];
...
function bindHome(){
 ...
 document.getElementById("openChapterTwo")?.addEventListener("click",openChapterTwo);
...
async function requestFullGameReset(){
 ...
 showToast("オンライン記録とギルド所属を安全に初期化しています…");
 ...
 onlinePartyController=null;powerRankingUi={state:readPowerRankingCache(),profile:null,selectedPlayerId:null,loadingList:false,loadingProfile:false,listTimedOut:false};powerRankingLastSignature="";powerRankingLastPublishedAt=0;game=null;battle=null;snapshot=null;activeEnemy=null;selected=null;equipmentTarget=null;skillTarget=null;
 monsterManage={editing:false,selected:new Set()};equipmentManage={editing:false,selected:new Set()};
 fullResetInFlight=false;screen="home";render();showToast("ゲームデータ・レイド記録・ギルド所属を初期化しました");
}
async function redeemSettingsGameMasterCode(event){
 ...
 const backup=typeof structuredClone==="function"?structuredClone(save.state):JSON.parse(JSON.stringify(save.state));
 const result=applyGameMasterReward(save.state,validation.kind);
 ...
 normalizeEquipmentState();input.value="";
 app.insertAdjacentHTML("beforeend",Modal(result.kind==="floor99Unlock362"?"GM出発先の解放完了":"GM支援パック認証完了",`<div class="serial-reward-result"><div>🛠️</div><p><b>${result.message}</b></p><small>${["progressPack356","finalePack359"].includes(result.kind)?"配布のみです。階層・討伐記録・現在の編成は変更していません。":"出発階層の解放は最高到達階・実績・クリア状態を変更しません。"}</small></div>`,`確認`));
 topModalButton().onclick=()=>{closeTopModal();render()};
}
function bindPlayerNameForm(root){
 const form=root.querySelector("[data-player-name-form]");if(!form)return;
 const input=form.querySelector("[data-player-name-input]"),status=form.querySelector("[data-player-name-status]");let composing=false;
 ...
 input.addEventListener("input",()=>{input.removeAttribute("aria-invalid");status.textContent="";status.classList.remove("is-error")});
 form.addEventListener("submit",event=>{
  event.preventDefault();if(composing)return;
  const controller=ensureOnlinePartyController(),result=controller.setDisplayName(input.value);
  status.classList.toggle("is-error",!result.ok);input.setAttribute("aria-invalid",String(!result.ok));
  if(!result.ok){status.textContent=result.message;input.focus();return}
  input.value=result.name;input.blur();
  publishPowerRankingSnapshot({force:true});
  status.textContent=controller.connectionReady?"名前を保存しました。ランキングの更新を送信しています。":"名前を保存しました。オンラインに接続するとランキングにも反映されます。";
  const ranking=document.querySelector("[data-power-record-modal]");if(ranking?.dataset.powerRecordTab==="ranking")renderCombatPowerRecordModal(ranking,"ranking");
 });
}
function openPlayerNameEditor(ranking){
 app.insertAdjacentHTML("beforeend",Modal("プレイヤー名",PlayerNameForm(powerRankingDisplayName(),"rankingPlayerName"),"閉じる"));
 const modal=topModal();modal.classList.add("player-name-modal363");bindPlayerNameForm(modal);
 const close=()=>{modal.remove();ranking?.querySelector("[data-player-name-edit]")?.focus()};
 modal._onDismiss=close;modal.querySelector("[data-modal-primary]").onclick=close;
 modal.querySelector("[data-player-name-input]")?.focus();
}
function bindSettings(){
 bindPlayerNameForm(app);
...
const INLINE_EQUIPMENT_SLOT_LABELS={weaponRight:"右手",weaponLeft:"左手",armorBody:"胴",armorSupport:"補助",accessoryNeck:"首",accessoryFinger:"指"};
function openInventoryEquipPicker(item){
 closeInventoryContext();
 const party=save.state.party.map(id=>save.state.monsters.find(monster=>monster.id===id)).filter(Boolean);
 const rows=party.map(monster=>`<article class="inventory-equip-target"><header>${monsterVisual(monster,"",{className:"equip-picker-monster375"})}<div><b>${escapeAttribute(displayName(monster))}</b><small>Lv.${monster.level}</small></div></header><div class="equip-picker-slots375">${compatibleSubslots(item).map(subslot=>{
  const required=Math.max(SLOT_UNLOCK_LEVEL[subslot]??1,equipmentRequiredMonsterLevel(item)),allowed=canEquipInSubslot(item,monster,subslot),current=save.state.equipment.find(gear=>gear.id===monster.equipment?.[subslot]),same=current?.id===item.id;
  const reason=signatureEquipmentOwnerId(item)&&!signatureEquipmentMatchesMonster(item,monster)?"専用キャラのみ":`必要Lv.${required}`;
  return`<section><b class="equip-picker-slot375">${equipmentSubslotLabel(subslot)}</b><div class="equip-picker-current375">${current?equipmentVisual(current):""}<span>現在：${current?escapeAttribute(current.name):"装備なし"}</span></div><small class="equip-picker-change375">→ ${escapeAttribute(item.name)}</small><button type="button" data-inline-equip="${item.id}" data-inline-target="${monster.id}" data-inline-subslot="${subslot}" ${allowed&&!same?"":"disabled"}>${same?"装備中":allowed?"この仲間に装備":reason}</button></section>`;
 ...
 app.insertAdjacentHTML("beforeend",Modal("装備先を選択",`<div class="inventory-inline-equip"><header class="equip-picker-item375">${equipmentVisual(item)}<div><b>${escapeAttribute(item.name)}</b><small>${slotLabel(item.slot)}・必要Lv.${equipmentRequiredMonsterLevel(item)}</small></div></header><p>誰に装備しますか？ 装備する枠を選んでください。</p>${rows||'<div class="empty">パーティーに魔物がいません。</div>'}${remove}</div>`,"戻る"));
 const modal=topModal();modal.classList.add("equip-picker-modal375");
...
function replacePartyMember(outgoingId,incomingId,inherit=false){
 ...
 const inheritedCircle=equippedMagicCircle(outgoing,save.state);
 ...
 if(inherit){
  ...
  equipMagicCircle(save.state,incoming,inheritedCircle.instanceId??"none");
...
function bindFormation(){
 ...
 document.querySelectorAll("[data-formation-circle]").forEach(button=>button.onclick=()=>{equipmentTarget=button.dataset.formationCircle;equipmentFocusItemId=null;navigationOrigin="formation";go("equipment")});
...
const ONLINE_RAID_EXCHANGE_PRICES=Object.freeze({character:240,equipment:180,circle:120});
const ONLINE_WEEKLY_RAID_REWARDS=Object.freeze({
 "abyss-amalga":Object.freeze({name:"終焉融骸・アビス＝マルガ",contractName:"融骸幼体アマルガ",speciesId:"juvenile_amalga",attribute:"dark",visualBase:"./assets/online/raid/juvenile-amalga",equipmentName:"終焉喰らいの大刃",circleId:"reincarnation",circleName:"輪廻の魔法陣"}),
 "zero-sovereign":Object.freeze({name:"零界凍皇・ニヴル＝レギア",contractName:"零界皇ニヴルシア",speciesId:"frost_sovereign",attribute:"ice",visualBase:"./assets/monsters/169_frost_sovereign",equipmentName:"凍星断界剣",circleId:"raid_zero_sovereign",circleName:"零界凍結陣"}),
 "vajra-beast":Object.freeze({name:"雷獄天獣・ヴァジュリオン",contractName:"雷帝獣ヴァジュラ",speciesId:"thunder_emperor",attribute:"lightning",visualBase:RAID_VAJRA_SPRITE,equipmentName:"天雷轟断牙",circleId:"raid_vajra_beast",circleName:"天雷轟界陣"})
});
...
function exchangeOnlineRaidReward(kind,cost){
 ...
 else if(base==="equipment"){const item=createEquipment("weapon",{rarity:"神話"});item.name=rewardDef.equipmentName;item.level=Math.max(1,Math.min(10000,Math.round((Number(save.state.player.maxFloor)||1)*1.35)));item.plus=30;item.raidLimited=true;item.weeklyRaidBossId=bossId;item.ruleOverrides={...(item.ruleOverrides??{}),unsellable:true,raidResonance:true,weeklyRaidBossId:bossId};normalizeVajraRaidWeapon(item);const result=receiveEquipment(save.state,item,{bossReward:true});message=`限定神話武器「${rewardDef.equipmentName}」を獲得（${result.message}）`}
 else if(base==="circle"){if(!rewardDef.circleId)return{ok:false,message:"このボスに交換可能な魔法陣はありません"};const instance=createMagicCircleInstance(save.state,rewardDef.circleId,{level:1,source:"weeklyRaidExchange",locked:false});if(!instance)return{ok:false,message:"魔法陣の現物を追加できませんでした"};save.state.magicCircles.unlocked[rewardDef.circleId]=true;message=`専用魔法陣「${rewardDef.circleName}」を獲得しました。通常の魔法陣として装備できます`}
...
function onlineExploreWorld(room){
	 const expedition=room?.expedition,objects=expedition?.objects??[],multiplayer=(room?.members?.length??0)>=2&&expedition?.coop?.enabled===true,floor=Math.max(1,Number(expedition?.floor)||1),selfPosition=room?.members?.find(member=>member.playerId===game?.onlineSelfId)?.dungeonPosition??expedition?.start,currentSectionId=selfPosition?.sectionId??expedition?.sectionByCell?.[`${selfPosition?.x},${selfPosition?.y}`]??expedition?.currentSectionId??expedition?.startSectionId,bossObjects=objects.filter(object=>object.type==="floorBoss"&&!object.hidden&&!object.resolved),bosses=bossObjects.map(object=>({...object,active:true,campaignBossId:object.bossId??object.bossProfile?.endgameBossId??object.bossProfile?.floorBossCatalogId??null,onlineBossMonster:onlineExploreBossMonster(object,floor)})),trophyChests=objects.filter(object=>object.type==="campaignTrophy"&&!object.hidden).map(object=>({...object,active:true,open:Boolean(object.resolved),claimed:Boolean(object.resolved),locksOpened:object.resolved?CAMPAIGN_KEYS_PER_FLOOR:Math.max(0,Number(object.locksOpened)||0)})),hotSpringObject=objects.find(object=>object.type==="hotSpring"&&!object.hidden),secretRoomObject=objects.find(object=>object.type==="secretRoom"&&!object.hidden&&!object.resolved),exitObject=objects.find(object=>object.type==="exit");
	...
	return{cols:Math.max(1,Number(expedition?.cols)||1),rows:Math.max(1,Number(expedition?.rows)||1),shape:expedition?.shape??"section-dungeons",layoutVersion:expedition?.layoutVersion??318,sections:expedition?.sections??[],rooms:expedition?.rooms??expedition?.sections??[],sectionGraph:expedition?.sectionGraph??[],sectionPortals:expedition?.sectionPortals??[],sectionByCell:expedition?.sectionByCell??{},startSectionId:expedition?.startSectionId??null,currentSectionId,currentRoomId:currentSectionId,roomAttributes:expedition?.roomAttributes??[],treasureRoom:Boolean(expedition?.treasureRoom),treasureRealm:Boolean(multiplayer&&expedition?.coop?.rare?.realmActive),tiles:(expedition?.tiles??[]).map(row=>[...row].map(tile=>tile==="."?0:1)),start:{...(expedition?.start??{x:1,y:1})},exit:{...(expedition?.exit??{x:1,y:1}),active:exitObject?!exitObject.hidden:Boolean(expedition?.exitReached)},shop:secretRoomObject?{x:secretRoomObject.x,y:secretRoomObject.y,sectionId:secretRoomObject.sectionId,roomId:secretRoomObject.roomId??secretRoomObject.id,active:true,rotation:secretRoomObject.rotation??0}:null,bosses,boss:bosses[0]??null,onlineBossMonster:bosses[0]?.onlineBossMonster??null,trophyChests,trophyChest:trophyChests.find(chest=>!chest.open)??trophyChests[0]??null,chests:objects.filter(object=>!object.hidden&&object.type==="chest").map(object=>({...object,open:Boolean(object.resolved),locked:Boolean(object.locked),onlineType:object.type})),campaignKeys:objects.filter(object=>object.type==="campaignKey").map(object=>({...object,collected:Boolean(object.resolved)})),decorations,onlineObjects:objects.filter(object=>!object.hidden&&["campaignKey","resonanceChest","deluxeChest","coopSwitch","resonanceVault","coopElite","relaySeal","keyFragment","combinedKey","rareGoldenMonster","rareMerchant","rarePortal","rarePortalGuardian","rarePortalChest","rareReturnPortal"].includes(object.type)).map(object=>({...object})),hotSpring:hotSpringObject?{...hotSpringObject,active:true,used:Boolean(hotSpringObject.used||hotSpringObject.resolved)}:null,encountering:false}
}
function syncOnlineExploreMembers(room,selfId,{snap=false}={}){
 ...
 for(const entry of game.onlineMembers){const position=entry.member.dungeonPosition??room?.expedition?.start;if(!position)continue;active.add(entry.member.playerId);let entity=game.onlineEntities.get(entry.member.playerId);if(!entity){entity=new Entity(position.x,position.y,position.facing);game.onlineEntities.set(entry.member.playerId,entity)}else reconcileOnlineMotion(entity,position,{snap});if(entry.member.playerId===selfId)game.player=entity}
...
function onlineExploreLoop(now){
 if(!game?.online||!game.running)return;game.performanceProfile??=currentExplorePerformanceProfile();if(game!==frameGame||game?.royal)return;
...
function autoEquipMonster(monsterId){
 ...
 autoEquipMagicCircle(save.state,monster);normalizeEquipmentState();restoreVitalSnapshot(monster,snapshot);
}
...
function unequipMonsterAll(monsterId,confirmFirst=true){const monster=save.state.monsters.find(m=>m.id===monsterId);if(!monster)return;const ids=Object.values(monster.equipment??{}).filter(Boolean);if(!ids.length&&equippedMagicCircle(monster,save.state).id==="none"){if(confirmFirst)alert("解除する装備がありません");return}if(confirmFirst&&!confirm(`${displayName(monster)}の装備をすべて解除しますか？`))return;const beforeStats=calculatedStats(monster),beforeMp=maxMp(monster);ids.forEach(id=>{const item=save.state.equipment.find(i=>i.id===id);if(item)item.equippedBy=null});monster.equipment=emptyEquipmentLoadout();equipMagicCircle(save.state,monster,"none");preserveVitals(monster,beforeStats,beforeMp);save.save();if(confirmFirst)render()}
...
function openGacha(){
 const campaigns=currentGachaCampaigns(),weekday=weekdayGachaSchedule(),tutorialFree=Math.max(0,Number(save.state.gacha?.tutorialFreeSummons)||0);save.save();const weekdayButtons=weekday.kind==="sunday"
  ?weekday.factions.map(kind=>`<button type="button" class="guerrilla-entry active" data-weekday-gacha="${kind}"><span class="gacha-event-mark"></span><b>日曜・深淵召喚</b><small>カテゴリ全体 ${(WEEKDAY_ENDGAME_RATE*100).toFixed(1)}%・外れは通常モンスター</small><em>${weekdayGachaCost(kind)}個</em></button>`).join("")
...
function darkMarketBody(){
 ...
 const recoveryRows=SECRET_ROOM_RECOVERY_ITEMS.map(item=>{const purchased=room?.recoveryPurchased?.[item.id]??0,remaining=Math.max(0,DARK_MARKET_ITEM_LIMIT-purchased);return`<article class="dark-market-recovery"><span>${item.icon}</span><div><b>${item.name}</b><small>${item.description}<br>所持 ${save.state.inventory[item.id]??0}</small></div><button data-market-recovery="${item.id}" ${remaining?"":"disabled"}>${remaining?`${(room?.recoveryPrices?.[item.id]??item.price).toLocaleString()}G`:"完売"}<small>${purchased}/${DARK_MARKET_ITEM_LIMIT}</small></button></article>`}).join("");
 return`<div class="dark-market"><div class="dark-market-wallet">所持 <b>${save.state.player.gold.toLocaleString()}G</b></div><small class="muted">装備・モンスターは各1点限り。所持魔物の最高Lv.${darkMarketPlayerMaxLevel(save.state)}＋100（Lv.${darkMarketPlayerMaxLevel(save.state)+100}）まではランダム価格。それを超える魔物にはLv・能力相応の最低価格が付きます。装備・回復用品はランダム価格です。</small><h3>一点物</h3><div class="dark-market-offers">${offerRows}</div><h3>回復用品</h3><div class="dark-market-recovery-list">${recoveryRows}</div></div>`;
}
...
function ensureExploreDecorations(world){
 if(!world)return[];
 if(world.chapterTwo)return world.decorations??[];
...
function maze(){
 const floor=save.state.player.currentFloor,rng=seeded(floorSeed(floor)),bossIds=campaignBossIdsForFloor(floor),layout=createCampaignDungeonLayout({floor,bossIds,random:rng,recentSignatures:recentDungeonShapeSignatures(floor)}),{cols,rows,tiles,sections}=layout,rooms=sections,startCell={...layout.start};rememberDungeonShapeSignatures(floor,layout.shapeSignatures);
 ...
 const availableBossSections=sections.filter(section=>section.id!==layout.startSectionId),assignedSections=new Set(),bossPlans=[];
...
function persistExpeditionSnapshot(source,{saveNow=true}={}){
 ...
 persistCampaignHeroField(source);
...
function hydrateExpeditionSnapshot(source){
 ...
 const player=new Entity(Number(playerData.x)||world.start.x,Number(playerData.y)||world.start.y,playerData.facing);player.rx=Number.isFinite(Number(playerData.rx))?Number(playerData.rx):player.x;player.ry=Number.isFinite(Number(playerData.ry))?Number(playerData.ry):player.y;
...
function currentSnapshot(){if(game?.chapterTwo){game.saveChapterTwo?.();return null}const result=expeditionSnapshotFromGame({halt:true});persistExpeditionSnapshot(result);return result}
window.addEventListener("pagehide",()=>{if(game?.chapterTwo&&!battle){game.saveChapterTwo?.();return}if(game?.royal&&!battle){royalState(save.state).position={x:game.player.x,y:game.player.y};save.save();return}if(battle)saveBattleCheckpoint();else if(game?.running)persistExpeditionSnapshot(expeditionSnapshotFromGame())});
...
function orderedCampaignStoryCharacters(scene){
 const preferred=scene.kind==="opening"||scene.storyTrack==="demon"||scene.storyPart==="report"?["lionel","sairan"]:HERO_PARTY_IDS,order=new Map(preferred.map((id,index)=>[id,index]));
...
function bindExplore(){
 ...
 game.input=createInputState();game.player??=new Entity(game.world.start.x,game.world.start.y);game.world.encountering=false;game.player.path=[];game.player.p=0;game.partyTrail=Array.isArray(game.partyTrail)&&game.partyTrail.length?game.partyTrail:[{x:game.player.rx??game.player.x,y:game.player.ry??game.player.y,facing:game.player.facing}];
...
function campaignHeroSpawnPoint(section){
 const canvas=game.canvas??game.camera.c,visible=p=>{const s=game.camera.world((p.x+.5)*TILE,(p.y+.5)*TILE);return s.x>40&&s.x<canvas.width-40&&s.y>canvas.height*.22&&s.y<canvas.height*.85};
 const blocked=[...(game.world.chests??[]),...(game.world.campaignKeys??[]),...campaignWorldBosses(game.world),...(section?.portals??[]),game.world.exit,game.world.shop].filter(Boolean);
 return chooseHeroSpawn(game.world,game.player,{sectionId:section?.id,visible,blocked});
}

function repairCampaignHeroPursuit(pursuit){
 if(!pursuit||!game?.world)return false;
 const point={x:Math.round(Number(pursuit.x)),y:Math.round(Number(pursuit.y))},sameSection=pursuit.sectionId===game.world.currentSectionId;
 if(sameSection&&game.world.tiles?.[point.y]?.[point.x]===0&&(pursuit.pendingArrival341||heroFieldRoute(game.world,point,game.player).length||Math.abs(point.x-game.player.x)+Math.abs(point.y-game.player.y)<=1)){
  Object.assign(pursuit,point,{rx:point.x,ry:point.y,move341:null});return false;
 }
 const section=activeExploreSection(),spawn=campaignHeroSpawnPoint(section);
 if(spawn)Object.assign(pursuit,spawn,{rx:spawn.x,ry:spawn.y,sectionId:section.id,pendingArrival341:false,move341:null,graceSeconds:1});
 else prepareHeroArrival(pursuit,{floor:save.state.player.currentFloor,sectionId:game.world.currentSectionId,anchor:game.player});
 return true;
}
function persistCampaignHeroField(source){
 const pursuit=source?.world?.campaignHeroPursuit;if(!pursuit)return;
 const ledger=campaignHeroLedger(),event=ledger.events?.[ledger.activeEncounterId];
 const saved=normalizeHeroPursuit({...pursuit,floor:source.floor??save.state.player.currentFloor},{encounterId:ledger.activeEncounterId,heroId:event?.heroId});
 if(saved){ledger.fieldPursuit341=saved;normalizeCampaignState(save.state).heroEncounters310=ledger}
}
function restoreCampaignHeroFieldPursuit(){
 const ledger=campaignHeroLedger(),event=ledger.events?.[ledger.activeEncounterId];
 if(!event||event.status!=="active"){game.world.campaignHeroPursuit=null;return}
 const options={encounterId:ledger.activeEncounterId,heroId:event.heroId},existing=normalizeHeroPursuit(game.world.campaignHeroPursuit,options),saved=normalizeHeroPursuit(ledger.fieldPursuit341,options);
 let pursuit=existing??saved;
 if(!pursuit){
  const section=activeExploreSection(),spawn=campaignHeroSpawnPoint(section);
  pursuit={...options,chaseSteps:0,portalTransfers:0,floor:save.state.player.currentFloor,state:"pursuing",sectionId:section.id,graceSeconds:1,revealed:true};
  if(spawn)Object.assign(pursuit,spawn,{rx:spawn.x,ry:spawn.y});else prepareHeroArrival(pursuit,{floor:save.state.player.currentFloor,sectionId:section.id,anchor:game.player});
 }else if(!existing&&(pursuit.floor!==save.state.player.currentFloor||pursuit.sectionId==null)){
  prepareHeroArrival(pursuit,{floor:save.state.player.currentFloor,sectionId:game.world.currentSectionId,anchor:game.world.start});
 }else repairCampaignHeroPursuit(pursuit);
 game.world.campaignHeroPursuit=pursuit;
 if(pursuit.playerChoice337==="fight"&&!campaignHeroCheckpointResumable(save.state.activeBattle,ledger))delete pursuit.playerChoice337;
 persistCampaignHeroField({floor:save.state.player.currentFloor,world:game.world});
}
function prepareCampaignHeroFloorTransfer(nextFloor){
 const pursuit=game?.world?.campaignHeroPursuit;if(!pursuit)return;
 const ledger=campaignHeroLedger(),event=ledger.events?.[ledger.activeEncounterId],carry=normalizeHeroPursuit(pursuit,{encounterId:ledger.activeEncounterId,heroId:event?.heroId});
 if(carry){Object.assign(carry,{floor:nextFloor,sectionId:null,pendingArrival341:true,arrivalAnchor341:null,graceSeconds:1});ledger.fieldPursuit341=carry;normalizeCampaignState(save.state).heroEncounters310=ledger}
}

function campaignHeroTouchesPlayer(pursuit){return Boolean(pursuit&&heroInContact(pursuit,{...game.player,sectionId:game.world.currentSectionId}))}

function showCampaignHeroFieldReveal(heroId){
 // Preload walking frames; the field sprite itself introduces the approaching hero.
 for(const frame of ["idle1","idle2","idle3","walk1","walk2"])explorationSpriteImage({speciesId:heroId,visualSpeciesId:heroId},frame);
}

function refreshCampaignHeroChaseHud(){
 const stage=document.querySelector(".explore-stage"),pursuit=game?.world?.campaignHeroPursuit;if(!stage)return;
 stage.querySelectorAll(".campaign-hero-chase-hud,.campaign-hero-edge-chip").forEach(node=>node.remove());
 if(!pursuit||pursuit.state==="resolved"){stage.classList.remove("is-hero-hunt");delete stage.dataset.heroHuntState;updateHeroTracker(stage,null,{});return}
 const canvas=game.canvas??game.camera.c,rect=canvas.getBoundingClientRect(),stageRect=stage.getBoundingClientRect();
 const tools=[...stage.querySelectorAll("#exploreAutoToggle,#miniMapToggle,.campaign-key-counter,.explore-scenery-badge")].map(e=>e.getBoundingClientRect()).filter(r=>r.top<stageRect.top+120);
 const top=Math.min(140,Math.max(8,...tools.map(r=>r.bottom-stageRect.top+4))),point=game.camera.world(((pursuit.rx??pursuit.x)+.5)*TILE,((pursuit.ry??pursuit.y)+.5)*TILE),direction=heroScreenIndicator({screenPoint:point,canvasWidth:canvas.width,canvasHeight:canvas.height,cssWidth:rect.width,cssHeight:rect.height,top:top+50,bottom:64});
 stage.classList.add("is-hero-hunt");stage.dataset.heroHuntState=pursuit.state;
 const model={heroId:pursuit.heroId,name:campaignHeroName(pursuit.heroId),portrait:monsterVisual({speciesId:pursuit.heroId,visualSpeciesId:pursuit.heroId},"勇者"),steps:pursuit.chaseSteps??0,status:pursuit.pendingArrival341?"区画へ接近中":pursuit.chaseSteps>=HERO_PURSUIT_STEPS?"まだ近くにいる":"追跡中",collapsed:Boolean(save.state.settings.heroTrackerCollapsed341),arriving:pursuit.pendingArrival341,top,...direction,x:direction.x+rect.left-stageRect.left,y:direction.y+rect.top-stageRect.top,busy:Boolean(game.paused||game.world.encountering||battle||document.querySelector(".game-modal"))};
 updateHeroTracker(stage,model,{toggle:()=>{save.state.settings.heroTrackerCollapsed341=!save.state.settings.heroTrackerCollapsed341;persistExpeditionSnapshot(expeditionSnapshotFromGame());refreshCampaignHeroChaseHud()},fight:engageCampaignHeroFromHud,locate:()=>{const p=game?.world?.campaignHeroPursuit;if(!p)return;const z=game.camera.z;game.camera.reset((p.rx??p.x)*TILE,(p.ry??p.y)*TILE);game.camera.z=z;game.camera.manual=true;game.camera.clamp(game.world);refreshCampaignHeroChaseHud()}});
}
function engageCampaignHeroFromHud(){
 const pursuit=game?.world?.campaignHeroPursuit;
 if(!pursuit||!game.running||game.paused||game.online||game.world.encountering||battle||document.querySelector(".game-modal"))return false;
 stopExploreAuto();cancelPendingExploreActions();pursuit.playerChoice337="fight";
 if(!persistExpeditionSnapshot(expeditionSnapshotFromGame(),{saveNow:false})||!save.save()){pursuit.playerChoice337="flee";showToast("戦闘開始を保存できませんでした。もう一度お試しください");return false}
 pursuit.state="contact";refreshCampaignHeroChaseHud();beginCampaignHeroContactBattle(pursuit);return true;
}
function updateCampaignHeroFieldMotion(dt){
 const pursuit=game?.world?.campaignHeroPursuit;if(!pursuit)return false;
 const result=advanceHeroField(pursuit,{world:game.world,player:game.player,dt,paused:game.paused||Boolean(battle)||Boolean(document.querySelector(".game-modal"))});
 if(result.contact){pursuit.state="contact";refreshCampaignHeroChaseHud();beginCampaignHeroContactBattle(pursuit);return true}
 if(result.blocked)repairCampaignHeroPursuit(pursuit);
 return false;
}
...
function showCampaignHeroEncounterChoice(){
 ...
 app.insertAdjacentHTML("beforeend",Modal("勇者と遭遇",`<section class="hero-encounter-choice"><small>HERO ENCOUNTER / CHOOSE YOUR ACTION</small><h3>${escapeAttribute(name)}が現れた</h3><p>自動探索を停止しました。どうするか選んでください。</p><p class="muted">選ぶまで追跡は進みません。フィールドで勇者に近づいても戦えます。逃げる場合は階層をまたいで500歩以上、十分に距離を離してください。</p><button type="button" class="ghost" data-hero-choice-flee>フィールドで行動する</button></section>`,"今すぐ戦う"));
 ...
 const choose=choice=>{
  ...
  else showExploreNotice("勇者が通路を歩いて追ってきます。近づくか「迎え撃つ」で戦えます","warning")
 };
...
function updateCampaignHeroPursuitOnStep(){
 const pursuit=game?.world?.campaignHeroPursuit;if(!pursuit)return armCampaignHeroEncounter();
 if(campaignHeroChoicePending(pursuit)){showCampaignHeroEncounterChoice();return true}
 pursuit.chaseSteps=(Number(pursuit.chaseSteps)||0)+1;
 if(campaignHeroTouchesPlayer(pursuit)){pursuit.state="contact";refreshCampaignHeroChaseHud();beginCampaignHeroContactBattle(pursuit);return true}
...
function transferCampaignHeroPursuit(section){
 const pursuit=game?.world?.campaignHeroPursuit;if(!pursuit)return;
 pursuit.portalTransfers=(Number(pursuit.portalTransfers)||0)+1;
 prepareHeroArrival(pursuit,{floor:save.state.player.currentFloor,sectionId:section.id,anchor:game.player});
 refreshCampaignHeroChaseHud();
}

function beginCampaignHeroContactBattle(pursuit){
 if(!pursuit||game.world.encountering)return;const contactLedger348=campaignHeroLedger();if(contactLedger348.events?.[pursuit.encounterId]){contactLedger348.events[pursuit.encounterId].battled348=true;normalizeCampaignState(save.state).heroEncounters310=contactLedger348;}if(!pursuit||game.world.encountering)return;const enemy=campaignHeroEncounter({heroId:pursuit.heroId,encounterId:pursuit.encounterId})[0];if(!enemy){showToast("勇者の戦闘準備ができませんでした。追跡は継続します");return false}beginEncounter([enemy],{campaignHero:true,heroId:pursuit.heroId,encounterId:pursuit.encounterId,priorVitals:capturePartyVitals()})}

function exploreAutoNavigation(){
 if(!game?.world)return null;
 const current=game.world.autoNavigation332;if(current&&typeof current==="object")return current;
 return game.world.autoNavigation332={lastTransition:null,reverseCount:0,pauseUntil:0};
}
function nearestAutoTarget(candidates){
 ...
 const entries=candidates.map(target=>{
 ...
 }).filter(entry=>entry.route?.length),last=exploreAutoNavigation()?.lastTransition,forward=entries.filter(entry=>!(last?.to===current&&entry.portal?.targetSectionId===last.from));
 return(forward.length?forward:entries).sort((a,b)=>a.score-b.score)[0]??null;
}
...
function fullFloorAutoTargets(){
 ...
 const floorState=campaignFloorState(save.state,save.state.player.currentFloor),current=world.currentSectionId??sectionIdAt(world,game.player.x,game.player.y),discovered=new Set(world.discoveredSections??[world.startSectionId]);
 const keys=(world.campaignKeys??[]).filter(entry=>!entry.collected),chests=(world.chests??[]).filter(entry=>!entry.open&&!entry.autoSkipped),currentObjects=[...keys,...chests].filter(entry=>campaignObjectSection(world,entry)===current),unvisited=(world.sections??[]).filter(section=>!discovered.has(section.id)).map(section=>({...section.center,sectionId:section.id,autoSectionTarget:true}));
 // AUTO priority: finish the present room, discover every untouched room,
 // then backtrack for remaining keys/chests.  This prevents two portals from
 // continuously winning the same nearest-target calculation.
 if(currentObjects.length)return currentObjects;
 if(unvisited.length)return unvisited;
 const remaining=[...keys,...chests];if(remaining.length)return remaining;
...
function applyExploreAutoPath(){
 ...
 const navigation=exploreAutoNavigation();if(Date.now()<Number(navigation?.pauseUntil||0))return;
...
function setExploreAutoMode(mode){
 if(mode!=="off"&&game?.world?.campaignHeroPursuit){stopExploreAuto();showExploreNotice("勇者との遭遇中は手動で行動を選んでください","warning");return"off"}
 const next=mode==="off"?"off":"floor";save.state.settings.exploreAutoMode=next;save.state.settings.exploreAutoMenuOpen=false;
 if(game?.player){game.player.path=[];game.player.p=0}if(next!=="off"&&game?.world)game.world.autoNavigation332={lastTransition:null,reverseCount:0,pauseUntil:Date.now()+260};save.save();updateExploreAutoToggleState();if(next!=="off"&&game?.running)requestAnimationFrame(applyExploreAutoPath);return next
}
...
function prepareEnemyEntry(entry,floor,{forceGear=false,economyFloor=null}={}){
 const source={...entry},f=Math.max(1,Math.floor(Number(floor)||1)),gearDepth=Math.max(1,Math.floor(Number(economyFloor)||f)),loadoutDepth=Math.max(gearDepth,Math.min(10000,Math.floor(Number(source.level)||0))),species=SPECIES[source.speciesId]??{},rank=source.faction??species.rarity??"N";
 const reroll=source.enemyLoadoutVersion!==5,boss=Boolean(source.boss),holder=forceGear||boss||source.equipped===true||(!reroll?Boolean(source.equipped):Math.random()<equipmentHolderRateForFloor(loadoutDepth));
 const slots=holder?Math.min(6,equipmentSlotsForFloor(loadoutDepth)+(boss?1:0)):0,rarity=holder?rollEnemyEquipmentRarity(loadoutDepth+(boss?Math.max(25,loadoutDepth*.18):0),rank):null,gearLevel=holder?enemyEquipmentLevelForFloor(loadoutDepth,{rank,boss}):0;
...
function randomEnemyGroup({forcedCount=null}={}){
 ...
 if(floor===1){const firstCapture=!contextualGuideState().disabled&&!contextGuideDone("battle_capture");if(firstCapture){recordFieldEncounter(SPECIES[FIRST_CAPTURE_SPECIES_ID]);return[firstCaptureEncounter()]}recordFieldEncounter(SPECIES.slime);return[prepareEnemyEntry({speciesId:"slime",level:1,boss:false},floor,{economyFloor:legacyDepth})]}
 const roll=Math.random(),count=Number.isInteger(fixedCount)?Math.max(1,Math.min(4,fixedCount)):campaignEncounterPartySize(floor,roll);
...
function floorBossParty(bossInfo,floor){
 const wanted=campaignBossSupportCount(floor),bossSpecies=String(bossInfo?.speciesId??""),bossName=String(bossInfo?.nameOverride??""),supports=[],seenIds=new Set([bossSpecies]),seenNames=new Set([bossName]);
 for(let attempt=0;attempt<4&&supports.length<wanted;attempt++)for(const entry of randomEnemyGroup({forcedCount:wanted-supports.length})){
  const species=SPECIES[entry.speciesId],name=String(entry.nameOverride??species?.name??entry.speciesId);if(seenIds.has(entry.speciesId)||seenNames.has(name))continue;
  seenIds.add(entry.speciesId);seenNames.add(name);supports.push({...entry,boss:false,statMultiplier:(Number(entry.statMultiplier)||1)*(.72+Math.min(100,Math.max(1,Number(floor)||1))*.0008),campaignBossSupport:true});if(supports.length>=wanted)break
 }
 return[bossInfo,...supports]
}
...
function loop(now){
 const frameGame=game;
 if(!game?.running||game.royal)return;
 if(campaignHeroChoicePending()&&!battle&&!game.world.encountering&&!document.querySelector(".game-modal"))showCampaignHeroEncounterChoice();
 if(game!==frameGame||game?.royal)return;
 const dt=Math.min(.05,(now-game.last)/1000||0);game.last=now;
 if(!game.paused)update(dt);
 if(!game?.running||game!==frameGame||game.royal)return;
 if(game.world.campaignHeroPursuit&&now-(game.heroHudPaint341??0)>160){game.heroHudPaint341=now;refreshCampaignHeroChaseHud()}
...
function interactExploreDecoration(decoration){
 ...
 if(decoration.type==="barrel"||decoration.type==="crate"){
  ...
  if(roll<.34){
   ...
   const amount=2+Math.floor(Math.random()*3);save.state.player.crystals=(save.state.player.crystals??0)+amount;resourceToast={icon:"crystal",amount};
...
function transitionCampaignSection(){
 if(game?.online||!game?.world?.sectionPortals?.length||game?.player?.path?.length)return false;const portal=game.world.sectionPortals.find(entry=>entry.sectionId===game.world.currentSectionId&&entry.x===game.player.x&&entry.y===game.player.y);if(!portal)return false;
 const from=game.world.currentSectionId,to=portal.targetSectionId,navigation=exploreAutoNavigation(),reverse=exploreAutoActive()&&navigation?.lastTransition?.from===to&&navigation.lastTransition.to===from;navigation.reverseCount=reverse?(Number(navigation.reverseCount)||0)+1:0;navigation.lastTransition={from,to,step:Number(game.world.steps)||0};navigation.pauseUntil=Date.now()+360;if(navigation.reverseCount>=2)stopExploreAuto("AUTO停止：同じ区画の往復を検知しました");
 const section=game.world.sections.find(entry=>entry.id===portal.targetSectionId);game.player.x=portal.arrivalX;game.player.y=portal.arrivalY;game.player.rx=portal.arrivalX;game.player.ry=portal.arrivalY;game.player.path=[];game.player.p=0;game.world.currentSectionId=portal.targetSectionId;game.world.currentRoomId=portal.targetSectionId;game.world.currentAttribute=section?.attribute??"neutral";game.partyTrail=[{x:game.player.x,y:game.player.y,facing:game.player.facing}];game.camera.reset(game.player.x*TILE,game.player.y*TILE);game.camera.clamp(game.world);revealCampaignArea();transferCampaignHeroPursuit(section);showSectionTransition(section);persistExpeditionSnapshot(expeditionSnapshotFromGame());return true
}
function update(dt){
 rearmPostclearEncounters(save.state,game?.world);
 ...
 if(updateCampaignHeroFieldMotion(dt))return;
 ...
 if(game.player.move(dt,7.5)){
  ...
  if(game.world.exit&&game.world.exit.active!==false&&!game.world.exit.locked&&game.player.x===game.world.exit.x&&game.player.y===game.world.exit.y){
   ...
   prepareCampaignHeroFloorTransfer(clearedFloor+1);stopGame();snapshot=null;clearExpeditionSnapshot({settleHeroPursuit:false});const rewindAdvance=advanceCampaignRewindFloor(campaignHeroLedger(),{clearedFloor});normalizeCampaignState(save.state).heroEncounters310=rewindAdvance.state;save.state.player.currentFloor++;
  ...
  if(!game.world.campaignHeroPursuit&&canPostclearEncounter(save.state,game.world)&&game.world.steps>=game.world.nextEncounter){
...
function exploreBandTheme(floor,attribute=game?.world?.currentAttribute){if(game?.chapterTwo)return game.world.theme;return!game?.online&&attribute?dungeonThemeForAttribute(attribute,floor):dungeonThemeForFloor(floor)}
...
function updateExplorationPartyTrail(){
 if(!game?.player)return;
 const point={x:game.player.rx,y:game.player.ry,facing:game.player.facing};
...
function explorationFollowerPosition(index){
 const spacing=2.25*index,trail=game.partyTrail??[],fallback={x:game.player.rx,y:game.player.ry,facing:game.player.facing};
 if(trail.length<2){
  const standby=[null,{x:-1.15,y:.78},{x:1.15,y:.78},{x:0,y:1.72}][index]??{x:0,y:index*.8};
  return{x:fallback.x+standby.x,y:fallback.y+standby.y,facing:fallback.facing}
 }
 let remaining=spacing;
 for(let i=0;i<trail.length-1;i++){
  const a=trail[i],b=trail[i+1],segment=Math.hypot(b.x-a.x,b.y-a.y);
  if(segment>=remaining){const ratio=segment?remaining/segment:0;return{x:a.x+(b.x-a.x)*ratio,y:a.y+(b.y-a.y)*ratio,facing:a.x!==b.x?(a.x<b.x?"left":"right"):a.facing??b.facing??"right"}}
  remaining-=segment
 }
 ...
 if(segment>.001)return{x:tail.x+(tail.x-prior.x)/segment*remaining,y:tail.y+(tail.y-prior.y)/segment*remaining,facing:prior.x!==tail.x?(prior.x<tail.x?"left":"right"):prior.facing??tail.facing??"right"};
 return tail
}
function drawCampaignHeroField(pursuit){
 const position={x:pursuit.rx??pursuit.x,y:pursuit.ry??pursuit.y,facing:pursuit.facing,moving341:Boolean(pursuit.moving341&&!game.paused&&!game.world.encountering)};
 drawExplorationMonster(position,{speciesId:pursuit.heroId,visualSpeciesId:pursuit.heroId,currentHp:1,onlineStats:{hp:1}},false,1.7,27);
 const point=game.camera.world(position.x*TILE,position.y*TILE),size=TILE*game.camera.z,c=game.ctx;
 c.save();c.font=`bold ${Math.max(10,12*game.camera.z)}px sans-serif`;c.textAlign="center";c.textBaseline="bottom";c.lineWidth=3;c.strokeStyle="#100811";c.fillStyle="#ffe0b0";
 c.strokeText(campaignHeroName(pursuit.heroId),point.x+size/2,point.y-size*1.65);c.fillText(campaignHeroName(pursuit.heroId),point.x+size/2,point.y-size*1.65);c.restore();
}
function drawExplorationMonster(position,monster,glow=false,scale=1,index=0){
 const moving=position.moving341??(Boolean(game.player.path?.length)||Math.abs((game.player.rx??game.player.x)-game.player.x)>.01||Math.abs((game.player.ry??game.player.y)-game.player.y)>.01);
 ...
 // All source sprites face right. Mirror only the sprite around its foot anchor.
 game.ctx.translate(contactX,contactY);if(position.facing==="left")game.ctx.scale(-1,1);
 game.ctx.drawImage(image,-32*pixelScale,-56*pixelScale,64*pixelScale,64*pixelScale);
 game.ctx.restore();
}
...
function explorationPartySceneObjects(){
 if(game?.online){return(game.onlineMembers??[]).map((entry,index)=>{const entity=game.onlineEntities?.get(entry.member.playerId),position=entity?{x:entity.rx,y:entity.ry,facing:entity.facing,moving341:Boolean(entity.path?.length)}:{...(entry.member.dungeonPosition??game.world.start),moving341:false};return{y:position.y+.88,order:80+index,draw:()=>{drawOnlineExploreCircle(position,entry.member.profile);drawExplorationMonster(position,entry.monster,false,(entry.member.playerId===game.onlineSelfId?1:.95)*partyMonsterArtScale(entry.monster),index)}}})}
 const members=explorationPartyMembers();
 const entries=members.map((monster,index)=>{
  const position=index?explorationFollowerPosition(index):{x:game.player.rx,y:game.player.ry,facing:game.player.facing};
  return{y:position.y+.88,order:80+index,draw:()=>drawExplorationMonster(position,monster,false,(index ? .95 : 1)*partyMonsterArtScale(monster),index)};
 });
 if(!entries.length){const position={x:game.player.rx,y:game.player.ry,facing:game.player.facing};entries.push({y:position.y+.88,order:80,draw:()=>drawExplorationMonster(position,{speciesId:"slime"},false,1,0)})}
 return entries
}
...
function drawExploreSceneObjects(world,floor,theme,stairsTexture){
 ...
 if(!world.treasureRealm&&world.exit&&world.exit.active!==false&&visibleExploreObject(world,world.exit))add(world.exit.y+.9,30,()=>drawExploreExit(world.exit,stairsTexture,theme));
 if(world.shop&&visibleExploreObject(world,world.shop))add(world.shop.y+.86,40,()=>drawExploreAtlas(world.shop,EXPLORE_ATLAS.entrance,{scale:1.72,rotation:world.shop.rotation??0,shadowColor:"#000",shadowBlur:6}));
	 campaignWorldBosses(world).filter(fieldBoss=>fieldBoss.active!==false&&visibleExploreObject(world,fieldBoss)).forEach((fieldBoss,index)=>{const boss=game?.chapterTwo?fieldBoss.chapterEnemy:game?.online?(fieldBoss.onlineBossMonster??world.onlineBossMonster):floorBossEnemy(fieldBoss);if(boss)add(fieldBoss.y+.9,60+index,()=>drawExplorationMonster(fieldBoss,{...boss,speciesId:boss.speciesId,visualSpeciesId:boss.visualSpeciesId,level:boss.level},true,1.92,9+index))});
	 const pursuit=world.campaignHeroPursuit;if(!game?.online&&pursuit&&!pursuit.pendingArrival341&&pursuit.sectionId===world.currentSectionId&&pursuit.state!=="resolved")add((pursuit.ry??pursuit.y)+.92,92,()=>drawCampaignHeroField(pursuit));
...
function draw(){
 ...
 if(game.chapterTwo)drawChapterTwoFieldLabels();
 ...
 drawMini(theme);
 if(!game?.chapterTwo)showTutorialPickupMarker();
}
function drawMini(theme=exploreBandTheme(game?.online?game.onlineFloor:save.state.player.currentFloor)){
 ...
 const w=game.world;
 if(!game.chapterTwo&&save.state.settings.minimapVisible===false){m.style.opacity=0;return}
...
function bindInput(c){
 ...
 c.onpointerup=e=>{
  ...
  if(drag||wasMulti){if(game.chapterTwo)game.saveChapterTwo?.();else queueExpeditionCheckpoint();}
  ...
  if(game.chapterTwo){game.chapterTwoTap?.(portalTapDestination(game.world,g)??g);return}
  const decoration=exploreDecorationAt(g.x,g.y);
...
async function animateBattleRecoveryGauge(entry){
 ...
 await runBattleGaugeAnimation({duration,isConnected:()=>bar.isConnected,update:progress=>{const eased=1-Math.pow(1-progress,3),shown=Math.min(state.current,Math.round(fromValue+gain*eased));label.textContent=`${resource.toUpperCase()} ${shown}/${state.max}`},complete:()=>{fill.style.transition="none";fill.style.width=`${toRate}%`;label.textContent=`${resource.toUpperCase()} ${state.current}/${state.max}`;bar.classList.remove(`is-${kind}-recovering`);element.classList.remove(`fx-${kind}-recover`);flash.remove()}});
...
function animateBattleHpGauge(target,element=battleTarget(target)){
 ...
 const animationBattle=battle;
 return runBattleGaugeAnimation({duration,isConnected:()=>bar.isConnected,update:progress=>{const eased=1-Math.pow(1-progress,3),shown=Math.max(state.current,Math.round(fromHp+(state.current-fromHp)*eased));label.textContent=`HP ${shown}/${state.max}`},complete:()=>{animationBattle.hpDisplayRates[state.key]=toRate;delete animationBattle.hpTrails[state.key];bar.classList.remove("is-hp-draining");fill.style.transition="none";fill.style.width=`${toRate}%`;label.textContent=`HP ${state.current}/${state.max}`}});
...
async function battleIntro(enemies){
 ...
 else if(battle?.specialBattle){const isEmergency=battle.specialBattleType==="emergency",isGauntlet=battle.specialBattleType==="gauntlet",isCampaignFinal=battle.specialBattleType==="campaignFinal",waveTotal=Math.max(1,Number(battle.specialWaveTotal)||1),waveIndex=Math.max(0,Number(battle.specialWaveIndex)||0),waveTitle=waveTotal>1?(waveIndex===waveTotal-1?"FINAL WAVE":`WAVE ${waveIndex+1}/${waveTotal}`):isEmergency?"世界異変":isGauntlet?"深淵の試練":isCampaignFinal?"予言の最終決戦":battle.specialBattleType==="chapterTwo"?"境界の森":"部隊戦";battleFlash(isEmergency||isCampaignFinal?"boss":"hit");await battleBanner(waveTitle,battle.specialTitle??(isGauntlet?"奈落回廊":isCampaignFinal?"勇者軍最終決戦":"4対4"),isEmergency||isCampaignFinal?"boss":"encounter",1100)}
...
function makeBattleEnemy(e,index=0){
 ...
 if(enemy.fixedTrialScaling){
  ...
  enemy.maxHp=Math.max(1,Math.round(enemy.maxHp*hpMultiplier));enemy.hp=enemy.maxHp;
	 ...
	 enemy.boss=Boolean(prepared.boss||enemy.boss);enemy.endgameBossId=prepared.endgameBossId??null;enemy.floorBossCatalogId=prepared.floorBossCatalogId??null;enemy.campaignBossId=prepared.campaignBossId??prepared.endgameBossId??prepared.floorBossCatalogId??null;enemy.combatRarity=enemy.endgameBossId?(prepared.faction==="tenGod"?"十神":"深淵"):enemy.floorBossCatalogId?"BOSS":prepared.combatRarity??enemy.combatRarity??null;enemy.visualSpeciesId=prepared.visualSpeciesId??prepared.endgameBossId??enemy.visualSpeciesId??null;enemy.faction=prepared.faction??profile?.faction??null;enemy.powerRate=prepared.powerRate??null;enemy.manifestationLabel=prepared.manifestationLabel??null;enemy.endgameSupport=Boolean(prepared.endgameSupport);enemy.campaignBossSupport=Boolean(prepared.campaignBossSupport);enemy.uncapturable=Boolean(prepared.uncapturable);enemy.trialElement=normalizedElement(prepared.trialElement??prepared.attribute??profile?.element??sp?.element);enemy.id=`enemy-${Date.now()}-${index}-${Math.random().toString(36).slice(2,7)}`;
 ...
 applyCampaignEndgameBalance(enemy,hiddenFloor,{campaign:Boolean(prepared.floorMilestoneId)});
 if(enemy.campaignHeroId)applyCampaignHeroLoadout(enemy);
 if(enemy.campaignHeroFinal)tuneFinalHero(enemy,{count:royalState(save.state).attempt?.memory?4:Math.max(1,campaignRemainingHeroes(campaignHeroLedger()).length),stage:royalState(save.state).attempt?.memory?(royalState(save.state).attempt.stage??revengeStage(save.state)):0});
...
function saveBattleCheckpoint(){
 ...
 syncPersistentAilments(battle);
	 ...
	 const explorationSnapshot=battle.specialBattleType==='chapterTwo'?null:persistExpeditionSnapshot(snapshot??expeditionSnapshotFromGame(),{saveNow:false})??save.state.expeditionSnapshot??null;
 save.state.activeBattle={
  ...(battle.specialBattleType==='chapterTwo'?{chapterTwoToken:battle.chapterTwoToken,chapterTwoEncounter:battle.chapterTwoEncounter}:{}),
  ultimates358:battle.ultimates358??null,ultimateActionKey358:battle.ultimateActionKey358??null,heroAlliance348:battle.heroAlliance348??{},heroFortitude361:battle.heroFortitude361??{},heroShields348:Object.fromEntries(battle.party.map(u=>[u.id,u.heroShield348??0])),heroChains348:Object.fromEntries(battle.party.map(u=>[u.id,u.heroChain348??null])),battleId:battle.battleId,floor:save.state.player.currentFloor,enemies:battle.enemies,turn:battle.turn,turnQueue:battle.turnQueue,queueIndex:battle.queueIndex,
...
function clearBattleCheckpoint(){cleanupUltimateBattle(battle);const options=arguments[0]&&typeof arguments[0]==="object"?arguments[0]:{},saveNow=options.saveNow!==false;delete save.state.activeBattle;if(saveNow)save.save()}
function resumeSavedBattle(){
 if(save.state.activeBattle?.specialBattleType==='chapterTwo'&&(!chapterTwoUnlocked(save.state)||chapterTwoState(save.state)?.run?.pending?.token!==save.state.activeBattle.chapterTwoToken)){delete save.state.activeBattle;save.save();screen='home';return false}
 ...
 if(data.tutorialCaptureEligible&&!contextGuideDone("battle_capture")){
  const previous=data.enemies.find(enemy=>!enemy.captured);
  if(previous&&previous.firstCaptureProfileVersion!==369){
   const replacement=tuneFirstCaptureEnemy(makeBattleEnemy(firstCaptureEncounter(),0)),rate=Math.max(0,Math.min(1,Number(previous.hp)/Math.max(1,Number(previous.maxHp)||1)));
   replacement.id=previous.id;replacement.hp=Math.max(1,Math.round(replacement.maxHp*rate));
   data.enemies=[replacement];data.targetEnemyId=replacement.id;
  }
 }
 ...
 for(const u of battle.party){u.heroShield348=Math.max(0,Number(data.heroShields348?.[u.id])||0);u.heroChain348=data.heroChains348?.[u.id]??null;}for(const e of battle.enemies)if(e.campaignHeroId&&Number(e.heroLoadoutVersion348??0)<3){const rate=e.hp/Math.max(1,e.maxHp),mpRate=e.currentMp/Math.max(1,e.maxMp),shieldRate=Math.max(0,Number(e.heroShield348)||0)/Math.max(1,e.maxHp);applyCampaignHeroLoadout(e);e.hp=Math.round(e.maxHp*rate);e.currentMp=Math.floor(e.maxMp*mpRate);e.heroShield348=Math.floor(e.maxHp*shieldRate)}
...
function recoverBattleHp(target,amount,maximum=null){
 ...
 if(enemy)target.hp=after;else target.currentHp=after;const actual=enemy?target.hp:target.currentHp;queueBattleRecovery(target,"hp",before,actual);const gained=actual-before;heroOverheal(battle,enemy?"enemy":"ally",target,Math.max(0,Math.floor((Number(amount)||0)*healFactor))-gained,max);
...
function tryUnyielding(monster){
 if(hasHeroFortitude(monster)&&tryHeroFortitude(battle,monster,1))return true;
 ...
 if(hasHeroFortitude(monster)&&heroFortitudeUsed(battle,monster))return false;
...
async function trySeriesChainAttack(monster,enemy,damage){
 if(ultimateExtraBlocked(battle,monster))return;
...
function startBattle(encounter,options={}){
 if(options.specialBattleType==='chapterTwo'&&(!chapterTwoUnlocked(save.state)||chapterTwoState(save.state)?.run?.pending?.token!==options.chapterTwoToken))return;
 ...
 const entries=tutorialCaptureEligible&&!guideStepDone(guide,"battle_capture")?[firstCaptureEncounter()]:tutorialCaptureEligible?rawEntries.slice(0,1):rawEntries;
 ...
 if(options.specialBattleType!=='chapterTwo')entries.forEach(entry=>recordBiomeEncounter(save.state,options.memoryBattle?(options.memorySourceFloor??save.state.player.currentFloor):save.state.player.currentFloor,entry.speciesId));
 ...
 if(options.specialBattleType==='chapterTwo')enemies.forEach((enemy,index)=>tuneChapterTwoEnemy(enemy,options.chapterTwoEncounter,index));
 ...
 for(const u of battle.party){u.heroShield348=0;u.heroChain348=null;}initializeFloorBossDeathTracking();battle.heroResonanceCount=heroResonanceProfile(battle.party).count;battle.invincibleAlliance=invincibleAllianceReady();
...
function protectTutorialCaptureTarget(){
 const target=battle?.enemies?.find(enemy=>!enemy.captured);if(!battle?.tutorialCaptureEligible||contextGuideDone("battle_capture")||!target||target.hp>0)return false;target.hp=1;addBattleLog(battle,`${target.name}は捕獲できる程度の力を残して踏みとどまった`);return true
}
...
const BATTLE_EFFECT_DETAIL=Object.freeze({accuracyUp:["命中率",null,1],accuracyDown:["命中率",null,-1],evasionUp:["回避率",null,1],evasionDown:["回避率",null,-1],mpRecoveryDown:["MP回復量",null,-1],magicToPhysical:["魔力→物理",null,1],atkUp:["物理・魔法ATK","atk",1],atkDown:["物理・魔法ATK","atk",-1],defUp:["物理・魔法DEF","def",1],defDown:["物理・魔法DEF","def",-1],spdUp:["SPD","spd",1],spdDown:["SPD","spd",-1],critUp:["会心率",null,1],healDown:["回復量",null,-1],reviveSeal:["蘇生封印",null,-1],guaranteedHit:["必中",null,1],guaranteedCritical:["確定会心",null,1],guard:["被ダメージ軽減",null,1],vulnerable:["被ダメージ増加",null,-1],regen:["HP自動回復",null,1],counter:["反撃倍率",null,1],lifeSteal:["吸収率",null,1],taunt:["挑発",null,1],stun:["行動不能",null,-1]});
function formatBattleInteger(value){return Math.round(Number(value)||0).toLocaleString("ja-JP")}
function bindBattleDetailTap(node,open){
 // Commit before an animation replaces the battle DOM between down and click.
 node.onpointerdown=event=>{if(event.button!=null&&event.button!==0)return;event.preventDefault();event.stopPropagation();open()};
 node.onclick=event=>{event.stopPropagation();if(event.detail===0)open()};
}
function openBattleStatusDetail(id){
 ...
 const groups=kinds.map(kind=>{const detail=BATTLE_EFFECT_DETAIL[kind]??[kind,null,1],stack=effectStackBreakdown(battle,id,kind,targetType),total=stack.reduce((sum,effect)=>sum+effect.applied,0),base=detail[1]?Number(stats[detail[1]])||0:null,opposite=kind.endsWith("Up")?kind.replace(/Up$/,"Down"):kind.endsWith("Down")?kind.replace(/Down$/,"Up"):null,oppositeTotal=opposite?effectValue(battle,id,opposite,targetType):0,final=base==null?null:base*Math.max(.2,1+(kind.endsWith("Up")?total-oppositeTotal:oppositeTotal-total));return`<section class="battle-status-detail-group ${detail[2]<0?"negative":"positive"}"><header><span><small>${detail[2]<0?"DEBUFF":"BUFF"}</small><b>${detail[0]}</b></span><strong>${kind==="vulnerable"?"＋":detail[2]<0?"−":"＋"}${percent(total)}</strong></header>${base==null?"":`<p class="battle-status-final"><span><small>基礎値</small>${formatBattleInteger(base)}</span><b><small>効果反映後</small>${formatBattleInteger(final)}</b></p>`}<div class="battle-status-effect-entries">${stack.map(effect=>`<article><header><b>${effect.sourceSkillName??effect.name??kind}</b><em>残り${Math.max(0,Number(effect.turns)||0)}ターン</em></header><small class="battle-status-effect-source">発動元：${effect.sourceName??"効果元不明"}</small><dl><div><dt>元効果</dt><dd>${percent(effect.original)}</dd></div><div><dt>減衰率</dt><dd>${percent(effect.attenuation)}</dd></div><div class="applied"><dt>最終効果</dt><dd>${percent(effect.applied)}</dd></div></dl></article>`).join("")}</div><footer><span>この項目の合計</span><strong>${kind==="vulnerable"?"＋":detail[2]<0?"−":"＋"}${percent(total)}</strong></footer></section>`}).join("");
 ...
 const special=ultimateLabels(battle,target).map(label=>`<p>${escapeAttribute(label)}</p>`).join("")+(hasHeroFortitude(target)?`<p><b>勇者のふんばり</b>：${heroFortitudeUsed(battle,target)?"使用済み":"残り1回・致死ダメージをHP1で耐える"}</p>`:"")+(Number(target.heroShield348)>0?`<p><b>盾</b>：残り ${formatBattleInteger(target.heroShield348)}</p>`:"");
 app.insertAdjacentHTML("beforeend",Modal(`${escapeAttribute(sourceName)}・状態効果`,`<div class="battle-status-detail-list detailed"><small>開いた時点の状態です</small>${special?`<section class="battle-status-special364">${special}</section>`:""}${groups}${ailmentRows||""}${groups||ailmentRows||special?"":"<p>状態効果はありません</p>"}${ally?'<button type="button" data-battle-unit-info>能力・装備・スキルを見る</button>':""}</div>`,"閉じる"));
 const modal=topModal(),close=()=>modal.remove();modal.classList.add("battle-status-detail-modal");modal._onDismiss=close;modal.querySelector("[data-modal-primary]").onclick=close;
 modal.querySelector("[data-battle-unit-info]")?.addEventListener("click",()=>{close();showBattleMonsterDetail(id)});
...
function renderBattle(){
 prepareBattleUltimates358();
 if(!battle)return;
 sanitizeBattleParty();
 clearContextGuide();
 refreshBattleHpTrails();
 app.classList.add("battle-active");mountBattleScreen(app,BattleScreen(battle,save.state.inventory,save.state.settings,save.state.player.currentFloor));
 mountBattleBossLayout(document.querySelector('.battle-screen'));
 if(battle.heroFortitudeCue361?.length){showToast(battle.heroFortitudeCue361.splice(0).join(' ／ '));}
 ...
 document.querySelectorAll("[data-battle-item-target]").forEach(button=>{const type=battle.itemTargetType;button.onclick=()=>{if(battle?.itemTargetType===type)return useBattleItem(type,button.dataset.battleItemTarget)}});
 const backItem=document.getElementById("backBattleItemMenu");if(backItem)backItem.onclick=()=>{if(!battle?.itemMenu||battle.busy)return;battle.itemTargetType=null;battle.itemTargetActorId=null;renderBattle()};
 document.querySelectorAll("[data-battle-detail]").forEach(button=>bindBattleDetailTap(button,()=>openBattleStatusDetail(button.dataset.battleDetail)));
 document.querySelectorAll("[data-status-detail]").forEach(button=>bindBattleDetailTap(button,()=>openBattleStatusDetail(button.dataset.statusDetail)));
 ...
 document.querySelectorAll("[data-enemy-target]").forEach(button=>bindBattleDetailTap(button,()=>{if(battle.busy||battle.targetEnemyId===button.dataset.enemyTarget)return openBattleStatusDetail(button.dataset.enemyTarget);battle.targetEnemyId=button.dataset.enemyTarget;renderBattle()}));
 ...
 const closeItem=document.getElementById("closeItemMenu");if(closeItem){const close=()=>{if(!battle?.itemMenu)return;battle.itemMenu=false;battle.itemTargetType=null;battle.itemTargetActorId=null;renderBattle()};closeItem.onclick=close;closeItem.onpointerdown=event=>{if(event.button!=null&&event.button!==0)return;event.preventDefault();event.stopPropagation();close()}};
...
function openBattleItemTarget(type){
 if(!battle||battle.busy||battle.auto||!battle.itemMenu||!actor()||(save.state.inventory[type]??0)<=0)return;
 ...
 // Keep target selection inside the battle panel: a generic modal is behind
 // the battle screen's stacking context and cannot receive taps.
 battle.itemTargetType=type;battle.itemTargetActorId=actor().id;renderBattle();
}
async function useBattleItem(type,targetId,{automatic=false}={}){
 if(!battle||battle.busy||(!automatic&&(battle.auto||!battle.itemMenu))||(automatic&&!battle.auto))return;const a=actor();if(!a)return;const reject=message=>automatic?command("attack",null,{skipRandomCircle:true}):alert(message);
 if(!["potions","highPotions","partyPotions","manaPotions","highManaPotions","partyManaPotions","fullManaPotions","partyFullManaPotions","reviveLeaves","statusCures","partyStatusCures","fullHeals","partyFullHeals"].includes(type)||(save.state.inventory[type]??0)<=0)return automatic?command("attack",null,{skipRandomCircle:true}):undefined;
 ...
 if(single&&(!target||(!automatic&&(battle.itemTargetType!==type||battle.itemTargetActorId!==a.id))))return;
 if(type==="reviveLeaves"&&target.currentHp>0)return reject("戦闘不能の仲間を選んでください");
 if(type!=="reviveLeaves"&&single&&target.currentHp<=0)return reject("戦闘不能の仲間には使用できません");
 const hasAilment=m=>(battle.allyAilments?.[m.id]?.length??0)||(m.statuses?.length??0)||(m.ailments?.length??0)||m.status;
 ...
 if(!usable)return reject("今は使用する必要がありません");
 if(type==="reviveLeaves"&&!canBattleRevive())return reject("この戦闘の蘇生上限99回に達しました");
 if(type==="reviveLeaves"&&hasEffect(battle,target.id,"reviveSeal"))return reject("蘇生封印中のため使用できません");
 battle.busy=true;battle.itemMenu=false;battle.itemTargetType=null;battle.itemTargetActorId=null;battle.actionCommitted=true;save.state.inventory[type]--;
...
async function triggerInvincibleAlliance(source){const env=heroBattleEnvironment348();triggerHeroAlliance(battle,"ally",source,env);await showHeroEvents348(env.events)}
function heroBattleEnvironment348(){
 for(const unit of battle.party){unit.maxHp=calculatedStats(unit).hp;unit.maxMp=maxMp(unit);unit.heroSignature348=signatureResonance(unit);unit.heroSkillCosts348=Object.fromEntries(allLearnedSkills(unit).map(skill=>[skill.id,effectiveSkillMpCost(unit,skill)]))}
 const events=[];
 return{events,blocked:(unit,side)=>side==="ally"&&(battle.allyAilments?.[unit.id]??[]).some(e=>["sleep","freeze"].includes(e.id)&&e.turns>0),stats:unit=>Object.prototype.hasOwnProperty.call(unit,"currentHp")?calculatedStats(unit):{...unit,hp:unit.maxHp},
 cleanse:(unit,side)=>{if(side==="ally"){clearNegativeAllyEffects(battle,unit.id);clearAilments(unit)}else{battle.enemyStatuses[unit.id]=[]}},
 damage:(target,side,amount,{source,element,damageClass})=>{
  if(side==="enemy"){const applied=applyEnemyDamage(battle,target,Math.floor(amount*enemyDamageMultiplier(target)*(target.hiddenDamageTaken??1)*endgameIncomingDamageMultiplier(target,element)*(source.currentHp!==undefined?magicCircleDamageMultiplier(source):1)),{sourceId:heroId(source),element,damageClass,heroRulesApplied:true});if(source.currentHp!==undefined){recordBattleDamage(source,applied.damage);registerWeaponFinisher(source,target,applied.beforeHp)}return applied.damage}
  const before=target.currentHp;let damage=mitigateHeroDamage(battle,"ally",target,amount);
  for(const store of [battle.circleShields??{},battle.signatureShields??{}]){const absorbed=Math.min(store[target.id]??0,damage);store[target.id]=Math.max(0,(store[target.id]??0)-absorbed);damage-=absorbed}
  target.currentHp=Math.max(0,before-damage);if(!tryHeroLastStand(battle,"ally",target,before)&&target.currentHp<=0)tryUnyielding(target);recordBattleTaken(target,Math.max(0,before-target.currentHp));if(target.currentHp<=0)handleMagicCircleDeath(target);return Math.max(0,before-target.currentHp)
 },onSkill:(unit,skill,options)=>{if(unit.currentHp!==undefined&&skill){recordSkillUse(unit,skill.id);consumeMagicCircleActionCost(unit)}}}
}
async function showHeroEvents348(events){
 for(const event of events){addBattleLog(battle,`${event.actorName??"勇者"}：${event.label}${event.value?` ${Math.round(event.value).toLocaleString()}`:""}`);
  if(event.kind==="heroAction"){await battleBanner(event.label,event.followup?"勇者共鳴・70%・MP消費なし":"勇者固有スキル","synergy",event.followup?180:320);await animateAttack(event.actorId,true)}
  else if(event.kind==="damage"){await animateHit(event.targetId,event.critical);await floatText(`-${event.value}`,event.targetId,event.critical?"critical":"skill")}
  else if(event.kind==="heal"||event.kind==="revive"){const healer=battle.party.find(u=>u.id===event.actorId);if(healer){recordBattleHealing(healer,event.value);if(event.kind==="revive"){const row=battleContribution(healer);if(row)row.revives++}}await floatText(`+${event.value}`,event.targetId,"heal");}
  else if(event.kind==="shield")await floatText(`盾 +${event.value}`,event.targetId,"guard");
  else if(event.kind==="heroLastStand")await battleBanner("まだ終わってへんやろ","HP1で耐え、残る3人が即時共鳴","synergy",520);
  else if(event.kind==="ko")await animateDefeat(event.targetId);
 }
...
function prepareBattleUltimates358(){
 if(!battle)return;
 prepareUltimateBattle(battle,{stats:u=>battle.party.includes(u)?calculatedStats(u):{...u,hp:u.maxHp},mp:u=>battle.party.includes(u)?maxMp(u):u.maxMp,skills:u=>(battle.party.includes(u)?allLearnedSkills(u):endgameCharacter(u.endgameBossId)?.skills??[]).map(s=>({...s,_cost358:battle.party.includes(u)?effectiveSkillMpCost(u,s):Math.max(0,Number(s.mp)||0)})),statusProfile:u=>u.statusProfile??endgameCharacter(u.endgameBossId)?.statusProfile});
}
async function flushUltimateEvents358(){
 if(!battle)return;finishUltimateAction(battle);
 for(const event of drainUltimateEvents(battle)){
  addBattleLog(battle,`${event.actorName}：${event.label}${event.value?` ${event.value.toLocaleString()}`:""}`);
  const unit=battle.party.find(u=>u.id===event.targetId)??battle.enemies.find(u=>u.id===event.targetId);
  if(event.kind==='revive'&&unit){unit.visualFrame='idle';battle.affectionDeathRecorded??={};battle.affectionDeathRecorded[unit.id]=false;}
  if(event.kind==='ultimateDeath'&&unit)await animateDefeat(unit.id);
  else await floatText(`${event.label}${event.value?` ${event.value.toLocaleString()}`:""}`,event.targetId,['heal','revive'].includes(event.kind)?'heal':event.kind==='damage'?'skill':'guard');
 }
}
...
async function command(type,skillId=null,{skipRandomCircle=false}={}){
 ...
 prepareBattleUltimates358();
 if(ultimateIsolated(battle,a))return finishCurrentAction();
 if(ultimateBasicOnly(battle,a)&&type!=='attack'){if(battle.auto)return command('attack');return alert('領域中は通常攻撃のみ使用できます')}
 battle.busy=true;beginUltimateAction(battle,a,skillId?skillById(skillId):null);
 if(type==='skill'&&isEndgameUltimate(skillId)){if(!learnedSkills(a).some(s=>s.id===skillId)){battle.busy=false;finishUltimateAction(battle);if(battle.auto)return command("attack",null,{skipRandomCircle:true});return}return performUltimate358(a,skillId)}
 if(isHeroResonanceSpecies(a.speciesId)&&(type==="attack"||type==="skill"&&heroAuthoredSkills(a).some(s=>s.id===skillId))){const chosen=type==="skill"?heroAuthoredSkills(a).find(s=>s.id===skillId):null;return performHeroCommand348(a,"ally",chosen,battle.targetEnemyId)}
 if(isHeroResonanceSpecies(a.speciesId)&&!reserveHeroAction(battle,"ally",a)){battle.busy=false;return finishCurrentAction()}
 const s=calculatedStats(a),e=selectedEnemy(battle);if(!e){battle.busy=false;if(!aliveEnemies(battle).length)return win(false,null);return finishCurrentAction()};battle.enemy=e;let triggerAlliance=false,signatureExtraAction=false;

 if(!skipRandomCircle&&hasCircleEffect(a,"randomSkill")&&(type==="attack"||type==="skill"&&!skillId)){
  const unique=new Map();for(const member of save.state.monsters??[])for(const skill of allLearnedSkills(member))unique.set(skill.id,skill);const pool=[...unique.values()].filter(skill=>!isEndgameUltimate(skill)&&canUseSkill(a,skill,cooldownRemaining(battle,a.id,skill.id)));
 ...
 if(type==="skill"&&skillId){
  ...
  if(skillCompleted){
   afterUltimateOrdinary(battle,a,{...skill,_cost358:listedMpCost},{targetId:battle.targetEnemyId});await flushUltimateEvents358();
 ...
 if(type==="item"){completeContextGuide("battle_item_open",{quiet:true});battle.busy=false;battle.auto=false;save.state.settings.autoBattle=false;battle.itemMenu=true;battle.itemTargetType=null;battle.itemTargetActorId=null;save.save();renderBattle();return}
 ...
 if(signatureExtraAction&&!ultimateExtraBlocked(battle,a)&&aliveEnemies(battle).length){battle.actionCommitted=false;battle.busy=false;renderBattle();await battleBanner("多動共鳴","追加行動を獲得","synergy",620,a);return continueBattleFlow()}
 battle.busy=false;
 await finishCurrentAction();
}
function chooseEnemyTarget(enemy=null,mode="normal"){
 const alive=battle.party.filter(monster=>monster.currentHp>0&&!ultimateIsolated(battle,monster));if(!alive.length)return null;
...
async function dealEnemyHit(e,target,multiplier=1,label="",criticalChance=.08,element=null,rules={}){
 if(ultimateIsolated(battle,target))return 0;beginUltimateAction(battle,e,{...rules,element:element??e.element});
 ...
 const heroBefore348=target.currentHp;d=mitigateHeroDamage(battle,"ally",target,d);target.currentHp=Math.max(0,target.currentHp-d);tryHeroLastStand(battle,"ally",target,heroBefore348);d=Math.max(0,heroBefore348-target.currentHp);recordBattleTaken(target,d);if(hasCircleEffect(target,"rage")&&d>0){const rage=rageCircleValues(target),cap=Math.max(rage.secondChainHits,Math.ceil(rage.maxDamageBonus/Math.max(.001,rage.damagePerHit)));target._circleRage=Math.min(cap,(target._circleRage??0)+1)}if(target.currentHp<=0&&tryUnyielding(target))addBattleLog(battle,`${displayName(target)}の復活・耐久効果が発動（${battle.reviveCount}/99）`);else addBattleLog(battle,`${displayName(target)}に${d}ダメージ`);await flushMagicCircleEvents();
...
async function resolveEnemySpecialAction(e,action){
 ...
 if(info.utility){
  ...
  if(info.revive&&canBattleRevive()){
	   ...
	   if(fallen){const before=fallen.hp,transfer=info.reviveTransferRate?Math.floor(e.hp*Math.min(.9,info.reviveTransferRate)):0;if(transfer>0){e.hp=Math.max(1,e.hp-transfer);storeFloorBossSacrifice(e,transfer);fallen.hp=Math.max(1,Math.min(fallen.maxHp,transfer));addBattleLog(battle,`${e.name}はHP${transfer.toLocaleString()}を${fallen.name}へ分与`)}else fallen.hp=Math.max(1,Math.floor(fallen.maxHp*info.revive));battle.reviveCount++;queueBattleRecovery(fallen,"hp",before,fallen.hp);if(info.reviveMp!==undefined){const oldMp=fallen.currentMp??0;fallen.currentMp=Math.max(oldMp,Math.floor((fallen.maxMp??0)*info.reviveMp));queueBattleRecovery(fallen,"mp",oldMp,fallen.currentMp);}for(const effect of info.revivedEffects??info.reviveEffects??[])applyBattleEffect(battle,fallen.id,{...effect,sourceKey:`${e.id}:${action}:revive`,sourceSkillName:info.label},"enemy");if(e.floorBossDomain?.effect==="healingFlare")e._floorBossHealedPowerReady=true;await flushBattleRecoveries()}
...
async function enemyTurn({skipUltimate=false}={}){
 if(battle.busy)return;const entry=currentTurnEntry(battle);if(entry?.type!=="enemy")return continueBattleFlow();
 battle.busy=true;const e=currentEnemy(battle);if(!e){battle.busy=false;return finishCurrentAction()}battle.enemy=e;prepareBattleUltimates358();const ultimate358=skipUltimate?null:chooseEndgameUltimate(battle,e);if(ultimate358)return performUltimate358(e,ultimate358.id);beginUltimateAction(battle,e);if(isHeroResonanceSpecies(e.speciesId)){heroBattleEnvironment348();return performHeroCommand348(e,"enemy",chooseHeroAllianceSkill(battle,"enemy",e))}if(e.fleeAfterTurns&&battle.turn>=e.fleeAfterTurns)return resolveEnemyFlee(e);let action=ultimateBasicOnly(battle,e)?ENEMY_ACTIONS.attack:chooseEnemyAction(e,{allies:battle.enemies,opponents:battle.party,battle}),mpCost=enemyActionMpCost(e,action);if(mpCost>(e.currentMp??0)){e.intent="魔力不足で通常攻撃へ切替";action=ENEMY_ACTIONS.attack;mpCost=0}e.currentMp=Math.max(0,(e.currentMp??0)-mpCost);addBattleLog(battle,`${e.name}：${e.intent}${mpCost?`（内部MP -${mpCost}）`:""}`);battle.actionCommitted=true;
 ...
 else if(specialActionInfo(action)){const ordinary358=specialActionInfo(action);beginUltimateAction(battle,e,ordinary358);commitCampaignHeroSkill(e,action,battle.turn);await resolveEnemySpecialAction(e,action);afterUltimateOrdinary(battle,e,{...ordinary358,id:ordinary358.id??action,name:ordinary358.name??ordinary358.label,_cost358:mpCost});await flushUltimateEvents358()}
 else{
  const target=chooseEnemyTarget(e,e.teamBattleTargetMode??e.campaignHeroTargetMode??(e.endgameBossId?"threat":"normal"));if(!target){battle.busy=false;if(!battle.party.some(u=>u.currentHp>0))return lose();return finishCurrentAction()};await animateAttack(e.id,action===ENEMY_ACTIONS.power);
...
async function finishCurrentAction(){
 if(!battle)return;await flushUltimateEvents358();
...
async function endRound(){
 battle.busy=true;prepareBattleUltimates358();finishUltimateAction(battle);
 ...
 for(const echo of dueSkillEchoes){const source=battle.party.find(monster=>monster.id===echo.sourceId),target=(battle.enemies??[]).find(enemy=>enemy.id===echo.targetId&&enemy.hp>0)??aliveEnemies(battle)[0];if(!target||ultimateExtraBlocked(battle,source))continue;const applied=applyEnemyDamage(battle,target,Math.max(1,Math.floor(Number(echo.amount)||0)),{sourceId:echo.sourceId,element:echo.element??source?.attribute??"neutral",damageClass:echo.damageClass??"physical"});if(source){recordBattleDamage(source,applied.damage);registerWeaponFinisher(source,target,applied.beforeHp)}addBattleLog(battle,`${source?displayName(source):"時の残響"}：${echo.skillName??"残響"}が再来し ${applied.damage.toLocaleString()}ダメージ`);await animateHit(target.id,true);await floatText(applied.damage?`残響 -${applied.damage}`:"完全ガード",target.id,applied.damage?"skill":"guard")}
 ...
 await flushHeroReactions348();tickCooldowns(battle);tickBattleEffects(battle);endUltimateRound(battle);await flushUltimateEvents358();
 battle.guards={};
...
async function continueBattleFlow(){
 ...
 if(queueFinished(battle))return endRound();
 const entry=currentTurnEntry(battle);prepareBattleUltimates358();
 const active358=entry?.type==='enemy'?currentEnemy(battle):currentAlly(battle),key358=`${battle.turn}:${battle.queueIndex}:${entry?.id}`;
 if(active358&&battle.ultimateActionKey358!==key358){battle.ultimateActionKey358=key358;if(beforeUltimateAction(battle,active358)){battle.busy=true;await flushUltimateEvents358();battle.busy=false;if(!aliveEnemies(battle).length)return win(false,null);if(!battle.party.some(u=>u.currentHp>0))return lose();return finishCurrentAction()}}
 renderBattle();
 ...
 if(entry?.type==="ally"){const current=currentAlly(battle);if(current&&await prepareMagicCircleTurn(current))return;if(battle.auto){const expectedBattle=battle,expectedTurn=battle.turn,expectedIndex=battle.queueIndex;await wait(220);if(battle!==expectedBattle||!battle.auto||battle.busy||battle.turn!==expectedTurn||battle.queueIndex!==expectedIndex)return;const a=currentAlly(battle);if(a){const ultimate358=chooseEndgameUltimate(battle,a,learnedSkills(a));if(ultimate358)return command("skill",ultimate358.id);for(const member of battle.party??[])member._maxHp=calculatedStats(member).hp;if(isHeroResonanceSpecies(a.speciesId)){heroBattleEnvironment348();const skill348=chooseHeroAllianceSkill(battle,"ally",a);return command(skill348?"skill":"attack",skill348?.id)}const hpRate=a.currentHp/Math.max(1,a._maxHp),rageFirstHit=hasCircleEffect(a,"rage")?rageCircleValues(a).firstChainHits:Infinity;if(hasCircleEffect(a,"rage")&&(a._circleRage??0)>=rageFirstHit&&Math.random()<Math.min(.75,(a._circleRage??0)*.06))return command("attack");const decision=chooseAutoBattleDecision(a,{...battle,autoBattleStats:{...(battle.autoBattleStats??{}),[a.id]:calculatedStats(a)}});if(decision.targetId&&(battle.enemies??[]).some(enemy=>enemy.id===decision.targetId&&enemy.hp>0))battle.targetEnemyId=decision.targetId;if(decision.kind==="skill"&&decision.skill&&["selfHeal","allHeal","revive","cleanse","mpHeal"].includes(decision.skill.type))return command("skill",decision.skill.id);if(hpRate<=.25){const item=["fullHeals","highPotions","potions"].find(type=>(save.state.inventory[type]??0)>0);if(item)return useBattleItem(item,a.id,{automatic:true})}if(decision.kind==="guard")return command("guard");if(decision.kind==="skill"&&decision.skill)return command("skill",decision.skill.id)}return command("attack")}}
}
...
function win(caught,m){
 ...
 const crystalRoll=defeated.reduce((sum,e)=>{if(e.speciesId==="mimic"&&!e.captured)return sum+mimicVictoryCrystals(economyDepth,Math.random);const chance=e.boss?1:e.gear?.28:.12;if(Math.random()<abyssExplorationChance(save.state,chance,null,{max:1}))return sum+(e.boss?30+Math.floor(e.level/8):2+Math.floor(Math.random()*2));return sum},0);if(crystalRoll){const current=Math.max(0,Math.min(Number.MAX_SAFE_INTEGER,Math.floor(Number(save.state.player.crystals)||0)));save.state.player.crystals=current+safeCurrencyGrant(current,crystalRoll)}
...
function drawChapterTwoFieldLabels(){
 const c=game.ctx,tile=TILE*game.camera.z,r=chapterTwoState(save.state).run;
 c.save();c.textAlign='center';c.textBaseline='middle';c.font=`900 ${Math.max(10,Math.min(18,tile*.23))}px serif`;c.lineWidth=3;c.strokeStyle='#08090b';
 for(const o of game.chapterTwoObjects??[]){const point=game.camera.world((o.x+.5)*TILE,(o.y+1.3)*TILE),locked=o.type==='door'&&o.targetSectionId==='forest-5'&&chapterTwoObjective(r).seals<2,label=locked?'封印中・東西を解放':o.label;c.fillStyle=locked?'#ec9aa8':o.type==='spring'?'#a6eef1':'#f2dda7';c.strokeText(label,point.x,point.y);c.fillText(label,point.x,point.y)}c.restore();
}
