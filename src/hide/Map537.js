...
// Check decoration continuously: sparse samples can otherwise miss a table corner,
// producing a path that the actual character body cannot follow.
function crosses537(a,b,o,r){let lo=0,hi=1;for(const [k,half] of [['x',o.w/2+r-1e-6],['y',o.h/2+r-1e-6]]){const d=b[k]-a[k],min=o[k]-half,max=o[k]+half;if(Math.abs(d)<1e-9){if(a[k]<=min||a[k]>=max)return false;continue}const t1=(min-a[k])/d,t2=(max-a[k])/d;lo=Math.max(lo,Math.min(t1,t2));hi=Math.min(hi,Math.max(t1,t2));if(lo>hi)return false}return hi>=0&&lo<=1}
export function line537(m,a,b,r=0){const n=Math.max(1,Math.ceil(dist(a,b)/(r?8:20))),decor=new Set();for(let i=0;i<=n;i++){const x=a.x+(b.x-a.x)*i/n,y=a.y+(b.y-a.y)*i/n;if(!free537(m,x,y,r))return false;for(const o of localDecor(m,x,y))decor.add(o)}for(const o of decor)if(crosses537(a,b,o,r))return false;return true}
