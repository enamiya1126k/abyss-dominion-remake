...
import {generateSectionDungeon} from '../core/DungeonSectionSystem.js?v=3.1.60-build380';
...
export function chapterTwoLayout(serial=1,area=0){
 ...
 const layout=generateSectionDungeon({count:6,random,attributes:area?[region.element,region.element,'water',region.element,region.element,region.element]:['nature','nature','water','nature','dark','dark'],sizeTiers:['small','standard','large','standard','large','huge'],patterns:[['rootClaws','rootRing','sanctumCross','abyssIslands','terraces','coreRings'],['rootClaws','abyssIslands','sanctumCross','rootClaws','abyssIslands','rootRing'],['abyssIslands','rootRing','sanctumCross','abyssIslands','coreRings','coreRings'],['sanctumCross','abyssIslands','sanctumCross','rootRing','sanctumCross','coreRings'],['coreRings','abyssIslands','sanctumCross','coreRings','rootRing','coreRings']][area],customTopology:{nodes:region.positions.map(([gx,gy],n)=>({id:id(n),gx,gy})),edges:region.rooms.flatMap(r=>Object.entries(r.links).filter(([,n])=>n>r.id).map(([direction,n])=>({a:id(r.id),b:id(n),direction})))}});
...
export function createChapterTwoWorld(run,rooms,encounters){
 ...
 const keyDrops=bosses.filter(b=>encounters[b.id].seal&&run.defeated.includes(b.id)&&!(run.keys380??run.defeated).includes(b.id)).map(b=>({id:b.id,...pick(Number(b.sectionId.split('-').at(-1)),7),collected:false}));
 const vault=run.vault380??{defeated:false,chests:[]},vaultId=`vault${run.area??0}`,ve=encounters[vaultId];
 if(ve)bosses.push({id:vaultId,...pick(2,85),active:!vault.defeated,hidden:false,chapterEnemy:{speciesId:ve.species[0],endgameBossId:ve.authorities?.[0],level:ve.level}});
 if(run.completed)for(const n of [1,3,4]){const rid=`roam${run.area??0}_${n}`,e=encounters[rid];if(e)bosses.push({id:rid,...pick(n,9),active:!(run.roaming380??[]).includes(rid),hidden:false,chapterEnemy:{speciesId:e.species[0],level:e.level}})}
 const treasures=ve&&vault.defeated?Array.from({length:10},(_,n)=>({id:`vaultChest${n}`,vaultIndex:n,...pick(2,25+n*13),open:vault.chests.includes(n),kind:'box'})):[];
 const chest={id:'forest-chest' ,...pick(2,18),open:run.chest,kind:'box'},spring={...pick(2,45),active:true,used:false,scale:4.5};
 return {...layout,chapterTwo:true,currentSectionId:sectionId,currentRoomId:sectionId,currentAttribute:sections[run.room].attribute,discoveredSections:run.visited.map(id),decorations:[],bosses,chests:[chest,...treasures],hotSpring:spring,campaignKeys:keyDrops,enemies:[],bossDefeated:run.completed,exit:{...layout.start,active:false,locked:true},shop:null,theme:chapterTwoTheme(run.room,run.area??0)};
}
