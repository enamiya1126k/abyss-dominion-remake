...
import {createMonster} from '../models/Monster.js?v=3.1.61-build381';
import {createSignatureEquipment} from '../core/SignatureWeaponSystem.js?v=3.1.60-build380';
...
import {applyTotalExperience,totalExperience} from '../models/Monster.js?v=3.1.61-build381';
...
export function chapterTwoState(state){
 ...
 p.areaClears378??={};p.areaClears378[0]=Math.max(count(p.areaClears378[0]),p.clears);p.runs378??={};p.vaults380??={};p.stories378??={};p.challengeClears378??={};
 ...
 if(p.run){const r=p.run;r.area=CHAPTER_TWO_AREAS[r.area]?r.area:0;const area=chapterTwoArea(r);
  ...
  r.roaming380??=[];r.visit380??=0;r.vault380=p.vaults380[r.area]??={defeated:false,chests:[]};
...
export function beginChapterTwoEncounter(state,id){
 ...
 if(e.vault?e.area!==r.area||r.room!==2||r.vault380.defeated:e.roaming?e.area!==r.area||!r.completed||r.room!==e.room||r.roaming380.includes(id):r.completed||chapterTwoRooms(r)[r.room].encounter!==id||r.defeated.includes(id))return{ok:false};
 if(r.pending&&r.pending.encounter!==id)return{ok:false};
 r.pending??={token:ENCOUNTERS[id].roaming?`roam:${r.serial}:${r.visit380}:${id}`:`forest:${r.serial}:${id}`,encounter:id,partyIds:[...new Set(state.party??[])].slice(0,4)};
...
export function settleChapterTwoEncounter(state,token,{won=false}={}){
 ...
 if(encounter.vault)r.vault380.defeated=true;else if(encounter.roaming){if(r.roaming380.includes(attempt.encounter))return{ok:false,duplicate:true};r.roaming380.push(attempt.encounter)}else r.defeated.push(attempt.encounter);
 ...
 let equipment=null;
 if(encounter.roaming&&r.roaming380.length===3){const item=createEquipment('weapon',{rarity:'LR'});item.level=area.level;const receipt=receiveEquipment(state,item,{bossReward:true});equipment={name:item.name,rarity:item.rarity,level:item.level,plus:item.plus,id:item.id,receipt:receipt.message};}
 ...
 const result={ok:true,won:true,token,area:area.id,challenge:!!r.challenge,encounter:attempt.encounter,experience,gold,crystals,firstClear,members,equipment,experiencePacks};p.lastReward=result;recordChapterTwo380(state,result);return result;
}
...
export function chapterTwoEnemyEntries(id,run=null){
 ...
 return e.species.map((speciesId,i)=>({speciesId,endgameBossId:e.authorities?.[i]??null,faction:e.authorities?.[i]?.startsWith('ten_')?'tenGod':e.authorities?.[i]?'abyss':null,teamBattle:!!e.area,teamBattleRole:e.roles?.[i],level:Math.round(e.level*(1+(run?.challengeTier??0)*.18)),chapterTwoEncounter:id,chapterTwoIndex:i,boss:i===0&&(e.boss||e.seal||e.vault)||false,nameOverride:i===0&&(e.boss||e.seal||e.vault)?e.name:undefined,uncapturable:!!e.authorities?.[i]||i===0&&(e.boss||e.seal||e.vault)||false,enemyLoadoutVersion:5,enemyGear:[],enemyMagicCircle:null,equipped:false,fixedTrialScaling:true,enemyFloor:100,statMultiplier:1}));
}
export function tuneChapterTwoEnemy(enemy,id,index=0,run=null){
 const e=ENCOUNTERS[id];if(!e)return;
 const support=index>0&&Boolean(e.boss||e.seal||e.vault),rate=support?.55:1;
 ...
 if(support)enemy.role=(index===1?'healer':'attacker');
...
export function recordChapterTwo380(state,result){const p=chapterTwoState(state);p.expedition380??={gold:0,experience:0,crystals:0,experiencePacks:0,battles:0,equipment:[]};const x=p.expedition380;for(const k of ['gold','experience','crystals','experiencePacks'])x[k]+=Number(result[k])||0;if(result.won)x.battles++;if(result.equipment)x.equipment.push(result.equipment);}
...
export function refreshChapterTwoRoaming380(state){const p=chapterTwoState(state);if(!p?.run?.completed||p.run.pending)return;const r=p.run;r.visit380++;r.roaming380=[];}
...
export function recruitChapterTwoHeroes380(state){const p=chapterTwoState(state);if(!p?.introComplete||p.alliance380)return{ok:false};state.monsters??=[];const joined=[],weapons=[];for(const id of ['myth_enami','myth_yori','myth_hide','myth_rion']){if(!state.monsters.some(m=>m.speciesId===id)){const m=createMonster(id,{level:1000,rank:1});m.favorite=true;m.locked=true;state.monsters.push(m);joined.push(m.id)}const item=createSignatureEquipment(id,0);item.level=1;item.plus=0;item.favorite=true;item.locked=true;receiveEquipment(state,item,{bossReward:true});weapons.push(item.name)}p.alliance380={joined,weapons,at:Date.now()};return{ok:true,joined,weapons};}
export function chapterTwoHint380(id){const e=ENCOUNTERS[id];return e?.vault?'本編以上の強敵。回復役の護衛を先に倒し、長期戦に備えよう。':e?.species?.length>1?'回復役の護衛を先に倒すと、主敵にダメージを集中できます。':'属性相性を確認して、有利な仲間で挑もう。';}
