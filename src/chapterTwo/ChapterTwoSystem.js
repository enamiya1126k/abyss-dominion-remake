import {rollChestSupplies427,grantChestSupplies427,mergeChestSupplies427} from '../core/ChestSupplies427.js';
...
export function openChapterTwoChest(state,{random=Math.random}={}){const p=chapterTwoState(state),r=p?.run;if(!r||r.pending||r.room!==2||r.chest)return{ok:false};r.chest=true;return{ok:true,gold:currency(state,'gold',80000*(chapterTwoArea(r).id+1)),supplies427:grantChestSupplies427(state,rollChestSupplies427({random}))};}
...
export function recordChapterTwo380(state,result){const p=chapterTwoState(state);p.expedition380??={gold:0,experience:0,crystals:0,experiencePacks:0,battles:0,equipment:[]};const x=p.expedition380;if(result.supplies427?.length)x.supplies427=mergeChestSupplies427([...(x.supplies427??[]),...result.supplies427]);for(const k of ['gold','experience','crystals','experiencePacks'])x[k]+=Number(result[k])||0;if(result.won)x.battles++;if(result.equipment)x.equipment.push(result.equipment);if(result.spoils394?.relic)x.equipment.push(result.spoils394.relic);if(result.spoils394?.circle)(x.circles395??=[]).push(result.spoils394.circle);}
...
export function openVaultChest380(state,index,{random=Math.random}={}){const p=chapterTwoState(state),r=p?.run;if(!r||r.pending||r.room!==2||!r.vault380.defeated||!Number.isInteger(index)||index<0||index>9||r.vault380.chests.includes(index))return{ok:false};r.vault380.chests.push(index);const area=chapterTwoArea(r);let result={ok:true,gold:0,experiencePacks:0};
 ...
 result.supplies427=grantChestSupplies427(state,rollChestSupplies427({slots:3,random}));recordChapterTwo380(state,result);return result;}
