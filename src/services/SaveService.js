...
function initialState(){
 ...
 return{schemaVersion:5,appVersion:APP_VERSION,player:{gold:1000,crystals:20,maxFloor:1,currentFloor:1,checkpoint:1,inRun:false,nextShopFloor:4,floorSeeds:{},openedChests:{},bossRewards:{}},monsters,party:monsters.map(m=>m.id),equipment:[],inventory:{potions:3,captureCrystals:5},settings:{minimapVisible:true,autoBattle:true,equipmentSort:"rarity",battleSpeed:1},records:{kills:0,captures:0,chests:0,purchases:0}};
}
export class SaveService{
 ...
 migrate(s){s.schemaVersion=5;s.appVersion=APP_VERSION;s.player??={};s.player.currentFloor??=1;s.player.checkpoint??=1;s.player.inRun??=false;s.player.nextShopFloor??=4;s.player.floorSeeds??={};s.player.openedChests??={};s.player.bossRewards??={};s.monsters??=[];s.party??=[];s.equipment??=[];s.inventory??={potions:0,captureCrystals:0};s.settings??={};s.settings.minimapVisible??=true;s.settings.autoBattle??=true;s.settings.equipmentSort??="rarity";s.settings.battleSpeed??=1;s.records??={kills:0,captures:0,chests:0,purchases:0};s.records.purchases??=0;s.monsters.forEach(m=>{m.currentHp??=null;m.currentMp??=maxMp(m);m.currentMp=Math.min(m.currentMp,maxMp(m));m.equipment??={weapon:null,armor:null,accessory:null}});s.equipment.forEach(i=>{i.favorite??=false;i.locked??=false;i.equippedBy??=null;i.plus??=0;i.level??=1});return s}
