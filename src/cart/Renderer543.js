import {floorSurface556,springBarrel556} from './Surfaces556.js';
import {parking557} from './Parking557.js';
import {frostPoint558} from './Projection558.js';
...
const images={};function art(name){if(!images[name]){const i=new Image();i.decoding='async';const folder={wax:'cart546',sale:'cart546',obstacle:'cart545'}[name]??'cart544';i.src=name==='woven-runner'?'./assets/cart557/woven-runner.png':['frost-harbor','foundry-quay','moonlit-bazaar','spring-barrel'].includes(name)?`./assets/cart556/${name}.png`:`./assets/${folder}/${name}.webp`;images[name]=i}return images[name]}
...
export function point543(r,x,y){const depth=clamp543(y/C.edge,0,1.2),scale=1-depth*.56,front=r.width>r.height?.83:.91,screenY=front-y/C.edge*(front-.22);if(r.floor558==='frost-harbor')return frostPoint558(r,x,screenY,C.width);return{x:r.width*.5+x/C.width*r.width*.82*scale,y:r.height*screenY,scale}}
...
export function renderer543(canvas){return{canvas,ctx:canvas.getContext('2d',{alpha:false}),terrain:document.createElement('canvas'),track:art('track'),cart:art('cart'),wax:art('wax'),sale:art('sale'),crates:art('obstacle'),spring556:art('spring-barrel'),rug557:art('woven-runner'),scenes556:Object.fromEntries(['frost-harbor','foundry-quay','moonlit-bazaar'].map(n=>[n,art(n)])),width:0,height:0,dpr:1,points:[],cacheBuilds:0}}
...
function background(r,g){const course=course544(g),variant=r.scenes556[course.scene]??(course.id==='wax'?r.wax:course.id==='sale'?r.sale:r.track),track=ready(variant)?variant:r.track,key=[r.width,r.height,r.dpr,track?.src,!!ready(track),!!ready(r.crates),!!ready(r.rug557),course.id].join(':');r.floor558=ready(variant)&&course.scene==='frost-harbor'?'frost-harbor':null;if(r.key===key)return;r.key=key;r.cacheBuilds++;const c=r.terrain.getContext('2d');r.terrain.width=r.canvas.width;r.terrain.height=r.canvas.height;c.setTransform(r.dpr,0,0,r.dpr,0,0);c.fillStyle='#358d87';c.fillRect(0,0,r.width,r.height);
 ...
 for(const z of course.zones){
  floorSurface556(c,(x,y)=>point543(r,x,y),z,r.rug557);
 ...
 // The last narrow strip is an increasing risk bonus, continuous with gold.
 const danger=c.createLinearGradient(0,point543(r,0,30).y,0,far.y);danger.addColorStop(0,'#eab38636');danger.addColorStop(1,'#ffc96808');band(r,c,GOLD544.to,29.63,danger);
...
export function paint543(r,g,selfId,u,at){if(!r.width||!r.height||!g.players.length)return;const c=r.ctx;background(r,g);
 ...
 for(const [i,b] of course544(g).bumpers.entries()){const hit=g.events.findLast(e=>e.type==='spring'&&e.bumper===i);springBarrel556(c,(x,y)=>point543(r,x,y),b,r.spring556,u.reduced||!hit?Infinity:elapsed-hit.at)}
 ...
 for(const p of [...g.players].sort((a,b)=>b.y-a.y)){
  ...
  const footprint=point543(r,x+C.radius,y).x-v.x;
  ...
  c.beginPath();c.ellipse(0,size*.16,footprint+3,Math.max(4,size*.13),0,0,Math.PI*2);c.fillStyle=parkingBonus544(p)?'#ffdf6599':'#143c3977';c.fill();c.strokeStyle=parking557(p).kind==='edge'?'#ffbc88':parkingBonus544(p)?'#fff0a2':color;c.lineWidth=p.playerId===selfId?2.8:2;c.stroke();
 ...
 for(const e of g.events){const age=elapsed-e.at;if(age<0||age>1100||!['bump','fall','boost','park','block','wall','spring'].includes(e.type))continue;const p=point543(r,e.x,e.y);c.save();c.globalAlpha=1-age/1100;
  ...
  if(e.type==='spring'){text(c,'カーン！',p.x,p.y-25-age/50,12,'#fff4b0')}
  ...
  if(e.type==='boost')text(c,e.direction==='back'?'逆回転！':e.direction==='left'||e.direction==='right'?'横取り！':'速達！！',p.x,p.y-26-age/50,12,'#fff1a7');
  if(e.type==='park'&&e.perfect){text(c,e.parking557==='edge'?`キワ停め！ ${e.points*g.multiplier}点`:`金枠！ ＋${e.bonus*g.multiplier}`,p.x,p.y-30-age/65,13,'#fff4a3');if(!u.reduced)for(let i=0;i<8;i++){const a=i*Math.PI/4;text(c,'✦',p.x+Math.cos(a)*(18+age/60),p.y+Math.sin(a)*(18+age/60),8,'#ffe28a')}}c.restore();
 }
 // Screen-space labels remain readable as the camera moves closer to the action.
 c.setTransform(r.dpr,0,0,r.dpr,0,0);
 for(const [y,label,tint,side,width]of [[28.1,'＋'+50*g.multiplier,'#ffe6a7',1,36],[29.8,'MAX '+200*g.multiplier,'#ffdbb1',-1,56]]){const p=point543(r,side*4.4,y),x=clamp543(p.x*zoom+tx+side*(width/2+3),width/2+5,r.width-width/2-5),sy=p.y*zoom+ty;if(sy<r.height*.12||sy>r.height-18)continue;c.fillStyle='#173e34ed';c.beginPath();c.roundRect(x-width/2,sy-8,width,16,3);c.fill();c.strokeStyle='#d8b777';c.lineWidth=.8;c.stroke();text(c,label,x,sy,8,tint)}
 return{elapsed};
}
