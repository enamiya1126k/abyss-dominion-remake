...
export function makeCart543({id,code,partyId,hostId,now,members}){return{id,code,game:'cart',rules543:5,partyId462:partyId,hostId,createdAt:now,updatedAt:now,serverAt:now,revision:0,phase:'lobby',members:copy(members),players:[],events:[],eventId:0,elapsed:0,round:0,history:[],results:[],winnerIds:[]}}
function setup(g,now){g.round++;g.roundAt=now+C.countdown;g.phase='countdown';g.phaseAt=now;g.elapsed=0;g.multiplier=g.round===C.rounds?2:1;g.stillAt=null;g.events=[];g.contacts={};g.roundResults=[];
 for(const p of g.players){const lane=(p.seat+g.round-1)%4;Object.assign(p,{x:(lane-1.5)*1.8,y:1.1,vx:0,vy:0,lane,chargeAt:null,pullPower547:0,launched:false,launchAt:null,fallenAt:null,power:null,roundScore:0,lastHitAt:-9999,auto:p.ai,botStart:350+random(g)*4800,aim545:0,bodyAngle545:0,spin545:0,boosted:false,parkedAt:null,parkAnnounced:false,botPower:course544(g).botPower-.075+random(g)*.15});p.aim545=course544(g).blocks.length?(lane<2?-1:1)*(lane===0||lane===3?.12:.32):0;if(course544(g).blocks.length)p.botPower+=(lane===0||lane===3?-.115:-.02);p.botPower=powerForSpeed549(4.5+7.5*p.botPower);}
 event(g,'round',{round:g.round});
}
export function startCart543(g,now,seed=543){g.rules543=5;g.seed=seed||543;g.players=g.members.filter(m=>!m.departed).map((m,seat)=>({playerId:m.playerId,name:m.name,seat,ai:!!m.ai,color499:m.color499,speciesId:m.choice?.speciesId??'slime',score:0,best:0,perfects:0,bumps:0,falls:0,lastSeq:0}));
...
export function physics543(g,dt=C.step/1000){
 ...
 while(remaining>1e-9){
  ...
  for(const p of active){
   const v=Math.hypot(p.vx,p.vy),next=Math.max(0,v-resistance544(g,p.y)*step),ratio=v?next/v:0,oldY=p.y;
   p.x+=p.vx*step*(1+ratio)/2;p.y+=p.vy*step*(1+ratio)/2;p.vx*=ratio;p.vy*=ratio;
   const belt=course544(g).zones.find(z=>z.kind==='boost');
   if(belt&&!p.boosted&&p.vy>0&&p.y>=belt.from&&oldY<=belt.to){p.boosted=true;const speed=Math.hypot(p.vx,p.vy),factor=(speed+belt.boost)/speed;p.vx*=factor;p.vy*=factor;event(g,'boost',{seat:p.seat,x:p.x,y:p.y})}
   rotate545(p,step);
  }
  ...
  for(const p of active){
   ...
   else if(Math.hypot(p.vx,p.vy)<.06){p.vx=p.vy=0;if(p.launched){p.parkedAt??=g.elapsed;if(!p.parkAnnounced&&g.elapsed-p.parkedAt>=250){p.parkAnnounced=true;event(g,'park',{seat:p.seat,x:p.x,y:p.y,perfect:parkingBonus544(p)>0,points:score543(p)})}}}
...
function step(g,now,inputs,auto){g.elapsed=now-g.roundAt;
 for(const p of g.players){p.auto=p.ai||auto.has(p.playerId);const list=inputs.get(p.playerId)??[];inputs.delete(p.playerId);
  ...
  if(!p.launched&&p.fallenAt==null&&g.elapsed>=C.launchWindow)launch543(g,p,!p.auto&&p.pullPower547>0?p.pullPower547:powerForSpeed549(4.5+7.5*(course544(g).botPower-.10)));
 }
 physics543(g);
