...
export function conveyorMotion549(c,project,zone,at,reduced){
 const f=zone.from+.13,t=zone.to-.13,phase=reduced?0:(at%1100)/1100*.36;
 c.save();quad(c,project,-3.79,f,3.79,t);c.clip();
 // Individual cylindrical slats roll toward the sea, with a highlight on each.
 for(let y=f-.36+phase;y<t+.36;y+=.36){
  const a=project(0,y),b=project(0,y+.29),g=metal(c,b.y,a.y,['#283b32','#687968','#bcc3a4','#e3d7b4','#61715c','#263c32']);
  quad(c,project,-3.77,y,3.77,y+.29,g,'#23372b');
  for(const x of [-2.55,0,2.55])quad(c,project,x-.035,y+.015,x+.035,y+.28,'#314b3a55');
 }
 c.restore();
 // Inlaid amber chevrons are compact and follow the same perspective.
 for(const y of [f+(t-f)*.33,f+(t-f)*.68]){
  const a=project(-.60,y-.11),b=project(0,y+.33),d=project(.60,y-.11);
  c.beginPath();c.moveTo(a.x,a.y);c.lineTo(b.x,b.y);c.lineTo(d.x,d.y);c.strokeStyle='#112e29a0';c.lineWidth=4;c.lineJoin='round';c.stroke();
  c.strokeStyle='#edd69a';c.lineWidth=2;c.stroke();
 }
 for(const side of [-1,1])for(let i=0;i<3;i++){
  const y=f+(t-f)*(i+.5)/3,p=project(side*3.97,y),bright=reduced||((Math.floor(at/150)+i)%3===0);
  c.fillStyle=bright?'#fff0b8':'#69836a';c.beginPath();c.ellipse(p.x,p.y,1.5*p.scale,2*p.scale,0,0,Math.PI*2);c.fill();
 }
}
