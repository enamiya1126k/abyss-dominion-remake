...
const server=http.createServer((request,response)=>{
 if(request.url==="/health"){const weekly=store.raid.weeklyState();response.writeHead(200,{"content-type":"application/json; charset=utf-8","cache-control":"no-store"});response.end(JSON.stringify({ok:true,service:"ABYSS DOMINION CO-OP SERVER",protocol:"1.14.0",weeklyRaid:{weekId:weekly.weekId,bossId:weekly.boss.id,modifierId:weekly.modifier.id,endsAt:weekly.endsAt},rooms:store.rooms.size,expeditions:[...store.rooms.values()].filter(room=>room.phase==="expedition").length,battles:[...store.rooms.values()].filter(room=>room.expedition?.battle).length,raids:[...store.rooms.values()].filter(room=>room.phase==="raid").length,teamBattles:[...store.rooms.values()].filter(room=>room.phase==="team").length,players:[...store.sessions.values()].filter(session=>session.connected).length,time:new Date().toISOString()}));return}
...
wss.on("connection",socket=>{
 ...
 if(message.type==="hello"){if(message.protocol!=="1.14.0")return fail(socket,{code:"PROTOCOL_MISMATCH",message:"ゲームとオンラインサーバーのバージョンが一致しません"});const result=store.hello(socket,message);if(!result.ok)return fail(socket,result);const activeTrade=store.trade.activeFor(result.playerId);reply(socket,{type:"helloAck",protocol:"1.14.0",playerId:result.playerId,resumeToken:result.resumeToken,resumed:result.resumed,room:result.room,activeTradeIds:activeTrade?[activeTrade.tradeId]:[]});store.deliverPendingRewards(socket.session);return}
