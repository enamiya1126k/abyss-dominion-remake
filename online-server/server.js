...
function shutdown(){clearInterval(worldRaidClock438);clearInterval(battleClock);clearInterval(heartbeat);if(powerRankingRolloverTimer)clearTimeout(powerRankingRolloverTimer);for(const socket of clients)try{socket.close(1001,"server shutdown")}catch{};server.close(()=>process.exit(0));setTimeout(()=>process.exit(0),1500).unref()}
