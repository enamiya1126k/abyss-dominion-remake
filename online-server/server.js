import{RaceCoordinator451}from'./src/RaceCoordinator451.js';
import http from"node:http";
import process from"node:process";
import{WebSocketServer,WebSocket}from"ws";
import{RoomStore}from"./src/RoomStore.js";
import {WorldRaidCoordinator440} from "./src/WorldRaidCoordinator440.js";
...
const worldRaid=new WorldRaidCoordinator440({sessions:store.sessions,stateFile:process.env.WORLD_RAID_STATE_FILE||"./data/world-raid.json",send:(playerId,message)=>{const session=store.sessions.get(playerId);if(session?.connected)reply(session.connection,message)},isBusy:session=>store.trade.blocksContent(session.playerId)});
const monsterRace451=new RaceCoordinator451({sessions:store.sessions,stateFile:process.env.RACE_STATE_FILE||"./data/monster-races451.json",send:(id,message)=>{const session=store.sessions.get(id);if(session?.connected)reply(session.connection,message)},isBusy:session=>Boolean(session.roomId)||worldRaid.active(session.playerId)||store.trade.blocksContent(session.playerId),canJoin:(session,members)=>!members.some(m=>store.friends.interactionBlocked(session.playerId,m.playerId))});
...
const server=http.createServer((request,response)=>{
 if(request.url==="/health"){const weekly=store.raid.weeklyState(),settlementJournal=store.settlementStatus(),persistence={friends:store.friends.persistenceHealthy(),guilds:store.guilds.persistenceHealthy(),ranking:store.powerRanking.persistenceHealthy(),settlements:store.settlementPersistenceHealthy(),worldRaid:worldRaid.healthy(),monsterRace:monsterRace451.healthy()},healthy=persistence.friends&&persistence.guilds&&persistence.ranking&&persistence.settlements&&persistence.worldRaid&&persistence.monsterRace;response.writeHead(healthy?200:503,{"content-type":"application/json; charset=utf-8","cache-control":"no-store"});response.end(JSON.stringify({ok:healthy,service:"ABYSS DOMINION CO-OP SERVER",protocol:ONLINE_PROTOCOL,compatibleProtocols:[...COMPATIBLE_PROTOCOLS],persistence,settlementJournal,weeklyRaid:{weekId:weekly.weekId,bossId:weekly.boss.id,modifierId:weekly.modifier.id,endsAt:weekly.endsAt},rooms:store.rooms.size,guilds:store.guilds.guilds.size,powerRankingRecords:store.powerRanking.recordCount(),expeditions:[...store.rooms.values()].filter(room=>room.phase==="expedition").length,battles:[...store.rooms.values()].filter(room=>room.expedition?.battle).length,raids:[...store.rooms.values()].filter(room=>room.phase==="raid").length,teamBattles:[...store.rooms.values()].filter(room=>room.phase==="team").length,resonanceMazes:0,activeHallGames:[...store.rooms.values()].filter(room=>store.hallMinigames.active(room)).length,recoveryOutboxes:store.recoveryOutboxes.size,tradeRecoveries:store.trade.recoveryStatus(),players:[...store.sessions.values()].filter(session=>session.connected).length,time:new Date().toISOString()}));return}
...
wss.on("connection",socket=>{
 ...
 socket.on("message",raw=>{
  ...
  if(message.type==="raceRequest451"){monsterRace451.handle(session,message);return}
  if(monsterRace451.active(session.playerId)&&(WORLD_RAID_BUSY_REQUESTS.has(message.type)||["worldRaidStart","worldRaidReserve430"].includes(message.type)))return fail(socket,{code:"MONSTER_RACE_ACTIVE",message:"パーティーのゲームが終わるまでお待ちください"});
  if(WORLD_RAID_REQUESTS.has(message.type)){worldRaid.handle(session,message);return}
  if(worldRaid.active(session.playerId)&&WORLD_RAID_BUSY_REQUESTS.has(message.type))return fail(socket,{code:"WORLD_RAID_ACTIVE",message:"共闘レイドの挑戦を終えてから移動してください"});
...
const towerClock517=setInterval(()=>monsterRace451.advanceTower517(),50);towerClock517.unref?.();
const monsterRaceClock451=setInterval(()=>monsterRace451.advance(),250);monsterRaceClock451.unref?.();
const battleClock=setInterval(()=>{store.advanceBattles()},250);battleClock.unref?.();
const worldRaidClock438=setInterval(()=>worldRaid.advance(),50);worldRaidClock438.unref?.();
...
function shutdown(){clearInterval(towerClock517);clearInterval(monsterRaceClock451);clearInterval(worldRaidClock438);clearInterval(battleClock);clearInterval(heartbeat);if(powerRankingRolloverTimer)clearTimeout(powerRankingRolloverTimer);for(const socket of clients)try{socket.close(1001,"server shutdown")}catch{};server.close(()=>process.exit(0));setTimeout(()=>process.exit(0),1500).unref()}
