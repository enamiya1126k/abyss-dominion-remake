...
const server=http.createServer((request,response)=>{
 if(request.url==="/health"){const weekly=store.raid.weeklyState(),settlementJournal=store.settlementStatus(),persistence={friends:store.friends.persistenceHealthy(),guilds:store.guilds.persistenceHealthy(),settlements:store.settlementPersistenceHealthy()},healthy=persistence.friends&&persistence.guilds&&persistence.settlements;response.writeHead(healthy?200:503,{"content-type":"application/json; charset=utf-8","cache-control":"no-store"});response.end(JSON.stringify({ok:healthy,service:"ABYSS DOMINION CO-OP SERVER",protocol:"1.16.0",persistence,settlementJournal,weeklyRaid:{weekId:weekly.weekId,bossId:weekly.boss.id,modifierId:weekly.modifier.id,endsAt:weekly.endsAt},rooms:store.rooms.size,guilds:store.guilds.guilds.size,expeditions:[...store.rooms.values()].filter(room=>room.phase==="expedition").length,battles:[...store.rooms.values()].filter(room=>room.expedition?.battle).length,raids:[...store.rooms.values()].filter(room=>room.phase==="raid").length,teamBattles:[...store.rooms.values()].filter(room=>room.phase==="team").length,resonanceMazes:[...store.rooms.values()].filter(room=>room.phase==="resonance").length,recoveryOutboxes:store.recoveryOutboxes.size,tradeRecoveries:store.trade.recoveryStatus(),players:[...store.sessions.values()].filter(session=>session.connected).length,time:new Date().toISOString()}));return}
