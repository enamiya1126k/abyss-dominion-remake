...
const BACKGROUND_REQUESTS=new Set(["ping","powerSnapshotSubmit","powerRankingList","powerRankingProfile","setConnectionMode","rewardAck","expeditionVitalsAck","hostWorldDeltaAck","battleDefeatedAck","expeditionResultAck","tradeAck"]);
...
wss.on("connection",socket=>{
 ...
 socket.on("message",raw=>{
  ...
  let message;try{message=JSON.parse(raw.toString())}catch{return fail(socket,{code:"BAD_JSON",message:"通信データを読み取れません"})}if(!message||typeof message.type!=="string")return;
	 if(message.type==="hello"){if(socket.session)return fail(socket,{code:"ALREADY_READY",message:"この接続では開始処理が完了しています"});if(message.protocol!=="1.16.0")return fail(socket,{code:"PROTOCOL_MISMATCH",message:"ゲームとオンラインサーバーのバージョンが一致しません"});const result=store.hello(socket,message);if(!result.ok)return fail(socket,result);const pendingExpeditionResult=Boolean(socket.session?.pendingMessages?.some(entry=>entry?.type==="expeditionResult"));reply(socket,{type:"helloAck",protocol:"1.16.0",capabilities:{roomListingsV1:true,friendsV1:true,guildsV1:true,guildPartyRecruitmentV1:true,guildActivityHistoryV1:true,guildPlansV1:true,guildPlanGatheringV1:true,guildPlanRemindersV1:true,settlementJournalV1:true,onlineSafetyV1:true,expeditionResultsV1:true,hostWorldReceiptsV1:true,battleRecordsV1:true,fullResetRaidV1:true,battleRosterV1:true,hallMinigamesV1:true,powerRankingsV1:true,backgroundConnectionV1:true},playerId:result.playerId,resumeToken:result.resumeToken,resumed:result.resumed,recovered:Boolean(result.recovered),backgroundOnly:Boolean(result.backgroundOnly),resumableRoom:Boolean(result.resumableRoom),room:result.room,friendState:result.friendState,guildState:result.guildState,activeTradeIds:store.trade.protectedTradeIdsFor(result.playerId),pendingExpeditionResult});if(!store.deliverPendingRewards(socket.session)){fail(socket,{code:"SETTLEMENT_PERSISTENCE",message:"保留中の受取を安全に確認できません。サーバーの保存先を確認してください"});return}reply(socket,{type:"recoveryComplete",orphanedExpedition:Boolean(!result.resumableRoom&&!pendingExpeditionResult)});return}
  const session=socket.session;if(!session)return fail(socket,{code:"NOT_READY",message:"先に接続処理を完了してください"});if(session.connection!==socket){try{socket.close(4001,"superseded connection")}catch{}return}if(!["ping","listRoomListings"].includes(message.type))store.markSessionActivity(session);let result={ok:false,code:"UNKNOWN_MESSAGE",message:"未対応の通信です"};
  ...
  if(message.type==="friendRequest")result=store.requestFriend(session,message.targetId);
	  ...
	  else if(message.type==="hallGameJoin")result=store.joinHallGame(session,message);
	  else if(message.type==="hallGameReady")result=store.setHallGameReady(session,message);
	  else if(message.type==="hallGameStart")result=store.startHallGame(session,message);
	  else if(message.type==="hallGameAction")result=store.submitHallGameAction(session,message);
	  else if(message.type==="hallGameLeave")result=store.leaveHallGame(session,message);
	  else if(message.type==="hallGameReset")result=store.resetHallGame(session,message);
