...
test("leader starts a ready four-player shared expedition and discoveries resolve once",()=>{
 ...
 const room=store.rooms.get(roomId);room.expedition.encountersEnabled=false;for(const object of room.expedition.objects.filter(entry=>entry.type==="encounter"))object.resolved=true;let target=room.expedition.objects.find(object=>object.type==="chest");if(!target){const point=room.expedition.decorations.find(object=>["barrel","crate","bones","crystal","water"].includes(object.type));room.expedition.decorations=room.expedition.decorations.filter(object=>object!==point);target={id:"fixture-chest",type:"chest",x:point.x,y:point.y,kind:"box",locked:false,mimic:false,resolved:false};room.expedition.objects.push(target)}target.locked=false;target.mimic=false;const path=shortestPath(room.expedition,players[0].session.dungeonPosition,target);for(const point of path)assert.equal(store.moveExpedition(players[0].session,point).ok,true);assert.equal(target.resolved,true);const discoveries=room.expedition.discoveries,rewardCounts=players.map(player=>player.session.pendingRewards.length);assert.ok(discoveries>=1);assert.equal(rewardCounts.every(count=>count>=1),true);
...
function enterFirstEncounter(store,room,session){
 const choice=room.expedition.objects.filter(object=>object.type==="encounter"&&!object.hidden&&!object.resolved).map(target=>({target,path:shortestPath(room.expedition,session.dungeonPosition,target)})).sort((a,b)=>a.path.length-b.path.length)[0];if(choice)for(const point of choice.path){assert.equal(store.moveExpedition(session,point).ok,true);if(room.expedition.battle)break}if(!room.expedition.battle)store._startBattle(room,{id:"fixture-random-encounter",type:"encounter",...session.dungeonPosition,resolved:true,randomEncounter:true});assert.ok(room.expedition.battle);if(choice&&room.expedition.battle.encounterId===choice.target.id)assert.equal(choice.target.resolved,true);return room.expedition.battle;
}

test("four players submit synchronized actions, create a perfect link, and a disconnected member is AI-controlled",()=>{
 let now=50_000;const profile={maxFloor:301,battleStats:{hp:20_000,mp:200,atk:4_000,matk:3_500,def:2_000,mdef:2_000,spd:900,crit:10,evasion:5},skills:[{id:"burst",name:"共鳴斬",kind:"attack",mp:12,power:1.2,hits:1}],captureStock:3},store=new RoomStore({now:()=>now,randomRoomCode:()=>"SYNC24",random:()=>.1}),players=[1,2,3,4].map(index=>hello(store,index,profile)),created=store.createRoom(players[0].session),roomId=created.room.roomId;for(const player of players.slice(1))store.joinRoom(player.session,roomId);assert.equal(store.setFloor(players[0].session,301).ok,true);for(const player of players)store.setReady(player.session,true);assert.equal(store.startExpedition(players[0].session).ok,true);
 const room=store.rooms.get(roomId),battle=enterFirstEncounter(store,room,players[0].session),targetId=battle.enemies[0].id;assert.ok(battle.enemies.length>=2&&battle.enemies.length<=4);assert.equal(store.setBattleSpeed(players[1].session,2).code,"LEADER_ONLY");assert.equal(store.setBattleSpeed(players[0].session,2).ok,true);
...
test("capture is an individual one-charge attempt and queues an idempotent contract reward",()=>{
 ...
 assert.equal(store.submitBattleAction(player.session,{kind:"capture",targetId:enemy.id}).ok,true);assert.equal(battle.players[player.session.playerId].captureCharges,0);assert.equal(enemy.hp,0);const reward=player.session.pendingRewards.find(entry=>entry.source.kind==="battleCapture");assert.ok(reward);assert.equal(reward.reward.captureCrystalCost,1);assert.equal(reward.reward.captureSuccess,true);assert.equal(reward.reward.capture.speciesId,enemy.speciesId);assert.equal(store.ackReward(player.session,reward.rewardId).ok,true);assert.equal(player.session.pendingRewards.some(entry=>entry.rewardId===reward.rewardId),false);
});

test("floor-one enemies are based on the leader floor, not a helper's endgame stats",()=>{
 const store=new RoomStore({random:()=>.1}),battle={id:"floor-one",floor:1};
 const weak=[{profile:{battleStats:{hp:100,atk:10,def:5}}}],endgame=[{profile:{battleStats:{hp:999_999_999,atk:999_999_999,def:999_999_999}}}];
 const weakEnemy=store._createBattleEnemies(battle,weak,1)[0],helperEnemy=store._createBattleEnemies(battle,endgame,1)[0];
 assert.equal(helperEnemy.maxHp,weakEnemy.maxHp);assert.ok(helperEnemy.maxHp<1_000);assert.equal(helperEnemy.level,1);
});

test("all members standing on the stairs automatically advance the leader's floor",()=>{
 let now=80_000;const profile={maxFloor:20,battleStats:{hp:1_000,mp:80,atk:100,matk:100,def:80,mdef:80,spd:50,crit:5,evasion:3}},store=new RoomStore({now:()=>now,randomRoomCode:()=>"STAIR2",random:()=>.35}),leader=hello(store,1,profile),helper=hello(store,2,profile),created=store.createRoom(leader.session),roomId=created.room.roomId;store.joinRoom(helper.session,roomId);store.setFloor(leader.session,1);store.setReady(leader.session,true);store.setReady(helper.session,true);store.startExpedition(leader.session);const room=store.rooms.get(roomId),firstId=room.expedition.id;room.expedition.encountersEnabled=false;for(const object of room.expedition.objects)if(object.type!=="exit")object.resolved=true;for(const decoration of room.expedition.decorations??[])decoration.used=true;
 for(const player of [leader,helper]){const path=shortestPath(room.expedition,player.session.dungeonPosition,room.expedition.exit);for(const point of path)assert.equal(store.moveExpedition(player.session,point).ok,true)}
 assert.equal(room.expedition.stairsMemberIds.size,2);assert.equal(room.expedition.stairsCountdownEndsAt,now+3_000);now+=3_001;store.advanceBattles();assert.equal(room.expedition.floor,2);assert.notEqual(room.expedition.id,firstId);assert.ok(leader.session.pendingRewards.some(entry=>entry.reward.leaderFloorUnlock===2));assert.equal(helper.session.pendingRewards.some(entry=>entry.reward.leaderFloorUnlock===2),false);
});

test("three-turn party buffs, shields and magic-circle data are authoritative in co-op and raid",()=>{
 let now=120_000;const skill={id:"hyper-focus",name:"天才の多動領域",kind:"buff",mp:18,allAllies:true,partyShieldRate:.22,effects:[{kind:"atkUp",value:.32,turns:3,allies:true},{kind:"defUp",value:.3,turns:3,allies:true}]},profile={maxFloor:200,battleStats:{hp:10_000,mp:200,atk:1_000,matk:1_000,def:800,mdef:800,spd:500,crit:5,evasion:3},skills:[skill],circleId:"aegis",circleName:"半月障壁陣",circleLevel:9,circleEffect:"shield"},store=new RoomStore({now:()=>now,randomRoomCode:()=>"BUFF24",random:()=>.1}),player=hello(store,1,profile),created=store.createRoom(player.session);store.setReady(player.session,true);store.startExpedition(player.session);const room=store.rooms.get(created.room.roomId),battle=enterFirstEncounter(store,room,player.session),actor=battle.players[player.session.playerId];assert.equal(actor.shield,5000);assert.equal(store.submitBattleAction(player.session,{kind:"skill",skillId:"hyper-focus",targetId:player.session.playerId}).ok,true);assert.equal(actor.effects.find(effect=>effect.kind==="atkUp")?.turns,3);assert.equal(actor.effects.find(effect=>effect.kind==="defUp")?.value,.3);assert.ok(battle.lastEvents.some(event=>event.kind==="shield"&&event.value>=2200));
 battle.outcome=null;now=battle.nextRoundAt+1;store.advanceBattles();assert.equal(actor.effects.find(effect=>effect.kind==="atkUp")?.turns,2);
 store._finishExpedition(room,{completed:false});store.setReady(player.session,true);assert.equal(store.startRaid(player.session).ok,true);const raid=room.raid,raider=raid.players[player.session.playerId];assert.equal(raider.circleEffect,"shield");assert.equal(raider.shield,5000);assert.equal(store.submitRaidAction(player.session,{kind:"skill",skillId:"hyper-focus",targetId:player.session.playerId,enemyTargetId:"abyss-amalga"}).ok,true);assert.equal(raider.effects.find(effect=>effect.kind==="atkUp")?.turns,3);assert.ok(raid.lastEvents.some(event=>event.kind==="shield"));
});
