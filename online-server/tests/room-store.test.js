...
test("leader starts a ready four-player shared expedition and discoveries resolve once",()=>{
 ...
 const room=store.rooms.get(roomId);for(const object of room.expedition.objects.filter(entry=>entry.type==="encounter"))object.resolved=true;const target=room.expedition.objects.find(object=>["chest","bone","shrine"].includes(object.type)),path=shortestPath(room.expedition,players[0].session.dungeonPosition,target);for(const point of path)assert.equal(store.moveExpedition(players[0].session,point).ok,true);assert.equal(target.resolved,true);const discoveries=room.expedition.discoveries,rewardCounts=players.map(player=>player.session.pendingRewards.length);assert.ok(discoveries>=1);assert.equal(target.type==="shrine"||rewardCounts.every(count=>count>=1),true);
...
test("all members standing on the stairs automatically advance the leader's floor",()=>{
 let now=80_000;const profile={maxFloor:20,battleStats:{hp:1_000,mp:80,atk:100,matk:100,def:80,mdef:80,spd:50,crit:5,evasion:3}},store=new RoomStore({now:()=>now,randomRoomCode:()=>"STAIR2",random:()=>.35}),leader=hello(store,1,profile),helper=hello(store,2,profile),created=store.createRoom(leader.session),roomId=created.room.roomId;store.joinRoom(helper.session,roomId);store.setFloor(leader.session,1);store.setReady(leader.session,true);store.setReady(helper.session,true);store.startExpedition(leader.session);const room=store.rooms.get(roomId),firstId=room.expedition.id;for(const object of room.expedition.objects)if(object.type==="encounter")object.resolved=true;
