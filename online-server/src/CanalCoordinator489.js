...
export function createCanal489(c,p,members){return makeCanal489({id:`cn489-${++c.data.serial}-${randomBytes(4).toString('hex')}`,code:p.code,partyId:p.id,hostId:p.hostId,members,now:c.now(),seed:randomBytes(4).readUInt32LE()})}
...
export function queueCanal489(c,session,m){
 ...
 if(!g||!p||m.gameId!==g.id||m.canalVersion489!==1||!['countdown','playing'].includes(g.phase)||!Array.isArray(m.inputs)||m.inputs.length>24)return;
 const key=g.id+':'+p.playerId;c.canalQueue489??=new Map();const queue=c.canalQueue489.get(key)??[];
 for(const x of m.inputs){if(queue.length>=CANAL489.maxQueue)break;if(!Number.isSafeInteger(x.seq)||x.seq<=p.lastSeq||x.seq>CANAL489.maxSequence||queue.some(y=>y.seq===x.seq)||!Number.isInteger(x.lane)||x.lane<0||x.lane>3||typeof x.held!=='boolean'||typeof x.pulse!=='boolean'||typeof x.burst!=='boolean')continue;queue.push({seq:x.seq,lane:x.lane,held:x.held,pulse:x.pulse,burst:x.burst,receivedAt:c.now()})}
...
export function handleCanal489(c,session,m){
 ...
 if(m.kind==='select'){
  ...
  if(p.members.some(x=>x.canalVersion489!==1))throw Error('全員の本体をBuild489へ更新してください');
