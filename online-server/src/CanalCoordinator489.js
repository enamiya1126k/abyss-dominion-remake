...
export function createCanal489(c,p,members){return {...makeCanal489({id:`cn489-${++c.data.serial}-${randomBytes(4).toString('hex')}`,code:p.code,partyId:p.id,hostId:p.hostId,members,now:c.now(),seed:randomBytes(4).readUInt32LE()}),rules492:1,rules494:1,netEnergy:0,netUntil:0,megaCount:0,teamCatches:0,bySpecies:[0,0,0,0]}}
...
export function queueCanal489(c,session,m){
 ...
 if(!g||!p||m.gameId!==g.id||m.canalVersion489!==1||(g.rules492===1&&m.canalRules492!==1)||(g.rules494===1&&m.canalRules494!==1)||!['countdown','playing'].includes(g.phase)||!Array.isArray(m.inputs)||m.inputs.length>24)return;
...
export function handleCanal489(c,session,m){
 ...
 if(m.kind==='select'){
  ...
  if(m.canalRules494!==1||p.members.some(x=>x.canalRules494!==1))throw Error('全員の本体をBuild494へ更新してください');
  ...
  g.rules492=1;g.rules494=1;startCanal489(g,c.now());
