...
function runtime(c,g){let r=runtimes(c).get(g.id);if(!r){r={g:structuredClone(g),inputs:[],sequences:new Map(),lastSave:c.now(),lastSend:0,phase:g.phase};runtimes(c).set(g.id,r)}return r}
export function handleRicochet550(c,session,m){if(m.op!=='ricochet550')return false;const g=ricochetFor550(c,session.playerId),party=Object.values(c.data.parties462??{}).find(p=>p.id===g?.partyId462),me=g?.members.find(p=>p.playerId===session.playerId&&!p.departed);
 if(!g||!party||!me||m.gameId!==g.id)throw Error('ゲームが切り替わりました');if(m.ricochetVersion550!==1)throw Error('本体をBuild550へ更新してください');if(g.phase!=='lobby')throw Error('準備画面で設定してください');
 ...
 else if(m.kind==='start'){if(party.hostId!==session.playerId)throw Error('部屋主が開始できます');if(party.members.some(p=>p.ricochetVersion550!==1))throw Error('全員がBuild550へ更新してください');if(party.members.some(p=>!p.ready||p.atHome||!c.sessions.get(p.playerId)?.connected)||g.members.some(p=>!p.choice))throw Error('全員の相棒と準備OKを確認してください');if(party.members.some(p=>c.isBusy(c.sessions.get(p.playerId))))throw Error('ほかの対戦の終了を待っています');g.aiColors500={...(party.aiColors500??{})};start550(g,c.now(),randomBytes(4).readUInt32LE());runtimes(c).delete(g.id)}else throw Error('未対応の操作です');
 g.updatedAt=c.now();g.revision++;return true;
}
export function queueRicochet550(c,session,m){const saved=ricochetFor550(c,session.playerId);if(!session?.connected||!saved||m.gameId!==saved.id||m.ricochetVersion550!==1||!Number.isSafeInteger(m.seq)||m.seq<1||m.seq>1e9)return false;
 ...
 if(!p||p.ai||m.round!==g.round||m.seq<=Math.max(p.lastSeq,r.sequences.get(p.playerId)??0)||party?.members.find(x=>x.playerId===p.playerId)?.atHome||!['pick','pull','shoot','cancel'].includes(m.action))return false;
 if(m.action==='pick'){if(g.phase!=='draft'||p.picked||!p.offers.includes(m.item)||c.now()>=g.deadline)return false}
 else{if(g.phase!=='play'||p.launched||c.now()>=g.roundAt+C.launch)return false;if(m.action!=='cancel'&&(!Number.isFinite(m.angle)||Math.abs(m.angle)>Math.PI||!Number.isFinite(m.power)||m.power>1||m.power<(m.action==='shoot'?.08:0)))return false}
 if(r.inputs.filter(i=>i.playerId===p.playerId).length>=16)return false;
 const input={playerId:p.playerId,seq:m.seq,round:m.round,action:m.action,angle:m.angle,power:m.power,item:m.item,at:c.now()};if(m.action==='pull'){const previous=r.inputs.at(-1);if(previous?.playerId===p.playerId&&previous.action==='pull')r.inputs.pop()}r.inputs.push(input);r.sequences.set(p.playerId,m.seq);return true;
}
export function advanceRicochets550(c){const now=c.now();for(const saved of Object.values(c.data.ricochetRooms550??{})){
 ...
 advance550(g,now,r.inputs,auto);const changed=r.phase!==g.phase;
 if(changed||now-r.lastSave>=2000){c.transaction(()=>{c.data.ricochetRooms550[g.code]=structuredClone(g)});r.lastSave=now;r.phase=g.phase}
