...
import {makeLuck511,startLuck511,drawPlan511,chooseLuck511,advanceLuck511,rounds528} from '../../src/luck/Rules511.js';
...
export function handleLuck511(c,session,m){
 ...
 if(m.kind==='rounds'){
  ...
  if(![8,16].includes(m.rounds))throw Error('8か16ラウンドを選んでください');
  if(rounds528(g)!==m.rounds){g.rounds=m.rounds;for(const member of p.members)member.ready=false;g.updatedAt=c.now();g.revision++;}
  ...
  g.aiColors500={...(p.aiColors500??{})};startLuck511(g,c.now(),drawPlan511(randomInt,rounds528(g)));
