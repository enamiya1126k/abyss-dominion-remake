...
export function createGorilla502(c,p,members){return makeGorilla503({id:`gg502-${++c.data.serial}-${randomBytes(4).toString('hex')}`,code:p.code,partyId:p.id,hostId:p.hostId,members:members.map(m=>({...m,choice:m.owned.find(x=>x.id===p.members.find(pm=>pm.playerId===m.playerId)?.slotOne476)??m.owned[0]})),now:c.now(),seed:randomBytes(4).readUInt32LE()})}
export function handleGorilla502(c,session,m){
 ...
 if(m.kind==='select'){
  ...
  g.aiColors500={...(p.aiColors500??{})};(g.rules503===1?startGorilla503:startGorilla502)(g,c.now(),{badHair:randomInt(g.rules503===1?GORILLA503.hairs:6),firstSeat:randomInt(4)});
 }else if(m.kind==='pull')(g.rules503===1?pullGorilla503:pullGorilla502)(g,session.playerId,m.hair,m.turn,c.now());
...
export function advanceGorillas502(c){
 const at=c.now();for(const g of Object.values(c.data.gorillaRooms502??{})){
  if(!['lobby','result'].includes(g.phase)&&at>=g.nextAt){c.transaction(()=>(g.rules503===1?advanceGorilla503:advanceGorilla502)(g,at));c.broadcast()}
