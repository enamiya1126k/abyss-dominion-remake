...
export function createGorilla502(c,p,members){return makeGorilla504({id:`gg502-${++c.data.serial}-${randomBytes(4).toString('hex')}`,code:p.code,partyId:p.id,hostId:p.hostId,members:members.map(m=>({...m,choice:m.owned.find(x=>x.id===p.members.find(pm=>pm.playerId===m.playerId)?.slotOne476)??m.owned[0]})),now:c.now(),seed:randomBytes(4).readUInt32LE()})}
export function handleGorilla502(c,session,m){
 ...
 if(m.kind==='select'){
  ...
  (g.rules504===1?startGorilla504:g.rules503===1?startGorilla503:startGorilla502)(g,c.now(),options);
...
export function advanceGorillas502(c){
 const at=c.now();for(const g of Object.values(c.data.gorillaRooms502??{})){
  if(!['lobby','result'].includes(g.phase)&&at>=g.nextAt){c.transaction(()=>(g.rules504===1?advanceGorilla504:g.rules503===1?advanceGorilla503:advanceGorilla502)(g,at));c.broadcast()}
