...
export function settleCrystals474(c,g){if(!rewardMode474(g)||g.phase!=='result'||g.economy474.settled)return;for(const p of g.players.filter(p=>!p.ai)){const entry=g.economy474.entries[p.playerId],r=g.results.find(r=>r.playerId===p.playerId)?.crystals474;if(!entry||!r)throw Error('報酬戦の参加記録がありません');issue(c,p.playerId,{id:`sg474:${entry.requestId}`,kind:'result',gameId:g.id,playerId:p.playerId,requestId:entry.requestId,fee:entry.amount,place:g.results.find(r=>r.playerId===p.playerId).place,tied:g.results.filter(r=>r.place===g.results.find(r=>r.playerId===p.playerId).place).length,prize:r.prize,journey:r.journey,...(r.economy480?{economy480:1,raw480:r.raw480,stake480:r.stake480,pool480:r.pool480,humanCount480:r.humanCount480,humanRaw480:r.humanRaw480,adjustment480:r.adjustment480}:{}),...(r.crystalRules477?{crystalRules477:1,shrines477:r.shrines477,events477:r.events477}:{}),crystals:r.total});c.data.accounts[p.playerId].sgDecisions474[entry.requestId].status='settled'}g.economy474.settled=true}
export function handleCrystal474(c,session,m){
 ...
 if(Number(m.rulesVersion)<18||!validFee474(m.amount)||!/^sg463-\d+-[a-f0-9]{8}$/.test(m.gameId)||!/^[\w-]{8,100}$/.test(m.requestId))throw Error('参加情報を確認してください');
...
export function abandon480(c,g,id){const entry=g.economy474?.entries?.[id];if(!entry||g.economy474.settled)return;issue(c,id,{id:`sg474:${entry.requestId}`,kind:'abandon480',gameId:g.id,playerId:id,requestId:entry.requestId,fee:entry.amount,crystals:0});c.data.accounts[id].sgDecisions474[entry.requestId].status='settled';g.economy474.settled=true;}
