...
export const partyRace462=(c,p)=>p?.raceCode?(p.game==='quiz'?c.data.quizRooms513?.[p.raceCode]:p.game==='luck'?c.data.luckRooms507?.[p.raceCode]:p.game==='gorilla'?c.data.gorillaRooms502?.[p.raceCode]:p.game==='canal'?c.data.canalRooms489?.[p.raceCode]:p.game==='sugoroku'?c.data.boardRooms463?.[p.raceCode]:p.game==='cabbage'?c.data.cabbageRooms484?.[p.raceCode]:c.data.rooms[p.raceCode])??null:null;
...
function deleteGame463(c,r){if(r?.game==='quiz'){delete c.data.quizRooms513[r.code]}else if(r?.game==='luck'){delete c.data.luckRooms507[r.code]}else if(r?.game==='gorilla'){delete c.data.gorillaRooms502[r.code]}else if(r?.game==='canal'){delete c.data.canalRooms489[r.code];for(const[key]of c.canalQueue489??[])if(key.startsWith(r.id+':'))c.canalQueue489.delete(key)}else if(r?.game==='cabbage'){delete c.data.cabbageRooms484[r.code];for(const [key]of c.cabbageQueue484??[])if(key.startsWith(r.id+':'))c.cabbageQueue484.delete(key)}else if(r?.game==='sugoroku'){cancelEntries474(c,r);delete c.data.boardRooms463[r.code]}else if(r)delete c.data.rooms[r.code]}
export function openGame462(c,p,game){if(!PARTY_GAMES462.some(g=>g.id===game))fail('このゲームはまだ遊べません');const old=partyRace462(c,p);if(old&&!['lobby','result'].includes(old.phase))fail('今のゲームが終わってから切り替えられます');if(p.members.some(m=>c.sessions.get(m.playerId)&&c.isBusy(c.sessions.get(m.playerId))))fail('ほかのオンラインコンテンツの終了を待っています');if(old)deleteGame463(c,old);const people=p.members.map((m,i)=>({playerId:m.playerId,name:m.name,color499:color499(m.color499,i).id,owned:m.owned,choice:null,ai:false}));const r=game==='quiz'?createQuiz513(c,p,people):game==='luck'?createLuck507(c,p,people):game==='gorilla'?createGorilla502(c,p,people):game==='canal'?createCanal489(c,p,people):game==='sugoroku'?createBoard463(c,p,people):game==='cabbage'?createCabbage484(c,p,people):c.makeRoom(p.code,people,8);r.hostId=p.hostId;r.partyId462=p.id;if(game==='sugoroku'&&old?.economy474){const fee=Math.min(old.economy474.fee,minimumFee474(p));r.economy474={mode:old.economy474.mode==='crystal'&&fee>0?'crystal':'practice',fee:fee||500,epoch:0,entries:{}}}if(old?.game!=='quiz'&&old?.game!=='luck'&&old?.game!=='gorilla'&&old?.game!=='canal'&&old?.game!=='sugoroku'&&old?.game!=='cabbage'&&old&&game==='race'){r.track459=old.track459;r.course=old.course==='芝'?'砂':'芝';r.aiFatigue455=old.aiFatigue455??{}}if(game==='quiz'){c.data.quizRooms513??={};c.data.quizRooms513[p.code]=r}else if(game==='luck'){c.data.luckRooms507??={};c.data.luckRooms507[p.code]=r}else if(game==='gorilla'){c.data.gorillaRooms502??={};c.data.gorillaRooms502[p.code]=r}else if(game==='canal'){c.data.canalRooms489??={};c.data.canalRooms489[p.code]=r}else if(game==='sugoroku'){c.data.boardRooms463??={};c.data.boardRooms463[p.code]=r}else if(game==='cabbage'){c.data.cabbageRooms484??={};c.data.cabbageRooms484[p.code]=r}else c.data.rooms[p.code]=r;p.raceCode=p.code;p.game=game;for(const m of p.members)m.ready=false}
export function handleParty462(c,session,m){
 ...
 if(m.op==='partyKick485'){
  ...
  if(r){r.updatedAt=c.now();if(r.game==='quiz'||r.game==='luck'||r.game==='gorilla'||r.game==='canal'||r.game==='sugoroku'||r.game==='cabbage')r.revision=(r.revision??0)+1}
  return true;
 }
