...
export function handleParty462(c,session,m){
 ...
 if(m.op==='partyCreate462'||m.op==='partyJoin462'){
  ...
  person.seatToken485=randomBytes(8).toString('hex');person.canalVersion489=m.canalVersion489===1?1:0;person.canalRules492=m.canalRules492===1?1:0;person.cabbageVersion484=m.cabbageVersion484===1?1:0;person.cabbageScoring491=m.cabbageScoring491===1?1:0;person.slotOne476=slot476(person.owned,m.slotOne476);
  c.data.parties462??={};
 ...
 if(m.op==='partyRoster462'){me.canalVersion489=m.canalVersion489===1?1:0;me.canalRules492=m.canalRules492===1?1:0;me.cabbageVersion484=m.cabbageVersion484===1?1:0;me.cabbageScoring491=m.cabbageScoring491===1?1:0;me.crystals474=crystalBalance474(m.crystals474);me.owned=c.roster(m.roster);me.slotOne476=slot476(me.owned,Object.hasOwn(m,'slotOne476')?m.slotOne476:me.slotOne476);if(r?.phase==='lobby'){const rm=c.member(r,id);if(rm){rm.owned=me.owned;if(rm.choice){rm.choice=rm.owned.find(x=>x.id===rm.choice.id)??null;me.ready=false}}}return true}
 if(m.op==='partyReady462'){if(r?.phase!=='lobby')fail('ゲームの準備画面で押してください');if(r.game==='canal'&&(m.canalVersion489!==1||m.canalRules492!==1))fail('新しい用水路には本体のBuild492更新が必要です');if(r.game==='cabbage'&&(m.cabbageVersion484!==1||m.cabbageScoring491!==1))fail('新しい減点ルールには本体のBuild491更新が必要です');const rm=c.member(r,id);if(!rm?.choice)fail('コマにする魔物を選んでください');if(me.atHome)fail('準備画面へ戻ってください');if(r.game==='sugoroku'&&r.economy474?.mode==='crystal'){if(m.ready===true&&!r.economy474.entries[id])fail('表示された💎の参加費を支払って準備してください');if(m.ready!==true)refundEntry474(c,r,id)}if(r.game==='canal'){me.canalVersion489=1;me.canalRules492=1}if(r.game==='cabbage'){me.cabbageVersion484=1;me.cabbageScoring491=1}me.ready=m.ready===true;return true}
