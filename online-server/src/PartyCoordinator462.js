import{createCart543}from'./CartCoordinator543.js';
import{createBomb542}from'./BombCoordinator542.js';
import{createTetra539}from'./TetraCoordinator539.js';
import{createHide536}from'./HideCoordinator536.js';
import{createFishing524}from'./FishingCoordinator524.js';
import{createSumo523}from'./SumoCoordinator523.js';
import{createTower517}from'./TowerCoordinator517.js';
import{createQuiz513}from'./QuizCoordinator513.js';
...
export const partyRace462=(c,p)=>p?.raceCode?(p.game==='cart'?c.data.cartRooms543?.[p.raceCode]:p.game==='bomb'?c.data.bombRooms542?.[p.raceCode]:p.game==='tetra'?c.data.tetraRooms539?.[p.raceCode]:p.game==='hide'?c.data.hideRooms536?.[p.raceCode]:p.game==='fishing'?c.data.fishingRooms524?.[p.raceCode]:p.game==='sumo'?c.data.sumoRooms523?.[p.raceCode]:p.game==='tower'?c.data.towerRooms517?.[p.raceCode]:p.game==='quiz'?c.data.quizRooms513?.[p.raceCode]:p.game==='luck'?c.data.luckRooms507?.[p.raceCode]:p.game==='gorilla'?c.data.gorillaRooms502?.[p.raceCode]:p.game==='canal'?c.data.canalRooms489?.[p.raceCode]:p.game==='sugoroku'?c.data.boardRooms463?.[p.raceCode]:p.game==='cabbage'?c.data.cabbageRooms484?.[p.raceCode]:c.data.rooms[p.raceCode])??null:null;
...
function deleteGame463(c,r){if(r?.game==='cart'){delete c.data.cartRooms543[r.code];c.cartRuntime543?.delete(r.id)}else if(r?.game==='bomb'){delete c.data.bombRooms542[r.code];c.bombRuntime542?.delete(r.id)}else if(r?.game==='tetra'){delete c.data.tetraRooms539[r.code];c.tetraRuntime539?.delete(r.id)}else if(r?.game==='hide'){delete c.data.hideRooms536[r.code];c.hideRuntime536?.delete(r.id)}else if(r?.game==='fishing'){delete c.data.fishingRooms524[r.code];c.fishingRuntime524?.delete(r.id)}else if(r?.game==='sumo'){delete c.data.sumoRooms523[r.code];c.sumoRuntime523?.delete(r.id)}else if(r?.game==='tower'){delete c.data.towerRooms517[r.code];c.towerRuntime517?.delete(r.id)}else if(r?.game==='quiz'){delete c.data.quizRooms513[r.code]}else if(r?.game==='luck'){delete c.data.luckRooms507[r.code]}else if(r?.game==='gorilla'){delete c.data.gorillaRooms502[r.code]}else if(r?.game==='canal'){delete c.data.canalRooms489[r.code];for(const[key]of c.canalQueue489??[])if(key.startsWith(r.id+':'))c.canalQueue489.delete(key)}else if(r?.game==='cabbage'){delete c.data.cabbageRooms484[r.code];for(const [key]of c.cabbageQueue484??[])if(key.startsWith(r.id+':'))c.cabbageQueue484.delete(key)}else if(r?.game==='sugoroku'){cancelEntries474(c,r);delete c.data.boardRooms463[r.code]}else if(r)delete c.data.rooms[r.code]}
export function openGame462(c,p,game){if(!PARTY_GAMES462.some(g=>g.id===game))fail('このゲームはまだ遊べません');const old=partyRace462(c,p);if(old&&!['lobby','result'].includes(old.phase))fail('今のゲームが終わってから切り替えられます');if(p.members.some(m=>c.sessions.get(m.playerId)&&c.isBusy(c.sessions.get(m.playerId))))fail('ほかのオンラインコンテンツの終了を待っています');if(old)deleteGame463(c,old);const people=p.members.map((m,i)=>({playerId:m.playerId,name:m.name,color499:color499(m.color499,i).id,owned:m.owned,choice:null,ai:false}));const r=game==='cart'?createCart543(c,p,people):game==='bomb'?createBomb542(c,p,people):game==='tetra'?createTetra539(c,p,people):game==='hide'?createHide536(c,p,people):game==='fishing'?createFishing524(c,p,people):game==='sumo'?createSumo523(c,p,people):game==='tower'?createTower517(c,p,people):game==='quiz'?createQuiz513(c,p,people):game==='luck'?createLuck507(c,p,people):game==='gorilla'?createGorilla502(c,p,people):game==='canal'?createCanal489(c,p,people):game==='sugoroku'?createBoard463(c,p,people):game==='cabbage'?createCabbage484(c,p,people):c.makeRoom(p.code,people,8);r.hostId=p.hostId;r.partyId462=p.id;if(game==='hide'&&old?.game==='hide')r.role=old.role;if(game==='luck'&&old?.game==='luck')r.rounds=old.rounds===16?16:8;if(game==='sugoroku'&&old?.economy474){const fee=Math.min(old.economy474.fee,minimumFee474(p));r.economy474={mode:old.economy474.mode==='crystal'&&fee>0?'crystal':'practice',fee:fee||500,epoch:0,entries:{}}}if(old?.game!=='cart'&&old?.game!=='bomb'&&old?.game!=='tetra'&&old?.game!=='hide'&&old?.game!=='fishing'&&old?.game!=='sumo'&&old?.game!=='tower'&&old?.game!=='quiz'&&old?.game!=='luck'&&old?.game!=='gorilla'&&old?.game!=='canal'&&old?.game!=='sugoroku'&&old?.game!=='cabbage'&&old&&game==='race'){r.track459=old.track459;r.course=old.course==='芝'?'砂':'芝';r.aiFatigue455=old.aiFatigue455??{}}if(game==='cart'){c.data.cartRooms543??={};c.data.cartRooms543[p.code]=r}else if(game==='bomb'){c.data.bombRooms542??={};c.data.bombRooms542[p.code]=r}else if(game==='tetra'){c.data.tetraRooms539??={};c.data.tetraRooms539[p.code]=r}else if(game==='hide'){c.data.hideRooms536??={};c.data.hideRooms536[p.code]=r}else if(game==='fishing'){c.data.fishingRooms524??={};c.data.fishingRooms524[p.code]=r}else if(game==='sumo'){c.data.sumoRooms523??={};c.data.sumoRooms523[p.code]=r}else if(game==='tower'){c.data.towerRooms517??={};c.data.towerRooms517[p.code]=r}else if(game==='quiz'){c.data.quizRooms513??={};c.data.quizRooms513[p.code]=r}else if(game==='luck'){c.data.luckRooms507??={};c.data.luckRooms507[p.code]=r}else if(game==='gorilla'){c.data.gorillaRooms502??={};c.data.gorillaRooms502[p.code]=r}else if(game==='canal'){c.data.canalRooms489??={};c.data.canalRooms489[p.code]=r}else if(game==='sugoroku'){c.data.boardRooms463??={};c.data.boardRooms463[p.code]=r}else if(game==='cabbage'){c.data.cabbageRooms484??={};c.data.cabbageRooms484[p.code]=r}else c.data.rooms[p.code]=r;p.raceCode=p.code;p.game=game;for(const m of p.members)m.ready=false}
export function handleParty462(c,session,m){
 ...
 if(m.op==='partyKick485'){
  ...
  if(r){r.updatedAt=c.now();if(r.game==='cart'||r.game==='bomb'||r.game==='tetra'||r.game==='hide'||r.game==='fishing'||r.game==='sumo'||r.game==='tower'||r.game==='quiz'||r.game==='luck'||r.game==='gorilla'||r.game==='canal'||r.game==='sugoroku'||r.game==='cabbage')r.revision=(r.revision??0)+1}
  return true;
 }
 ...
 if(m.op==='partyRoster462'){me.cartVersion543=m.cartVersion543===2?2:0;me.bombVersion542=m.bombVersion542===1?1:0;me.tetraVersion539=m.tetraVersion539===3?3:0;me.hideVersion536=m.hideVersion536===5?5:0;me.minigamesVersion528=m.minigamesVersion528===1?1:0;me.fishingVersion524=m.fishingVersion524===8?8:0;me.sumoVersion523=m.sumoVersion523===3?3:0;me.towerVersion517=m.towerVersion517===5?5:0;me.quizVersion514=m.quizVersion514===1?1:0;me.quizVersion513=m.quizVersion513===1?1:0;me.luckVersion511=m.luckVersion511===1?1:0;me.luckVersion509=m.luckVersion509===1?1:0;me.luckVersion508=m.luckVersion508===1?1:0;me.luckVersion507=m.luckVersion507===1?1:0;me.gorillaRules505=m.gorillaRules505===1?1:0;me.gorillaRules504=m.gorillaRules504===1?1:0;me.gorillaRules503=m.gorillaRules503===1?1:0;me.gorillaVersion502=m.gorillaVersion502===1?1:0;me.canalVersion489=m.canalVersion489===1?1:0;me.canalRules492=m.canalRules492===1?1:0;me.canalRules494=m.canalRules494===1?1:0;me.canalRules496=m.canalRules496===1?1:0;me.canalRules541=m.canalRules541===1?1:0;me.canalRules500=m.canalRules500===1?1:0;me.cabbageVersion484=m.cabbageVersion484===1?1:0;me.cabbageScoring491=m.cabbageScoring491===1?1:0;me.crystals474=crystalBalance474(m.crystals474);me.owned=c.roster(m.roster);me.slotOne476=slot476(me.owned,Object.hasOwn(m,'slotOne476')?m.slotOne476:me.slotOne476);if(r?.phase==='lobby'){const rm=c.member(r,id);if(rm){rm.owned=me.owned;if(rm.choice){rm.choice=rm.owned.find(x=>x.id===rm.choice.id)??null;me.ready=false}}}return true}
 if(m.op==='partyReady462'){if(['sugoroku','gorilla','luck'].includes(r?.game)&&m.minigamesVersion528!==1)fail('このゲームには本体のBuild528更新が必要です');me.minigamesVersion528=m.minigamesVersion528===1?1:0;if(r?.phase!=='lobby')fail('ゲームの準備画面で押してください');if(r.game==='cart'&&m.cartVersion543!==2)fail('暴走カートには本体のBuild544更新が必要です');if(r.game==='bomb'&&m.bombVersion542!==1)fail('爆弾リレーには本体のBuild542更新が必要です');if(r.game==='tetra'&&m.tetraVersion539!==3)fail('テトラポット渡りには本体のBuild541更新が必要です');if(r.game==='hide'&&m.hideVersion536!==5)fail('かくれんぼには本体のBuild541更新が必要です');if(r.game==='fishing'&&m.fishingVersion524!==8)fail('ぬし釣り大騒動には本体のBuild534更新が必要です');if(r.game==='sumo'&&m.sumoVersion523!==3)fail('魔獣ぶっとばし相撲には本体のBuild535更新が必要です');if(r.game==='tower'&&m.towerVersion517!==5)fail('本体をBuild529へ更新してください');if(r.game==='quiz'&&m.quizVersion514!==1)fail('〇×サバイバルには本体のBuild514更新が必要です');if(r.game==='luck'&&r.rules511===1&&m.luckVersion511!==1)fail('運だけ大運動会には本体のBuild511更新が必要です');if(r.game==='luck'&&r.rules509===1&&m.luckVersion509!==1)fail('運だけ大運動会には本体のBuild509更新が必要です');if(r.game==='luck'&&r.rules508===1&&m.luckVersion508!==1)fail('運だけ大運動会には本体のBuild508更新が必要です');if(r.game==='luck'&&m.luckVersion507!==1)fail('運だけ大運動会には本体のBuild507更新が必要です');if(r.game==='gorilla'&&r.rules505===1&&m.gorillaRules505!==1)fail('胸毛抜きには本体のBuild505更新が必要です');if(r.game==='gorilla'&&r.rules504===1&&m.gorillaRules504!==1)fail('胸毛抜きには本体のBuild504更新が必要です');if(r.game==='gorilla'&&r.rules503===1&&m.gorillaRules503!==1)fail('胸毛抜きには本体のBuild503更新が必要です');if(r.game==='gorilla'&&m.gorillaVersion502!==1)fail('胸毛抜きには本体のBuild502更新が必要です');if(r.game==='canal'&&(m.canalVersion489!==1||m.canalRules492!==1))fail('新しい用水路には本体のBuild492更新が必要です');if(r.game==='canal'&&m.canalRules494!==1)fail('新しい用水路には本体のBuild494更新が必要です');if(r.game==='canal'&&m.canalRules496!==1)fail('共有水場には本体のBuild496更新が必要です');if(r.game==='cabbage'&&(m.cabbageVersion484!==1||m.cabbageScoring491!==1))fail('新しい減点ルールには本体のBuild491更新が必要です');const rm=c.member(r,id);if(!rm?.choice)fail('コマにする魔物を選んでください');if(me.atHome)fail('準備画面へ戻ってください');if(r.game==='sugoroku'&&r.economy474?.mode==='crystal'){if(m.ready===true&&!r.economy474.entries[id])fail('表示された💎の参加費を支払って準備してください');if(m.ready!==true)refundEntry474(c,r,id)}if(r.game==='cart')me.cartVersion543=2;if(r.game==='bomb')me.bombVersion542=1;if(r.game==='tetra')me.tetraVersion539=3;if(r.game==='hide')me.hideVersion536=5;if(r.game==='fishing')me.fishingVersion524=8;if(r.game==='sumo')me.sumoVersion523=3;if(r.game==='tower')me.towerVersion517=5;if(r.game==='quiz'){me.quizVersion513=1;me.quizVersion514=1;}if(r.game==='luck'){me.luckVersion511=m.luckVersion511===1?1:0;me.luckVersion509=m.luckVersion509===1?1:0;me.luckVersion508=m.luckVersion508===1?1:0;me.luckVersion507=1;}if(r.game==='canal'){me.canalVersion489=1;me.canalRules492=1;me.canalRules494=1;me.canalRules496=1;me.canalRules541=m.canalRules541===1?1:0;me.canalRules500=m.canalRules500===1?1:0}if(r.game==='gorilla'){me.gorillaVersion502=1;me.gorillaRules505=m.gorillaRules505===1?1:0;me.gorillaRules504=m.gorillaRules504===1?1:0;me.gorillaRules503=m.gorillaRules503===1?1:0;}if(r.game==='cabbage'){me.cabbageVersion484=1;me.cabbageScoring491=1}me.ready=m.ready===true;return true}
