...
export const partyRace462=(c,p)=>p?.raceCode?(p.game==='gorilla'?c.data.gorillaRooms502?.[p.raceCode]:p.game==='canal'?c.data.canalRooms489?.[p.raceCode]:p.game==='sugoroku'?c.data.boardRooms463?.[p.raceCode]:p.game==='cabbage'?c.data.cabbageRooms484?.[p.raceCode]:c.data.rooms[p.raceCode])??null:null;
...
function deleteGame463(c,r){if(r?.game==='gorilla'){delete c.data.gorillaRooms502[r.code]}else if(r?.game==='canal'){delete c.data.canalRooms489[r.code];for(const[key]of c.canalQueue489??[])if(key.startsWith(r.id+':'))c.canalQueue489.delete(key)}else if(r?.game==='cabbage'){delete c.data.cabbageRooms484[r.code];for(const [key]of c.cabbageQueue484??[])if(key.startsWith(r.id+':'))c.cabbageQueue484.delete(key)}else if(r?.game==='sugoroku'){cancelEntries474(c,r);delete c.data.boardRooms463[r.code]}else if(r)delete c.data.rooms[r.code]}
export function openGame462(c,p,game){if(!PARTY_GAMES462.some(g=>g.id===game))fail('このゲームはまだ遊べません');const old=partyRace462(c,p);if(old&&!['lobby','result'].includes(old.phase))fail('今のゲームが終わってから切り替えられます');if(p.members.some(m=>c.sessions.get(m.playerId)&&c.isBusy(c.sessions.get(m.playerId))))fail('ほかのオンラインコンテンツの終了を待っています');if(old)deleteGame463(c,old);const people=p.members.map((m,i)=>({playerId:m.playerId,name:m.name,color499:color499(m.color499,i).id,owned:m.owned,choice:null,ai:false}));const r=game==='gorilla'?createGorilla502(c,p,people):game==='canal'?createCanal489(c,p,people):game==='sugoroku'?createBoard463(c,p,people):game==='cabbage'?createCabbage484(c,p,people):c.makeRoom(p.code,people,8);r.hostId=p.hostId;r.partyId462=p.id;if(game==='sugoroku'&&old?.economy474){const fee=Math.min(old.economy474.fee,minimumFee474(p));r.economy474={mode:old.economy474.mode==='crystal'&&fee>0?'crystal':'practice',fee:fee||500,epoch:0,entries:{}}}if(old?.game!=='gorilla'&&old?.game!=='canal'&&old?.game!=='sugoroku'&&old?.game!=='cabbage'&&old&&game==='race'){r.track459=old.track459;r.course=old.course==='芝'?'砂':'芝';r.aiFatigue455=old.aiFatigue455??{}}if(game==='gorilla'){c.data.gorillaRooms502??={};c.data.gorillaRooms502[p.code]=r}else if(game==='canal'){c.data.canalRooms489??={};c.data.canalRooms489[p.code]=r}else if(game==='sugoroku'){c.data.boardRooms463??={};c.data.boardRooms463[p.code]=r}else if(game==='cabbage'){c.data.cabbageRooms484??={};c.data.cabbageRooms484[p.code]=r}else c.data.rooms[p.code]=r;p.raceCode=p.code;p.game=game;for(const m of p.members)m.ready=false}
export function handleParty462(c,session,m){
 ...
 if(m.op==='partyKick485'){
  ...
  if(r){r.updatedAt=c.now();if(r.game==='gorilla'||r.game==='canal'||r.game==='sugoroku'||r.game==='cabbage')r.revision=(r.revision??0)+1}
  return true;
 }
 ...
 if(m.op==='partyRoster462'){me.gorillaRules503=m.gorillaRules503===1?1:0;me.gorillaVersion502=m.gorillaVersion502===1?1:0;me.canalVersion489=m.canalVersion489===1?1:0;me.canalRules492=m.canalRules492===1?1:0;me.canalRules494=m.canalRules494===1?1:0;me.canalRules496=m.canalRules496===1?1:0;me.canalRules500=m.canalRules500===1?1:0;me.cabbageVersion484=m.cabbageVersion484===1?1:0;me.cabbageScoring491=m.cabbageScoring491===1?1:0;me.crystals474=crystalBalance474(m.crystals474);me.owned=c.roster(m.roster);me.slotOne476=slot476(me.owned,Object.hasOwn(m,'slotOne476')?m.slotOne476:me.slotOne476);if(r?.phase==='lobby'){const rm=c.member(r,id);if(rm){rm.owned=me.owned;if(rm.choice){rm.choice=rm.owned.find(x=>x.id===rm.choice.id)??null;me.ready=false}}}return true}
 if(m.op==='partyReady462'){if(r?.phase!=='lobby')fail('ゲームの準備画面で押してください');if(r.game==='gorilla'&&r.rules503===1&&m.gorillaRules503!==1)fail('胸毛抜きには本体のBuild503更新が必要です');if(r.game==='gorilla'&&m.gorillaVersion502!==1)fail('胸毛抜きには本体のBuild502更新が必要です');if(r.game==='canal'&&(m.canalVersion489!==1||m.canalRules492!==1))fail('新しい用水路には本体のBuild492更新が必要です');if(r.game==='canal'&&m.canalRules494!==1)fail('新しい用水路には本体のBuild494更新が必要です');if(r.game==='canal'&&m.canalRules496!==1)fail('共有水場には本体のBuild496更新が必要です');if(r.game==='cabbage'&&(m.cabbageVersion484!==1||m.cabbageScoring491!==1))fail('新しい減点ルールには本体のBuild491更新が必要です');const rm=c.member(r,id);if(!rm?.choice)fail('コマにする魔物を選んでください');if(me.atHome)fail('準備画面へ戻ってください');if(r.game==='sugoroku'&&r.economy474?.mode==='crystal'){if(m.ready===true&&!r.economy474.entries[id])fail('表示された💎の参加費を支払って準備してください');if(m.ready!==true)refundEntry474(c,r,id)}if(r.game==='canal'){me.canalVersion489=1;me.canalRules492=1;me.canalRules494=1;me.canalRules496=1;me.canalRules500=m.canalRules500===1?1:0}if(r.game==='gorilla'){me.gorillaVersion502=1;me.gorillaRules503=m.gorillaRules503===1?1:0;}if(r.game==='cabbage'){me.cabbageVersion484=1;me.cabbageScoring491=1}me.ready=m.ready===true;return true}
