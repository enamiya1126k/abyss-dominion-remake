...
export const partyRace462=(c,p)=>p?.raceCode?(p.game==='tower'?c.data.towerRooms517?.[p.raceCode]:p.game==='quiz'?c.data.quizRooms513?.[p.raceCode]:p.game==='luck'?c.data.luckRooms507?.[p.raceCode]:p.game==='gorilla'?c.data.gorillaRooms502?.[p.raceCode]:p.game==='canal'?c.data.canalRooms489?.[p.raceCode]:p.game==='sugoroku'?c.data.boardRooms463?.[p.raceCode]:p.game==='cabbage'?c.data.cabbageRooms484?.[p.raceCode]:c.data.rooms[p.raceCode])??null:null;
...
export function handleParty462(c,session,m){
 ...
 if(m.op==='partyKick485'){
  ...
  if(r){r.updatedAt=c.now();if(r.game==='tower'||r.game==='quiz'||r.game==='luck'||r.game==='gorilla'||r.game==='canal'||r.game==='sugoroku'||r.game==='cabbage')r.revision=(r.revision??0)+1}
  return true;
 }
