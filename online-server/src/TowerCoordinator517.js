...
export function queueTower517(c,session,m){const saved=towerFor517(c,session.playerId);if(!saved||saved.id!==m.gameId||m.towerVersion517!==5)return false;const rt=runtime(c,saved),g=rt.g,p=g.players.find(p=>p.playerId===session.playerId);if(g.phase!=='play'||!p||!p.alive||p.ai||!Number.isSafeInteger(m.seq)||m.seq<=p.lastSeq||m.seq>1e9)return false;
 const at=c.now(),previous=rt.inputs.get(p.playerId);if(p.role==='drop'&&previous&&at-previous.receivedAt<20&&!m.drop)return false;
 if(p.role==='run'&&m.kind==='move'){if(!Number.isFinite(m.axis)||Math.abs(m.axis)>1)return false;rt.inputs.set(p.playerId,{axis:m.axis,jump:previous?.jump||m.jump===true,superJump:previous?.superJump||m.superJump===true,receivedAt:at})}
...
export function advanceTowers517(c){const now=c.now();for(const saved of Object.values(c.data.towerRooms517??{})){
 ...
 for(const p of g.players){if(!p.ai&&(!c.sessions.get(p.playerId)?.connected||party?.members.find(m=>m.playerId===p.playerId)?.atHome))auto.add(p.playerId);const input=rt.inputs.get(p.playerId);if(input&&now-input.receivedAt>350){input.axis=0;input.jump=false;input.superJump=false}}
