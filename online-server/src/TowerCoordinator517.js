...
export function queueTower517(c,session,m){const saved=towerFor517(c,session.playerId);if(!saved||saved.id!==m.gameId||m.towerVersion517!==1)return false;const rt=runtime(c,saved),g=rt.g,p=g.players.find(p=>p.playerId===session.playerId);if(g.phase!=='play'||!p||!p.alive||p.ai||!Number.isSafeInteger(m.seq)||m.seq<=p.lastSeq||m.seq>1e9)return false;
 const at=c.now(),previous=rt.inputs.get(p.playerId);if(previous&&at-previous.receivedAt<20)return false;
