...
export function handleHide536(c,session,m){if(m.op!=='hide536')return false;const g=hideFor536(c,session.playerId),party=Object.values(c.data.parties462??{}).find(p=>p.id===g?.partyId462),me=g?.members.find(x=>x.playerId===session.playerId&&!x.departed);
 if(!g||!party||!me||g.id!==m.gameId)throw Error('ゲームが切り替わりました');if(m.hideVersion536!==3)throw Error('本体をBuild538へ更新してください');if(g.phase!=='lobby')throw Error('開始前に設定してください');
...
export function queueHide536(c,session,m){
 ...
 const old=r.inputs.get(p.playerId)??{};r.inputs.set(p.playerId,{steer:Object.hasOwn(m,'steer')?(m.steer?{...m.steer}:null):Object.hasOwn(m,'target')?null:old.steer,target:Object.hasOwn(m,'steer')?null:Object.hasOwn(m,'target')?(m.target?{...m.target}:null):old.target,attack:m.attack??old.attack,kind:m.prop??old.kind,receivedAt:c.now()});p.lastSeq=m.seq;return true;
}
export function advanceHides536(c){const now=c.now();for(const saved of Object.values(c.data.hideRooms536??{})){
 if(saved.rulesVersion!==3&&['hiding','play'].includes(saved.phase)){c.transaction(()=>{const fresh=makeHide536({id:saved.id,code:saved.code,partyId:saved.partyId462,hostId:saved.hostId,members:saved.members,now});fresh.role=saved.role;fresh.migrationNote='館と庭園のフィールドに更新されました。準備OKで新しいゲームを始めよう';c.data.hideRooms536[saved.code]=fresh;const party=Object.values(c.data.parties462??{}).find(p=>p.id===saved.partyId462);party?.members.forEach(m=>m.ready=false)});runtimes(c).delete(saved.id);c.broadcast();continue}
