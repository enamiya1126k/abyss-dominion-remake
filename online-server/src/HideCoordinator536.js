...
export function handleHide536(c,session,m){if(m.op!=='hide536')return false;const g=hideFor536(c,session.playerId),party=Object.values(c.data.parties462??{}).find(p=>p.id===g?.partyId462),me=g?.members.find(x=>x.playerId===session.playerId&&!x.departed);
 if(!g||!party||!me||g.id!==m.gameId)throw Error('ゲームが切り替わりました');if(m.hideVersion536!==6)throw Error('本体をBuild559へ更新してください');if(g.phase!=='lobby')throw Error('開始前に設定してください');
...
export function advanceHides536(c){const now=c.now();for(const saved of Object.values(c.data.hideRooms536??{})){
 if(saved.rulesVersion!==6&&['hiding','play'].includes(saved.phase)){c.transaction(()=>{const fresh=makeHide536({id:saved.id,code:saved.code,partyId:saved.partyId462,hostId:saved.hostId,members:saved.members,now});fresh.role=saved.role;fresh.migrationNote='封印５か所から３つを解く新ルールになりました。準備OKで始めよう';c.data.hideRooms536[saved.code]=fresh;const party=Object.values(c.data.parties462??{}).find(p=>p.id===saved.partyId462);party?.members.forEach(m=>m.ready=false)});runtimes(c).delete(saved.id);c.broadcast();continue}
