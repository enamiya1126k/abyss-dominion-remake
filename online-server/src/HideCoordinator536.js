...
import {makeHide536,startHide536,advanceHide536,publicHide536,objects536,HIDE536,PROPS536} from '../../src/hide/Rules536.js';
...
export function handleHide536(c,session,m){if(m.op!=='hide536')return false;const g=hideFor536(c,session.playerId),party=Object.values(c.data.parties462??{}).find(p=>p.id===g?.partyId462),me=g?.members.find(x=>x.playerId===session.playerId&&!x.departed);
 if(!g||!party||!me||g.id!==m.gameId)throw Error('ゲームが切り替わりました');if(m.hideVersion536!==7)throw Error('本体をBuild563へ更新してください');if(g.phase!=='lobby')throw Error('開始前に設定してください');
...
export function queueHide536(c,session,m){
 ...
 if(m.inspect563!=null&&(typeof m.inspect563!=='string'||m.inspect563.length>16))return false;
 ...
 const r=runtime(c,saved),g=r.g,p=g.players.find(p=>p.playerId===session.playerId);if(!session.connected||!['hiding','play'].includes(g.phase)||!p||!p.alive||p.ai||Object.values(c.data.parties462??{}).find(q=>q.id===g.partyId462)?.members.find(q=>q.playerId===p.playerId)?.atHome||m.seq<=p.lastSeq)return false;
 if(m.inspect563!=null&&(!p.hunter||g.phase!=='play'||!objects536(g,p.playerId).some(o=>o.id===m.inspect563&&!o.checked)))return false;
 const old=r.inputs.get(p.playerId)??{};r.inputs.set(p.playerId,{inspect563:Object.hasOwn(m,'inspect563')?m.inspect563:Object.hasOwn(m,'target')||Object.hasOwn(m,'steer')||m.attack?null:old.inspect563,steer:Object.hasOwn(m,'steer')?(m.steer?{...m.steer}:null):Object.hasOwn(m,'target')?null:old.steer,target:Object.hasOwn(m,'steer')?null:Object.hasOwn(m,'target')?(m.target?{...m.target}:null):old.target,ability541:m.ability541===true||old.ability541,attack:m.attack??old.attack,kind:m.prop??old.kind,receivedAt:c.now()});p.lastSeq=m.seq;return true;
}
export function advanceHides536(c){const now=c.now();for(const saved of Object.values(c.data.hideRooms536??{})){
 if(saved.rulesVersion!==7&&['hiding','play'].includes(saved.phase)){c.transaction(()=>{const fresh=makeHide536({id:saved.id,code:saved.code,partyId:saved.partyId462,hostId:saved.hostId,members:saved.members,now});fresh.role=saved.role;fresh.migrationNote='封印５か所から３つを解く新ルールになりました。準備OKで始めよう';c.data.hideRooms536[saved.code]=fresh;const party=Object.values(c.data.parties462??{}).find(p=>p.id===saved.partyId462);party?.members.forEach(m=>m.ready=false)});runtimes(c).delete(saved.id);c.broadcast();continue}
