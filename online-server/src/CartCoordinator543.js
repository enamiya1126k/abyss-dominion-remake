...
export function handleCart543(c,session,m){if(m.op!=='cart543')return false;const g=cartFor543(c,session.playerId),party=Object.values(c.data.parties462??{}).find(p=>p.id===g?.partyId462),me=g?.members.find(x=>x.playerId===session.playerId&&!x.departed);
 if(!g||!party||!me||g.id!==m.gameId)throw Error('ゲームが切り替わりました');if(m.cartVersion543!==5)throw Error('本体をBuild549へ更新してください');if(g.phase!=='lobby')throw Error('開始前に設定してください');
...
export function queueCart543(c,session,m){
 ...
 if(!session.connected||!saved||saved.id!==m.gameId||m.cartVersion543!==5||!Number.isSafeInteger(m.seq)||m.seq<1||m.seq>1e9||!['pull','shoot','cancel'].includes(m.action))return false;
 ...
 if(g.phase!=='play'||!p||p.ai||p.fallenAt!=null||p.launched||m.round!==g.round||m.seq<=p.lastSeq||c.now()>=g.roundAt+C.launchWindow||party?.members.find(x=>x.playerId===p.playerId)?.atHome)return false;
...
export function advanceCarts543(c){const now=c.now();for(const saved of Object.values(c.data.cartRooms543??{})){
 if(saved.rules543!==5&&['countdown','play','intermission'].includes(saved.phase)){c.transaction(()=>{const fresh=makeCart543({id:saved.id,code:saved.code,partyId:saved.partyId462,hostId:saved.hostId,members:saved.members,now});c.data.cartRooms543[saved.code]=fresh;const party=Object.values(c.data.parties462??{}).find(p=>p.id===saved.partyId462);party?.members.forEach(m=>m.ready=false)});runtimes(c).delete(saved.id);c.broadcast();continue}
 ...
 if(now-r.lastSend>=50){r.lastSend=now;for(const m of party?.members??[])if(c.sessions.get(m.playerId)?.connected&&c.subscribers.has(m.playerId))c.send(m.playerId,{type:'cartFrame543',selfId:m.playerId,serverNow:now,cart:publicCart543(g,m.playerId,{frame:true})})}
