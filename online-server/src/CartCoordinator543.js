...
import {makeCart543,startCart543,advanceCart543,publicCart543} from '../../src/cart/Rules543.js';
...
export function handleCart543(c,session,m){if(m.op!=='cart543')return false;const g=cartFor543(c,session.playerId),party=Object.values(c.data.parties462??{}).find(p=>p.id===g?.partyId462),me=g?.members.find(x=>x.playerId===session.playerId&&!x.departed);
 if(!g||!party||!me||g.id!==m.gameId)throw Error('ゲームが切り替わりました');if(m.cartVersion543!==2)throw Error('本体をBuild544へ更新してください');if(g.phase!=='lobby')throw Error('開始前に設定してください');
...
export function queueCart543(c,session,m){
 ...
 if(!session.connected||!saved||saved.id!==m.gameId||m.cartVersion543!==2||!Number.isSafeInteger(m.seq)||m.seq<1||m.seq>1e9||!['hold','release','cancel'].includes(m.action))return false;
 ...
 if(g.phase!=='play'||!p||p.ai||p.launched||m.round!==g.round||m.seq<=p.lastSeq||party?.members.find(x=>x.playerId===p.playerId)?.atHome)return false;
 const actions=r.inputs.get(p.playerId)??[];if(actions.length>=6)return false;
 actions.push({action:m.action,at:c.now()});r.inputs.set(p.playerId,actions);p.lastSeq=m.seq;return true;
}
export function advanceCarts543(c){const now=c.now();for(const saved of Object.values(c.data.cartRooms543??{})){
 if(saved.rules543!==2&&['countdown','play','intermission'].includes(saved.phase)){c.transaction(()=>{const fresh=makeCart543({id:saved.id,code:saved.code,partyId:saved.partyId462,hostId:saved.hostId,members:saved.members,now});c.data.cartRooms543[saved.code]=fresh;const party=Object.values(c.data.parties462??{}).find(p=>p.id===saved.partyId462);party?.members.forEach(m=>m.ready=false)});runtimes(c).delete(saved.id);c.broadcast();continue}
