...
import {makeQuiz513,startQuiz513,chooseQuiz513,advanceQuiz513} from '../../src/quiz/Rules513.js';
import {drawQuestions513} from './quiz/Questions513.js';
...
export function handleQuiz513(c,session,m){
 ...
 if(m.quizVersion513!==1)throw Error('〇×サバイバルには本体のBuild513更新が必要です');
 if(m.kind==='select'){
  ...
  if(p.members.some(x=>x.quizVersion513!==1))throw Error('全員がBuild513へ更新してから開始してください');
  ...
  g.aiColors500={...(p.aiColors500??{})};startQuiz513(g,c.now(),drawQuestions513(randomInt,p.recentQuestionIds513??[]),randomInt);
...
export function advanceQuizzes513(c){const at=c.now();for(const g of Object.values(c.data.quizRooms513??{})){
 if(!['lobby','result'].includes(g.phase)&&Number.isFinite(g.nextAt)&&at>=g.nextAt){c.transaction(()=>{advanceQuiz513(g,at);if(g.phase==='reveal'){const p=Object.values(c.data.parties462??{}).find(p=>p.id===g.partyId462);if(p)p.recentQuestionIds513=[...(p.recentQuestionIds513??[]).filter(id=>id!==g.reveal.id),g.reveal.id].slice(-30)}});c.broadcast()}
