...
import {makeSumo523,startSumo523,advanceSumo523,publicSumo523} from '../../src/sumo/Rules523.js';
...
export function advanceSumos523(c){const now=c.now();for(const saved of Object.values(c.data.sumoRooms523??{})){
 ...
 for(const p of g.players){if(!p.ai&&(!c.sessions.get(p.playerId)?.connected||party?.members.find(m=>m.playerId===p.playerId)?.atHome))auto.add(p.playerId);const input=r.inputs.get(p.playerId);if(input&&now-input.receivedAt>350){input.x=0;input.y=0;input.press=false;input.release=false;input.cancel=true}}
