...
export function handleFishing524(c,session,m){if(m.op!=='fishing524')return false;const g=fishingFor524(c,session.playerId),p=Object.values(c.data.parties462??{}).find(p=>p.id===g?.partyId462),me=g?.members.find(x=>x.playerId===session.playerId&&!x.departed);if(!g||!p||!me||g.id!==m.gameId)throw Error('ゲームが切り替わりました');if(m.fishingVersion524!==3)throw Error('本体をBuild526へ更新してください');if(g.phase!=='lobby')throw Error('開始前に設定してください');
 ...
 else if(m.kind==='start'){if(p.hostId!==session.playerId)throw Error('部屋主が開始できます');if(p.members.some(x=>x.fishingVersion524!==3))throw Error('全員がBuild526へ更新してください');if(p.members.some(x=>!x.ready||x.atHome||!c.sessions.get(x.playerId)?.connected)||g.members.some(x=>!x.choice))throw Error('全員が相棒を選んで準備OKを押してください');if(p.members.some(x=>c.isBusy(c.sessions.get(x.playerId))))throw Error('ほかの対戦の終了を待っています');g.aiColors500={...(p.aiColors500??{})};startFishing524(g,c.now(),randomBytes(4).readUInt32LE());runtimes(c).delete(g.id)}else throw Error('未対応の操作です');g.updatedAt=c.now();g.revision++;return true;
}
export function queueFishing524(c,session,m){const saved=fishingFor524(c,session.playerId);if(!saved||saved.id!==m.gameId||m.fishingVersion524!==3||m.kind!=='input'||!Number.isSafeInteger(m.seq)||m.seq>1e9)return false;const r=runtime(c,saved),g=r.g,p=g.players.find(p=>p.playerId===session.playerId);if(g.phase!=='play'||!p||p.ai||m.seq<=p.lastSeq)return false;
 if(typeof m.reel!=='boolean'||m.cast&&(!Number.isFinite(m.cast.x)||!Number.isFinite(m.cast.y)||m.cast.x<0||m.cast.x>1||m.cast.y<0||m.cast.y>1)||m.choice&&!['keep','bait','continue'].includes(m.choice))return false;
 const old=r.inputs.get(p.playerId),order=m.cast?(r.castOrder526=(r.castOrder526??0)+1):old?.castOrder526;r.inputs.set(p.playerId,{reel:m.reel,cast:m.cast?{x:m.cast.x,y:m.cast.y,bait:m.cast.bait===true}:old?.cast,castOrder526:order,castReceivedAt526:m.cast?c.now():old?.castReceivedAt526,choice:m.choice??old?.choice,receivedAt:c.now()});p.lastSeq=m.seq;return true;
}
