 ...
 handle(session,m){try{
  ...
  this.transaction(()=>{let r=this.roomFor(session.playerId),member=this.member(r,session.playerId);const name=text(m.displayName??session.profile?.displayName??session.profile?.name??'冒険者',16);
   ...
   if(m.op==='bet'){
    ...
    const valid=r?.id===m.raceId&&member&&r.phase==='parade'&&this.now()<r.deadline&&!member.ticket&&!member.passed&&validTicket451(m.ticket)&&(r.rulesVersion>=4||['win','pair','exact'].includes(m.ticket.kind));
   ...
   if(m.op==='create'||m.op==='join'){
    ...
    if(m.op==='create'){if(Object.keys(this.data.rooms).length>=1000)fail('部屋がいっぱいです');let code;do{code=randomBytes(3).toString('hex').toUpperCase()}while(this.data.rooms[code]);this.data.rooms[code]=this.makeRoom(code,[person],m.rulesVersion>=5?5:m.rulesVersion>=4?4:m.rulesVersion>=3?3:m.rulesVersion>=2?2:1);return}
