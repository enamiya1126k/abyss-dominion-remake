...
export class RaceCoordinator451{
 ...
 handle(session,m){try{
  ...
  this.transaction(()=>{let r=this.roomFor(session.playerId),member=this.member(r,session.playerId);const name=text(m.displayName??session.profile?.displayName??session.profile?.name??'冒険者',16);
   ...
   if(m.op==='create'||m.op==='join'){
    ...
    if(m.op==='create'){if(Object.keys(this.data.rooms).length>=1000)fail('部屋がいっぱいです');let code;do{code=randomBytes(3).toString('hex').toUpperCase()}while(this.data.rooms[code]);this.data.rooms[code]=this.makeRoom(code,[person],m.rulesVersion>=4?4:m.rulesVersion>=3?3:m.rulesVersion>=2?2:1);return}
   ...
   if(m.op==='boost'){if(r.rulesVersion<4||r.phase!=='race'||m.raceId!==r.id)fail('このレースでは今は操作できません');const i=r.racers.findIndex(x=>x.ownerId===session.playerId);advanceSimulation456(r.sim456,r.racers,this.now()-r.startAt,this.autoIndices456(r));boost456(r.sim456,i,m.seq);return}
 ...
 }catch(error){this.send(session.playerId,{type:'raceError451',op:m.op,requestId:m.requestId,message:error.message,held:m.op==='bet'});this.push(session)}}
