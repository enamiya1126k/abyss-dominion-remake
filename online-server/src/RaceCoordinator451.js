...
export class RaceCoordinator451{
 ...
 makeRoom(code,members,rulesVersion=1){const id=`r451-${++this.data.serial}-${randomBytes(4).toString('hex')}`;return{id,code,rulesVersion,hostId:members[0].playerId,phase:'lobby',createdAt:this.now(),updatedAt:this.now(),course:randomInt(2)?'芝':'砂',members:members.map(m=>({...m,ticket:null,passed:false,departed:false})),system:(()=>{const choices=[...(rulesVersion>=3?racePool455:pool)];return Array.from({length:4},()=>this.entrant(choices.splice(randomInt(choices.length),1)[0],null,null,rulesVersion))})()}}
 ...
 handle(session,m){try{
  ...
  this.transaction(()=>{let r=this.roomFor(session.playerId),member=this.member(r,session.playerId);const name=text(m.displayName??session.profile?.displayName??session.profile?.name??'冒険者',16);
   ...
   if(m.op==='create'||m.op==='join'){
    ...
    if(m.op==='create'){if(Object.keys(this.data.rooms).length>=1000)fail('部屋がいっぱいです');let code;do{code=randomBytes(3).toString('hex').toUpperCase()}while(this.data.rooms[code]);this.data.rooms[code]=this.makeRoom(code,[person],m.rulesVersion>=3?3:m.rulesVersion>=2?2:1);return}
