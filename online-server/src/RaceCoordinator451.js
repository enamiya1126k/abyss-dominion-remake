...
export class RaceCoordinator451{
 ...
 roster(source){if(!Array.isArray(source))fail('手持ち魔物を読み込めません');const seen=new Set(),rows=[];for(const m of source.slice(0,500)){const id=text(m.id,100),speciesId=canonicalRaceIdentity455(text(m.raceSpeciesId??m.speciesId,80));if(!id||!raceSpecies451(speciesId)||seen.has(id))continue;seen.add(id);rows.push({id,speciesId})}if(!rows.length)fail('出走できる魔物がいません');return rows}
 entrant(speciesId,ownerId=null,monsterId=null,version=2){const sp=raceSpecies451(speciesId);return{speciesId,name:sp.name,ownerId,monsterId,rulesVersion:version,profile:raceProfile451(speciesId,version),condition:randomInt(5)}}
 makeRoom(code,members,rulesVersion=1){const id=`r451-${++this.data.serial}-${randomBytes(4).toString('hex')}`;return{id,code,rulesVersion,hostId:members[0].playerId,phase:'lobby',createdAt:this.now(),updatedAt:this.now(),course:rulesVersion>=4?(this.data.serial%2?'芝':'砂'):(randomInt(2)?'芝':'砂'),members:members.map(m=>({...m,ticket:null,passed:false,departed:false})),system:(()=>{const choices=[...(rulesVersion>=3?racePool455:pool)];return Array.from({length:4},()=>this.entrant(choices.splice(randomInt(choices.length),1)[0],null,null,rulesVersion))})()}}
 ...
 handle(session,m){try{
  ...
  this.transaction(()=>{let r=this.roomFor(session.playerId),member=this.member(r,session.playerId);const name=text(m.displayName??session.profile?.displayName??session.profile?.name??'冒険者',16);
   ...
   if(m.op==='create'||m.op==='join'){
    ...
    if(m.op==='create'){if(Object.keys(this.data.rooms).length>=1000)fail('部屋がいっぱいです');let code;do{code=randomBytes(3).toString('hex').toUpperCase()}while(this.data.rooms[code]);this.data.rooms[code]=this.makeRoom(code,[person],m.rulesVersion>=6?6:m.rulesVersion>=5?5:m.rulesVersion>=4?4:m.rulesVersion>=3?3:m.rulesVersion>=2?2:1);return}
    const target=this.data.rooms[text(m.code,6).toUpperCase()];if(!target||target.phase!=='lobby')fail('入室できる部屋が見つかりません');if((target.rulesVersion??1)>Number(m.rulesVersion??1))fail('この部屋は更新版です。ゲームを再読み込みしてから参加してください');if(target.members.length>=4)fail('この部屋は満員です');if(!this.canJoin(session,target.members))fail('この部屋には参加できません');target.members.push(person);return;
   }
   if(!r||!member)fail('先にレースの部屋へ参加してください');
   if(m.op==='course'){if(r.rulesVersion<4||r.phase!=='lobby'||r.hostId!==session.playerId)fail('部屋主が開始前にコースを選べます');if(!['芝','砂'].includes(m.course))fail('コースを選んでください');r.course=m.course;return}
   if(m.op==='boost'){if(r.rulesVersion<4||r.phase!=='race'||m.raceId!==r.id)fail('このレースでは今は操作できません');const i=r.racers.findIndex(x=>x.ownerId===session.playerId);advanceSimulation456(r.sim456,r.racers,this.now()-r.startAt,this.autoIndices456(r));if(r.rulesVersion>=5){if(!Number.isInteger(m.seq)||m.seq<1||m.seq>5)fail('操作番号が不正です');while(r.sim456.runners[i].boostSeq<m.seq)boost456(r.sim456,i,r.sim456.runners[i].boostSeq+1)}else boost456(r.sim456,i,m.seq);return}
 ...
 advance(){if(this.loadError)return;for(const room of Object.values(this.data.rooms))try{const at=this.now(),phase=room.phase;
  if(room.rulesVersion>=4&&phase==='race'&&room.sim456?.order.length<8&&at-room.startAt-room.sim456.elapsed>=500){this.transaction(()=>{advanceSimulation456(room.sim456,room.racers,at-room.startAt,this.autoIndices456(room));if(room.sim456.order.length===8){room.outcome=simulationOutcome456(room.sim456);room.finishAt456=room.startAt+room.sim456.elapsed}});this.broadcast()}
