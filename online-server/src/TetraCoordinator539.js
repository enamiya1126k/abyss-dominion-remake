...
export function handleTetra539(c,session,m){if(m.op!=='tetra539')return false;const g=tetraFor539(c,session.playerId),party=Object.values(c.data.parties462??{}).find(p=>p.id===g?.partyId462),me=g?.members.find(x=>x.playerId===session.playerId&&!x.departed);
 if(!g||!party||!me||g.id!==m.gameId)throw Error('ゲームが切り替わりました');if(m.tetraVersion539!==1)throw Error('本体をBuild539へ更新してください');if(g.phase!=='lobby')throw Error('開始前に設定してください');
...
export function queueTetra539(c,session,m){const saved=tetraFor539(c,session.playerId);if(!session.connected||!saved||saved.id!==m.gameId||m.tetraVersion539!==1||!Number.isSafeInteger(m.seq)||m.seq<1||m.seq>1e9||!['begin','aim','release','cancel'].includes(m.action)||!Number.isFinite(m.angle)||Math.abs(m.angle)>Math.PI)return false;
 ...
 const actions=r.inputs.get(p.playerId)??[];if(actions.length>=12)return false;const a={action:m.action,angle:m.angle,at:c.now()-g.startAt};if(a.action==='aim'&&actions.at(-1)?.action==='aim')actions[actions.length-1]=a;else actions.push(a);r.inputs.set(p.playerId,actions);p.lastSeq=m.seq;return true;
}
