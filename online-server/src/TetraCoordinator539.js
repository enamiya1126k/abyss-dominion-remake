...
export function handleTetra539(c,session,m){if(m.op!=='tetra539')return false;const g=tetraFor539(c,session.playerId),party=Object.values(c.data.parties462??{}).find(p=>p.id===g?.partyId462),me=g?.members.find(x=>x.playerId===session.playerId&&!x.departed);
 if(!g||!party||!me||g.id!==m.gameId)throw Error('ゲームが切り替わりました');if(m.tetraVersion539!==2)throw Error('本体をBuild540へ更新してください');if(g.phase!=='lobby')throw Error('開始前に設定してください');
...
export function advanceTetras539(c){const now=c.now();for(const saved of Object.values(c.data.tetraRooms539??{})){
 if(saved.rulesVersion!==2&&['countdown','play'].includes(saved.phase)){c.transaction(()=>{const fresh=makeTetra539({id:saved.id,code:saved.code,partyId:saved.partyId462,hostId:saved.hostId,members:saved.members,now});c.data.tetraRooms539[saved.code]=fresh;const party=Object.values(c.data.parties462??{}).find(p=>p.id===saved.partyId462);party?.members.forEach(m=>m.ready=false)});runtimes(c).delete(saved.id);c.broadcast();continue}
