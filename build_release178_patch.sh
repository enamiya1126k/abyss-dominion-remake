...
base=${1:?release177 baseline directory is required}
work=${2:-$(cd "$(dirname "$0")" && pwd)}
output=${3:-$(cd "$work/.." && pwd)/abyss-dominion-remake-main-178-patch-from-177.zip}
expected_tree="a5e19651d36313564ab044568ee0ff9cd8f8eefc36b41d65edb68f5adb593752"

if [[ ! -d "$base" ]]; then
  echo "release177 baseline directory not found: $base" >&2
  exit 1
fi
if [[ ! -d "$work" ]]; then
  echo "release178 working directory not found: $work" >&2
  exit 1
fi
...
if [[ "$actual_tree" != "$expected_tree" ]]; then
  echo "release177 baseline mismatch: $actual_tree" >&2
  exit 1
fi

payload_files=(
  "BUILD237_GUILD_PLAN_GATHERING_UPDATE.md"
  "README.md"
  "README_178.txt"
  "README_PATCH_178.txt"
  "build_release178_patch.sh"
  "index.html"
  "online-server/server.js"
  "online-server/src/GuildCoordinator.js"
  "online-server/src/RoomStore.js"
  "online-server/tests/build236-guild-plans.test.js"
  "online-server/tests/build237-guild-plan-gathering.test.js"
  "src/Styles/build237.css"
  "src/main.js"
...
for relative in "${payload_files[@]}"; do
  if [[ -z "$relative" || "$relative" = /* || "$relative" = .* || "$relative" == */../* || "$relative" == ../* || "$relative" == */.. ]]; then
    echo "unsafe release178 payload path: $relative" >&2
    exit 1
  fi
  if [[ -n "${listed[$relative]:-}" ]]; then
    echo "duplicate release178 payload path: $relative" >&2
    exit 1
  fi
  if is_excluded "$relative"; then
    echo "excluded path cannot be included in release178: $relative" >&2
    exit 1
  fi
  listed[$relative]=1
done
...
if ((${#unexpected[@]})); then
  echo "release178 has changed files missing from the explicit payload list:" >&2
  printf '  %s\n' "${unexpected[@]}" >&2
  exit 1
fi
if ((${#deleted[@]})); then
  echo "release178 contains deletions, but this patch supports replacement files only:" >&2
  printf '  %s\n' "${deleted[@]}" >&2
  exit 1
fi
...
while IFS= read -r relative; do
  ...
  if [[ ! -f "$source_file" ]]; then
    echo "release178 payload file is missing: $relative" >&2
    exit 1
  fi
  if [[ ! -s "$source_file" ]]; then
    echo "release178 payload file is empty: $relative" >&2
    exit 1
  fi
  if [[ -f "$base/$relative" ]] && cmp -s "$source_file" "$base/$relative"; then
    echo "release178 payload file is unchanged from release177: $relative" >&2
    exit 1
  fi
...
if find "$root" -type f -empty -print -quit | grep -q .; then
  echo "release178 patch contains an empty file" >&2
  exit 1
fi

echo "release178 patch payload:"
...
if [[ ! -f "$verify_root/PATCH_FILES.txt" || ! -f "$verify_root/PATCH_MANIFEST.sha256" ]]; then
  echo "release178 patch metadata is missing after extraction" >&2
  exit 1
fi
